...
9. Verify unit/model/SDK/package tests cover strict Coordination Fabric schemas and protocol envelopes, ownership/fencing/lease/request/message/operation invariants, the complete failure matrix, generated all-or-nothing grant and stale-session cases, activation and claim-GC preflight consumers, bounded diagnostics, old-epoch/self-conflict refusal, and standalone package-boundary scans.
...
Phase 27 locks contracts and safety preflight only. Do not claim that the package already ships the transactional coordinator, durable supervisors, cross-process claim negotiation, offline mailbox replay, recoverable operation executor, reservation cutover, deadlock engine, or legacy database migration.
