...
Autopilot tests are deterministic by default. They do not call live providers, networks, real models, or user/global Pi state while validating `/autopilot`, `/autopilot-onboard`, `/autopilot-handoff`, `/autopilot-close`, `/autopilot-abort`, `context_budget`, contracts/templates, Quality vNext contract schemas, perfect-quality prompt doctrine, scope/protected-path adjudication, work-item lifecycle/closure gates, runtime close/merge/abort, forced-output/status, state-store helpers, `autopilot-agent-run`, runner CLI/bin behavior, fake-Pi paths, offline SDK/RPC/package gates, and package payloads.
...
- `test:sdk` loads the extension through the real Pi SDK with isolated temp `cwd`/`agentDir`, in-memory managers, no built-in tools by default, and offline environment variables. It asserts `/autopilot`, `/autopilot-onboard`, `/autopilot-handoff`, `/autopilot-close`, `/autopilot-abort`, active-workstream handoff behavior, and parent-session tool exposure.
