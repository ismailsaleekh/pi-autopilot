...
Autopilot tests are deterministic by default. They do not call live providers, networks, real models, or user/global Pi state while validating `/autopilot`, `/autopilot-onboard`, `/autopilot-handoff`, `context_budget`, contracts/templates, Quality vNext contract schemas, perfect-quality prompt doctrine, forced-output/status, state-store helpers, `autopilot-agent-run`, runner CLI/bin behavior, fake-Pi paths, offline SDK/RPC/package gates, and package payloads.
...
Current tests validate shipped commands, `context_budget`, parent/onboard/handoff prompts, contracts/templates, Quality vNext contract schemas, deterministic spec/status gates, durable purpose state, execution-audit generation/classification/coherence, perfect-quality prompt doctrine, forced-output/status, state-store helpers, runner CLI/bin, fake-Pi and e2e witnesses, offline SDK/RPC/package gates, and `pack:dry-run`. Remaining coverage gaps are compiled scheduling UI, PTY/TUI behavior, migration tooling for older runtime folders, default automated live-provider child execution, and later full lifecycle/adjudication/terminal-closure automation.
