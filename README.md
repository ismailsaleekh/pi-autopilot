...
- `/autopilot-coordination status|doctor` queries the authenticated local coordinator without starting an LLM turn. Status reports durable runs, session/child leases, acquisition groups, edit leases, claim requests, and mailbox counts; doctor reports database/schema integrity and expired-heartbeat recovery classifications without releasing WRITE authority.
