...
- `/autopilot-coordination status|doctor` queries the authenticated local coordinator without starting an LLM turn. Status reports durable runs, session/child leases, acquisition groups, edit leases, claim requests, mailbox cursors/counts, and accepted reconciliation evidence; doctor reports database/schema integrity, startup reconciliation, and expired-heartbeat recovery classifications without releasing WRITE authority from age alone.
...
Autopilot currently supplies the package extension, commands, `context_budget`, scheduler/runtime quality gates, isolated worktrees, runner/close/abort flows, Coordination Fabric contracts, the transactional coordinator, durable run supervisors, session/child leases, generation fencing, durable mailbox cursors/replay, handoff continuity, end-to-end peer claim negotiation, automatic terminal-evidence release-condition reconciliation, startup recovery, diagnostics, deterministic export, multiprocess restart/negotiation proof, and standalone packed-install coverage. Owner-scoped recoverable Git/worktree sagas, edit-lease/change-reservation cutover, the full deadlock/starvation engine, contradiction arbitration, and legacy database cutover remain later Coordination Fabric phases. It does not include a compiled scheduler UI, PTY/TUI coverage, default automated live-provider execution, network push/PR creation, or hosted PR automation. Provider-backed child runs require explicit operator approval, subscription Pi channels, and the `autopilot-agent-run` path; the default package gate remains deterministic, offline, network-free, and isolated from user/global Pi state.
