// Shared terminal close/abort crash-recovery harness (Phase 40 / D70 change C4).
//
// Extracted VERBATIM from tests/crash/terminal-cleanup-resume.test.ts so the
// close × boundary and abort × boundary crash-recovery matrices can be sharded
// into sibling files without changing a single assertion or the real
// process-death subprocess behaviour. Every export is a PURE helper — there is
// NO module-level mutable state — so importing it from concurrent test files is
// race-free (node:test runs one process per file). `runBoundary` performs the
// identical real SIGKILL-at-boundary + recovery it did before the split.
//
// Path note: `packageRoot`/`processClient` resolve from THIS file's location
// (tests/helpers/), which is the same depth under the package root as
// tests/crash/, so `../../` still names the package root and the process-client
// helper lives beside this file.
...
const processClient = fileURLToPath(new URL('./terminal-cleanup-process-client.ts', import.meta.url));

export type TerminalAction = 'close' | 'abort';
...
export async function runBoundary(action: TerminalAction, boundary: AutopilotTerminalCleanupBoundary, options: { readonly deleteS2BindingBeforeRecovery?: boolean } = {}): Promise<void> {
