...
fn close_creates_result_ref_and_does_not_push_land_move_target_or_write_operator_checkout() {
    ...
    assert_eq!(
        fs::read_to_string(report.archive_dir.join("protected.txt")).expect("archived evidence"),
        "proof\n"
    );
...
fn close_refuses_unproven_cleanup_and_leaves_that_work_present() {
    ...
    let result = lifecycle.close(CloseRequest {
        ...
        target_ref: "refs/heads/main".to_owned(),
        evidence: Vec::new(),
        cleanup: vec![CleanupProof {
            artifact: CleanupArtifact::PackageWorktree(unsafe_tree.clone()),
            proven_safe: false,
        }],
    });
    assert_eq!(
        result,
        Err(drivers::lifecycle::LifecycleError::UnsafeCleanup)
    );
    assert!(unsafe_tree.exists(), "unproven cleanup is not removed");
    assert!(
        fixture
            .vcs
            .read_tip(&fixture.source, "refs/autopilot/results/ws/run-unsafe")
            .is_err(),
        "failed close creates no result ref"
    );
    assert!(
        !fixture.owner.join("archive/ws/run-unsafe").exists(),
        "failed close creates no closed archive"
    );
}
...
fn abort_preserves_dirty_work_refs_and_evidence_and_deletes_nothing() {
    ...
    fixture
        .vcs
        .swap(&fixture.source, RUN_MAIN_REF, &fixture.base, ZERO)
        .expect("D76 run main");
    ...
    assert_eq!(
        fs::read_to_string(dirty.join("dirty.txt")).expect("dirty after abort"),
        "uncommitted\n"
    );
    assert_eq!(
        fixture
