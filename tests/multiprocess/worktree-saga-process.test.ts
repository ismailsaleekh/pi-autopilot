...
void describe('owner-scoped worktree saga multiprocess execution', () => {
  void it('serializes two recovery executors and creates one branch/worktree/effect', async () => {
    ...
    try {
      ...
      const runResponse = await client.mutate('attach-run', { repoId, workstreamRun: runId, sessionId: null, fencingGeneration: null, expectedVersion: 0, idempotencyKey: 'attach-run-process' }, { repo_key: repoId, canonical_root: repo, git_common_dir: join(repo, '.git'), autopilot_id: autopilotId, workstream: 'work-process' });
      ...
      const active: ActiveAutopilotRow = { schema_version: 'autopilot.active_parent.v1', autopilot_id: autopilotId, workstream: 'work-process', workstream_run: runId, repo_key: repoId, source_repo: repo, git_common_dir: join(repo, '.git'), worktree_root: join(stateRoot, 'worktrees', repoId), main_worktree_path: join(taskRoot, 'main'), branch: `autopilot/${runId}`, runtime_root: join(taskRoot, 'main', '.pi', 'autopilot', 'work-process'), target_branch: 'master', target_base_sha: git(repo, ['rev-parse', 'HEAD']), origin_url: null, pid: process.pid, boot_id: 'boot-process', status: 'active', started_at: '2026-07-11T00:00:00.000Z', active_run_epoch: 1, active_epoch_started_at: '2026-07-11T00:00:00.000Z', active_run_receipt_id: 'receipt-process' };
