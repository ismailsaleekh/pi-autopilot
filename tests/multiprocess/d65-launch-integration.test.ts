...
import { beginD65LaunchBootstrap, detectD65CharterComplete, publishD65FirstGraphAndSuccessorHeartbeat, registerD65LaunchPolicyAndInitialHeartbeat } from '../../src/core/coordination/d65-launch-integration.ts';
...
async function buildLaunchFixture(suffix: string): Promise<LaunchFixture> {
  ...
  const stateRoot = join(root, 'state'); const sessionRoot = join(coordinatorRuntimePaths({ ...process.env, [AUTOPILOT_STATE_ROOT_ENV]: stateRoot }).sessionsRoot);
  ...
  const rosterSha256 = sha256('fixed D65 Pi-subscription roster');
  ...
  const signer = new TestOperatorSigner({ privateKeyPem, programId, workstream, trustRef, trustSha256, programEvidenceRoot, packageCommit, packageTree, b0Commit, b0Tree, rosterSha256, rosterProvider, stateRoot });
