...
void describe('exact aa3e377 coordinator upgrade choreography', () => {
  void it('git-archives, builds, runs, and upgrades the actual executable predecessor to schema 9', async () => {
    ...
    try {
      ...
      assert.equal(response.payload['protocol_version'], '1.3');
      assert.equal(response.payload['database_schema_version'], 9);
      ...
      assert.equal(schemaVersion(paths.databasePath), 9);
  ...
  void it('keeps a real stale 1.2 binary powerless after the target exceeds the predecessor freshness window', async () => {
    ...
    try {
      ...
      assert.equal((await new CoordinatorClient({ env, autoStart: false }).query('status')).payload['protocol_version'], '1.3');
      // Simulate a wall-clock correction that invalidates every old-format boot
      // estimate, then remove lock defense entirely. Schema 9 itself must keep
  ...
  void it('recovers a hard-crashed upgrade owner and serializes racing target clients', async () => {
    ...
    try {
      ...
      const deterministicBackup = join(paths.backupsRoot, `coordinator.final.pre-upgrade-1.2-to-1.3.${interrupted.upgrade_id}.db`);
      ...
      assert.equal(responses.every((response) => response.payload['database_schema_version'] === 9), true);
