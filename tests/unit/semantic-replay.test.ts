...
function currentDatabasePath(paths: ReturnType<typeof coordinatorRuntimePaths>): string {
  const current = readCurrentStoreGeneration(paths);
  if (current === null) throw new Error('semantic replay assertion requires a published current generation');
  return current.database_path;
}

function eventCount(paths: ReturnType<typeof coordinatorRuntimePaths>): number {
  const database = new DatabaseSync(currentDatabasePath(paths), { readOnly: true });
  try {
    const row = database.prepare('SELECT COUNT(*) AS count FROM events').get();
    const count = row?.['count'];
    if (typeof count !== 'number') throw new Error('event count is not numeric');
    return count;
