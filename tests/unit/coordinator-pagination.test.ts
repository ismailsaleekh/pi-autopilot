...
void describe('BUG-176 protocol-1.6 byte-bounded coordinator pagination', () => {
  ...
  void it('migrates an oversized cf42 idempotency result into an exact compact replay plus durable detail pages', async () => {
    ...
    try {
      ...
      database.exec('DROP TABLE result_details; DROP TABLE result_receipts; DROP TABLE mailbox_delivery_items; DROP TABLE mailbox_deliveries; DROP TABLE reconciliation_details; DROP TABLE reconciliation_receipts; DELETE FROM schema_migrations WHERE version=12; PRAGMA user_version=11;');
