...
void describe('Coordination Fabric legacy migration and cutover', () => {
  ...
  void it('rejects arbitrary but well-formed migration checksums at both schema-6 and schema-9 boundaries', async () => {
    await withMigrationTestFixture(async (fixture) => {
      ...
      await assert.rejects(() => runCoordinationMigration({ command: 'dry-run', repoKey: fixture.repoKey, env: fixture.env, clock: migrationTestClock() }), /exact locked schema-6\/7\/8\/9 package lineage/u);
    });
    await withMigrationTestFixture(async (fixture) => {
      const paths = coordinatorRuntimePaths(fixture.env);
    ...
    await withFixture(async (fixture) => {
      ...
      try {
        const attached = store.handle({
          schema_version: 'autopilot.coordinator_request.v1', protocol_version: '1.3', request_id: 'mixed-attach', action: 'attach-run', idempotency_key: 'mixed-attach',
  ...
  void it('preserves mixed terminal coordinator state while fencing unmatched legacy WRITE authority as recovery work', async () => {
    await withFixture(async (fixture) => {
      ...
      try {
        const attached = store.handle({
          schema_version: 'autopilot.coordinator_request.v1', protocol_version: '1.3', request_id: 'terminal-mixed-attach', action: 'attach-run', idempotency_key: 'terminal-mixed-attach', repo_id: fixture.repoKey, workstream_run: active.workstream_run, session_id: null, fencing_generation: null, expected_version: 0,
  ...
  void it('uses an existing coordinator repository identity when empty legacy state has no active row and no --repo-root', async () => {
    await withEmptyMigrationTestFixture(async (fixture) => {
      ...
      try {
        const attached = store.handle({
          schema_version: 'autopilot.coordinator_request.v1', protocol_version: '1.3', request_id: 'empty-existing-repository', action: 'attach-run', idempotency_key: 'empty-existing-repository', repo_id: fixture.repoKey, workstream_run: 'coordinator-only-run', session_id: null, fencing_generation: null, expected_version: 0,
