...
import { coordinationRootForRepo, readActiveAutopilots, readPathClaims, writePathClaims } from '../../src/core/parallel-runtime.ts';
...
async function fixtureActive(fixture: { repoKey: string; env: NodeJS.ProcessEnv }): Promise<Awaited<ReturnType<typeof readActiveAutopilots>>[number]> {
...
void describe('real legacy corpus migration compatibility', () => {
  ...
  void it('bulk-retains every explicitly reviewed claim through one recovery-only fenced attachment', async () => {
    await withMigrationTestFixture(async (fixture) => {
      const active = await fixtureActive(fixture);
      const root = coordinationRootForRepo(fixture.repoKey, fixture.env);
      const claim = (await readPathClaims(root))[0];
      if (claim === undefined) throw new Error('missing fixture claim');
      await writePathClaims(root, [claim, { ...claim, path: 'src/second-future.ts', acquired_at: '2026-07-12T11:01:01.000Z' }]);
