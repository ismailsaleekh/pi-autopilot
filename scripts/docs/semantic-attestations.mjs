 ...
 * against the frontmatter hash. Existing artifacts are always validated too so a
 * stale receipt cannot sit in-tree silently.
 *
...
/**
 * @param {{ docId: string, coversSources: readonly string[] }} doc
 * @param {{ bodyHash: string }} hashes
...
export function validateSemanticAttestation(doc, hashes, attestation) {
  ...
  if (record.schema_version !== ATTESTATION_SCHEMA) failures.push(`semantic attestation has wrong schema_version (${String(record.schema_version)})`);
