...
export interface AutopilotToolCallContextLike {
  readonly cwd?: string;
}

export interface AutopilotGitGuardPolicy {
...
export interface AutopilotGuardAllowDecision {
  readonly block?: false;
  readonly reason?: undefined;
}

export type AutopilotGuardDecision = AutopilotGuardBlockDecision | AutopilotGuardAllowDecision | undefined;

type ShellSeparator = '&&' | '||' | ';' | '|';

interface ShellSegment {
  readonly text: string;
  readonly nextSeparator: ShellSeparator | null;
}

interface SegmentEvaluation {
  readonly blockReason: string | null;
  readonly nextCwd: string | null | undefined;
}
...
export function evaluateAutopilotWorktreeToolCall(
  event: AutopilotToolCallEventLike,
  ctx: AutopilotToolCallContextLike,
  policy: AutopilotGitGuardPolicy,
...
function evaluateShellCommandForGit(
  command: string,
  initialCwd: string,
  root: string,
  label: string,
  ...
  if (depth > 8) return `${label}: nested shell command depth exceeded while checking git worktree scope.`;
  const substitutionReason = evaluateCommandSubstitutions(command, initialCwd, root, label, depth);
  if (substitutionReason !== null) return substitutionReason;

  let currentCwd: string | null = initialCwd;
  for (const segment of splitShellSegments(command)) {
    const evaluation = evaluateShellSegment(segment.text, currentCwd, root, label, depth);
    if (evaluation.blockReason !== null) return evaluation.blockReason;
    if (evaluation.nextCwd !== undefined) {
      if (segment.nextSeparator === ';' || segment.nextSeparator === '&&') currentCwd = evaluation.nextCwd;
    }
...
function evaluateShellSegment(
  ...
  if (tokens.length === 0) return { blockReason: null, nextCwd: undefined };
  ...
  if (wrapped !== null) return { blockReason: wrapped, nextCwd: undefined };
  ...
  if (gitIndexes.length > 0) {
    ...
    if (envReason !== null) return { blockReason: envReason, nextCwd };
  }
  for (const index of gitIndexes) {
    const reason = evaluateGitInvocation(tokens, index, cwd, root, label);
    if (reason !== null) return { blockReason: reason, nextCwd };
  }
  return { blockReason: null, nextCwd };
}

function evaluateGitInvocation(
  ...
  for (let index = gitIndex + 1; index < tokens.length; index += 1) {
    ...
    if (token === '-C') {
      ...
      if (raw === undefined || raw.length === 0) return `${label}: git -C requires a path.`;
...
function gitEnvironmentAssignmentReason(tokens: readonly string[], label: string): string | null {
...
function isGitScopeEnvironmentKey(key: string): boolean {
...
function isGitScopeConfigPair(configPair: string): boolean {
  return /^core\.(?:worktree|bare)=/iu.test(configPair);
}

function branchMutationRequested(tokens: readonly string[]): boolean {
...
function tagMutationRequested(tokens: readonly string[]): boolean {
  if (tokens.some((token) =>
    token === '-d' ||
    token === '--delete' ||
    token === '-f' ||
...
function containsBranchOrTagNameArgument(tokens: readonly string[], readOnlyModes: ReadonlySet<string>): boolean {
...
function evaluateShellWrapper(
  ...
  if (command === undefined || !SHELL_WRAPPERS.has(command)) return null;
...
function shellScriptArgumentIndex(tokens: readonly string[], start: number): number | null {
  for (let index = start; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token === undefined) continue;
    if (token === '-c' || token === '-lc' || token === '-cl') return index + 1 < tokens.length ? index + 1 : null;
    if (token.startsWith('-') && token.includes('c')) return index + 1 < tokens.length ? index + 1 : null;
  }
  return null;
}

function cdTargetFromTokens(tokens: readonly string[], cwd: string | null): string | null | undefined {
  const commandIndex = firstCommandIndex(tokens);
  if (commandIndex === null) return undefined;
  if (tokens[commandIndex] !== 'cd') return undefined;
...
function firstCommandIndex(tokens: readonly string[]): number | null {
  for (let index = 0; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token === undefined) continue;
    if (isEnvironmentAssignment(token)) continue;
    if (token === 'command' || token === 'builtin' || token === 'time') continue;
...
function gitTokenIndexes(tokens: readonly string[]): readonly number[] {
  const indexes: number[] = [];
  for (let index = 0; index < tokens.length; index += 1) {
    if (tokens[index] === 'git') indexes.push(index);
  }
  return Object.freeze(indexes);
}

function evaluateCommandSubstitutions(
  command: string,
  cwd: string,
  root: string,
  label: string,
...
function splitShellSegments(command: string): readonly ShellSegment[] {
  ...
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (quote !== null) {
      buffer += char;
      if ((quote === 'single' && char === "'") || (quote === 'double' && char === '"')) quote = null;
      continue;
    }
    if (char === "'") {
      quote = 'single';
...
function pushSegment(segments: ShellSegment[], raw: string, nextSeparator: ShellSeparator | null): void {
  const text = raw.trim();
  if (text.length > 0) segments.push({ text, nextSeparator });
}
...
function extractDollarParenCommands(command: string): readonly string[] {
  const out: string[] = [];
  let index = 0;
  while (index < command.length) {
    ...
    if (depth === 0) out.push(command.slice(start + 2, cursor - 1));
    index = cursor;
  }
  return Object.freeze(out);
}

function extractBacktickCommands(command: string): readonly string[] {
  const out: string[] = [];
  let cursor = 0;
...
function containsUnquotedGrouping(segment: string): boolean {
  let quote: 'single' | 'double' | null = null;
  for (let index = 0; index < segment.length; index += 1) {
    const char = segment.charAt(index);
    if (quote !== null) {
...
function canonicalAllowedWriteRoots(paths: readonly string[], fallbackRoot: string): readonly string[] {
  return Object.freeze(paths.map((path) => canonicalCandidatePath(path, fallbackRoot)));
}

function canonicalCandidatePath(path: string, fallbackRoot: string): string {
  const absolute = isAbsolute(path) ? resolve(path) : resolve(fallbackRoot, path);
  if (existsSync(absolute)) return resolve(realpathSync(absolute));
...
function block(reason: string): AutopilotGuardBlockDecision {
