...
export function parseAutopilotExecutionCommit(value: unknown): AutopilotExecutionCommit {
  assertExecutionCommitShape(value);
  const commit = value as AutopilotExecutionCommit;
...
function assertStateShape(value: unknown): void {
  ...
  if (record !== undefined) {
    ...
    if (Array.isArray(record['operator_questions']) && record['operator_questions'].length > 0) issues.push('/operator_questions: operator decisions are coordinator-owned planning-contradiction packets and must not be synthesized in progress state');
...
function assertExecutionAuditShape(value: unknown): void {
  ...
  if (record !== undefined) {
    ...
    if (hasKey(record, 'baseline_head') && record['baseline_head'] !== null) expectString(record['baseline_head'], '/baseline_head', issues, { max: 80 });
    if (hasKey(record, 'post_run_head') && record['post_run_head'] !== null) expectString(record['post_run_head'], '/post_run_head', issues, { max: 80 });
    optionalEnum(record, 'head_change_kind', AUTOPILOT_HEAD_CHANGE_KIND_VALUES, '/head_change_kind', issues);
    if (hasKey(record, 'committed_changed_paths')) expectStringArray(record['committed_changed_paths'], '/committed_changed_paths', issues, 500);
...
function assertExecutionCommitShape(value: unknown): void {
  const issues: string[] = [];
  ...
  if (record !== undefined) {
    ...
    expectString(record['unit_id'], '/unit_id', issues, { pattern: UNIT_ID, max: 128 });
    expectEnum(record['role'], ['implement', 'fix'] as const, '/role', issues);
    expectInteger(record['attempt'], '/attempt', issues, 1, 999);
    expectString(record['cwd'], '/cwd', issues, { max: 1024 });
    ...
    if (hasKey(record, 'commit_shas')) expectStringArray(record['commit_shas'], '/commit_shas', issues, 500, 1, 80);
...
function semanticExecutionCommitIssues(commit: AutopilotExecutionCommit): string[] {
  const issues: string[] = [];
  for (const [field, path] of [
    ...
    for (const value of path) issues.push(...relativePathIssues(value, `${field} entry`));
  }
  if (commit.edited_claimed_paths.length === 0) issues.push('edited_claimed_paths must not be empty');
  ...
  if (commit.after_head !== commit.commit_sha) issues.push('after_head must equal commit_sha');
  if (commit.before_head === commit.after_head) issues.push('before_head and after_head must differ');
...
export interface AutopilotRuntimeReferenceResolution {
  readonly runtime_relative_ref: string;
 ...
 */
export function resolveAutopilotRuntimeReference(workstream: string, ref: string): AutopilotRuntimeReferenceResolution | null {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(workstream) || ref.length > 512 || ref !== ref.normalize('NFC') || ref.split('/').some((segment) => segment.length === 0) || relativePathIssues(ref, 'runtime reference').length > 0) return null;
  ...
  if (ref.startsWith(reservedPrefix)) {
    ...
    if (!ref.startsWith(exactPrefix)) return null;
    runtimeRelativeRef = ref.slice(exactPrefix.length);
  }
  if (runtimeRelativeRef.length === 0 || runtimeRelativeRef.split('/').some((segment) => segment.length === 0) || relativePathIssues(runtimeRelativeRef, 'runtime-relative reference').length > 0) return null;
...
function deriveRuntimeRootFromArtifactOutputs(spec: AutopilotUnitSpec): string | null {
  ...
  ].filter((root): root is string => root !== null && root.endsWith(expectedSuffix));
  for (const root of roots) {
    if (roots.filter((candidate) => candidate === root).length >= 2) return root;
  }
  const [only] = roots;
  return roots.length === 1 && only !== undefined ? only : null;
}

function rootBeforeArtifactSegment(pathValue: string, segment: 'statuses' | 'receipts' | 'evidence'): string | null {
