...
import { gitQueryNulStrings, gitQueryText, runGitMutation } from './git-process.ts';
...
export async function commitAutopilotExecution(input: {
  ...
  if (dirtyClaimedPaths.length > 0) commitSubject = `autopilot runtime commit ${input.spec.unit_id} attempt ${String(input.spec.attempt)}`;
  const commitPath = input.commitPath ?? deriveAutopilotExecutionCommitPath(input.spec);
  let durableRecord: AutopilotExecutionCommit | null = null;
  const persistExecutionCommit = async (): Promise<AutopilotExecutionCommit> => {
    ...
    if (afterSourceDirty.length > 0) fail('post-commit-source-dirty', 'runtime commit left source paths dirty after commit.', afterSourceDirty);
  ...
  await executeOwnedWorktreeSaga({
    ...
    action: async () => {
      ...
      await runGitMutation({ descriptor: { kind: 'stage-paths', paths: dirtyClaimedPaths }, cwd: input.spec.cwd, env: runtimeGitEnv(input.env) });
      ...
      await runGitMutation({ descriptor: { kind: 'commit', message: commitSubject }, cwd: input.spec.cwd, env: runtimeGitEnv(input.env) });
      runtimeCommitCreated = gitHead(input.spec.cwd) !== beforeHead;
    },
    finalize: async () => {
...
function committedDiffPaths(cwd: string, beforeHead: string, afterHead: string): readonly string[] {
  return Object.freeze(gitQueryNulStrings({ descriptor: { kind: 'diff-paths', from: beforeHead, to: afterHead }, cwd }).map((path) => path.replace(/\\/gu, '/')).sort());
}

function commitRange(cwd: string, beforeHead: string, afterHead: string): readonly string[] {
  const output = gitQueryText({ descriptor: { kind: 'rev-list-range', fromExclusive: beforeHead, toInclusive: afterHead, reverse: true }, cwd });
...
function currentBranch(cwd: string): string {
  const branch = gitQueryText({ descriptor: { kind: 'current-branch' }, cwd }).trim();
  if (branch.length === 0 || branch === 'HEAD') fail('detached-execution-head', 'source-changing runtime commit requires the unit worktree to be on a named branch.');
  return branch;
}
