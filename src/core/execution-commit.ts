...
export async function commitAutopilotExecution(input: {
  ...
  const inspectCommit = (): WorktreeSagaInspection => {
    ...
    if (currentDirty.some((path) => !dirtyClaimedPaths.includes(path))) return { outcome: 'unsafe', proof: currentDirty.map((path) => `unexpected_dirty=${path}`) };
    if (currentDirty.length > 0) return { outcome: 'not-applied', proof: currentDirty.map((path) => `dirty=${path}`) };
    if (currentHead !== beforeHead) {
      ...
      if (actualPaths.length !== expectedPaths.length || actualPaths.some((path, index) => path !== expectedPaths[index])) return { outcome: 'unsafe', proof: [...expectedPaths.map((path) => `expected_path=${path}`), ...actualPaths.map((path) => `actual_path=${path}`)] };
  ...
  await executeOwnedWorktreeSaga({
    ...
    verify: async () => {
      ...
      if (inspected.outcome !== 'satisfied') fail('commit-saga-postcondition', 'execution commit saga did not produce a clean advanced HEAD.', inspected.proof);
