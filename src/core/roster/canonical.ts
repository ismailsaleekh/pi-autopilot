...
export type RosterJsonPrimitive = string | number | boolean | null;
export type RosterJsonValue = RosterJsonPrimitive | readonly RosterJsonValue[] | RosterJsonObject;
export interface RosterJsonObject {
  readonly [key: string]: RosterJsonValue | undefined;
}

export type RosterSha256Digest = `sha256:${string}`;

export const AUTOPILOT_ROSTER_CANONICAL_HASH_ALGORITHM =
  'autopilot.phase37.canonical-json.sha256.v1' as const;

const SHA256_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const WHITESPACE = new Set([' ', '\n', '\r', '\t']);

type ObjectScanExpectation = 'keyOrEnd' | 'colon' | 'value' | 'commaOrEnd';
type ArrayScanExpectation = 'valueOrEnd' | 'commaOrEnd';

interface ObjectScanFrame {
  readonly kind: 'object';
  readonly keys: Set<string>;
  expect: ObjectScanExpectation;
}

interface ArrayScanFrame {
  readonly kind: 'array';
  expect: ArrayScanExpectation;
}
...
export function assertRosterSha256Digest(value: string): asserts value is RosterSha256Digest {
...
export function parseRosterJsonWithDuplicateKeyRejection(text: string): unknown {
  assertNoDuplicateJsonObjectKeys(text);
  return JSON.parse(text) as unknown;
}

export function canonicalRosterJson(value: unknown): string {
  return `${canonicalizeJsonValue(value, '$')}\n`;
}

export function rosterCanonicalSha256(value: unknown): RosterSha256Digest {
  return sha256Utf8(canonicalRosterJson(value));
}

export function rosterCanonicalSha256Hex(value: unknown): string {
  return rosterCanonicalSha256(value).slice('sha256:'.length);
}

export function rosterCanonicalSha256OmittingOwnFields(
  value: unknown,
...
export function rosterCanonicalSha256OmittingOwnField(
  value: unknown,
  omittedField: string,
): RosterSha256Digest {
...
export function sha256Utf8(text: string): RosterSha256Digest {
  return `sha256:${createHash('sha256').update(text, 'utf8').digest('hex')}`;
}

export function sha256Bytes(bytes: Uint8Array): RosterSha256Digest {
...
export function assertNoDuplicateJsonObjectKeys(text: string): void {
  ...
  const currentFrame = (): ScanFrame | undefined => stack[stack.length - 1];
  ...
  const afterValue = (): void => {
    ...
    if (frame.kind === 'object') {
      if (frame.expect !== 'value') fail('object value appeared outside a property value position');
      frame.expect = 'commaOrEnd';
      return;
    }
    if (frame.expect !== 'valueOrEnd') fail('array value appeared outside an array value position');
    frame.expect = 'commaOrEnd';
  };
  const closeContainer = (kind: 'object' | 'array'): void => {
    const frame = currentFrame();
  ...
  const parseStringToken = (): string | null => {
    const start = index;
    index += 1;
  ...
  while (index < text.length && issues.length === 0) {
    ...
    if (frame.kind === 'object') {
      if (frame.expect === 'keyOrEnd') {
        ...
        if (key !== null) {
          if (frame.keys.has(key)) fail(`duplicate object member ${JSON.stringify(key)}`);
          frame.keys.add(key);
          frame.expect = 'colon';
        }
        continue;
  ...
  if (issues.length === 0) {
    skipWhitespace();
    if (stack.length > 0) issues.push('unterminated JSON container');
    if (!rootComplete) issues.push('missing root JSON value');
    if (index < text.length) issues.push(`unexpected trailing bytes at byte ${String(index)}`);
  }
  if (issues.length > 0) throw new AutopilotRosterCanonicalizationError(issues);
}

function canonicalizeJsonValue(value: unknown, path: string): string {
...
function omitOwnFields(value: unknown, omittedFields: readonly string[]): RosterJsonObject {
  if (!isPlainObject(value)) {
  ...
  const output: Record<string, RosterJsonValue> = {};
...
function toJsonValue(value: unknown, path: string): RosterJsonValue {
  canonicalizeJsonValue(value, path);
  return value as RosterJsonValue;
}
