...
export const PROVIDER_RECIPES: readonly ProviderRecipe[] = Object.freeze(
  (JSON.parse(PROVIDER_RECIPES_JSON) as ProviderRecipe[]).map((recipe) => Object.freeze(recipe)),
);

export const SEED_ROSTERS: readonly Roster[] = Object.freeze(
  (JSON.parse(SEED_ROSTERS_JSON) as Roster[]).map((roster) => Object.freeze(roster)),
);

export const SEED_CANDIDATES: readonly RosterCandidate[] = Object.freeze(
  (JSON.parse(SEED_CANDIDATES_JSON) as RosterCandidate[]).map((candidate) => Object.freeze(candidate)),
);
...
export const PROVIDER_RECIPE_REGISTRY: ProviderRecipeRegistry = Object.freeze(computeProviderRecipeRegistry());
...
export const SEED_CANDIDATE_REGISTRY: SeedCandidateRegistry = Object.freeze(computeSeedCandidateRegistry());
...
export function verifyProviderRecipeSeeds(
  ...
  for (const recipe of sortProviderRecipes(recipes)) {
    ...
    seen.add(identity);
    const expectedRecipeHash = hashObjectOmitting(recipe as unknown as Record<string, unknown>, 'recipe_sha256');
...
export function verifySeedCandidateRegistry(candidates: readonly RosterCandidate[] = SEED_CANDIDATES): readonly string[] {
  ...
  for (const candidate of sortRosterCandidates(candidates)) {
    ...
    seen.add(identity);
    const expected = hashObjectOmitting(candidate as unknown as Record<string, unknown>, 'candidate_sha256');
...
export function buildAssignment(
  ...
  const routeDiagnostics = validateRouteConformance(assignment);
...
export function buildRosterFromRecipe(options: {
  ...
  const assignments = sortAssignmentsByRole(
    options.profile.role_templates.map((roleTemplate) => buildAssignment(roleTemplate, options.routePolicy, options.provider)),
  );
...
function exactQualificationManifestForRecipe(
  ...
  if (
    manifest.schema_version !== 'autopilot.certification_manifest.v1' ||
    manifest.subject_kind !== 'provider_recipe' ||
    manifest.subject_id !== recipe.recipe_id ||
    ...
    manifest.qualification_state !== 'synthetic-test-ready'
  ) {
    return false;
  }
  if (new Date(manifest.expires_at).getTime() <= new Date(manifest.issued_at).getTime()) {
    return false;
  }
  const roleOrder = manifest.role_results.map((result) => result.role);
  if (JSON.stringify(roleOrder) !== JSON.stringify(ROSTER_ROLE_ORDER)) {
    return false;
  }
  if (!manifest.role_results.every((result) => result.state === 'synthetic-pass' && result.evidence_refs.length > 0)) {
    return false;
  }
  const allEvidence = [...manifest.required_evidence, ...manifest.live_evidence, ...manifest.role_results.flatMap((role) => role.evidence_refs)];
  if (allEvidence.length === 0 || !allEvidence.every((evidence) => evidence.secret_free === true)) {
    return false;
  }
  if (!manifest.role_results.every((result) => result.evidence_refs.every((evidence) => evidence.kind === 'synthetic-fixture'))) {
    return false;
  }
  return hashObjectOmitting(manifest as unknown as Record<string, unknown>, 'manifest_sha256') === manifest.manifest_sha256;
}
...
function readinessForCandidate(
  ...
  const hasPriorityProof = qualificationManifest?.required_evidence.some((evidence) => evidence.kind === 'billing-proof' && evidence.secret_free === true) ?? false;
  if (exactQualificationManifestForRecipe(recipe, qualificationManifest) && (!priorityProofRequired || hasPriorityProof)) {
    return {
      candidate_state: 'synthetic-fixture-ready',
      launch_readiness: 'synthetic-fixture-only',
      qualification_state: 'synthetic-test-ready',
      synthetic_fixture_ready_only: true,
      diagnostic_codes: [],
    };
  }
  return {
...
export function resolveRecipe(
  ...
  diagnostics.push(...validateInventoryForProfile(normalizedInventory, recipe, profile, routePolicy).map((diagnostic) => diagnostic.code));
