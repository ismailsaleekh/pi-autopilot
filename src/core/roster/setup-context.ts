...
export async function resolveRosterSetupInventoryFromContext(
  ...
  for (const model of models) {
    ...
    const routePolicy = findRoutePolicyForProviderApi(
      providerId,
      api,
      ROUTE_POLICIES,
      routeAuthClass,
...
function inventoryAuthClass(
  ...
  if (!status.configured) return null;
  if (isUsingOAuth) return 'oauth';
  const policy = routePolicy ?? findRoutePolicyForProviderApi(providerId, api, ROUTE_POLICIES);
  const concreteAllowed = policy?.allowed_auth_classes.filter(
    (entry) => entry !== 'none' && entry !== 'unknown' && entry !== 'oauth',
  ) ?? [];
