...
import type { AutopilotReceipt, AutopilotReceiptV2, AutopilotStatusEntry, AutopilotUnitSpec } from '../contracts/types.ts';
import {
  autopilotExpectedIdentityHash,
  autopilotObservedProfileMismatches,
...
export interface AutopilotValidatedStatusEvidence {
  ...
  readonly finalModelMetadata: Readonly<Record<string, unknown>> | null;
}
...
export async function validateAutopilotStatusEvidence(input: {
  ...
  readonly observedExecution?: AutopilotObservedExecutionEvidence | null;
  ...
  const expectedIdentityHash = autopilotExpectedIdentityHash(
    expectedAutopilotStatusIdentityFromSpec(spec, providerIdentity, input.rosterExecutionIdentity),
  );
...
async function validateRosterReceiptEvidence(input: {
  ...
  readonly rawReceipt: Record<string, unknown>;
  readonly providerIdentity: AutopilotProviderIdentity;
  readonly schemaSha256: `sha256:${string}`;
  readonly expectedIdentityHash: `sha256:${string}`;
  ...
  let carrierReceipt: AutopilotReceiptV2;
  try {
...
function rosterCarrierReceiptMismatches(input: {
  readonly receipt: AutopilotReceiptV2;
  readonly spec: AutopilotUnitSpec;
  readonly schemaSha256: `sha256:${string}`;
  readonly expectedIdentityHash: `sha256:${string}`;
  readonly statusSha256: `sha256:${string}`;
...
function parseJsonObjectText(text: string, path: string, label: string): Record<string, unknown> {
  let parsed: unknown;
  try {
...
async function writeReceiptJson(path: string, receipt: AutopilotReceiptV2): Promise<void> {
