...
type AuthenticatedRosterAuthority = {
  readonly roster: ReturnType<typeof parseAutopilotRoster>;
  readonly paths: ReturnType<typeof resolveRosterScopePaths>;
};
...
async function readAuthenticatedRosterForSelection(input: {
  ...
  readonly stateRoot: string | undefined;
}): Promise<AuthenticatedRosterAuthority> {
...
async function readAuthenticatedRosterForSavedRef(input: {
  ...
  const matches: AuthenticatedRosterAuthority[] = [];
  ...
  if (match === undefined) throw new Error('saved target roster match disappeared');
  return match;
}
