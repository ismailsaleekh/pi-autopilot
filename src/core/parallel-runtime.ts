...
import { authorityCandidatesForSpec, deriveAutopilotAuthority, type AutopilotAuthorityArtifact } from './authority.ts';
import {
  AUTOPILOT_CHECKOUT_PROFILE_SNAPSHOT_FILE,
  checkoutProfileSnapshotFromResolved,
  parseCheckoutProfileSnapshot,
  readCheckoutProfileSnapshot,
  resolveAutopilotCheckoutProfile,
  sparseIncludePatternsForPaths,
  type AutopilotCheckoutDiskGateConfig,
  type AutopilotCheckoutMode,
  type AutopilotCheckoutProfileOrigin,
} from './checkout-profile.ts';
import { assertAutopilotDiskGate, AutopilotDiskGateError } from './disk-gate.ts';
import { buildS2CoordinationFailureDiagnostic } from './coordination/s2-diagnostics.ts';
import { clearDurableS2RetentionDiskPressure, readS2RetentionProgressState, recordS2RetentionDiskPressure, s2RetentionPressureStatePath, s2RetentionRunsPausedForWorktreeCreation } from './coordination/s2-retention-state-machine.ts';
import { applySparseCheckoutSet, createAutopilotGitWorktree } from './sparse-worktree.ts';
...
import { assertD65OrdinaryBoundaryFromEnvironment, assertD65RecoveryBoundaryFromEnvironment } from './coordination/d65-runtime-dispatch.ts';
import { parseCoordinationChildLease, parseCoordinationRun, parseCoordinationRunResource, parseCoordinationUnitAttempt, parseCoordinationWorktree, parseCoordinationWorktreeOperation } from './coordination/contracts.ts';
...
function s2RetentionRootForWorktreeRoot(worktreeRoot: string): string {
  return join(worktreeRoot, '_retention');
}

async function clearRecoveredS2WorktreeDiskPressure(input: { readonly worktreeRoot: string; readonly workstreamRun: string }): Promise<void> {
  ...
  if (!s2RetentionRunsPausedForWorktreeCreation(state).includes(input.workstreamRun)) return;
  const lane = state.lanes.find((candidate) => candidate.workstream_run === input.workstreamRun);
  if (lane?.disk_pressure_event_seq === null || lane?.disk_pressure_event_seq === undefined) return;
  await clearDurableS2RetentionDiskPressure(statePath, input.workstreamRun, lane.disk_pressure_event_seq);
}

async function recordS2WorktreeDiskPressure(input: { readonly worktreeRoot: string; readonly workstreamRun: string; readonly error: AutopilotDiskGateError; readonly now: Date }): Promise<void> {
  if (input.error.code !== 'insufficient-space') return;
  const retentionRoot = s2RetentionRootForWorktreeRoot(input.worktreeRoot);
  const eventSeq = input.now.getTime();
  await recordS2RetentionDiskPressure(s2RetentionPressureStatePath(retentionRoot), { offendingRun: input.workstreamRun, reason: input.error.code, eventSeq });
  await writeJsonAtomic(join(retentionRoot, 'diagnostics', input.workstreamRun, `disk-pressure-${String(eventSeq)}.json`), {
    schema_version: 'autopilot.s2_retention.disk_pressure_diagnostic.v1',
...
async function assertDiskGateOrRecordS2Pressure(input: { readonly active?: ActiveAutopilotRow; readonly worktreeRoot: string; readonly workstreamRun: string; readonly now: Date; readonly path: string; readonly projection: Parameters<typeof assertAutopilotDiskGate>[0]['projection'] }): Promise<void> {
  try {
    ...
    if (error instanceof AutopilotDiskGateError) await recordS2WorktreeDiskPressure({ worktreeRoot: input.worktreeRoot, workstreamRun: input.workstreamRun, error, now: input.now });
    throw error;
  }
}
...
export interface AutopilotTaskInfo {
  ...
  readonly status: AutopilotParentStatus;
  readonly checkout_mode: 'sparse' | 'full';
  readonly checkout_profile_ref: string | null;
  readonly checkout_profile_sha256: `sha256:${string}` | null;
  readonly checkout_profile_origin: AutopilotCheckoutProfileOrigin | 'legacy-full';
}
...
export interface AutopilotUnitInfo extends AutopilotUnitBranchInfo {
  ...
  readonly checkout_profile_ref: string | null;
  readonly materialized_paths_ref: string | null;
}
...
export async function prepareAutopilotUnitWorktree(input: {
  ...
  return await withAutopilotFileLock(lockPath, `unit-worktree:${input.active.autopilot_id}:${input.unitId}:${String(input.attempt)}`, async () => {
    ...
    let resumableUnitInfo: AutopilotUnitInfo | null = null;
    if (found !== undefined) {
      ...
      if (input.active.coordination_authority !== 'coordinator-edit-leases-v1') return { unitInfo: info, created: false, resumed: true };
      const client = await OwnedWorktreeSagaClient.fromEnvironment(env);
      const pending = (await client.operations()).filter((operation) => operation.owner.repo_id === input.active.repo_key && operation.owner.autopilot_id === input.active.autopilot_id && operation.owner.workstream_run === input.active.workstream_run && operation.owner.unit_id === input.unitId && operation.owner.attempt === input.attempt && operation.operation_type === 'create' && operation.stage !== 'committed' && operation.stage !== 'compensated' && operation.stage !== 'failed');
      if (pending.length === 0) return { unitInfo: info, created: false, resumed: true };
      if (pending.length !== 1) throw new CoordinationRuntimeError('store-corrupt', 'unit index owner has multiple incomplete durable create operations', pending.map((operation) => operation.operation_id));
    ...
    await assertDiskGateOrRecordS2Pressure({
      worktreeRoot: input.active.worktree_root,
    ...
    const pendingCreates = input.active.coordination_authority === 'coordinator-edit-leases-v1'
      ? (await (await OwnedWorktreeSagaClient.fromEnvironment(env)).operations()).filter((operation) => operation.owner.repo_id === input.active.repo_key && operation.owner.autopilot_id === input.active.autopilot_id && operation.owner.workstream_run === input.active.workstream_run && operation.owner.unit_id === input.unitId && operation.owner.attempt === input.attempt && operation.operation_type === 'create' && operation.stage !== 'compensated' && operation.stage !== 'failed')
      : [];
    if (pendingCreates.length > 1) throw new CoordinationRuntimeError('store-corrupt', 'unit worktree preparation found multiple durable create operations for one owner', pendingCreates.map((operation) => operation.operation_id));
    const pendingCreate = pendingCreates[0];
    let baseSha = gitHead(input.active.main_worktree_path);
    if (pendingCreate !== undefined) {
      const intent = pendingCreate.intent;
      if (!('worktree_path' in intent) || !('branch' in intent) || !('base_sha' in intent) || !('reason' in intent)) throw new CoordinationRuntimeError('store-corrupt', 'unit worktree create operation has a metadata-reconcile intent', [pendingCreate.operation_id]);
      if (intent.base_sha === null) throw new CoordinationRuntimeError('store-corrupt', 'unit worktree create operation lost its immutable base SHA');
      if (intent.worktree_path !== unitWorktreePath || intent.branch !== branch || intent.git_common_dir !== input.active.git_common_dir || intent.reason !== 'prepare source-changing unit worktree') throw new CoordinationRuntimeError('idempotency-conflict', 'durable unit worktree create operation differs from the requested deterministic owner identity', [pendingCreate.operation_id]);
      baseSha = intent.base_sha;
    }
    const unitInfo: AutopilotUnitInfo = resumableUnitInfo ?? {
    ...
    const createPhysicalWorktree = async (): Promise<void> => {
      if (existsSync(unitAttemptRoot) && !existsSync(unitWorktreePath) && (await readdir(unitAttemptRoot, { withFileTypes: true })).length > 0) fail('unit-worktree-path-exists', 'unit attempt path contains unrelated residue before recoverable worktree creation.', [unitAttemptRoot]);
    ...
    const finalizeUnitWorktree = async (): Promise<void> => {
      if (input.unitSpec !== undefined) await ensureFutureOwnedParentDirs(unitWorktreePath, input.unitSpec.owned_paths);
    ...
    if (input.active.coordination_authority === 'coordinator-edit-leases-v1') {
      ...
      }, { action: createPhysicalWorktree, finalize: finalizeUnitWorktree }, env);
    } else {
      await createPhysicalWorktree();
      await finalizeUnitWorktree();
    }
    return { unitInfo, created: resumableUnitInfo === null, resumed: resumableUnitInfo !== null };
  });
}

async function recoverUnitCreateSagaMetadata(active: ActiveAutopilotRow, operation: CoordinationWorktreeOperation, env: ProcessEnvLike): Promise<CoordinationWorktreeOperation> {
  if (operation.operation_type !== 'create' || operation.owner.unit_id === 'main') fail('unit-create-recovery-invalid', 'unit create recovery requires an incomplete unit create operation.', [operation.operation_id]);
  ...
  if (operation.intent.worktree_path !== unitWorktreePath || operation.intent.branch !== `autopilot/unit/${active.workstream_run}/${unitId}/attempt-${String(attempt)}`) fail('unit-create-recovery-owner-mismatch', 'durable unit create intent disagrees with deterministic owner paths.', [operation.operation_id]);
  const checkout = await checkoutMetadataForTaskRoot(taskRoot);
  const checkoutMode = operation.intent.checkout_mode === 'full' ? 'full' : 'sparse';
  const baseSha = operation.intent.base_sha;
  if (baseSha === null || operation.intent.checkout_mode === null) fail('unit-create-recovery-invalid', 'durable unit create intent lacks base SHA or checkout mode.', [operation.operation_id]);
  const unitAttemptRoot = dirname(unitWorktreePath);
  ...
  const result = await executeOwnedWorktreeSaga({
    ...
    action: async () => {
      if (existsSync(unitAttemptRoot) && !existsSync(unitWorktreePath) && (await readdir(unitAttemptRoot, { withFileTypes: true })).length > 0) fail('unit-worktree-path-exists', 'unit attempt path contains unrelated residue before recoverable worktree creation.', [unitAttemptRoot]);
      await mkdir(unitAttemptRoot, { recursive: true });
      if (!existsSync(unitWorktreePath)) await createAutopilotGitWorktree({ repoRoot: active.source_repo, worktreePath: unitWorktreePath, branch: operation.intent.branch, startPoint: baseSha, mode: operation.intent.checkout_mode ?? 'full', sparsePatterns: operation.intent.sparse_patterns, env: { ...env, [AUTOPILOT_RUNTIME_ENV]: AUTOPILOT_RUNTIME_VALUE } });
  ...
  if (result.operation === null) fail('unit-create-recovery-unmanaged', 'durable unit create recovery unexpectedly ran without coordinator authority.', [operation.operation_id]);
...
export async function recoverAutopilotWorktreeSagas(input: { readonly active: ActiveAutopilotRow; readonly env?: ProcessEnvLike }): Promise<readonly CoordinationWorktreeOperation[]> {
  ...
  if (!Array.isArray(childPayload) || !Array.isArray(attemptPayload) || !Array.isArray(worktreePayload)) throw new AutopilotParallelRuntimeError('coordination-status-invalid', 'coordinator status omitted child, attempt, or worktree projections during preflight rollback recovery.');
...
export async function updateUnitBranchStatus(input: {
  ...
  readonly env?: ProcessEnvLike;
  readonly recovery?: 'unit-recovery';
}): Promise<void> {
  const env = input.env ?? process.env;
  if (input.recovery === 'unit-recovery') await assertD65RecoveryBoundaryFromEnvironment('unit-recovery', { attached_session_current: true, policy_trust_current: true, no_pending_publication: true, terminal_prepared_cancellable: false, terminal_after_commit: false, accepted_continuation_reason: null, covered_semantic_reason: null, attach_terminal_recovery: false }, env);
  else await assertD65OrdinaryBoundaryFromEnvironment('ordinary-state-advance', env);
  const taskRoot = taskRootForActiveAutopilot(input.active);
...
export async function releaseReadClaimsForUnitPaths(input: {
  ...
  return await withAutopilotFileLock(join(input.context.coordinationRoot, '.locks', 'path-claims.lock'), `read-claim-release:${input.context.active.autopilot_id}:${input.unitId}:${String(input.attempt)}`, async () => {
    const current = await readPathClaims(input.context.coordinationRoot);
    const released = current.filter((claim) =>
...
interface TaskCheckoutMetadata {
  readonly mode: AutopilotCheckoutMode | 'legacy-full';
  readonly checkoutProfileRef: string | null;
...
async function checkoutMetadataForTaskRoot(taskRoot: string): Promise<TaskCheckoutMetadata> {
...
function materializationPathsForCheckoutBootstrap(spec: AutopilotUnitSpec): readonly string[] {
  return sortedUnique(authorityCandidatesForSpec(spec).map((candidate) => normalizeRepoRelativePath(candidate.path.replace(/\/\*\*$/u, ''))));
}

async function ensureFutureOwnedParentDirs(worktreePath: string, ownedPaths: readonly string[]): Promise<void> {
...
async function recoverCoordinatorBootstrapWorkstream(input: {
  ...
  if (!Array.isArray(runValues)) fail('coordinator-status-invalid', 'coordinator status omitted durable runs during bootstrap recovery.');
  ...
  if (candidates.length > 1) fail('ambiguous-bootstrap-run', 'multiple durable bootstrap runs require exact recovery before activation.', candidates.map((run) => run.workstream_run));
  ...
  if (!Array.isArray(operationValues)) fail('coordinator-status-invalid', 'coordinator status omitted bootstrap operations.');
  ...
  if (createOperations.length > 1) fail('bootstrap-operation-duplicate', 'durable bootstrap run owns duplicate main create operations.', [run.workstream_run]);
  ...
  if (!isRecord(bootstrap) || bootstrap['schema_version'] !== 'autopilot.worktree_bootstrap.v1') fail('bootstrap-state-invalid', 'worktree bootstrap state is malformed.', [bootstrapPath]);
  const parsedRows = parseLegacyActiveAutopilots([bootstrap['active']]);
  const active = parsedRows[0];
  if (active === undefined || active.repo_key !== input.repo.repoKey || active.workstream !== input.workstream || active.workstream_run !== run.workstream_run || active.autopilot_id !== run.autopilot_id || active.main_worktree_path !== expectedMainPath) fail('bootstrap-owner-mismatch', 'bootstrap state does not match durable coordinator ownership.', [bootstrapPath]);
  ...
  const recoveryEnv = { ...input.env, [AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV]: attachment.contextPath };
  {
    await executeOwnedWorktreeSaga({
      ...
      action: async () => {
        ...
        if (!existsSync(active.main_worktree_path)) await createAutopilotGitWorktree({ repoRoot: active.source_repo, worktreePath: active.main_worktree_path, branch: active.branch, startPoint: active.target_base_sha, mode: profileSnapshot.profile.mode, sparsePatterns: profileSnapshot.base_patterns, env: { ...input.env, [AUTOPILOT_RUNTIME_ENV]: AUTOPILOT_RUNTIME_VALUE } });
  ...
  if (!existsSync(active.main_worktree_path)) fail('bootstrap-recovery-incomplete', 'main worktree remains absent after durable create recovery.', [active.main_worktree_path]);
  await mkdir(active.runtime_root, { recursive: true });
  await writeJsonAtomic(join(taskRoot, AUTOPILOT_CHECKOUT_PROFILE_SNAPSHOT_FILE), profileSnapshot);
...
function requestedClaimsForAuthority(active: ActiveAutopilotRow, spec: AutopilotUnitSpec, authorityArtifact: AutopilotAuthorityArtifact, reason: string): readonly AutopilotPathClaim[] {
  ...
  for (const observation of authorityArtifact.observations) add(observation.path, 'READ');
  for (const edit of authorityArtifact.edit_intentions) add(edit.path, 'WRITE');
  for (const exclusive of authorityArtifact.exclusives) add(exclusive.path, 'EXCLUSIVE');
...
function effectiveClaims(stored: readonly AutopilotPathClaim[], requested: readonly AutopilotPathClaim[]): readonly AutopilotPathClaim[] {
  return Object.freeze(requested.map((claim) => {
    const effective = stored.find((candidate) =>
      candidate.autopilot_id === claim.autopilot_id &&
      candidate.workstream_run === claim.workstream_run &&
      candidate.unit_id === claim.unit_id &&
      candidate.attempt === claim.attempt &&
    ...
    if (effective === undefined) fail('claim-persistence-mismatch', 'granted durable claim is absent after atomic claim persistence.', [claim.path]);
    return effective;
  }));
}
...
async function upsertUnitBranchInfo(taskRoot: string, branchInfo: AutopilotUnitBranchInfo): Promise<void> {
  ...
  if (!existsSync(path)) fail('unit-branch-authority-missing', 'unit branch projection requires the durable run-main branch authority.', [path]);
  const branches = parseBranchesInfo(await readJson(path));
...
function assertBranchAvailable(repoRoot: string, branch: string): void {
  if (!runGitQuery({ descriptor: { kind: 'ref-exists', ref: `refs/heads/${branch}` }, cwd: repoRoot }).negative) fail('branch-exists', 'Autopilot branch already exists before worktree creation.', [branch]);
}

export interface AutopilotFileLockOptions {
  /** Bounded acquisition wait; exposed for deterministic fail-closed QA. */
  readonly timeoutMs?: number;
}

export async function withAutopilotFileLock<T>(lockPath: string, holderId: string, run: () => Promise<T>, options: AutopilotFileLockOptions = {}): Promise<T> {
...
async function acquireFileLock(lockPath: string, holderId: string, timeoutMs = DEFAULT_LOCK_TIMEOUT_MS): Promise<FileLockHandle> {
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > DEFAULT_LOCK_TIMEOUT_MS) fail('invalid-lock-timeout', 'Autopilot runtime lock timeout must be a positive bounded integer.', [String(timeoutMs)]);
...
async function reclaimStaleLockIfEligible(lockPath: string): Promise<void> {
  let text: string;
  try {
  ...
  if (parsed['schema_version'] !== 'autopilot.lock.v1' || typeof parsed['holder_id'] !== 'string' || typeof acquiredAtRaw !== 'string' || typeof pidRaw !== 'number' || !Number.isSafeInteger(pidRaw) || typeof bootIdRaw !== 'string') return;
  ...
  if (stale) {
    ...
    if (current === text) await unlink(lockPath).catch((error: unknown) => {
...
function optionalCheckoutMode(record: Record<string, unknown>, field: string): 'sparse' | 'full' | 'legacy-full' {
  const value = record[field];
  if (value === undefined) return 'legacy-full';
  if (value === 'sparse' || value === 'full' || value === 'legacy-full') return value;
  if (value === 'claim-minimal' || value === 'exclude-heavy') return 'sparse';
