...
export async function prepareAutopilotUnitWorktree(input: {
  ...
  return await withAutopilotFileLock(lockPath, `unit-worktree:${input.active.autopilot_id}:${input.unitId}:${String(input.attempt)}`, async () => {
    ...
    await executeOwnedWorktreeSaga({
      ...
      inspect: (): WorktreeSagaInspection => {
        if (!existsSync(unitWorktreePath)) return { outcome: 'not-applied', proof: ['unit_worktree_absent'] };
        const actualBranch = runGit(['rev-parse', '--abbrev-ref', 'HEAD'], unitWorktreePath, env).trim();
        if (actualBranch !== branch) return { outcome: 'unsafe', proof: [`expected_branch=${branch}`, `actual_branch=${actualBranch}`] };
        if (checkout.mode !== 'legacy-full' && checkout.mode !== 'full' && !isSparseCheckoutEnabled(unitWorktreePath, env)) return { outcome: 'not-applied', proof: ['unit_worktree_registered', 'sparse_configuration_incomplete'] };
      ...
      verify: async () => {
        ...
        if (actualBranch !== branch || actualHead !== baseSha) fail('unit-worktree-create-postcondition', 'unit worktree create saga postcondition failed.', [actualBranch, actualHead, branch, baseSha]);
        ...
        if (indexed === undefined || indexed.worktree_path !== unitWorktreePath || indexed.branch !== branch) fail('unit-worktree-create-postcondition', 'unit worktree metadata is incomplete after saga action.', metadataRefs);
...
async function recoverUnitCreateSagaMetadata(active: ActiveAutopilotRow, operation: CoordinationWorktreeOperation, env: ProcessEnvLike): Promise<CoordinationWorktreeOperation> {
  ...
  const inspect = (): WorktreeSagaInspection => {
    ...
    const actualBranch = runGit(['rev-parse', '--abbrev-ref', 'HEAD'], unitWorktreePath, env).trim();
    const actualHead = gitHead(unitWorktreePath);
    if (actualBranch !== operation.intent.branch || actualHead !== baseSha) return { outcome: 'unsafe', proof: [`expected_branch=${operation.intent.branch}`, `actual_branch=${actualBranch}`, `expected_head=${baseSha}`, `actual_head=${actualHead}`] };
    if (operation.intent.checkout_mode !== 'full' && !isSparseCheckoutEnabled(unitWorktreePath, env)) return { outcome: 'not-applied', proof: ['unit_worktree_registered', 'sparse_configuration_incomplete'] };
...
async function recoverCoordinatorBootstrapWorkstream(input: {
  ...
  {
    const inspect = (): WorktreeSagaInspection => {
      if (!existsSync(active.main_worktree_path)) return { outcome: 'not-applied', proof: ['main_worktree_absent'] };
      ...
      if (actualBranch !== active.branch || actualHead !== active.target_base_sha) return { outcome: 'unsafe', proof: [`actual_branch=${actualBranch}`, `actual_head=${actualHead}`] };
      if (profileSnapshot.profile.mode !== 'full' && !isSparseCheckoutEnabled(active.main_worktree_path, input.env)) return { outcome: 'not-applied', proof: ['main_worktree_registered', 'sparse_configuration_incomplete'] };
    ...
    await executeOwnedWorktreeSaga({
      ...
      verify: () => { const inspected = inspect(); if (inspected.outcome !== 'satisfied') fail('bootstrap-recovery-postcondition', 'recovered main create postcondition failed.', inspected.proof); return inspected.proof; },
    }, recoveryEnv);
  }
...
async function createNewWorkstream(input: {
  ...
  const inspectMainCreate = (): WorktreeSagaInspection => {
    if (!existsSync(mainWorktreePath)) return { outcome: 'not-applied', proof: ['main_worktree_absent'] };
    ...
    if (actualBranch !== branch || actualHead !== input.repo.headSha) return { outcome: 'unsafe', proof: [`expected_branch=${branch}`, `actual_branch=${actualBranch}`, `expected_head=${input.repo.headSha}`, `actual_head=${actualHead}`] };
    if (checkoutProfile.profile.mode !== 'full' && !isSparseCheckoutEnabled(mainWorktreePath, input.env)) return { outcome: 'not-applied', proof: ['main_worktree_registered', 'sparse_configuration_incomplete'] };
  ...
  await executeOwnedWorktreeSaga({
    ...
    verify: () => {
      ...
      if (inspected.outcome !== 'satisfied') fail('main-worktree-create-postcondition', 'main worktree create saga postcondition failed.', inspected.proof);
