...
export function deriveD65BootstrapTransaction(input: D65BootstrapTransactionInput): D65BootstrapRows {
  ...
  if (typeof resourceTargetBaseSha !== 'string' || resourceTargetBaseSha !== bootstrap.content_commit) throw new CoordinationRuntimeError('invalid-request', 'bootstrap run resource target_base_sha must equal the content-result commit (content_commit)', [String(resourceTargetBaseSha), bootstrap.content_commit]);
