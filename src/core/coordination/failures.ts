...
export type CoordinationFailureClass = 'client-invalid' | 'retryable-contention' | 'fenced-client' | 'owned-recovery' | 'contradiction-review' | 'system-fatal';
export type CoordinationRetryPolicy = 'never' | 'same-idempotency-key' | 'after-reattach' | 'after-reconciliation';

export interface CoordinationFailureDefinition {
  readonly code: CoordinationFailureCode;
  readonly failure_class: CoordinationFailureClass;
  readonly retry_policy: CoordinationRetryPolicy;
  readonly operator_decision: boolean;
  readonly description: string;
}

const definitions: readonly CoordinationFailureDefinition[] = [
...
export function coordinationFailureDefinition(code: CoordinationFailureCode): CoordinationFailureDefinition {
  const definition = COORDINATION_FAILURE_TAXONOMY.find((candidate) => candidate.code === code);
  if (definition === undefined) throw new Error(`Coordination failure taxonomy is incomplete for ${code}`);
  return definition;
}

export interface CoordinationFailureScenario {
  readonly scenario: string;
  readonly failure_code: CoordinationFailureCode | null;
  readonly durable_effect: 'none' | 'single-committed-effect' | 'unknown-until-replay' | 'prepared-recovery-work' | 'safety-halt';
  readonly required_response: string;
  readonly forbidden_response: string;
}

export const COORDINATION_FAILURE_MATRIX: readonly CoordinationFailureScenario[] = Object.freeze([
...
export function sanitizeCoordinationDiagnosticText(value: string, maximumCodePoints = COORDINATION_DIAGNOSTIC_MAX_TEXT_CODE_POINTS): string {
