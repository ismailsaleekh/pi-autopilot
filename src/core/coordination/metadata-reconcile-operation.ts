...
export interface PersistedMetadataReconcileBatchEntry {
  readonly client: CoordinatorClient;
  readonly session: CoordinatorSessionContext;
  readonly worktree_root: string;
  readonly approval: PersistedMetadataReconcileApproval;
}
...
interface PreparedBatchEntry extends PersistedMetadataReconcileBatchEntry {
  readonly operation: MetadataOperation;
}

async function markBatchRecovering(entries: readonly PreparedBatchEntry[], persistedOperationIds: ReadonlySet<string>, errorCode: string): Promise<readonly unknown[]> {
  const failures: unknown[] = [];
  for (const entry of entries) {
    if (!persistedOperationIds.has(entry.operation.operation_id)) continue;
    try {
...
export async function executeApprovedMetadataReconcileBatch(input: {
  readonly entries: readonly PersistedMetadataReconcileBatchEntry[];
  ...
  if (input.entries.length === 0) throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation operation consumer requires at least one approval');
  const prepared: PreparedBatchEntry[] = input.entries.map((entry) => ({ ...entry, operation: validateApproval(entry.session, entry.approval) }));
  ...
  if (operationIds.size !== prepared.length || canonicalIds.size !== prepared.length) throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation batch contains duplicate operation or canonical worktree authority');
  const first = prepared[0];
  if (first === undefined) throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation operation set disappeared');
  for (const entry of prepared) {
    ...
    if (entry.session.repo_id !== first.session.repo_id
      ...
      || entry.operation.intent.git_common_dir !== first.operation.intent.git_common_dir) throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation operation batch crosses repository or package-owned evidence authority');
  }
  ...
  try {
    ...
    if (transitionFailures.length > 0) throw new AggregateError([error, ...transitionFailures], 'metadata reconciliation preparation failed and durable recovery-state publication was incomplete');
    throw error;
  }
  ...
  for (const path of batch.evidence_paths) {
    ...
    if (evidencePaths.has(canonicalId)) throw new CoordinationRuntimeError('invalid-state', 'metadata reconciliation batch published duplicate operation evidence', [canonicalId]);
  ...
  for (const entry of prepared) {
    try {
      ...
      if (path === undefined) throw new CoordinationRuntimeError('invalid-state', 'metadata reconciliation batch omitted operation evidence', [entry.operation.intent.canonical_worktree_id]);
      const evidence = await evidenceRef(entry.worktree_root, entry.session, path);
      if (current.stage === 'prepared') current = await transition({ client: entry.client, session: entry.session, operation: current, stage: 'in-progress', completedSteps: [], currentStep: 'preflight-probe', recoveryAttempts: current.recovery_attempts, evidence: null, errorCode: null, worktreeState: entry.approval.worktree.state });
      if (current.stage === 'in-progress' || current.stage === 'reconciling') current = await transition({ client: entry.client, session: entry.session, operation: current, stage: 'verified', completedSteps: COMPLETED_STEPS, currentStep: null, recoveryAttempts: current.recovery_attempts, evidence, errorCode: null, worktreeState: entry.approval.worktree.state });
      if (current.stage === 'verified') current = await transition({ client: entry.client, session: entry.session, operation: current, stage: 'committed', completedSteps: COMPLETED_STEPS, currentStep: null, recoveryAttempts: current.recovery_attempts, evidence, errorCode: null, worktreeState: entry.approval.worktree.state });
    ...
    } catch (error) { transitionFailures.push(error); }
  }
  if (transitionFailures.length > 0) throw new AggregateError(transitionFailures, 'metadata reconciliation Git postcondition succeeded but one or more durable operation commits failed');
