...
import { AUTOPILOT_ROLE_VALUES, parseAutopilotExecutionAudit, parseAutopilotReceipt, parseAutopilotReceiptV2, parseAutopilotStatusEntry, parseAutopilotUnitSpec, parseAutopilotUnitSpecV2 } from '../contracts/index.ts';
...
const ACCEPTED_VERDICTS = new Set<AutopilotVerdict>(['DONE', 'PASS', 'NEEDS_FIX', 'BLOCKED']);

export interface AutopilotChildTerminalAcceptance {
  readonly schema_version: typeof AUTOPILOT_CHILD_TERMINAL_ACCEPTANCE_SCHEMA;
  ...
  readonly status: CoordinationEvidenceRef;
  readonly receipt: CoordinationEvidenceRef;
  readonly audit: CoordinationEvidenceRef;
  readonly tool_call_id: string;
  readonly carrier_status_sha256: `sha256:${string}`;
  readonly audit_disposition: 'zero-change' | 'accounted-changes';
...
function exact(value: JsonRecord, fields: readonly string[], label: string): void {
  ...
  if (actual.length !== expected.length || actual.some((field, index) => field !== expected[index])) throw new CoordinationRuntimeError('invalid-state', `${label} fields are not exact`, actual);
}

function text(value: JsonRecord, field: string, label: string, maximum = 512): string {
  const entry = value[field];
  if (typeof entry !== 'string' || entry.length < 1 || entry.length > maximum || entry.includes('\u0000')) throw new CoordinationRuntimeError('invalid-state', `${label}.${field} must be bounded non-empty text`);
  return entry;
}

function identifier(value: JsonRecord, field: string, label: string): string {
  const entry = text(value, field, label, 192);
  if (!/^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,191}$/u.test(entry)) throw new CoordinationRuntimeError('invalid-state', `${label}.${field} is not an identifier`);
  return entry;
}
...
function evidence(value: unknown, label: string): CoordinationEvidenceRef {
  ...
  if (ref.startsWith('/') || ref.startsWith('../') || ref.includes('/../') || ref === '..' || /^[A-Za-z]:/u.test(ref)) throw new CoordinationRuntimeError('invalid-state', `${label}.ref is not run-relative`);
  if (!SHA256.test(digest)) throw new CoordinationRuntimeError('invalid-state', `${label}.sha256 is not a SHA-256 digest`);
  return { ref, sha256: digest as `sha256:${string}` };
}

export function parseAutopilotChildTerminalAcceptance(value: unknown): AutopilotChildTerminalAcceptance {
  ...
  if (!ACCEPTED_VERDICTS.has(verdict)) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance verdict is invalid');
  if (entry['transport_result'] !== 'accepted') throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance transport_result must be accepted');
  const carrierStatusSha256 = text(entry, 'carrier_status_sha256', label, 71);
  if (!SHA256.test(carrierStatusSha256)) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance carrier_status_sha256 is invalid');
  const auditDisposition = text(entry, 'audit_disposition', label);
  if (auditDisposition !== 'zero-change' && auditDisposition !== 'accounted-changes') throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance audit_disposition is invalid');
...
function relativeEvidence(root: string, path: string, bytes: Uint8Array, label: string): CoordinationEvidenceRef {
  const ref = relative(resolve(root), resolve(path)).replace(/\\/gu, '/');
  if (ref.length === 0 || ref.startsWith('../') || ref.startsWith('/') || /^[A-Za-z]:/u.test(ref)) throw new CoordinationRuntimeError('unauthorized-client', `${label} is outside the durable run main worktree`, [path]);
...
export async function writeAutopilotChildTerminalAcceptance(input: {
  ...
  readonly now?: Date;
}): Promise<{ readonly path: string; readonly evidence: CoordinationEvidenceRef; readonly acceptance: AutopilotChildTerminalAcceptance }> {
  ...
  const existingAcceptance = async (): Promise<{ readonly path: string; readonly evidence: CoordinationEvidenceRef; readonly acceptance: AutopilotChildTerminalAcceptance } | null> => {
    ...
    if (existing.spec.ref !== expectedSpec.ref || existing.status.ref !== expectedStatus.ref || existing.receipt.ref !== expectedReceipt.ref || existing.audit.ref !== expectedAudit.ref) throw new CoordinationRuntimeError('idempotency-conflict', 'existing terminal acceptance artifact refs differ from the exact parent-validated inputs', [path]);
  ...
  try {
    try { await link(stagingPath, path); }
      ...
      if (raced === null) throw new CoordinationRuntimeError('recovery-required', 'terminal acceptance create race lost without a durable artifact', [path]);
      return raced;
    }
...
function parseTerminalUnitSpec(bytes: Uint8Array): { readonly original: AutopilotUnitSpec | AutopilotUnitSpecV2; readonly lowered: AutopilotUnitSpec } {
...
function parseTerminalReceipt(bytes: Uint8Array, spec: AutopilotUnitSpec | AutopilotUnitSpecV2): AutopilotReceipt | AutopilotReceiptV2 {
  ...
  if (isJsonRecord(parsed) && parsed['schema_version'] === 'autopilot.receipt.v2') {
    if (spec.schema_version !== 'autopilot.unit_spec.v2') throw new CoordinationRuntimeError('invalid-state', 'receipt.v2 terminal evidence requires unit_spec.v2 bytes');
    return parseAutopilotReceiptV2(parsed);
  }
  if (spec.schema_version === 'autopilot.unit_spec.v2') throw new CoordinationRuntimeError('invalid-state', 'unit_spec.v2 terminal evidence requires native receipt.v2 bytes');
...
function assertTerminalReceiptV2Compatibility(input: {
  ...
  readonly acceptance: AutopilotChildTerminalAcceptance;
}): void {
...
export function autopilotAuditProvesZeroSourceChange(audit: AutopilotExecutionAudit): boolean {
...
export function assertAutopilotChildTerminalAcceptanceChain(input: {
...
}): { readonly spec: AutopilotUnitSpec; readonly originalSpec: AutopilotUnitSpec | AutopilotUnitSpecV2; readonly status: AutopilotStatusEntry; readonly receipt: AutopilotReceipt | AutopilotReceiptV2; readonly audit: AutopilotExecutionAudit } {
  const { acceptance, child } = input;
  if (acceptance.repo_id !== child.owner.repo_id || acceptance.autopilot_id !== child.owner.autopilot_id || acceptance.workstream_run !== child.owner.workstream_run || acceptance.unit_id !== child.owner.unit_id || acceptance.attempt !== child.owner.attempt || acceptance.child_lease_id !== child.child_lease_id) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance identity differs from the authenticated child lease');
  ...
  ] as const) if (sha256(bytes) !== expected) throw new CoordinationRuntimeError('invalid-state', `terminal acceptance ${label} hash differs from its artifact bytes`);
  ...
  if (parsedSpec.original.workstream !== acceptance.workstream || parsedSpec.original.unit_id !== acceptance.unit_id || parsedSpec.original.role !== acceptance.role || parsedSpec.original.attempt !== acceptance.attempt || status.workstream !== acceptance.workstream || status.unit_id !== acceptance.unit_id || status.role !== acceptance.role || status.attempt !== acceptance.attempt || status.verdict !== acceptance.verdict || receipt.workstream !== acceptance.workstream || receipt.unit_id !== acceptance.unit_id || receipt.role !== acceptance.role || receipt.attempt !== acceptance.attempt || audit.workstream !== acceptance.workstream || audit.unit_id !== acceptance.unit_id || audit.role !== acceptance.role || audit.attempt !== acceptance.attempt) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance spec/status/receipt/audit identities disagree');
  if (receipt.tool_name !== 'autopilot_emit_status' || receipt.tool_call_id !== acceptance.tool_call_id || receipt.status_sha256 !== acceptance.status.sha256 || receipt.status_sha256 !== acceptance.carrier_status_sha256) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance does not bind the exact accepted forced-output carrier');
  if (receipt.schema_version === 'autopilot.receipt.v2') assertTerminalReceiptV2Compatibility({ specBytes: input.specBytes, receiptBytes: input.receiptBytes, acceptance });
  const disposition = autopilotAuditProvesZeroSourceChange(audit) ? 'zero-change' : 'accounted-changes';
  if (acceptance.audit_disposition !== disposition) throw new CoordinationRuntimeError('invalid-state', 'terminal acceptance audit disposition differs from the exact execution audit');
