...
const EXTRA_PERSISTED_ARTIFACT_SCHEMAS = Object.freeze([
  ...
  'autopilot.coordination_preflight.v1', 'autopilot.coordination_recovery_operation.v1', 'autopilot.coordinator_cursor.v1',
  ...
  'autopilot.saga_execution_lock.v1', 'autopilot.scheduler_config.v1', 'autopilot.semantic_graph.v1', 'autopilot.semantic_graph_authority_shard.v1',
...
const extraFamilies = EXTRA_PERSISTED_ARTIFACT_SCHEMAS
  ...
  .map((schemaVersion) => currentOnlyFamily(schemaVersion, schemaVersion.includes('coordinator_') ? 'transport-or-page' : 'runtime-evidence', Object.freeze(['schema_version']), Object.freeze(['schema_version']), 'schema-bearing persisted artifact discovered in runtime sources; exact hot-file parser hookup deferred'));
...
export function selectVersionedIngressProducer(input: {
  ...
  const generation = input.producer_generation ?? range.first_generation;
  if (!Number.isSafeInteger(generation) || generation < range.first_generation || generation > range.last_generation) throw new CoordinationRuntimeError('protocol-mismatch', 'persisted artifact producer generation is outside its exact producer_build fence', [family.family, input.producer_build, String(generation)]);
...
export function parseVersionedPersistedArtifact(input: {
  ...
  const selection = selectVersionedIngressProducer({ family: input.family, producer_build: input.producer_build, ...(input.producer_generation === undefined ? {} : { producer_generation: input.producer_generation }), ...(input.registry === undefined ? {} : { registry: input.registry }) });
...
export function parseVersionedUnitFailureIngress(input: {
  ...
  const ingress = parseVersionedPersistedArtifact({ family: 'autopilot.unit_failure.v1', producer_build: input.producer_build, bytes: input.bytes, ...(input.producer_generation === undefined ? {} : { producer_generation: input.producer_generation }) });
