...
function text(value: JsonMap, field: string, label: string): string {
  ...
  if (typeof entry !== 'string' || entry.length === 0 || entry.length > 2048) throw new CoordinationRuntimeError('invalid-state', `${label}.${field} must be bounded non-empty text`);
  return entry;
}
...
export function validateReconciliationEvidenceDocument(bytes: Uint8Array, expected: ReconciliationEvidenceIdentity): void {
  ...
  switch (expected.source) {
    case 'child-process':
      if (schema !== 'autopilot.receipt.v1') throw new CoordinationRuntimeError('invalid-state', 'child-terminal evidence must be an autopilot.receipt.v1 artifact');
      if (text(document, 'workstream', 'reconciliation evidence') !== expected.workstream) throw new CoordinationRuntimeError('invalid-state', 'child receipt workstream does not match durable ownership');
      if (expected.unitId === null || expected.attempt === null || text(document, 'unit_id', 'reconciliation evidence') !== expected.unitId || integer(document, 'attempt', 'reconciliation evidence') !== expected.attempt) throw new CoordinationRuntimeError('invalid-state', 'child receipt identity does not match its process lease');
      if (text(document, 'tool_name', 'reconciliation evidence') !== 'autopilot_emit_status') throw new CoordinationRuntimeError('invalid-state', 'child receipt does not prove the forced status carrier');
      return;
    case 'unit-merge':
