...
export type HistoricalUnitFailureGeneration = (typeof HISTORICAL_UNIT_FAILURE_GENERATIONS)[keyof typeof HISTORICAL_UNIT_FAILURE_GENERATIONS];

export interface HistoricalUnitFailureEvidenceProvenance {
  readonly kind: 'coordinator-accepted-before-schema10';
...
export interface HistoricalUnitFailureEvidenceFacts {
  ...
  readonly action: 'reset' | 'abort';
  readonly unitWorktreePath: string;
  readonly captureCommitSha: null;
  readonly captureRef: null;
  readonly originalSha256: `sha256:${string}`;
  readonly originalFields: readonly string[];
}

export type UnitFailureEvidenceIngress =
  | { readonly kind: 'current'; readonly facts: UnitFailureEvidenceFacts }
  | { readonly kind: 'historical'; readonly facts: HistoricalUnitFailureEvidenceFacts; readonly provenance: HistoricalUnitFailureEvidenceProvenance };
...
function assertExactFields(document: JsonMap, expectedFields: readonly string[], label: string): void {
  const actual = Object.keys(document).sort();
  if (actual.length !== expectedFields.length || actual.some((field, index) => field !== expectedFields[index])) throw new CoordinationRuntimeError('invalid-state', `${label} fields are incompatible`, actual);
}

function parseFailureAction(document: JsonMap): 'quarantine' | 'reset' | 'preserve' | 'abort' {
...
function assertHistoricalCommonFields(document: JsonMap, expected: ReconciliationEvidenceIdentity): void {
  ...
  if (!Number.isFinite(Date.parse(createdAt))) throw new CoordinationRuntimeError('invalid-state', 'historical unit failure evidence created_at is invalid');
...
export function parseUnitFailureEvidenceIngress(bytes: Uint8Array, expected: ReconciliationEvidenceIdentity, provenance: HistoricalUnitFailureEvidenceProvenance | null): UnitFailureEvidenceIngress {
  try {
    return { kind: 'current', facts: parseUnitFailureEvidenceFacts(bytes, expected) };
  } catch (currentError) {
    if (provenance === null) throw currentError;
...
export function validateReconciliationEvidenceDocument(bytes: Uint8Array, expected: ReconciliationEvidenceIdentity, provenance: HistoricalUnitFailureEvidenceProvenance | null = null): void {
  ...
  switch (expected.source) {
    ...
    case 'quarantine-capture': {
      ...
      if (facts.kind === 'current' && facts.facts.captureCommitSha === null) throw new CoordinationRuntimeError('invalid-state', 'quarantine evidence requires a non-null capture commit');
      return;
    }
    case 'run-close':
    case 'run-abort': {
