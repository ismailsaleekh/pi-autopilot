...
function quarantinePostcondition(requestInput: WorktreePostconditionRequest): WorktreePostconditionResult {
  ...
  if (branchQuery.negative) return result('unsafe', ['owned_capture_ref_absent', `expected_ref=${expectedCaptureRef}`], 'owned-git-ref');
  ...
  if (mismatchedRegistrations.length > 0) return result('unsafe', mismatchedRegistrations.map((entry) => `registration_head=${entry.head_sha}:capture_head=${capture}`), 'owned-git-ref');
  const parents = gitQueryText({ descriptor: { kind: 'rev-list-parents', revision: capture }, cwd: intent.repo_root, ...(request.env === undefined ? {} : { env: request.env }) }).trim().split(/\s+/u).slice(1);
  const paths = exactDiffPaths(request, intent.base_sha, capture);
  if (parents.length !== 1 || parents[0] !== intent.base_sha || !samePaths(intent.paths, paths)) return result('unsafe', [`capture_sha=${capture}`, `owned_ref=${expectedCaptureRef}`, ...parents.map((parent) => `actual_parent=${parent}`), ...intent.paths.map((path) => `expected_path=${path}`), ...paths.map((path) => `actual_path=${path}`)], 'owned-git-ref');
