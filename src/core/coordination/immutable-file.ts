...
export function readImmutableFileBytes(input: {
  ...
  readonly errorCode?: CoordinationFailureCode;
  readonly minimumBytes?: number;
  readonly allowMultipleLinks?: boolean;
}): Uint8Array {
  const code = input.errorCode ?? 'recovery-required';
  const minimum = input.minimumBytes ?? 1;
  if (!Number.isSafeInteger(input.maximumBytes) || input.maximumBytes < 1 || !Number.isSafeInteger(minimum) || minimum < 0 || minimum > input.maximumBytes) throw new CoordinationRuntimeError('invalid-request', 'immutable file byte bounds are invalid', [input.label]);
  ...
  try {
    ...
    if (!before.isFile() || before.isSymbolicLink() || !validLinkCount || before.size < minimum || before.size > input.maximumBytes) throw new CoordinationRuntimeError(code, `${input.label} must be a bounded ${linkContract}`, [input.path, `size=${String(before.size)}`, `link_count=${String(before.nlink)}`]);
    ...
    if (!opened.isFile() || !validOpenedLinkCount || !sameIdentity(before, opened)) throw new CoordinationRuntimeError(code, `${input.label} identity changed while opening`, [input.path]);
    ...
    if (offset !== bytes.byteLength || !afterDescriptor.isFile() || !validAfterDescriptorLinks || !sameIdentity(opened, afterDescriptor) || !afterPath.isFile() || afterPath.isSymbolicLink() || !validAfterPathLinks || !sameIdentity(opened, afterPath)) throw new CoordinationRuntimeError(code, `${input.label} identity changed during descriptor-pinned read`, [input.path]);
    return bytes;
  } catch (error) {
