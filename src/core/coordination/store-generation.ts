...
import { COORDINATOR_API_SCHEMA_VERSION, COORDINATOR_STORE_SCHEMA_VERSION, enforcePrivateAuthorityPath, ensureCoordinatorPrivateRoots, type CoordinatorRuntimePaths } from './runtime-paths.ts';
import { CoordinatorWriterGuard } from './writer-guard.ts';

export const COORDINATOR_STORE_GENERATION_SCHEMA = 'autopilot.coordinator_store_generation.v1' as const;
export const COORDINATOR_STORE_POINTER_SCHEMA = 'autopilot.coordinator_store_pointer.v1' as const;
export const COORDINATOR_FIXED_PATH_BARRIER_SCHEMA = 'autopilot.cf50_fixed_path_barrier.v1' as const;
...
interface SqlRow { readonly [key: string]: SQLOutputValue }

export type CoordinatorStoreGenerationSourceKind = 'cf50-fixed-schema12' | 's1-generation-restore';

export interface CoordinatorStoreGenerationPublication {
  readonly schema_version: typeof COORDINATOR_STORE_GENERATION_SCHEMA;
  readonly generation_id: string;
  readonly store_schema_version: typeof COORDINATOR_STORE_SCHEMA_VERSION;
  readonly source_kind: CoordinatorStoreGenerationSourceKind;
  readonly source_generation_id: string | null;
  readonly source_database_sha256: `sha256:${string}`;
  readonly publication_database_sha256: `sha256:${string}`;
  readonly migration_checksums: readonly string[];
  readonly created_at: string;
}

export interface CoordinatorStorePointer {
  readonly schema_version: typeof COORDINATOR_STORE_POINTER_SCHEMA;
  readonly generation_id: string;
  readonly relative_generation_path: string;
  readonly store_schema_version: typeof COORDINATOR_STORE_SCHEMA_VERSION;
  readonly publication_sha256: `sha256:${string}`;
  readonly previous_generation_id: string | null;
  readonly published_at: string;
}

export interface StoreDatabaseFileIdentity {
  readonly device: number;
  readonly inode: number;
}

export interface CurrentStoreGeneration {
  readonly pointer: CoordinatorStorePointer;
  readonly pointer_sha256: `sha256:${string}`;
  readonly publication: CoordinatorStoreGenerationPublication;
  readonly generation_path: string;
  readonly database_path: string;
  readonly database_file_identity: StoreDatabaseFileIdentity;
...
export type StorePublicationBoundary = (typeof STORE_PUBLICATION_BOUNDARIES)[number];

export interface StoreGenerationMigrationAdapter {
  prepareFreshSchema12(databasePath: string): Promise<void>;
  migrateSchema12To13(databasePath: string): Promise<readonly string[]>;
  verifySchema13(databasePath: string): Promise<void>;
}

export interface StoreGenerationOptions {
  readonly now?: () => Date;
  readonly onBoundary?: (boundary: StorePublicationBoundary) => void | Promise<void>;
}
...
function fileSha256(path: string): `sha256:${string}` {
  return sha256Bytes(readFileSync(path));
}

function canonicalTimestamp(value: unknown, label: string): string {
  if (typeof value !== 'string' || value.length !== 24) throw new CoordinationRuntimeError('store-corrupt', `${label} must be a canonical ISO timestamp`);
  try {
    if (new Date(value).toISOString() !== value) throw new Error('timestamp differs after canonicalization');
  } catch {
...
function exactRecord(value: unknown, fields: readonly string[], label: string): Readonly<Record<string, unknown>> {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new CoordinationRuntimeError('store-corrupt', `${label} must be an object`);
  const record = value as Readonly<Record<string, unknown>>;
  const actual = Object.keys(record).sort();
  const expected = [...fields].sort();
  if (actual.length !== expected.length || actual.some((field, index) => field !== expected[index])) throw new CoordinationRuntimeError('store-corrupt', `${label} fields are closed`, actual);
  return record;
}

function parseSha256(value: unknown, label: string): `sha256:${string}` {
  if (typeof value !== 'string' || !SHA256_PATTERN.test(value)) throw new CoordinationRuntimeError('store-corrupt', `${label} is not a canonical SHA-256 digest`);
  return value as `sha256:${string}`;
}

function parseGenerationId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !STORE_GENERATION_ID_PATTERN.test(value)) throw new CoordinationRuntimeError('store-corrupt', `${label} is not a generation address`);
  return value;
}

export function parseStoreGenerationPublication(value: unknown): CoordinatorStoreGenerationPublication {
  ...
  if (record['schema_version'] !== COORDINATOR_STORE_GENERATION_SCHEMA || record['store_schema_version'] !== COORDINATOR_STORE_SCHEMA_VERSION) throw new CoordinationRuntimeError('store-corrupt', `${label} schema identity is invalid`);
  const sourceKind = record['source_kind'];
  if (sourceKind !== 'cf50-fixed-schema12' && sourceKind !== 's1-generation-restore') throw new CoordinationRuntimeError('store-corrupt', `${label}.source_kind is invalid`);
  const sourceGeneration = record['source_generation_id'];
  if (sourceGeneration !== null && (typeof sourceGeneration !== 'string' || !STORE_GENERATION_ID_PATTERN.test(sourceGeneration))) throw new CoordinationRuntimeError('store-corrupt', `${label}.source_generation_id is invalid`);
  if ((sourceKind === 'cf50-fixed-schema12') !== (sourceGeneration === null)) throw new CoordinationRuntimeError('store-corrupt', `${label} source kind/generation identity is contradictory`);
  const checksums = record['migration_checksums'];
  if (!Array.isArray(checksums) || checksums.some((entry) => typeof entry !== 'string' || !/^[a-f0-9]{64}$/u.test(entry))) throw new CoordinationRuntimeError('store-corrupt', `${label}.migration_checksums is invalid`);
  return Object.freeze({
    ...
    migration_checksums: Object.freeze([...checksums] as string[]),
    created_at: canonicalTimestamp(record['created_at'], `${label}.created_at`),
  });
}

export function parseStorePointer(value: unknown): CoordinatorStorePointer {
  ...
  if (record['schema_version'] !== COORDINATOR_STORE_POINTER_SCHEMA || record['store_schema_version'] !== COORDINATOR_STORE_SCHEMA_VERSION) throw new CoordinationRuntimeError('store-corrupt', `${label} schema identity is invalid`);
  ...
  if (relativePath !== `stores/${generationId}` || isAbsolute(relativePath) || relativePath.split('/').includes('..')) throw new CoordinationRuntimeError('store-corrupt', `${label}.relative_generation_path is invalid`);
  const previous = record['previous_generation_id'];
  if (previous !== null && (typeof previous !== 'string' || !STORE_GENERATION_ID_PATTERN.test(previous) || previous === generationId)) throw new CoordinationRuntimeError('store-corrupt', `${label}.previous_generation_id is invalid`);
  return Object.freeze({
...
function parseJsonBytes(bytes: Uint8Array, path: string, label: string): unknown {
  let parsed: unknown;
  try { parsed = JSON.parse(Buffer.from(bytes).toString('utf8')) as unknown; }
  catch (error) { throw new CoordinationRuntimeError('store-corrupt', `${label} is unreadable or invalid JSON`, [path, error instanceof Error ? error.message : String(error)]); }
  return parsed;
}
...
function assertOwnedPrivateObject(path: string, kind: 'file' | 'directory', requireSingleLink: boolean): void {
  const metadata = lstatSync(path);
  const validKind = kind === 'file' ? metadata.isFile() : metadata.isDirectory();
  if (!validKind || metadata.isSymbolicLink()) throw new CoordinationRuntimeError('system-fatal', `store generation ${kind} is a symbolic alias or wrong object kind`, [path]);
  if (requireSingleLink && metadata.nlink !== 1) throw new CoordinationRuntimeError('system-fatal', 'live store generation refuses hardlink aliases', [path, `link_count=${String(metadata.nlink)}`]);
  if (platform() !== 'win32') {
    const getuid = process.getuid;
    if (getuid !== undefined && metadata.uid !== getuid()) throw new CoordinationRuntimeError('system-fatal', 'store generation owner differs from the coordinator process user', [path, `uid=${String(metadata.uid)}`]);
  }
}

function storeDatabaseFileIdentity(path: string): StoreDatabaseFileIdentity {
  assertOwnedPrivateObject(path, 'file', true);
  const metadata = lstatSync(path);
  return Object.freeze({ device: metadata.dev, inode: metadata.ino });
}

function sameStoreDatabaseFileIdentity(left: StoreDatabaseFileIdentity, right: StoreDatabaseFileIdentity): boolean {
  return left.device === right.device && left.inode === right.inode;
}

function assertContainedNoSymlinks(root: string, target: string): void {
  const canonicalRoot = resolve(root);
  const canonicalTarget = resolve(target);
  const lexical = relative(canonicalRoot, canonicalTarget);
  if (lexical.length === 0 || lexical === '..' || lexical.startsWith(`..${sep}`) || isAbsolute(lexical)) throw new CoordinationRuntimeError('system-fatal', 'store generation path escapes its private root', [canonicalRoot, canonicalTarget]);
  ...
  for (const component of lexical.split(sep)) {
    ...
    if (existsSync(current)) {
      ...
      if (info.isSymbolicLink()) throw new CoordinationRuntimeError('system-fatal', 'store generation path contains a symlink segment', [current]);
    }
  }
  ...
  if (physical === '..' || physical.startsWith(`..${sep}`) || isAbsolute(physical)) throw new CoordinationRuntimeError('system-fatal', 'store generation path physically escapes its private root', [canonicalTarget]);
}
...
function sqliteValue(row: SqlRow | undefined, field: string, label: string): SQLOutputValue {
  const value = row?.[field];
  if (value === undefined) throw new CoordinationRuntimeError('store-corrupt', `${label} omitted ${field}`);
  return value;
}

function verifyDatabase(path: string, schema: number): void {
  assertOwnedPrivateObject(path, 'file', true);
  ...
  try {
    ...
    if (integrity !== 'ok' || version !== schema) throw new CoordinationRuntimeError('store-corrupt', 'store generation database integrity/schema identity is invalid', [path, `integrity=${String(integrity)}`, `schema=${String(version)}`]);
  } finally { database.close(); }
}

function generationPaths(paths: CoordinatorRuntimePaths, generationId: string): { readonly directory: string; readonly database: string; readonly publication: string } {
...
async function writeSyncedFile(path: string, body: string): Promise<void> {
...
async function atomicReplacePointer(paths: CoordinatorRuntimePaths, pointer: CoordinatorStorePointer): Promise<`sha256:${string}`> {
...
function readStableCurrentStoreGeneration(paths: CoordinatorRuntimePaths, verifyDatabaseContents: boolean): CurrentStoreGeneration | null {
  if (!existsSync(paths.currentStorePointerPath)) return null;
  ...
  if (pointer.relative_generation_path !== expectedRelative) throw new CoordinationRuntimeError('store-corrupt', 'store pointer generation path disagrees with its ID');
  ...
  if (sha256Bytes(publicationBytes) !== pointer.publication_sha256) throw new CoordinationRuntimeError('store-corrupt', 'store pointer publication digest is invalid', [generation.publication]);
  ...
  if (publication.generation_id !== pointer.generation_id || publication.store_schema_version !== pointer.store_schema_version) throw new CoordinationRuntimeError('store-corrupt', 'store generation publication identity disagrees with its pointer');
  if (publication.source_kind === 'cf50-fixed-schema12' && pointer.previous_generation_id !== null) throw new CoordinationRuntimeError('store-corrupt', 'first-generation publication has a contradictory predecessor pointer');
  if (publication.source_kind === 's1-generation-restore' && pointer.previous_generation_id !== publication.source_generation_id) throw new CoordinationRuntimeError('store-corrupt', 'restored generation source disagrees with its predecessor pointer');
  if (verifyDatabaseContents) verifyDatabase(generation.database, COORDINATOR_STORE_SCHEMA_VERSION);
 ...
 */
export function readCurrentStoreAdmissionGeneration(paths: CoordinatorRuntimePaths): CurrentStoreGeneration | null {
  return readStableCurrentStoreGeneration(paths, false);
}

export function assertCurrentStoreGenerationAuthority(paths: CoordinatorRuntimePaths, expected: CurrentStoreGeneration): CurrentStoreGeneration {
...
export function readCurrentStoreGeneration(paths: CoordinatorRuntimePaths): CurrentStoreGeneration | null {
...
function barrierTriggerName(table: string, operation: 'INSERT' | 'UPDATE' | 'DELETE'): string {
  return `autopilot_s1_deny_${createHash('sha256').update(table, 'utf8').digest('hex').slice(0, 20)}_${operation.toLowerCase()}`;
}

interface FixedBarrierRecord {
  readonly source_database_sha256: `sha256:${string}`;
  readonly generation_id: string;
  readonly publication_sha256: `sha256:${string}`;
}

function fixedBarrierRecord(database: DatabaseSync): FixedBarrierRecord | null {
  ...
  const row = rows[0];
  if (rows.length !== 1 || row === undefined || row['schema_version'] !== COORDINATOR_FIXED_PATH_BARRIER_SCHEMA) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier record is malformed');
...
function verifyFixedBarrier(database: DatabaseSync, expected?: FixedBarrierRecord): FixedBarrierRecord {
  const version = sqliteValue(database.prepare('PRAGMA user_version').get(), 'user_version', 'fixed barrier schema');
  if (version !== COORDINATOR_API_SCHEMA_VERSION) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier does not retain API/schema-12 identity');
  const record = fixedBarrierRecord(database);
  if (record === null) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path mutation-deny barrier is absent');
  if (expected !== undefined && canonicalJson(record) !== canonicalJson(expected)) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier evidence disagrees with the published generation');
  const tables = database.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all().map((row) => {
    const name = row['name'];
    if (typeof name !== 'string') throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier found an invalid user table');
    return name;
  });
  for (const table of tables) for (const operation of ['INSERT', 'UPDATE', 'DELETE'] as const) {
    ...
    if (trigger?.['tbl_name'] !== table || typeof trigger['sql'] !== 'string' || !trigger['sql'].includes(`RAISE(ABORT, '${FIXED_BARRIER_MESSAGE}')`)) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier does not deny every user-table mutation', [table, operation]);
  }
  return record;
}
...
function installFixedBarrier(paths: CoordinatorRuntimePaths, evidence: FixedBarrierRecord): void {
  ...
  try {
    ...
    const tables = database.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all().map((row) => {
      const name = row['name'];
      if (typeof name !== 'string') throw new CoordinationRuntimeError('store-corrupt', 'fixed source found an invalid user table');
      return name;
    });
    for (const table of tables) for (const operation of ['INSERT', 'UPDATE', 'DELETE'] as const) database.exec(`CREATE TRIGGER ${quoteIdentifier(barrierTriggerName(table, operation))} BEFORE ${operation} ON ${quoteIdentifier(table)} BEGIN SELECT RAISE(ABORT, '${FIXED_BARRIER_MESSAGE}'); END`);
    verifyFixedBarrier(database, evidence);
...
export async function storeGenerationPublicationPresent(paths: CoordinatorRuntimePaths): Promise<boolean> {
  if (existsSync(paths.currentStorePointerPath)) return true;
  if (!existsSync(paths.storesRoot)) return false;
...
export function fixedStoreBarrierPublished(paths: CoordinatorRuntimePaths): boolean {
...
function verifyPublishedFixedBarrier(paths: CoordinatorRuntimePaths): FixedBarrierRecord {
  if (!existsSync(paths.databasePath)) throw new CoordinationRuntimeError('store-corrupt', 'published S1 authority has no fixed-path schema-12 barrier');
  const fixed = new DatabaseSync(paths.databasePath, { readOnly: true, timeout: 5_000 });
  let barrier: FixedBarrierRecord;
  try { barrier = verifyFixedBarrier(fixed); }
  finally { fixed.close(); }
  ...
  assertOwnedPrivateObject(generation.publication, 'file', true);
  const publicationBytes = readFileSync(generation.publication);
  if (sha256Bytes(publicationBytes) !== barrier.publication_sha256) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier publication digest is not recoverable');
  const publication = parseStoreGenerationPublication(parseJsonFile(generation.publication, 'fixed barrier source publication'));
  if (publication.generation_id !== barrier.generation_id || publication.source_kind !== 'cf50-fixed-schema12' || publication.source_generation_id !== null || publication.source_database_sha256 !== barrier.source_database_sha256) throw new CoordinationRuntimeError('store-corrupt', 'fixed-path barrier evidence disagrees with its first publication');
  verifyDatabase(generation.database, COORDINATOR_STORE_SCHEMA_VERSION);
  return barrier;
}

interface BarrierRecoveryCandidate {
  readonly pointer: CoordinatorStorePointer;
  readonly publication: CoordinatorStoreGenerationPublication;
  readonly generation_path: string;
  readonly database_path: string;
...
async function recoverBarrieredFirstPublication(paths: CoordinatorRuntimePaths, writerGuard: CoordinatorWriterGuard, options: StoreGenerationOptions): Promise<CurrentStoreGeneration | null> {
  if (!existsSync(paths.databasePath)) return null;
  const fixed = new DatabaseSync(paths.databasePath, { readOnly: true, timeout: 5_000 });
  let barrier: FixedBarrierRecord | null;
  try { barrier = fixedBarrierRecord(fixed); if (barrier !== null) verifyFixedBarrier(fixed, barrier); }
  finally { fixed.close(); }
  if (barrier === null) return null;
  const candidates: BarrierRecoveryCandidate[] = [];
  for (const name of await readdir(paths.storesRoot)) {
    if (!STORE_GENERATION_ID_PATTERN.test(name)) continue;
    const generation = generationPaths(paths, name);
    if (!existsSync(generation.publication) || !existsSync(generation.database)) continue;
    const publicationBytes = readFileSync(generation.publication);
    const publication = parseStoreGenerationPublication(parseJsonFile(generation.publication, 'orphan generation publication'));
    if (publication.generation_id !== name || name !== barrier.generation_id || sha256Bytes(publicationBytes) !== barrier.publication_sha256 || publication.source_database_sha256 !== barrier.source_database_sha256) continue;
    const pointer: CoordinatorStorePointer = { schema_version: COORDINATOR_STORE_POINTER_SCHEMA, generation_id: name, relative_generation_path: `stores/${name}`, store_schema_version: COORDINATOR_STORE_SCHEMA_VERSION, publication_sha256: barrier.publication_sha256, previous_generation_id: null, published_at: (options.now ?? (() => new Date()))().toISOString() };
  ...
  if (candidates.length !== 1) throw new CoordinationRuntimeError('store-corrupt', 'barriered fixed source has an ambiguous or missing complete first generation', [`candidate_count=${String(candidates.length)}`]);
  const candidate = candidates[0];
  if (candidate === undefined) throw new CoordinationRuntimeError('store-corrupt', 'barrier recovery candidate disappeared');
  verifyDatabase(candidate.database_path, COORDINATOR_STORE_SCHEMA_VERSION);
  if (fileSha256(candidate.database_path) !== candidate.publication.publication_database_sha256) throw new CoordinationRuntimeError('store-corrupt', 'barrier recovery generation database disagrees with its immutable publication hash', [candidate.database_path]);
  ...
  if (recovered === null) throw new CoordinationRuntimeError('store-corrupt', 'recovered store pointer disappeared');
  return recovered;
}
...
export async function publishRestoredStoreGeneration(paths: CoordinatorRuntimePaths, writerGuard: CoordinatorWriterGuard, sourceDatabasePath: string, sourceDatabaseSha256: `sha256:${string}`, sourceGenerationId: string, migration: StoreGenerationMigrationAdapter, options: StoreGenerationOptions = {}): Promise<CurrentStoreGeneration> {
  ...
  if (!SHA256_PATTERN.test(sourceDatabaseSha256) || fileSha256(sourcePath) !== sourceDatabaseSha256) throw new CoordinationRuntimeError('store-corrupt', 'restore source digest does not match its verified backup evidence', [sourcePath]);
  if (existsSync(`${sourcePath}-wal`) || existsSync(`${sourcePath}-shm`)) throw new CoordinationRuntimeError('store-corrupt', 'restore source carries foreign WAL/SHM components', [sourcePath]);
  ...
  try {
    ...
    if (integrity !== 'ok' || typeof version !== 'number' || !Number.isSafeInteger(version) || version !== COORDINATOR_API_SCHEMA_VERSION && version !== COORDINATOR_STORE_SCHEMA_VERSION) throw new CoordinationRuntimeError('store-corrupt', 'restore source must be exact schema 12 or schema 13 with physical integrity', [sourcePath, `integrity=${String(integrity)}`, `schema=${String(version)}`]);
  ...
  try {
    ...
    try { await backup(source, stagingDatabase); }
    finally { source.close(); }
    if (fileSha256(sourcePath) !== sourceDatabaseSha256) throw new CoordinationRuntimeError('store-corrupt', 'restore source changed during fresh-generation capture', [sourcePath]);
    ...
    if (sourceSchema === COORDINATOR_API_SCHEMA_VERSION) await options.onBoundary?.('migration-complete');
    await migration.verifySchema13(stagingDatabase);
    verifyDatabase(stagingDatabase, COORDINATOR_STORE_SCHEMA_VERSION);
    if (existsSync(`${stagingDatabase}-wal`) || existsSync(`${stagingDatabase}-shm`)) throw new CoordinationRuntimeError('store-corrupt', 'restored staging generation retained foreign WAL/SHM', [stagingDatabase]);
    await options.onBoundary?.('integrity-verified');
    syncFile(stagingDatabase);
    await options.onBoundary?.('database-fsynced');
    const publication: CoordinatorStoreGenerationPublication = {
      schema_version: COORDINATOR_STORE_GENERATION_SCHEMA,
      generation_id: generationId,
      store_schema_version: COORDINATOR_STORE_SCHEMA_VERSION,
    ...
    writerGuard.assertHeld();
    const pointer: CoordinatorStorePointer = {
      schema_version: COORDINATOR_STORE_POINTER_SCHEMA,
      generation_id: generationId,
    ...
    if (restored === null) throw new CoordinationRuntimeError('store-corrupt', 'restored current store pointer disappeared');
    return restored;
  } catch (error) {
...
export async function ensureCurrentStoreGeneration(paths: CoordinatorRuntimePaths, writerGuard: CoordinatorWriterGuard, migration: StoreGenerationMigrationAdapter, options: StoreGenerationOptions = {}): Promise<CurrentStoreGeneration> {
  ...
  for (const name of await readdir(paths.coordinatorRoot)) {
    if (!/^\.current-store\.\d+\.[a-f0-9]{16}\.tmp$/u.test(name)) continue;
  ...
  for (const name of await readdir(paths.storesRoot)) {
    if (!/^\.staging-generation-[a-f0-9]{32}$/u.test(name)) continue;
  ...
  try {
    ...
    await migration.verifySchema13(stagingDatabase);
    verifyDatabase(stagingDatabase, COORDINATOR_STORE_SCHEMA_VERSION);
    if (existsSync(`${stagingDatabase}-wal`) || existsSync(`${stagingDatabase}-shm`)) throw new CoordinationRuntimeError('store-corrupt', 'staging generation retained WAL/SHM after migration checkpoint', [stagingDatabase]);
    await options.onBoundary?.('integrity-verified');
    syncFile(stagingDatabase);
    await options.onBoundary?.('database-fsynced');
    const publication: CoordinatorStoreGenerationPublication = {
    ...
    if (published === null) throw new CoordinationRuntimeError('store-corrupt', 'published current store pointer disappeared');
    return published;
  } catch (error) {
    if (existsSync(stagingDirectory)) await rm(stagingDirectory, { recursive: true, force: true });
    throw error;
