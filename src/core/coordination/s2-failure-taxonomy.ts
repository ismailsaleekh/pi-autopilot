...
export type S2FailureCriticality = 'authority-critical' | 'progress-critical';
export type S2FailureScopeKind =
  | 'request-envelope'
  | 'coordinator-invariant'
  | 'connection-protocol'
  | 'store-schema-boundary'
  | 'ipc-frame'
  | 'client-authority-proof'
  ...
  | 'idempotency-key'
  | 'owner-run-recovery'
  | 'owner-operation-saga'
  | 'owner-operation-storage'
  | 'owner-operation-path'
  | 'planning-authority-set'
  | 'coordinator-store'
  | 'local-runtime';

export type S2FailureEvidencePublication = 'required-for-diagnosis' | 'required-before-repair-or-retry';
export type S2AuthorityScopeRule = 'fail-closed-at-exact-scope';
export type S2ProgressScopeRule = 'must-not-stop-unrelated-runs-or-coordinator';

interface S2FailureDecisionBase<Code extends CoordinationFailureCode> {
  readonly schema_version: typeof S2_FAILURE_TAXONOMY_SCHEMA_VERSION;
  readonly code: Code;
...
export interface S2AuthorityFailureDecision<Code extends CoordinationFailureCode = CoordinationFailureCode> extends S2FailureDecisionBase<Code> {
  readonly criticality: 'authority-critical';
  readonly scope_rule: S2AuthorityScopeRule;
  readonly evidence_publication: 'required-for-diagnosis';
}

export interface S2ProgressFailureDecision<Code extends CoordinationFailureCode = CoordinationFailureCode> extends S2FailureDecisionBase<Code> {
  readonly criticality: 'progress-critical';
  readonly scope_rule: S2ProgressScopeRule;
  readonly evidence_publication: 'required-before-repair-or-retry';
}

export type S2CoordinationFailureDecision<Code extends CoordinationFailureCode = CoordinationFailureCode> =
  | S2AuthorityFailureDecision<Code>
  | S2ProgressFailureDecision<Code>;

type S2DecisionByCode = { readonly [Code in CoordinationFailureCode]: S2CoordinationFailureDecision<Code> };

function authorityDecision<Code extends CoordinationFailureCode>(input: Omit<S2AuthorityFailureDecision<Code>, 'schema_version' | 'criticality' | 'scope_rule' | 'evidence_publication'>): Readonly<S2AuthorityFailureDecision<Code>> {
...
function progressDecision<Code extends CoordinationFailureCode>(input: Omit<S2ProgressFailureDecision<Code>, 'schema_version' | 'criticality' | 'scope_rule' | 'evidence_publication'>): Readonly<S2ProgressFailureDecision<Code>> {
  return Object.freeze({
    schema_version: S2_FAILURE_TAXONOMY_SCHEMA_VERSION,
...
const S2_DECISION_BY_CODE: Readonly<S2DecisionByCode> = Object.freeze({
...
export function decideS2CoordinationFailure<Code extends CoordinationFailureCode>(code: Code): S2CoordinationFailureDecision<Code> {
  return S2_DECISION_BY_CODE[code];
}

export function listS2CoordinationFailureDecisions(): readonly S2CoordinationFailureDecision[] {
...
export function assertS2FailureTaxonomyMatchesExistingRetryPolicy(): void {
  for (const code of COORDINATION_FAILURE_CODES) {
    ...
    if (actual !== expected) throw new Error(`S2 failure taxonomy retry policy mismatch for ${code}`);
  }
}
