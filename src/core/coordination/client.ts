...
import { parseKnownCompatibleCurrentCoordinatorLock, parsePredecessorCoordinatorLock } from './upgrade-contracts.ts';
...
export class CoordinatorClient {
      ...
      try {
        ...
        if (upgrade === null) {
          ...
          if (currentText !== null) {
            ...
            if (isProcessAlive(current.pid)) throw new CoordinationRuntimeError('coordinator-unavailable', `wire-compatible coordinator ${current.package_build} is live as pid ${String(current.pid)} but its socket is unavailable; auto-start will not replace or reinterpret its predecessor fence`);
  ...
  #assertCoordinatorCompatibility(response: CoordinatorResponseEnvelope): Exclude<CoordinatorRuntimeCompatibility, { readonly kind: 'incompatible' }> {
    ...
    if (compatibility.reason === 'unknown-build') throw new CoordinationRuntimeError('protocol-mismatch', `coordinator package build ${compatibility.package_build ?? '<missing>'} is outside the closed protocol-1.3/schema-9 compatibility lineage`);
