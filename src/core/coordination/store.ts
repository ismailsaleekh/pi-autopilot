...
import { claimModesConflict, coordinationPathsOverlap, parseCoordinationAcquisitionGroup, parseCoordinationChildLease, parseCoordinationClaimRequest, parseCoordinationEditLease, parseCoordinationMailboxCursor, parseCoordinationMessage, parseCoordinationReconciliationEvidence, parseCoordinationReleaseCondition, parseCoordinationRepository, parseCoordinationRequestedLease, parseCoordinationRun, parseCoordinationSessionLease, parseCoordinationUnitAttempt, parseCoordinationWorktree, parseCoordinationWorktreeOperation } from './contracts.ts';
...
import type { CoordinationAcquisitionGroup, CoordinationChildLease, CoordinationClaimRequest, CoordinationEditLease, CoordinationMailboxCursor, CoordinationMessage, CoordinationOwnerIdentity, CoordinationReconciliationEvidence, CoordinationReconciliationSource, CoordinationReconciliationSummary, CoordinationReleaseCondition, CoordinationReleaseConditionType, CoordinationRepository, CoordinationRequestedLease, CoordinationRun, CoordinationSessionLease, CoordinationUnitAttempt, CoordinationWorktree, CoordinationWorktreeOperation, CoordinatorRequestEnvelope, CoordinatorResponseEnvelope } from './types.ts';

const DATABASE_EXPORT_SCHEMA = 'autopilot.coordinator_export.v1';
const SHA256_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const RUN_OWNED_IDEMPOTENCY_ACTIONS = new Set(['acquire-group', 'acknowledge-grant', 'respond-claim-request', 'cancel-claim-request', 'cancel-acquisition-group', 'supersede-attempt', 'record-release-evidence', 'reconcile-run', 'prepare-operation', 'transition-operation']);
...
export class CoordinatorStore {
  ...
  recordReleaseEvidence(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      const evidence = this.#acceptReconciliationEvidence({
        ...
        targetId,
        evidence: {
          ref: payloadString(request.payload, 'evidence_ref'),
          sha256: payloadString(request.payload, 'evidence_sha256') as `sha256:${string}`,
        },
        seq,
      });
      ...
      const reconciliation = this.#reconcileOwnedRun(request.repo_id, workstreamRun, seq);
      return { sequence: seq, eventType: 'release-evidence-accepted', entityType: 'reconciliation-evidence', entityId: evidence.reconciliation_evidence_id, payload: { reconciliation_evidence: evidence, run: nextRun, reconciliation } };
    });
  }
  ...
  #verifyAcceptedEvidenceFile(run: CoordinationRun, source: CoordinationReconciliationSource, targetId: string, evidence: { readonly ref: string; readonly sha256: `sha256:${string}` }): void {
    ...
    if (relativeEvidence.length === 0 || relativeEvidence === '..' || relativeEvidence.startsWith(`..${sep}`) || isAbsolute(relativeEvidence)) throw new CoordinationRuntimeError('unauthorized-client', 'accepted reconciliation evidence escapes the run-owned main worktree');
    let bytes: Uint8Array;
    try {
      const evidenceStat = lstatSync(evidencePath);
      if (!evidenceStat.isFile() || evidenceStat.isSymbolicLink()) throw new CoordinationRuntimeError('unauthorized-client', 'accepted reconciliation evidence must be a regular non-symbolic file', [evidencePath]);
      ...
      if (relativeRealEvidence.length === 0 || relativeRealEvidence === '..' || relativeRealEvidence.startsWith(`..${sep}`) || isAbsolute(relativeRealEvidence)) throw new CoordinationRuntimeError('unauthorized-client', 'accepted reconciliation evidence resolves outside the run-owned main worktree', [evidencePath, realEvidencePath]);
      bytes = readFileSync(evidencePath);
    } catch (error) {
      if (error instanceof CoordinationRuntimeError) throw error;
      throw new CoordinationRuntimeError('recovery-required', 'accepted reconciliation evidence file is unreadable', [evidencePath, error instanceof Error ? error.message : String(error)]);
    }
    const actual = `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
    if (actual !== evidence.sha256) throw new CoordinationRuntimeError('invalid-state', 'accepted reconciliation evidence hash does not match the run-owned artifact', [evidencePath, `expected=${evidence.sha256}`, `actual=${actual}`]);
