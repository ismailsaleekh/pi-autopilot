...
function isD65RuntimeUnitSpecSchema(schemaVersion: string | null): boolean {
  return schemaVersion !== null && D65_RUNTIME_UNIT_SPEC_SCHEMAS.has(schemaVersion);
}

function isD65RuntimeReceiptSchema(schemaVersion: string | null): boolean {
  return schemaVersion !== null && D65_RUNTIME_RECEIPT_SCHEMAS.has(schemaVersion);
}

function parseD65RuntimeUnitSpecDocument(value: string, label: string): D65RuntimeUnitSpec {
...
function parseD65RuntimeReceiptDocument(value: string, label: string): D65RuntimeReceipt {
  const document = parseJsonObject(value, label);
...
export class CoordinatorStore {
   ...
   */
  #collectRunScopedStatusDetails(detailRuns: readonly { readonly repoId: string; readonly workstreamRun: string }[]): {
    readonly unitAttempts: CoordinationUnitAttempt[];
    readonly acquisitionGroups: CoordinationAcquisitionGroup[];
    readonly observations: CoordinationObservation[];
    readonly editLeases: CoordinationEditLease[];
    readonly reservationObligations: CoordinationReservationObligation[];
    readonly changeReservations: CoordinationChangeReservation[];
    readonly runTerminalIntents: CoordinationRunTerminalIntent[];
    readonly claimRequests: CoordinationClaimRequest[];
    readonly reconciliationEvidence: CoordinationReconciliationEvidence[];
    readonly reconciliationReceipts: CoordinationReconciliationReceipt[];
    readonly mailboxDeliveries: CoordinationMailboxDeliveryReceipt[];
    readonly resultReceipts: CoordinationResultReceipt[];
    readonly worktrees: CoordinationWorktree[];
    readonly worktreeOperations: CoordinationWorktreeOperation[];
    readonly waitForEdges: CoordinationWaitForEdge[];
    readonly deadlockResolutions: CoordinationDeadlockResolution[];
    readonly authoritativeArtifacts: CoordinationAuthoritativeArtifact[];
    readonly adjudicationAssignments: CoordinationAdjudicationAssignment[];
    readonly escalations: CoordinationEscalation[];
  } {
    const unitAttempts: CoordinationUnitAttempt[] = [];
    const acquisitionGroups: CoordinationAcquisitionGroup[] = [];
    const observations: CoordinationObservation[] = [];
    const editLeases: CoordinationEditLease[] = [];
    const reservationObligations: CoordinationReservationObligation[] = [];
    const changeReservations: CoordinationChangeReservation[] = [];
    const runTerminalIntents: CoordinationRunTerminalIntent[] = [];
    const claimRequests: CoordinationClaimRequest[] = [];
    const reconciliationEvidence: CoordinationReconciliationEvidence[] = [];
    const reconciliationReceipts: CoordinationReconciliationReceipt[] = [];
    const mailboxDeliveries: CoordinationMailboxDeliveryReceipt[] = [];
    const resultReceipts: CoordinationResultReceipt[] = [];
    const worktrees: CoordinationWorktree[] = [];
    const worktreeOperations: CoordinationWorktreeOperation[] = [];
    const waitForEdges: CoordinationWaitForEdge[] = [];
    const deadlockResolutions: CoordinationDeadlockResolution[] = [];
    const authoritativeArtifacts: CoordinationAuthoritativeArtifact[] = [];
    const adjudicationAssignments: CoordinationAdjudicationAssignment[] = [];
    const escalations: CoordinationEscalation[] = [];
    for (const { repoId, workstreamRun } of detailRuns) {
      for (const row of this.#db.prepare('SELECT * FROM unit_attempts WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) unitAttempts.push(unitAttemptFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM acquisition_groups WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) acquisitionGroups.push(acquisitionGroupFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM observations WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) observations.push(observationFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM edit_leases WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) editLeases.push(editLeaseFromRow(row));
      ...
      for (const reservation of this.#db.prepare('SELECT * FROM change_reservations WHERE repo_id=? ORDER BY entity_id').all(repoId).map(changeReservationFromRow)) if (reservation.workstream_run === workstreamRun || relevantReservationIds.has(reservation.reservation_id)) changeReservations.push(reservation);
      for (const row of this.#db.prepare('SELECT * FROM run_terminal_intents WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) runTerminalIntents.push(runTerminalIntentFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM claim_requests WHERE repo_id=? AND (requester_workstream_run=? OR owner_workstream_run=?) ORDER BY entity_id').all(repoId, workstreamRun, workstreamRun)) claimRequests.push(claimRequestFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM reconciliation_evidence WHERE repo_id=? AND workstream_run=? ORDER BY entity_id').all(repoId, workstreamRun)) reconciliationEvidence.push(reconciliationEvidenceFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM reconciliation_receipts WHERE repo_id=? AND workstream_run=? ORDER BY committed_event_seq, entity_id').all(repoId, workstreamRun)) reconciliationReceipts.push(reconciliationReceiptFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM mailbox_deliveries WHERE repo_id=? AND workstream_run=? ORDER BY delivery_id').all(repoId, workstreamRun)) mailboxDeliveries.push(mailboxDeliveryFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM result_receipts WHERE repo_id=? AND workstream_run=? ORDER BY committed_event_seq, entity_id').all(repoId, workstreamRun)) resultReceipts.push(resultReceiptFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM worktrees WHERE repo_id=? AND workstream_run=? AND is_current_canonical=1 ORDER BY canonical_worktree_id').all(repoId, workstreamRun)) worktrees.push(canonicalWorktreeFromRow(row));
      for (const row of this.#db.prepare('SELECT * FROM worktree_operations WHERE repo_id=? AND workstream_run=? AND canonical_worktree_id IS NOT NULL ORDER BY canonical_worktree_id,entity_id').all(repoId, workstreamRun)) worktreeOperations.push(canonicalWorktreeOperationFromRow(row));
      ...
      for (const resolution of this.#db.prepare('SELECT * FROM deadlock_resolutions WHERE repo_id=? ORDER BY entity_id').all(repoId).map(deadlockResolutionFromRow)) if (resolution.cycle_edge_ids.some((edgeId) => relevantEdgeIds.has(edgeId))) deadlockResolutions.push(resolution);
      for (const row of this.#db.prepare('SELECT * FROM authoritative_artifacts WHERE repo_id=? AND source_run=? ORDER BY entity_id').all(repoId, workstreamRun)) authoritativeArtifacts.push(authoritativeArtifactFromRow(row));
      for (const assignment of this.#db.prepare('SELECT * FROM adjudication_assignments WHERE repo_id=? ORDER BY entity_id').all(repoId).map(adjudicationAssignmentFromRow)) if (assignment.requesting_run === workstreamRun || assignment.participating_runs.includes(workstreamRun) || assignment.adjudicator.workstream_run === workstreamRun) adjudicationAssignments.push(assignment);
      for (const escalation of this.#db.prepare('SELECT * FROM escalations WHERE repo_id=? ORDER BY entity_id').all(repoId).map(escalationFromRow)) if (escalation.participating_runs.includes(workstreamRun)) escalations.push(escalation);
    }
  ...
  #d65ResolveConsumableProbe(run: CoordinationRun, unitId: string, attempt: number, successorSpecRef: string, successorSpecSha256: string, coordinatorTime: string): Readonly<{ artifact_id: string; sha256: `sha256:${string}`; probe_sequence: number; provider: string; trigger_continuation_sha256: `sha256:${string}`; coordinator_time: string }> {
    ...
    const exactRuntimeSpecEntry = (ref: string, sha256: string) => authorityEntries.filter((entry) => entry.ref === ref && entry.sha256 === sha256 && isD65RuntimeUnitSpecSchema(entry.document_schema_version));
    const exactRuntimeReceiptEntry = (ref: string, sha256: string) => authorityEntries.filter((entry) => entry.ref === ref && entry.sha256 === sha256 && isD65RuntimeReceiptSchema(entry.document_schema_version));
    ...
    if (d65RuntimeSpecGeneration(successorSpec) !== d65RuntimeSpecGeneration(failedSpec) || successorSpec.unit_id !== unitId || successorSpec.attempt !== attempt || !d65RuntimeSpecsPreserveRetryAuthority(failedSpec, successorSpec)) throw new CoordinationRuntimeError('invalid-state', 'probe successor spec changes the authorized unit/attempt/model tuple', [successorSpec.unit_id, String(successorSpec.attempt), d65RuntimeSpecProviderModelId(successorSpec)]);
