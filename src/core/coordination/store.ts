...
export class CoordinatorStore {
  ...
  #assertUnitFailureEvidenceFacts(run: CoordinationRun, source: 'attempt-reset' | 'quarantine-capture', targetId: string, facts: ReturnType<typeof parseUnitFailureEvidenceFacts>): void {
    ...
    if (commonRaw === null || head === null || branch === null) throw new CoordinationRuntimeError('recovery-required', 'coordinator could not verify unit failure Git postconditions');
    const actualCommon = realpathSync(isAbsolute(commonRaw) ? commonRaw : resolve(worktree.canonical_path, commonRaw));
    if (actualCommon !== realpathSync(worktree.git_common_dir) || actualCommon !== realpathSync(facts.gitCommonDir) || branch !== worktree.branch || branch !== facts.branch || head !== facts.gitHeadAfter || status.stdout.byteLength !== 0) throw new CoordinationRuntimeError('recovery-required', 'unit failure Git postconditions differ from durable worktree evidence', [`common=${actualCommon}`, `branch=${branch}`, `head=${head}`, `mutable=${String(status.stdout.byteLength)}`]);
