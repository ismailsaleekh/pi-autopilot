...
import { parseCoordinationChildLease, parseCoordinationMessage, parseCoordinationRepository, parseCoordinationRun, parseCoordinationSessionLease } from './contracts.ts';
import { CoordinationRuntimeError, type CoordinationFailureCode } from './failures.ts';
import { COORDINATOR_BUSY_TIMEOUT_MS, COORDINATOR_DATABASE_SCHEMA_VERSION, COORDINATOR_PACKAGE_BUILD, type CoordinatorRuntimePaths } from './runtime-paths.ts';
import type { CoordinationChildLease, CoordinationMessage, CoordinationRepository, CoordinationRun, CoordinationSessionLease, CoordinatorRequestEnvelope, CoordinatorResponseEnvelope } from './types.ts';
...
export class CoordinatorStore {
  ...
  private constructor(db: DatabaseSync, paths: CoordinatorRuntimePaths, clock: StoreClock, lastBackupPath: string | null) {
    this.#db = db;
    this.#paths = paths;
    this.#clock = clock;
  ...
  static async open(paths: CoordinatorRuntimePaths, clock: StoreClock = systemClock): Promise<CoordinatorStore> {
    ...
    try {
      ...
      if (migrationRow === undefined || sqlInteger(migrationRow, 'version') !== 1 || sqlString(migrationRow, 'checksum') !== expectedMigrationChecksum) throw new CoordinationRuntimeError('schema-mismatch', 'coordinator migration journal checksum does not match the package schema');
