...
import { COORDINATION_WORKTREE_STATES } from './types.ts';
import type { CoordinationAcquisitionGroup, CoordinationAcquisitionKind, CoordinationAdjudicationAssignment, CoordinationAuthoritativeArtifact, CoordinationChangeReservation, CoordinationChildLease, CoordinationClaimRequest, CoordinationDeadlockResolution, CoordinationEditLease, CoordinationEscalation, CoordinationMailboxCursor, CoordinationMessage, CoordinationOwnerIdentity, CoordinationReconciliationEvidence, CoordinationReconciliationSource, CoordinationReconciliationSummary, CoordinationReleaseCondition, CoordinationReleaseConditionType, CoordinationRepository, CoordinationRequestedLease, CoordinationReservationObligation, CoordinationRun, CoordinationRunTerminalIntent, CoordinationSessionLease, CoordinationUnitAttempt, CoordinationUnitRole, CoordinationWaitForEdge, CoordinationWorktree, CoordinationWorktreeOperation, CoordinatorRequestEnvelope, CoordinatorResponseEnvelope } from './types.ts';
...
export class CoordinatorStore {
  ...
  private constructor(db: DatabaseSync, clock: StoreClock, stateRoot: string, lastBackupPath: string | null) {
    this.#db = db;
    this.#clock = clock;
  ...
  static async open(paths: CoordinatorRuntimePaths, clock: StoreClock = systemClock): Promise<CoordinatorStore> {
    ...
    try {
      ...
      const store = new CoordinatorStore(db, clock, paths.stateRoot, lastBackupPath);
  ...
  heartbeatSession(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#sessionMutation(request, 'session-heartbeat', (session, seq) => {
      ...
      const reconciliation = this.#reconcileOwnedRun(request.repo_id, session.workstream_run, seq);
  ...
  prepareRunTerminal(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      if (runningChildren.length > 0) throw new CoordinationRuntimeError('recovery-required', 'run terminal preparation requires all child processes terminal', runningChildren);
      ...
      if (incompleteOperations.length > 0) throw new CoordinationRuntimeError('recovery-required', 'run terminal preparation requires all owned worktree sagas terminal', incompleteOperations);
      if (outcomeValue === 'closed') this.#assertRunCloseReservationReady(run);
      ...
      if (pendingGroups.length > 0) throw new CoordinationRuntimeError('recovery-required', 'run terminal preparation requires queued acquisition groups to be cancelled or superseded', pendingGroups.map((group) => `${group.acquisition_group_id}:${group.state}`));
      const seq = this.#nextEventSequence(run.repo_id);
      ...
      return { sequence: seq, eventType: 'run-terminal-prepared', entityType: 'run-terminal-intent', entityId: intent.terminal_intent_id, payload: { run_terminal_intent: intent } };
    });
  }
  ...
  cancelRunTerminal(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      return { sequence: seq, eventType: 'run-terminal-cancelled', entityType: 'run-terminal-intent', entityId: cancelled.terminal_intent_id, payload: { run_terminal_intent: cancelled, reason: payloadString(request.payload, 'reason') } };
    });
  }
  ...
  #maintainWaitForGraph(repoId: string, seq: number, fixedPointDepth = 0): void {
    if (fixedPointDepth > 1024) throw new CoordinationRuntimeError('store-corrupt', 'deadlock resolution did not reach a transactional fixed point');
    const requests = this.#db.prepare('SELECT * FROM claim_requests WHERE repo_id=? ORDER BY entity_id').all(repoId).map(claimRequestFromRow);
    const leases = this.#db.prepare('SELECT * FROM edit_leases WHERE repo_id=? ORDER BY entity_id').all(repoId).map(editLeaseFromRow);
    ...
    if (missingTypedResolution) this.#maintainWaitForGraph(repoId, seq, fixedPointDepth + 1);
  }
