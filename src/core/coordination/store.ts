...
import { activeCoordinationMigrationFreeze, assertCoordinationDispatchAllowed, assertCoordinationFrozenMutationAllowed, coordinationCutoverCommitted } from './migration-paths.ts';
...
export class CoordinatorStore {
  ...
  #lastStartupReconciliation: CoordinationReconciliationSummary | null = null;
  ...
  async #replayPendingSemanticEvents(paths: CoordinatorRuntimePaths): Promise<void> {
    ...
    try {
      ...
      if (receipt === null) {
        ...
        if (invariantErrors.length > 0) throw new CoordinationRuntimeError('invalid-state', 'semantic replay violates coordinator invariants', invariantErrors.slice(0, 100).map((finding) => `${finding.code}:${finding.entity}:${finding.detail}`));
  ...
  importLegacyCoordination(plan: CoordinationLegacyImportPlan): StoreEffect {
    ...
    try {
      ...
      if (invariantFindings.length > 0) throw new CoordinationRuntimeError('invalid-state', 'transactional legacy import violates coordinator invariants', invariantFindings.slice(0, 100).map((finding) => `${finding.code}:${finding.entity}:${finding.detail}`));
      this.#db.exec('COMMIT');
    } catch (error) {
      this.#db.exec('ROLLBACK');
      throw error;
  ...
  verifyMigrationImport(repoId: string, migrationId: string): { readonly invariant_findings: readonly CoordinationInvariantFinding[]; readonly integrity: string } {
    ...
    if (integrity !== 'ok' || errors.length > 0) throw new CoordinationRuntimeError('invalid-state', 'migration verification failed coordinator integrity or invariants', errors.slice(0, 100).map((finding) => `${finding.code}:${finding.entity}:${finding.detail}`));
  ...
  #dispatch(request: CoordinatorRequestEnvelope): StoreEffect {
    ...
    if (!queryActions.has(request.action)) {
      if (activeCoordinationMigrationFreeze(this.#stateRoot) !== null) assertCoordinationFrozenMutationAllowed(this.#stateRoot, request.repo_id, request.action);
  ...
  runCatalog(repoId: string, workstreamRun: string | null, payload: Readonly<Record<string, unknown>> = {}): StoreEffect {
    ...
    if (workstreamRun !== null && cursorRun !== null) throw new CoordinationRuntimeError('invalid-request', 'exact run catalog query cannot carry a pagination cursor');
    ...
    if (fetchedRuns.length !== fetchedResources.length || fetchedRuns.some((run, index) => fetchedResources[index]?.workstream_run !== run.workstream_run)) throw new CoordinationRuntimeError('store-corrupt', 'run catalog and immutable run resources are not in exact lockstep');
  ...
  attachSession(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      if (pendingRecovery.length > 0) throw new CoordinationRuntimeError('recovery-required', `run ${workstreamRun} cannot attach ordinary dispatch while migration recovery is pending`, pendingRecovery.map((work) => work.recovery_id));
  ...
  claimAdjudicationAssignment(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      if (assignments.length !== 1) throw new CoordinationRuntimeError('store-corrupt', 'adjudication attempt has multiple simultaneous assignments', assignments.map((assignment) => assignment.assignment_id));
  ...
  #convertUnitMergeToReservations(run: CoordinationRun, targetId: string, mergeEvidence: { readonly ref: string; readonly sha256: `sha256:${string}` }, seq: number): { readonly reservations: readonly CoordinationChangeReservation[]; readonly obligations: readonly CoordinationReservationObligation[] } {
    ...
    if (existing.length > 0) throw new CoordinationRuntimeError('invalid-state', 'partial or mismatched reservation conversion already exists', existing.map((reservation) => reservation.reservation_id));
  ...
  #assertRunTerminalExternalReady(run: CoordinationRun): void {
    ...
    if (pendingGroups.length > 0) throw new CoordinationRuntimeError('recovery-required', 'run terminal commit requires queued acquisition groups to be cancelled or superseded', pendingGroups.map((group) => `${group.acquisition_group_id}:${group.state}`));
  }
  ...
  #assertRunCloseReservationReady(run: CoordinationRun): void {
    ...
    if (unresolved.length > 0) throw new CoordinationRuntimeError('recovery-required', 'run close requires every reservation integration obligation to be resolved', unresolved.map((entry) => `${entry.obligation_id}:${entry.state}`));
  ...
  #requireCurrentSession(request: CoordinatorRequestEnvelope): CoordinationSessionLease {
    ...
    if (session.attachment_kind !== 'migration-recovery' && pendingRecovery.length > 0 && !['detach-session', 'heartbeat'].includes(request.action)) throw new CoordinationRuntimeError('recovery-required', 'ordinary session is fenced from dispatch while migration recovery remains pending', pendingRecovery.map((work) => work.recovery_id));
    return session;
  }
