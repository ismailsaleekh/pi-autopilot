...
export class CoordinatorStore {
  ...
  private constructor(db: DatabaseSync, clock: StoreClock, stateRoot: string, lastBackupPath: string | null, options: CoordinatorStoreOpenOptions) {
    this.#db = db;
    this.#clock = clock;
  ...
  static async open(paths: CoordinatorRuntimePaths, clock: StoreClock = systemClock, options: CoordinatorStoreOpenOptions = {}): Promise<CoordinatorStore> {
    ...
    try {
      ...
      if (currentVersion === 6 && existed && options.allowExistingSchemaMigration !== true) throw new CoordinationRuntimeError('schema-mismatch', 'existing predecessor schema 6 requires the private verified migration path; in-place server migration is forbidden');
  ...
  #allInvariantFindings(): readonly CoordinationInvariantFinding[] {
    ...
    for (const row of this.#db.prepare('SELECT repo_id FROM repositories ORDER BY repo_id').all()) findings.push(...checkCoordinationInvariants(this.#snapshotForRepository(sqlString(row, 'repo_id'))));
  ...
  importLegacyCoordination(plan: CoordinationLegacyImportPlan): StoreEffect {
    ...
    try {
      ...
      for (const worktree of plan.worktrees) {
        ...
        else { const current = worktreeFromRow(existing); if (current.canonical_path !== worktree.canonical_path || current.git_common_dir !== worktree.git_common_dir || current.branch !== worktree.branch || current.kind !== worktree.kind) throw new CoordinationRuntimeError('invalid-state', 'legacy worktree disagrees with existing coordinator ownership', [worktree.worktree_id]); }
      }
  ...
  #projectionPage(kind: 'status' | 'doctor', request: CoordinatorRequestEnvelope, initialComplete: Readonly<Record<string, unknown>> | null, sections: readonly string[], initialSnapshot: string | null): StoreEffect {
  ...
  registerAttempt(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      const attempt: CoordinationUnitAttempt = { schema_version: 'autopilot.unit_attempt.v1', owner, state: 'preflight', role: payloadUnitRole(request.payload, 'role'), spec: { ref: payloadString(request.payload, 'spec_ref'), sha256: payloadString(request.payload, 'spec_sha256') as `sha256:${string}` }, preemptible: payloadBoolean(request.payload, 'preemptible'), checkpoint_ordinal: 0, critical_section: null, version: 1 };
  ...
  registerAuthoritativeArtifact(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      if (verifiedCommit.status !== 0 || verifiedCommit.stdout.trim() !== gitCommit) throw new CoordinationRuntimeError('invalid-request', 'authoritative artifact git_commit is not the exact verified commit in its registered source repository', [gitCommit, verifiedCommit.stderr.trim()]);
      const sourceHead = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: sourceRoot, encoding: 'utf8', timeout: 30_000 });
      if (sourceHead.status !== 0 || sourceHead.stdout.trim() !== gitCommit) throw new CoordinationRuntimeError('invalid-request', 'authoritative artifact must be registered from the exact current source authority HEAD', [gitCommit, sourceHead.stdout.trim()]);
      ...
      if (shown.status !== 0) throw new CoordinationRuntimeError('invalid-request', 'authoritative artifact ref is not a blob at the immutable Git commit', [ref, shown.stderr.trim()]);
  ...
  prepareOperation(request: CoordinatorRequestEnvelope): IdempotentEffect {
    return this.#mutation(request, () => {
      ...
      if (existingWorktreeRow === undefined) {
        ...
        if (canonicalJson(existingWorktree) !== canonicalJson(worktree)) throw new CoordinationRuntimeError('idempotency-conflict', 'worktree identity was reused with different immutable authority');
      }
  ...
  #assertReservationIntegrationGitFacts(run: CoordinationRun, predecessorTerminalSha: string, integrationHead: string, protectedPaths: readonly string[], requireExactHead: boolean): void {
    ...
    if ((head.status ?? -1) !== 0) throw new CoordinationRuntimeError('invalid-state', 'reservation integration owned workstream HEAD is unreadable', [head.stderr.trim()]);
    if (currentHead !== integrationHead) {
      ...
      if ((diff.status ?? -1) !== 0) throw new CoordinationRuntimeError('invalid-state', 'failed to verify post-validation reservation path stability', [diff.stderr.trim()]);
  ...
  #assertUnitMergeGitFacts(run: CoordinationRun, facts: ReturnType<typeof parseUnitMergeReservationFacts>): void {
    ...
    const mainRoot = resolve(this.#stateRoot, 'worktrees', repository.repo_key, 'active', run.workstream_run, 'main');
    const git = (args: readonly string[]): { readonly status: number; readonly stdout: string; readonly stderr: string } => {
      const result = spawnSync('git', [...args], { cwd: mainRoot, encoding: 'utf8' });
      return { status: result.status ?? -1, stdout: result.stdout, stderr: result.stderr };
    };
    const head = git(['rev-parse', 'HEAD']);
    if (head.status !== 0 || facts.mergeCommitSha !== facts.integrationAfter) throw new CoordinationRuntimeError('invalid-state', 'unit-merge evidence integration head or merge commit is invalid', [head.stderr.trim(), `actual=${head.stdout.trim()}`, `evidence=${facts.integrationAfter}`]);
    const integrated = git(['merge-base', '--is-ancestor', facts.integrationAfter, head.stdout.trim()]);
    if (integrated.status !== 0) throw new CoordinationRuntimeError('invalid-state', 'unit-merge evidence integration head is not contained in the owned workstream HEAD', [facts.integrationAfter, head.stdout.trim(), integrated.stderr.trim()]);
    for (const sha of [facts.integrationBefore, facts.integrationAfter]) {
      const object = git(['cat-file', '-e', `${sha}^{commit}`]);
      if (object.status !== 0) throw new CoordinationRuntimeError('invalid-state', 'unit-merge evidence references a missing Git commit', [sha, object.stderr.trim()]);
    }
    const ancestor = git(['merge-base', '--is-ancestor', facts.integrationBefore, facts.integrationAfter]);
    if (ancestor.status !== 0) throw new CoordinationRuntimeError('invalid-state', 'unit-merge integration_before is not an ancestor of integration_after', [facts.integrationBefore, facts.integrationAfter]);
    const diff = git(['diff', '--name-only', '--no-renames', '-z', facts.integrationBefore, facts.integrationAfter]);
    if (diff.status !== 0) throw new CoordinationRuntimeError('invalid-state', 'failed to derive exact unit-merge Git diff', [diff.stderr.trim()]);
  ...
  #assertRunTerminalGitFacts(run: CoordinationRun, source: 'run-close' | 'run-abort', terminalSha: string): void {
    ...
    if ((head.status ?? -1) !== 0 || head.stdout.trim() !== terminalSha) throw new CoordinationRuntimeError('invalid-state', `${source} terminal commit is not the authoritative Git HEAD`, [head.stderr.trim(), `actual=${head.stdout.trim()}`, `evidence=${terminalSha}`]);
    ...
    for (const reservation of reservations) {
      ...
      if ((ancestor.status ?? -1) !== 0) throw new CoordinationRuntimeError('invalid-state', 'terminal commit does not contain every reserved accepted merge', [reservation.reservation_id, facts.integrationAfter, terminalSha]);
    }
  }
   ...
   */
  #reconcileSemanticWorktreeDuplicates(repoId: string, workstreamRun: string): void {
    ...
    for (let leftIndex = 0; leftIndex < active.length; leftIndex += 1) {
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < active.length; rightIndex += 1) {
        ...
        if (right === undefined || sameWorktreeAuthority(left, right)) continue;
        const sameOwner = canonicalJson(left.owner) === canonicalJson(right.owner);
        if (sameOwner || left.canonical_path === right.canonical_path) throw new CoordinationRuntimeError('store-corrupt', sameOwner
    ...
    for (const worktree of active) groups.set(worktreeOwnerKindKey(worktree), [...(groups.get(worktreeOwnerKindKey(worktree)) ?? []), worktree]);
    for (const candidates of groups.values()) {
      if (candidates.length < 2) continue;
      if (candidates.length !== 2) throw new CoordinationRuntimeError('store-corrupt', 'active worktree authority has more than two semantic projections', candidates.map((entry) => entry.worktree_id));
      const first = candidates[0];
      const second = candidates[1];
      if (first === undefined || second === undefined) throw new CoordinationRuntimeError('store-corrupt', 'semantic worktree duplicate set changed during reconciliation');
      if (!sameWorktreeAuthority(first, second)) throw new CoordinationRuntimeError('store-corrupt', 'active worktree owner/kind projections conflict in authority-bearing identity', candidates.map((entry) => entry.worktree_id));
      if (first.state !== second.state) throw new CoordinationRuntimeError('recovery-required', 'exact worktree projections disagree in lifecycle state', candidates.map((entry) => `${entry.worktree_id}:${entry.state}`));
      const operationsFor = (id: string): CoordinationWorktreeOperation[] => this.#db.prepare('SELECT * FROM worktree_operations WHERE repo_id=? AND workstream_run=? AND json_extract(payload_json, \'$.worktree_id\')=? ORDER BY entity_id').all(repoId, workstreamRun, id).map(worktreeOperationFromRow);
      ...
      if (incomplete.length > 0) throw new CoordinationRuntimeError('recovery-required', 'duplicate worktree projection has an incomplete operation history', incomplete.map((entry) => entry.operation_id));
      const normalizeHistory = (operations: readonly CoordinationWorktreeOperation[]): string => canonicalJson(operations.map(({ operation_id: _operationId, worktree_id: _worktreeId, ...operation }) => operation));
      if (firstOperations.length > 0 && secondOperations.length > 0 && normalizeHistory(firstOperations) !== normalizeHistory(secondOperations)) throw new CoordinationRuntimeError('recovery-required', 'duplicate worktree projections carry conflicting operation histories', candidates.map((entry) => entry.worktree_id));
      ...
      if (canonical === undefined) throw new CoordinationRuntimeError('recovery-required', 'duplicate worktree projections lack a mechanically selectable deterministic canonical row', candidates.map((entry) => entry.worktree_id));
      ...
      if (canonical.version !== redundant.version) {
        ...
        if (histories.length !== 1 || canonicalOperations.length === 0 || redundant.version !== 1 || canonical.version < redundant.version) throw new CoordinationRuntimeError('recovery-required', 'duplicate worktree projection versions are not explained by one canonical operation history', candidates.map((entry) => `${entry.worktree_id}:v${String(entry.version)}`));
      }
    ...
    const allGroups = new Map<string, CoordinationWorktree[]>();
    for (const worktree of all) allGroups.set(worktreeOwnerKindKey(worktree), [...(allGroups.get(worktreeOwnerKindKey(worktree)) ?? []), worktree]);
    for (const candidates of allGroups.values()) {
      if (candidates.length !== 2) continue;
      const first = candidates[0];
      if (first === undefined || first.kind !== 'unit') continue;
      const deterministicId = deterministicWorktreeId(first.owner, first.kind);
      ...
      if (canonical === undefined || redundant === undefined) continue;
      if (!sameWorktreeAuthority(canonical, redundant)) throw new CoordinationRuntimeError('store-corrupt', 'removed deterministic worktree and its active historical shadow disagree in authority-bearing identity', candidates.map((entry) => entry.worktree_id));
      if (redundant.state !== 'terminal' || redundant.version !== 1) throw new CoordinationRuntimeError('recovery-required', 'historical worktree shadow is not the exact untouched terminal migration projection', [`state=${redundant.state}`, `version=${String(redundant.version)}`]);
      const operationsFor = (id: string): CoordinationWorktreeOperation[] => this.#db.prepare('SELECT * FROM worktree_operations WHERE repo_id=? AND workstream_run=? AND json_extract(payload_json, \'$.worktree_id\')=? ORDER BY entity_id').all(repoId, workstreamRun, id).map(worktreeOperationFromRow);
      ...
      if (childExists || attemptExists) throw new CoordinationRuntimeError('recovery-required', 'historical worktree shadow cannot retire because its owner has child or attempt history', [canonical.owner.unit_id, String(canonical.owner.attempt)]);
      ...
      if (canonical.git_common_dir !== repository.git_common_dir || existsSync(canonical.canonical_path) || this.#gitWorktreeRegistered(repository.canonical_root, canonical.canonical_path) || this.#gitText(repository.canonical_root, ['rev-parse', '--verify', `refs/heads/${canonical.branch}`]) !== null) throw new CoordinationRuntimeError('recovery-required', 'historical worktree shadow cannot retire because the exact removed Git postcondition no longer holds', [canonical.canonical_path, canonical.branch]);
  ...
  #recordWorktreeProjectionRetirement(repoId: string, canonical: CoordinationWorktree, retired: CoordinationWorktree, seq: number, disposition: 'equivalent-active-projection' | 'removed-deterministic-projection-canonical', committedRemoveOperationId: string | null = null): void {
  ...
  #verifyOperationEvidenceFile(operation: CoordinationWorktreeOperation): void {
    ...
    if (!evidence.ref.startsWith(expectedPrefix) || evidence.ref !== `${expectedPrefix}${operation.operation_id}.json`) throw new CoordinationRuntimeError('unauthorized-client', 'operation evidence ref is not derived from its durable owner and operation', [evidence.ref]);
    ...
    try { parsed = parseJsonObject(Buffer.from(bytes).toString('utf8'), 'operation evidence'); } catch (error) { throw new CoordinationRuntimeError('invalid-state', 'operation evidence is not valid JSON', [error instanceof Error ? error.message : String(error)]); }
  ...
  #verifyMigrationReleasePostconditions(run: CoordinationRun, work: CoordinationMigrationRecoveryWork, claim: { readonly path: string; readonly mode: string; readonly unitId: string; readonly attempt: number; readonly editLeaseId: string }, source: Exclude<CoordinationReconciliationSource, 'child-process'>, targetId: string, evidence: { readonly ref: string; readonly sha256: `sha256:${string}` }): readonly string[] {
    ...
    if (source === 'unit-merge') {
      ...
      if (run.status !== expectedStatus || (main.state !== 'terminal' && main.state !== 'removed') || existsSync(main.canonical_path) || this.#gitWorktreeRegistered(resource.source_repo, main.canonical_path) || this.#gitText(resource.source_repo, ['rev-parse', '--verify', `refs/heads/${main.branch}`]) !== null || this.#gitText(resource.source_repo, ['rev-parse', '--verify', `refs/heads/${archiveRef}`]) !== terminalSha || source === 'run-close' && (resource.target_branch === null || this.#gitText(resource.source_repo, ['rev-parse', '--verify', `refs/heads/${resource.target_branch}`]) !== terminalSha)) throw new CoordinationRuntimeError('invalid-state', 'run-terminal migration recovery postconditions are not exact and terminal state was not changed', [run.status, main.state, main.canonical_path, main.branch, archiveRef, terminalSha]);
  ...
  #gitText(cwd: string, args: readonly string[]): string | null {
  ...
  #gitWorktreeRegistered(repoRoot: string, candidate: string): boolean {
    ...
    if (result.status !== 0) throw new CoordinationRuntimeError('recovery-required', 'Git worktree registration inspection failed', [repoRoot, result.stderr]);
