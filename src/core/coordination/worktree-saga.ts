...
import { gitWorktreeRegistrationFacts, inspectWorktreePostcondition, type WorktreePostconditionResult } from './worktree-postconditions.ts';
...
async function writeImmutableEvidence(input: {
  ...
  readonly proof: readonly string[];
  readonly proofSource: WorktreePostconditionResult['proof_source'] | null;
  readonly captureSha: string | null;
...
function assertSpecMatchesActiveAuthority(spec: OwnedWorktreeOperationSpec): void {
  ...
  if (spec.intent.branch !== expectedBranch) throw new CoordinationRuntimeError('unauthorized-client', 'worktree saga branch is not derived from its durable owner', [spec.intent.branch, expectedBranch]);
}
...
export class OwnedWorktreeSagaClient {
  ...
  async prepare(spec: OwnedWorktreeOperationSpec): Promise<{ readonly operation: CoordinationWorktreeOperation; readonly worktree: CoordinationWorktree; readonly replayed: boolean }> {
    ...
    if (spec.operationId !== undefined && !allOperations.some((entry) => entry.operation_id === spec.operationId)) throw new CoordinationRuntimeError('invalid-request', 'caller-supplied worktree operation ID is allowed only to resume an existing historical operation', [spec.operationId, canonicalOperationId]);
    ...
    if (semanticExisting.length > 1) throw new CoordinationRuntimeError('recovery-required', 'multiple historical operations match one canonical operation-key v2 identity', semanticExisting.map((entry) => entry.operation_id));
    ...
    if (semantic.length > 1) throw new CoordinationRuntimeError('recovery-required', 'multiple current worktree projections match one canonical semantic identity', semantic.map((entry) => entry.worktree_id));
    if (semantic.some((entry) => !sameWorktreeAuthority(entry, proposed))) throw new CoordinationRuntimeError('store-corrupt', 'current worktree projection disagrees in authority-bearing identity', semantic.map((entry) => entry.worktree_id));
    ...
    if (existing !== undefined) {
      if (!sameOwner(existing.owner, owner) || existing.operation_type !== spec.operationType || canonicalJson(existing.intent) !== canonicalJson(spec.intent)) throw new CoordinationRuntimeError('idempotency-conflict', 'worktree operation key was reused with different immutable intent');
  ...
  async writeEvidence(operation: CoordinationWorktreeOperation, stage: 'verified' | 'compensated' | 'failed', inspection: WorktreePostconditionResult | null, proof: readonly string[], errorCode: string | null): Promise<CoordinationEvidenceRef> {
...
function operationKind(owner: CoordinationOwnerIdentity): CoordinationWorktreeKind {
  return owner.unit_id === 'main' ? 'main' : 'unit';
}

function inspectOperation(operation: CoordinationWorktreeOperation, env: ProcessEnvLike): WorktreePostconditionResult {
  if (operation.operation_type === 'metadata-reconcile') throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation uses its dedicated exact-set runtime', [operation.operation_id]);
...
function operationTypeSet(...values: readonly CoordinationWorktreeOperationType[]): ReadonlySet<CoordinationWorktreeOperationType> {
  return new Set<CoordinationWorktreeOperationType>(values);
}

const COMMITTED_SUPERSEDING_OPERATIONS: Readonly<Record<CoordinationWorktreeOperationType, ReadonlySet<CoordinationWorktreeOperationType>>> = Object.freeze({
...
async function inspectCommittedOperation(client: OwnedWorktreeSagaClient, operation: CoordinationWorktreeOperation, env: ProcessEnvLike): Promise<WorktreePostconditionResult> {
  const direct = inspectOperation(operation, env);
  if (direct.outcome === 'satisfied') return direct;
  ...
  for (const candidate of later) {
    ...
    if (current.outcome !== 'satisfied') continue;
    return Object.freeze({
      outcome: 'satisfied',
...
async function sagaExecutionLockIsStale(lockPath: string): Promise<boolean> {
  try {
    ...
    if (typeof pid !== 'number' || !Number.isSafeInteger(pid) || pid < 1 || typeof bootId !== 'string') return false;
...
export async function executeOwnedWorktreeSaga(spec: OwnedWorktreeOperationSpec, callbacks: WorktreeSagaCallbacks, env: ProcessEnvLike = process.env): Promise<WorktreeSagaResult> {
  ...
  if (operation.stage === 'committed') {
    ...
    if (committedInspection.outcome !== 'satisfied') throw new CoordinationRuntimeError('recovery-required', 'committed worktree operation disagrees with its canonical postcondition', committedInspection.proof);
  ...
  if (operation.stage === 'verified') {
    ...
    if (verifiedInspection.outcome !== 'satisfied') throw new CoordinationRuntimeError('recovery-required', 'verified worktree operation no longer satisfies its canonical postcondition', verifiedInspection.proof);
...
function assertExternalWorktreeAuthority(spec: OwnedWorktreeOperationSpec, env: ProcessEnvLike, ownedWorktreeRoot: string): void {
  ...
  if (!existsSync(intent.worktree_path)) return;
...
function ownedMaterializationDirectory(worktreePath: string, path: string): string {
  const normalized = path.replace(/\\/gu, '/').replace(/^\.\//u, '');
  if (normalized.length === 0 || normalized.startsWith('/') || normalized.includes('\0') || normalized.split('/').some((segment) => segment === '..' || segment === '')) throw new CoordinationRuntimeError('invalid-state', 'materialize intent contains an unbounded repository path', [path]);
  ...
  if (rel.startsWith('..') || isAbsolute(rel)) throw new CoordinationRuntimeError('invalid-state', 'materialize intent escapes its owned worktree', [path, worktreePath]);
  return directory;
}

function sagaGitEnv(env: ProcessEnvLike): ProcessEnvLike {
  return {
    ...process.env,
    ...env,
    AUTOPILOT_RUNTIME: '1',
...
async function fixedAction(operation: CoordinationWorktreeOperation, env: ProcessEnvLike): Promise<void> {
  if (operation.operation_type === 'metadata-reconcile') throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation requires the exact-set metadata runtime', [operation.operation_id]);
  ...
  switch (operation.operation_type) {
    case 'create': {
      ...
      if (branchExists) {
        ...
        if (actual !== intent.base_sha) throw new CoordinationRuntimeError('recovery-required', 'create branch moved between canonical probe and action', [intent.branch, `expected=${intent.base_sha}`, `actual=${actual}`]);
      }
      ...
      if (!registered) await runGitMutation({ descriptor: { kind: 'worktree-add', path: intent.worktree_path, branch: intent.branch, startPoint: branchExists ? null : intent.base_sha, createBranch: !branchExists, noCheckout: intent.checkout_mode !== 'full' }, cwd: intent.repo_root, env: gitEnv });
    ...
    case 'materialize':
      if (intent.sparse_patterns.length > 0) await runGitMutation({ descriptor: { kind: 'sparse-checkout-add', patterns: intent.sparse_patterns }, cwd: intent.worktree_path, env: gitEnv });
      for (const path of intent.paths) mkdirSync(ownedMaterializationDirectory(intent.worktree_path, path), { recursive: true });
      return;
    case 'commit':
    case 'quarantine': {
      ...
      if (staged.negative) await runGitMutation({ descriptor: { kind: 'commit', message: `${operation.operation_type} ${operation.owner.workstream_run} ${operation.owner.unit_id} attempt ${String(operation.owner.attempt)}` }, cwd: intent.worktree_path, env: gitEnv });
      return;
    }
    case 'merge': {
      if (intent.target_sha === null) throw new CoordinationRuntimeError('invalid-state', 'merge operation intent lacks target_sha');
      if (inspectOperation(operation, env).outcome === 'satisfied') return;
      ...
      if (existsSync(isAbsolute(mergeHeadPath) ? mergeHeadPath : resolve(intent.worktree_path, mergeHeadPath))) {
        ...
        if (intent.base_sha === null || currentHead !== intent.base_sha) throw new CoordinationRuntimeError('recovery-required', 'interrupted merge cannot be safely compensated because target HEAD moved');
        ...
        if (afterAbort.outcome === 'unsafe') throw new CoordinationRuntimeError('recovery-required', 'interrupted merge abort did not restore safe repository state', afterAbort.proof);
    ...
    case 'remove': {
      ...
      if (!branchBefore.negative) {
        ...
        if (actualBefore !== intent.target_sha) throw new CoordinationRuntimeError('recovery-required', 'owned branch moved after operation intent; refusing retirement', [intent.branch, actualBefore, intent.target_sha]);
      }
      const registered = gitWorktreeRegistrationFacts(intent.repo_root, gitEnv).some((entry) => entry.worktree_path === canonicalPath(intent.worktree_path));
      if (existsSync(intent.worktree_path) || registered) await runGitMutation({ descriptor: { kind: 'worktree-remove', path: intent.worktree_path }, cwd: intent.repo_root, env: gitEnv });
      ...
      if (!branchAfter.negative) {
        if (intent.target_sha === null) throw new CoordinationRuntimeError('recovery-required', 'owned branch appeared during remove without an authorized expected SHA', [intent.branch]);
...
export async function ensureMainWorktreeSagaRegistered(input: { readonly active: ActiveAutopilotRow; readonly env?: ProcessEnvLike }): Promise<CoordinationWorktreeOperation> {
  ...
  if (registered.length === 1) {
    ...
    if (inspected.outcome !== 'satisfied') throw new CoordinationRuntimeError('recovery-required', 'committed main worktree registration disagrees with canonical postcondition truth', inspected.proof);
    return existing;
  }
...
export async function recoverOwnedWorktreeSagas(input: { readonly active: ActiveAutopilotRow; readonly env?: ProcessEnvLike }): Promise<readonly CoordinationWorktreeOperation[]> {
  ...
  for (const candidate of operations) {
    ...
    if (matchingWorktrees.length !== 1 || worktree === undefined) throw new CoordinationRuntimeError('store-corrupt', 'recoverable operation lacks exactly one canonical worktree owner', [candidate.operation_id, candidate.worktree_id, canonicalWorktreeId]);
