...
export interface OwnedWorktreeOperationSpec {
  ...
  readonly committedWorktreeState: CoordinationWorktreeState;
}

export interface WorktreeSagaInspection {
  readonly outcome: 'satisfied' | 'not-applied' | 'unsafe';
  readonly proof: readonly string[];
}

export const WORKTREE_SAGA_BOUNDARIES = ['after-prepare', 'before-probe', 'after-probe', 'after-start', 'before-action', 'after-action', 'after-action-report', 'before-verification', 'after-verification', 'after-evidence', 'after-verified-commit', 'after-terminal-commit'] as const;
export type WorktreeSagaBoundary = (typeof WORKTREE_SAGA_BOUNDARIES)[number];

export interface WorktreeSagaCallbacks {
  inspect(): Promise<WorktreeSagaInspection> | WorktreeSagaInspection;
  action(): Promise<void> | void;
  verify(): Promise<readonly string[]> | readonly string[];
  observeBoundary?(boundary: WorktreeSagaBoundary): Promise<void> | void;
}

async function observeBoundary(callbacks: WorktreeSagaCallbacks, boundary: WorktreeSagaBoundary): Promise<void> {
  await callbacks.observeBoundary?.(boundary);
}

export class WorktreeSagaCompensatedError extends Error {
  override readonly name = 'WorktreeSagaCompensatedError';
  readonly proof: readonly string[];

  constructor(message: string, proof: readonly string[]) {
    super(message);
    this.proof = Object.freeze([...proof]);
  }
}

export interface WorktreeSagaResult {
  readonly managed: boolean;
  readonly operation: CoordinationWorktreeOperation | null;
  readonly worktree: CoordinationWorktree | null;
  readonly replayed: boolean;
}

export type WorktreeSagaExecutionPhase = 'prepared' | 'preflight-probe' | 'start-report' | 'external-action' | 'action-report' | 'postcondition-verification' | 'evidence-write' | 'verified-report' | 'commit-report';

function phaseCauseEvidence(prefix: 'cause' | 'reconciliation', error: unknown, maximumDetails: number): readonly string[] {
...
function responseOperation(response: CoordinatorResponseEnvelope): CoordinationWorktreeOperation {
  return parseCoordinationWorktreeOperation(response.payload['operation']);
}

function responseWorktree(response: CoordinatorResponseEnvelope): CoordinationWorktree {
...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new CoordinationRuntimeError('invalid-request', 'saga evidence contains a non-finite number');
    return JSON.stringify(value);
  }
  if (Array.isArray(value)) return `[${value.map((entry) => canonicalJson(entry)).join(',')}]`;
  if (typeof value !== 'object') throw new CoordinationRuntimeError('invalid-request', 'saga evidence contains a non-JSON value');
  const object = value as Readonly<Record<string, unknown>>;
  return `{${Object.keys(object).sort().map((key) => `${JSON.stringify(key)}:${canonicalJson(object[key])}`).join(',')}}`;
}

function ownerFor(spec: OwnedWorktreeOperationSpec): CoordinationOwnerIdentity {
  return {
...
function operationEvidencePath(session: CoordinatorSessionContext, ref: string): string {
...
async function writeImmutableEvidence(input: {
  ...
  readonly stage: 'verified' | 'compensated' | 'failed';
  readonly proof: readonly string[];
...
function assertSessionOwnsSpec(session: CoordinatorSessionContext, spec: OwnedWorktreeOperationSpec): void {
  if (session.repo_id !== spec.active.repo_key || session.repo_key !== spec.active.repo_key || session.autopilot_id !== spec.active.autopilot_id || session.workstream_run !== spec.active.workstream_run) throw new CoordinationRuntimeError('unauthorized-client', 'worktree saga session does not own the requested active run');
  if (spec.attempt < 1 || spec.unitId.length === 0) throw new CoordinationRuntimeError('invalid-request', 'worktree saga requires a durable unit attempt identity');
  ...
  if (resolve(spec.intent.worktree_path) !== expectedPath) throw new CoordinationRuntimeError('unauthorized-client', 'worktree saga path is not derived from its durable owner', [spec.intent.worktree_path, expectedPath]);
}

async function durableRunExists(active: ActiveAutopilotRow, env: ProcessEnvLike): Promise<boolean> {
...
export class OwnedWorktreeSagaClient {
  ...
  static async fromEnvironment(env: ProcessEnvLike = process.env): Promise<OwnedWorktreeSagaClient> {
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0) throw new CoordinationRuntimeError('unauthorized-client', `${AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV} is required for owner-scoped worktree operations`);
  ...
  async operations(): Promise<readonly CoordinationWorktreeOperation[]> {
  ...
  async worktrees(): Promise<readonly CoordinationWorktree[]> {
    const status = await this.#client.query('status', this.#session.repo_id, this.#session.workstream_run);
    return Object.freeze(array(status.payload['worktrees'], 'worktrees').map((entry) => parseCoordinationWorktree(entry)));
  }

  async prepare(spec: OwnedWorktreeOperationSpec): Promise<{ readonly operation: CoordinationWorktreeOperation; readonly worktree: CoordinationWorktree; readonly replayed: boolean }> {
    assertSessionOwnsSpec(this.#session, spec);
    const owner = ownerFor(spec);
    const id = deterministicWorktreeId(owner, spec.kind);
    const proposed: CoordinationWorktree = {
  ...
  async transition(input: {
    readonly operation: CoordinationWorktreeOperation;
    readonly stage: CoordinationWorktreeOperation['stage'];
    readonly completedSteps: readonly string[];
    readonly currentStep: string | null;
    readonly recoveryAttempts: number;
    readonly verificationEvidence: CoordinationEvidenceRef | null;
    readonly errorCode: string | null;
    readonly worktreeState: CoordinationWorktreeState;
    readonly transitionKey: string;
  }): Promise<{ readonly operation: CoordinationWorktreeOperation; readonly worktree: CoordinationWorktree }> {
  ...
  async writeEvidence(operation: CoordinationWorktreeOperation, stage: 'verified' | 'compensated' | 'failed', proof: readonly string[], errorCode: string | null): Promise<CoordinationEvidenceRef> {
    return await writeImmutableEvidence({ session: this.#session, operation, stage, proof, errorCode });
  }

  #identity(idempotencyKey: string, expectedVersion: number) {
    return {
  ...
  #proof(): { readonly session_lease_id: string; readonly session_token: string } {
...
function errorCode(error: unknown): string {
  ...
  if (error instanceof Error && 'code' in error && typeof error.code === 'string') return error.code.slice(0, 128);
...
async function withSagaExecutionLock<T>(session: CoordinatorSessionContext, spec: OwnedWorktreeOperationSpec, run: () => Promise<T>): Promise<T> {
  const owner = ownerFor(spec);
  ...
  for (let attempt = 0; attempt < 1_400; attempt += 1) {
    ...
    try {
      ...
      try {
        ...
        await unlink(lockPath).catch((cleanupError: unknown) => {
      ...
      if (acquiredLock || !(error instanceof Error && 'code' in error && error.code === 'EEXIST')) throw error;
      let reclaimer: Awaited<ReturnType<typeof open>> | null = null;
      try {
        reclaimer = await open(reclaimPath, 'wx', 0o600);
        if (await sagaExecutionLockIsStale(lockPath)) await unlink(lockPath).catch((unlinkError: unknown) => {
          if (!(unlinkError instanceof Error && 'code' in unlinkError && unlinkError.code === 'ENOENT')) throw unlinkError;
        });
      } catch (reclaimError) {
        if (!(reclaimError instanceof Error && 'code' in reclaimError && reclaimError.code === 'EEXIST')) throw reclaimError;
      } finally {
        if (reclaimer !== null) {
...
export async function executeOwnedWorktreeSaga(spec: OwnedWorktreeOperationSpec, callbacks: WorktreeSagaCallbacks, env: ProcessEnvLike = process.env): Promise<WorktreeSagaResult> {
  ...
  if (contextPath === undefined || contextPath.trim().length === 0) {
    ...
    if (inspection.outcome === 'unsafe') throw new CoordinationRuntimeError('recovery-required', 'unmanaged legacy worktree operation found unsafe external state', inspection.proof);
    if (inspection.outcome === 'not-applied') await callbacks.action();
  ...
  if (operation.stage === 'compensated' || operation.stage === 'failed') throw new CoordinationRuntimeError('recovery-required', `worktree operation is terminal at ${operation.stage}`, [operation.operation_id]);
  ...
  try {
    ...
    if (inspection.outcome === 'unsafe') throw new CoordinationRuntimeError('recovery-required', 'worktree operation requires owned recovery before mutation', inspection.proof);
...
function git(args: readonly string[], cwd: string, env: ProcessEnvLike, input?: string): string {
  ...
  if (result.error !== undefined) throw new CoordinationRuntimeError('git-partial-effect', `git ${args.join(' ')} failed to spawn`, [result.error.message]);
  if ((result.status ?? -1) !== 0) throw new CoordinationRuntimeError('git-partial-effect', `git ${args.join(' ')} exited with status ${String(result.status ?? -1)}`, [result.stderr.trim(), result.stdout.trim()]);
...
function worktreeEntries(repoRoot: string, env: ProcessEnvLike): readonly string[] {
...
function registeredBranch(repoRoot: string, worktreePath: string, env: ProcessEnvLike): string | null {
  const expected = canonicalPath(worktreePath);
  let currentPath: string | null = null;
  for (const line of git(['worktree', 'list', '--porcelain'], repoRoot, env).split('\n')) {
    if (line.startsWith('worktree ')) currentPath = canonicalPath(line.slice('worktree '.length));
    else if (line.startsWith('branch ') && currentPath === expected) return line.slice('branch refs/heads/'.length);
  }
  return null;
}

function isRegistered(intent: CoordinationWorktreeOperationIntent, env: ProcessEnvLike): boolean {
  return worktreeEntries(intent.repo_root, env).includes(canonicalPath(intent.worktree_path));
}

function assertNoSymlinkSegments(ownedRoot: string, target: string, label: string): void {
  const lexicalRoot = resolve(ownedRoot);
  const lexicalTarget = resolve(target);
  const ownedRelative = relative(lexicalRoot, lexicalTarget);
  if (ownedRelative === '' || ownedRelative.startsWith('..') || isAbsolute(ownedRelative) || ownedRelative.split(sep).includes('..')) throw new CoordinationRuntimeError('unauthorized-client', `${label} path escapes its package-owned state root`, [lexicalRoot, lexicalTarget]);
  let cursor = lexicalRoot;
  for (const segment of ownedRelative.split(sep)) {
    if (existsSync(cursor) && lstatSync(cursor).isSymbolicLink()) throw new CoordinationRuntimeError('recovery-required', `symlink substitution detected in ${label} path`, [cursor]);
    cursor = join(cursor, segment);
  }
  if (existsSync(cursor) && lstatSync(cursor).isSymbolicLink()) throw new CoordinationRuntimeError('recovery-required', `symlink substitution detected at ${label} path`, [cursor]);
}

function assertExternalWorktreeAuthority(spec: OwnedWorktreeOperationSpec, env: ProcessEnvLike, ownedWorktreeRoot: string): void {
  ...
  if (!absentCreate && !absentRemove && (!present || !registered)) throw new CoordinationRuntimeError('recovery-required', 'external worktree path and Git registration disagree with durable authority', [`path_present=${String(present)}`, `git_registered=${String(registered)}`]);
  if (!present) {
    if (registered && registeredBranch(intent.repo_root, intent.worktree_path, env) !== intent.branch) throw new CoordinationRuntimeError('recovery-required', 'stale worktree metadata branch disagrees with durable authority', [intent.branch]);
    return;
  }
  ...
  if (actualCommon !== expectedCommon) throw new CoordinationRuntimeError('recovery-required', 'external worktree Git common-dir disagrees with durable repository authority', [`expected=${expectedCommon}`, `actual=${actualCommon}`]);
}

function changedPaths(path: string, env: ProcessEnvLike): readonly string[] {
...
function head(path: string, env: ProcessEnvLike): string {
  return git(['rev-parse', 'HEAD'], path, env);
}

function branch(path: string, env: ProcessEnvLike): string {
  return git(['rev-parse', '--abbrev-ref', 'HEAD'], path, env);
}

function fixedInspection(operation: CoordinationWorktreeOperation, env: ProcessEnvLike): WorktreeSagaInspection {
  ...
  if (present !== registered) {
    if (operation.operation_type === 'remove' && !present && registered) {
      ...
      if (metadataBranch !== intent.branch) return { outcome: 'unsafe', proof: [`expected_metadata_branch=${intent.branch}`, `actual_metadata_branch=${String(metadataBranch)}`] };
    } else return { outcome: 'unsafe', proof: [`path_present=${String(present)}`, `git_registered=${String(registered)}`] };
  }
  if (operation.operation_type === 'create') {
    if (!present) {
      const branchCheck = spawnSync('git', ['show-ref', '--verify', '--quiet', `refs/heads/${intent.branch}`], { cwd: intent.repo_root, encoding: 'utf8' });
      if ((branchCheck.status ?? -1) !== 0) return { outcome: 'not-applied', proof: ['path_absent', 'git_registration_absent', 'branch_absent'] };
    ...
    if (branch(intent.worktree_path, env) !== intent.branch) return { outcome: 'unsafe', proof: ['registered_branch_mismatch'] };
    const currentHead = head(intent.worktree_path, env);
    if (intent.base_sha !== null && currentHead !== intent.base_sha) return { outcome: 'unsafe', proof: [`expected_head=${intent.base_sha}`, `actual_head=${currentHead}`] };
    if (intent.checkout_mode !== null && intent.checkout_mode !== 'full') {
      ...
      if ((sparse.status ?? -1) !== 0 || sparse.stdout.trim() !== 'true') return { outcome: 'not-applied', proof: ['worktree_registered', 'sparse_configuration_incomplete'] };
    }
  ...
  if (operation.operation_type === 'remove') {
    ...
    if (branchPresent && intent.target_sha !== null) {
      const branchSha = git(['rev-parse', `refs/heads/${intent.branch}`], intent.repo_root, env);
      if (branchSha !== intent.target_sha) return { outcome: 'unsafe', proof: [`branch_expected=${intent.target_sha}`, `branch_actual=${branchSha}`] };
    }
    if (!present) return branchPresent ? { outcome: 'not-applied', proof: ['path_absent', 'git_registration_absent', 'branch_present'] } : { outcome: 'satisfied', proof: ['path_absent', 'git_registration_absent', 'branch_absent'] };
  ...
  if (!present) return { outcome: 'unsafe', proof: ['owned_worktree_missing'] };
  if (operation.operation_type === 'reset') {
    const dirty = changedPaths(intent.worktree_path, env);
    if (dirty.length > 0) return { outcome: 'unsafe', proof: dirty.map((path) => `uncaptured_dirty=${path}`) };
    const currentHead = head(intent.worktree_path, env);
  ...
  if (operation.operation_type === 'archive') {
    if (intent.archive_ref === null || intent.target_sha === null) return { outcome: 'unsafe', proof: ['archive_intent_incomplete'] };
  ...
  if (operation.operation_type === 'merge' && intent.archive_ref !== null && intent.target_sha !== null && intent.base_sha !== null) {
    ...
    if (currentBranch !== intent.archive_ref) return { outcome: 'unsafe', proof: [`expected_target_branch=${intent.archive_ref}`, `actual_branch=${currentBranch}`] };
    if (currentHead === intent.target_sha) return { outcome: 'satisfied', proof: [`target_head=${currentHead}`] };
  ...
  if (operation.operation_type === 'merge' && intent.target_sha !== null && intent.base_sha !== null) {
    ...
    const currentHead = head(intent.worktree_path, env);
    if (existsSync(isAbsolute(mergeHeadPath) ? mergeHeadPath : resolve(intent.worktree_path, mergeHeadPath))) return currentHead === intent.base_sha
    ...
    if (changedPaths(intent.worktree_path, env).length > 0) return { outcome: 'unsafe', proof: ['merge_target_dirty_without_owned_merge_state'] };
  ...
  if (operation.operation_type === 'commit' || operation.operation_type === 'quarantine') {
    const currentHead = head(intent.worktree_path, env);
    const dirty = changedPaths(intent.worktree_path, env);
    if (dirty.length > 0) return { outcome: 'not-applied', proof: dirty.map((path) => `dirty=${path}`) };
    if (intent.target_sha !== null) return currentHead === intent.target_sha ? { outcome: 'satisfied', proof: [`head=${currentHead}`, 'worktree_clean'] } : { outcome: 'unsafe', proof: [`expected_head=${intent.target_sha}`, `actual_head=${currentHead}`] };
    if (intent.base_sha === null || currentHead === intent.base_sha) return operation.operation_type === 'quarantine' && intent.paths.length === 0 ? { outcome: 'satisfied', proof: [`head=${currentHead}`, 'worktree_clean'] } : { outcome: 'not-applied', proof: [`head=${currentHead}`, 'worktree_clean'] };
    const parents = git(['rev-list', '--parents', '-n', '1', currentHead], intent.worktree_path, env).split(/\s+/u).slice(1);
  ...
  if (operation.operation_type === 'materialize') {
    const sparse = spawnSync('git', ['config', '--bool', 'core.sparseCheckout'], { cwd: intent.worktree_path, encoding: 'utf8' });
    if ((sparse.status ?? -1) !== 0 || sparse.stdout.trim() !== 'true') return { outcome: 'unsafe', proof: ['sparse_checkout_disabled'] };
    ...
    for (const path of intent.paths) {
      ...
      for (const line of result.stdout.split('\n').filter((entry) => entry.length > 2 && !entry.startsWith('S '))) {
        ...
        if (!fileInfo.isFile() || fileInfo.size > 1_024) continue;
        if (readFileSync(absolute).subarray(0, 256).toString().startsWith('version https://git-lfs.github.com/spec/v1')) lfsPointers.push(trackedPath);
      }
    }
    if (lfsPointers.length > 0) return { outcome: 'unsafe', proof: lfsPointers.map((path) => `lfs_pointer=${path}`) };
...
function fixedAction(operation: CoordinationWorktreeOperation, env: ProcessEnvLike): void {
  const intent = operation.intent;
  switch (operation.operation_type) {
    case 'create': {
      if (intent.base_sha === null || intent.checkout_mode === null) throw new CoordinationRuntimeError('invalid-state', 'create operation intent lacks base_sha or checkout_mode');
      const branchCheck = spawnSync('git', ['show-ref', '--verify', '--quiet', `refs/heads/${intent.branch}`], { cwd: intent.repo_root, encoding: 'utf8' });
      const branchExists = (branchCheck.status ?? -1) === 0;
      const registered = isRegistered(intent, env);
      if (!registered && intent.checkout_mode === 'full') git(branchExists ? ['worktree', 'add', intent.worktree_path, intent.branch] : ['worktree', 'add', '-b', intent.branch, intent.worktree_path, intent.base_sha], intent.repo_root, env);
      else if (!registered) git(branchExists ? ['worktree', 'add', '--no-checkout', intent.worktree_path, intent.branch] : ['worktree', 'add', '--no-checkout', '-b', intent.branch, intent.worktree_path, intent.base_sha], intent.repo_root, env);
      if (intent.checkout_mode !== 'full') {
        if (intent.sparse_patterns.length === 0) throw new CoordinationRuntimeError('invalid-state', 'sparse create operation lacks patterns');
    ...
    case 'materialize':
      if (intent.sparse_patterns.length > 0) git(['sparse-checkout', 'add', '--skip-checks', '--stdin'], intent.worktree_path, env, `${intent.sparse_patterns.join('\n')}\n`);
      for (const path of intent.paths) {
    ...
    case 'commit':
    case 'quarantine':
      if (changedPaths(intent.worktree_path, env).length === 0) return;
      ...
      return;
    case 'merge':
      if (intent.target_sha === null) throw new CoordinationRuntimeError('invalid-state', 'merge operation intent lacks target_sha');
      if (fixedInspection(operation, env).outcome !== 'satisfied') {
        if (intent.archive_ref !== null) git(['merge', '--ff-only', intent.target_sha], intent.repo_root, env);
        else {
          const mergeHeadPath = git(['rev-parse', '--git-path', 'MERGE_HEAD'], intent.worktree_path, env);
          if (existsSync(isAbsolute(mergeHeadPath) ? mergeHeadPath : resolve(intent.worktree_path, mergeHeadPath))) {
            if (intent.base_sha === null || head(intent.worktree_path, env) !== intent.base_sha) throw new CoordinationRuntimeError('recovery-required', 'interrupted merge cannot be safely compensated because target HEAD moved');
    ...
    case 'reset':
      ...
      return;
    case 'archive':
      if (intent.archive_ref === null || intent.target_sha === null) throw new CoordinationRuntimeError('invalid-state', 'archive operation intent lacks ref or target');
    ...
    case 'remove':
      if (existsSync(intent.worktree_path)) {
        if (changedPaths(intent.worktree_path, env).length > 0) throw new CoordinationRuntimeError('recovery-required', 'dirty worktree cannot be removed; quarantine capture is required first', intent.paths);
      ...
      }
      {
        ...
        if ((branchCheck.status ?? -1) === 0) {
          if (intent.target_sha !== null && git(['rev-parse', `refs/heads/${intent.branch}`], intent.repo_root, env) !== intent.target_sha) throw new CoordinationRuntimeError('recovery-required', 'owned branch moved after operation intent; refusing retirement', [intent.branch]);
          if (intent.target_sha === null) throw new CoordinationRuntimeError('invalid-state', 'remove operation intent lacks the expected branch SHA');
...
function missingOperationMetadata(operation: CoordinationWorktreeOperation): readonly string[] {
  ...
  if (existsSync(taskInfoPath)) {
    try {
      const taskInfo = record(JSON.parse(readFileSync(taskInfoPath, 'utf8')) as unknown, 'task info for operation metadata recovery');
...
function fixedVerify(operation: CoordinationWorktreeOperation, env: ProcessEnvLike): readonly string[] {
  ...
  if (inspected.outcome !== 'satisfied') throw new CoordinationRuntimeError('git-partial-effect', 'worktree operation postcondition is not satisfied', inspected.proof);
  const missingMetadata = missingOperationMetadata(operation);
  if (missingMetadata.length > 0) throw new CoordinationRuntimeError('git-partial-effect', 'worktree operation metadata postcondition is not satisfied', missingMetadata.map((ref) => `missing_metadata=${ref}`));
  const intent = operation.intent;
  if (existsSync(intent.worktree_path)) {
    ...
    if (normalizedActual !== expectedCommon) throw new CoordinationRuntimeError('unauthorized-client', 'worktree Git common-dir identity does not match durable authority', [normalizedActual, expectedCommon]);
  }
  return inspected.proof;
}

export async function ensureMainWorktreeSagaRegistered(input: { readonly active: ActiveAutopilotRow; readonly env?: ProcessEnvLike }): Promise<CoordinationWorktreeOperation> {
  ...
  if (registered.length > 1) throw new CoordinationRuntimeError('store-corrupt', 'durable run has duplicate main worktree create operations');
  if (registered.length === 1) {
    const existing = registered[0];
    if (existing === undefined) throw new CoordinationRuntimeError('store-corrupt', 'main worktree operation disappeared');
    if (existing.stage !== 'committed') throw new CoordinationRuntimeError('recovery-required', 'main worktree create operation must recover before registration can complete', [existing.operation_id, existing.stage]);
    return existing;
  }
  const inspect = (): WorktreeSagaInspection => {
    if (!existsSync(active.main_worktree_path)) return { outcome: 'unsafe', proof: ['main_worktree_missing'] };
    const intent: CoordinationWorktreeOperationIntent = {
    ...
    if (!isRegistered(intent, env)) return { outcome: 'unsafe', proof: ['main_worktree_not_git_registered'] };
  ...
  const result = await executeOwnedWorktreeSaga({
    ...
    verify: () => {
      ...
      if (inspected.outcome !== 'satisfied') throw new CoordinationRuntimeError('recovery-required', 'main worktree registration postcondition failed', inspected.proof);
      return inspected.proof;
    },
  }, env);
  if (result.operation === null) throw new CoordinationRuntimeError('invalid-state', 'durable main worktree registration unexpectedly used an unmanaged path');
  return result.operation;
}

export async function recoverOwnedWorktreeSagas(input: { readonly active: ActiveAutopilotRow; readonly env?: ProcessEnvLike }): Promise<readonly CoordinationWorktreeOperation[]> {
  const env = input.env ?? process.env;
  const client = await OwnedWorktreeSagaClient.fromEnvironment(env);
  if (client.session.repo_id !== input.active.repo_key || client.session.autopilot_id !== input.active.autopilot_id || client.session.workstream_run !== input.active.workstream_run) throw new CoordinationRuntimeError('unauthorized-client', 'worktree saga recovery session does not own the active run');
  const recovered: CoordinationWorktreeOperation[] = [];
  for (const candidate of await client.operations()) {
    ...
    if (result.operation !== null) recovered.push(result.operation);
  }
  return Object.freeze(recovered);
}

export function fixedWorktreeSagaCallbacks(operation: CoordinationWorktreeOperation, env: ProcessEnvLike = process.env): WorktreeSagaCallbacks {
  return {
