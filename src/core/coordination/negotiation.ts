    ...
    if ((input.acquisitionKind ?? 'initial') === 'initial') {
      ...
      if (rebound !== undefined) {
        ...
        if (verified !== undefined) {
          ...
          if (editLeases.length !== verified.requested_leases.length || verified.grant_event_seq === null) throw new CoordinationRuntimeError('invalid-state', 'migrated acquisition authority is incomplete and requires supervisor recovery');
