...
export const D65_ED25519_SPKI_BYTE_COUNT = 44 as const;
export const D65_ED25519_RAW_KEY_BYTE_COUNT = 32 as const;
export const D65_ED25519_SIGNATURE_BYTE_COUNT = 64 as const;
...
export type D65SignaturePurpose = keyof typeof D65_SIGNATURE_DOMAINS;

const BASE64URL_UNPADDED = /^[A-Za-z0-9_-]+$/u;
const BASE64URL_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';

export interface D65TrustAnchor {
  /** The exact 44 canonical SPKI bytes. */
  readonly spki: Uint8Array;
  /** `sha256:<64 lowercase hex>` of the 44 bytes. */
  readonly sha256: `sha256:${string}`;
  /** Node public key object reconstructed from the SPKI. */
 ...
 */
export function decodeUnpaddedBase64Url(value: string): Uint8Array | null {
  if (typeof value !== 'string' || value.length === 0 || !BASE64URL_UNPADDED.test(value)) return null;
...
export function encodeUnpaddedBase64Url(bytes: Uint8Array): string {
  ...
  if (bits > 0) out += BASE64URL_ALPHABET[(buffer << (6 - bits)) & 0x3f];
  return out;
}
 ...
 * fallback parser.
 */
export function parseD65TrustAnchorSpki(bytes: Uint8Array): D65TrustAnchor {
  if (!(bytes instanceof Uint8Array)) throw new CoordinationRuntimeError('invalid-state', 'trust anchor bytes must be a byte buffer');
  if (bytes.byteLength !== D65_ED25519_SPKI_BYTE_COUNT) throw new CoordinationRuntimeError('invalid-state', `trust anchor must be exactly ${String(D65_ED25519_SPKI_BYTE_COUNT)} canonical SPKI bytes`, [`bytes=${String(bytes.byteLength)}`]);
  for (let index = 0; index < D65_ED25519_SPKI_PREFIX.length; index += 1) {
    if (bytes[index] !== D65_ED25519_SPKI_PREFIX[index]) throw new CoordinationRuntimeError('invalid-state', 'trust anchor SPKI prefix is not the canonical Ed25519 DER header');
  }
  ...
  if (publicKey.asymmetricKeyType !== 'ed25519') throw new CoordinationRuntimeError('invalid-state', 'trust anchor key type must be ed25519');
  ...
  if (reexported.byteLength !== D65_ED25519_SPKI_BYTE_COUNT || !bytesEqual(reexported, spki)) throw new CoordinationRuntimeError('invalid-state', 'trust anchor SPKI is not the canonical minimal DER encoding');
...
export function verifyD65Signature(input: {
  readonly trustAnchor: D65TrustAnchor;
  readonly purpose: D65SignaturePurpose;
  readonly message: Uint8Array;
  readonly signature: string;
}): boolean {
  const signatureBytes = decodeUnpaddedBase64Url(input.signature);
  if (signatureBytes === null || signatureBytes.byteLength !== D65_ED25519_SIGNATURE_BYTE_COUNT) return false;
  // Reject any signature whose canonical re-encoding differs (no padding/alias).
  if (encodeUnpaddedBase64Url(signatureBytes) !== input.signature) return false;
