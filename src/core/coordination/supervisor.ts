...
function d65BootstrapContextPath(sessionsRoot: string, repoId: string, workstreamRun: string, sessionLeaseId: string): string {
  return join(sessionsRoot, `${createHash('sha256').update(`${repoId}\0${workstreamRun}\0${sessionLeaseId}`, 'utf8').digest('hex')}.json`);
}

const D65_STAGED_SESSION_TOKEN_SCHEMA = 'autopilot.d65_staged_session_token.v1' as const;
 ...
 */
async function writeD65StagedSessionToken(stagedPath: string, input: { readonly session_lease_id: string; readonly session_token: string }): Promise<void> {
 ...
 */
async function readD65StagedSessionToken(stagedPath: string, expectedLeaseId: string): Promise<string | null> {
  if (!existsSync(stagedPath)) return null;
  let value: unknown;
  try {
    value = JSON.parse(await readFile(stagedPath, 'utf8')) as unknown;
  } catch (error) {
  ...
  if (record['schema_version'] !== D65_STAGED_SESSION_TOKEN_SCHEMA) throw new CoordinationRuntimeError('invalid-state', 'D65 staged session token schema is incompatible', [stagedPath]);
  const leaseId = requireString(record, 'session_lease_id');
  const token = requireAuthorityToken(record, 'session_token');
  if (leaseId !== expectedLeaseId) throw new CoordinationRuntimeError('invalid-state', 'D65 staged session token names a different session lease', [leaseId, expectedLeaseId]);
  return token;
}

/** Descriptor-safe removal of the staged token residue (deterministic, crash-safe). */
async function removeD65StagedSessionToken(stagedPath: string): Promise<void> {
...
export class DurableRunSupervisorClient {
  ...
  async attachD65Bootstrap(input: {
    ...
    if (staged === null) await writeD65StagedSessionToken(stagedPath, { session_lease_id: sessionLeaseId, session_token: sessionToken });
    const attachSession = await this.#client.mutate('attach-session', {
      repoId,
      workstreamRun: run.workstream_run,
      sessionId,
  ...
  async #adoptExistingD65BootstrapSession(input: { readonly repo: AutopilotRepoIdentity; readonly active: ActiveAutopilotRow; readonly sessionLeaseId: string }, run: CoordinationRun): Promise<RunSupervisorAttachment | null> {
    ...
    if (existsSync(contextPath)) {
      ...
      if (staged !== null && staged !== context.session_token) throw new CoordinationRuntimeError('invalid-state', 'D65 staged session token conflicts with the published final context', [session.session_lease_id]);
      if (staged !== null) await removeD65StagedSessionToken(stagedPath);
