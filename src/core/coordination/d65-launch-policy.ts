...
export const D65_LAUNCH_POLICY_SCHEMA = 'autopilot.launch_policy.v1' as const;

export interface D65LaunchPolicy {
  readonly schema_version: typeof D65_LAUNCH_POLICY_SCHEMA;
  ...
  readonly bootstrap_graph_sha256: `sha256:${string}`;
  readonly bootstrap_receipt_event_seq: number;
  readonly roster_sha256: `sha256:${string}`;
  readonly parallel_cap: 1;
  readonly maximum_parallel_cap: number;
  readonly expected_checkout_units: 1;
  readonly program_evidence_root: string;
  readonly trust_anchor_ref: string;
  readonly trust_anchor_sha256: `sha256:${string}`;
  readonly prior_policy_sha256: `sha256:${string}` | null;
  readonly capacity_decision_ref: string | null;
  readonly capacity_decision_sha256: `sha256:${string}` | null;
  readonly issued_at: string;
  readonly signer_key_id: `sha256:${string}`;
  readonly signature: string;
}

function absolutePathField(record: Readonly<Record<string, unknown>>, field: string, label: string): string {
  const value = str(record, field, label, 1024);
  if (!value.startsWith('/') || value.includes('\u0000') || value.includes('/../') || value.endsWith('/..') || value.includes('//')) fail(label, `${field} must be a normalized absolute path`);
  return value;
}

function unpaddedBase64Url(record: Readonly<Record<string, unknown>>, field: string, label: string): string {
  const value = str(record, field, label, 128);
  if (!/^[A-Za-z0-9_-]+$/u.test(value)) fail(label, `${field} must be unpadded base64url`);
  return value;
}

export function parseD65LaunchPolicy(value: unknown): D65LaunchPolicy {
  ...
  if (record['parallel_cap'] !== 1) fail(label, 'parallel_cap must be exactly 1');
  if (record['expected_checkout_units'] !== 1) fail(label, 'expected_checkout_units must be exactly 1');
  ...
  if (policyVersion === 1) {
    ...
    if (priorPolicy !== null || capacityDecisionRef !== null || capacityDecisionSha !== null) fail(label, 'initial policy version 1 must have null prior/decision fields');
    if (maximumParallelCap !== 1) fail(label, 'initial policy maximum_parallel_cap must be exactly 1');
  } else {
    if (priorPolicy === null) fail(label, 'a superseding policy must name the prior policy digest');
    if (capacityDecisionRef === null || capacityDecisionSha === null) fail(label, 'a superseding policy requires an exact capacity decision reference');
  }
  return {
...
export const D65_CAPACITY_DECISION_SCHEMA = 'autopilot.capacity_decision.v1' as const;

export interface D65CapacityDecision {
  readonly schema_version: typeof D65_CAPACITY_DECISION_SCHEMA;
  readonly program_id: string;
  ...
  readonly repo_id: string;
  readonly workstream_run: string;
  readonly prior_policy_sha256: `sha256:${string}`;
  readonly requested_parallel_cap: number;
  readonly requested_maximum_parallel_cap: number;
  readonly requested_expected_checkout_units: number;
...
export function parseD65CapacityDecision(value: unknown): D65CapacityDecision {
  ...
  if (toVersion !== fromVersion + 1) fail(label, 'capacity decision versions must be contiguous (to_version = from_version + 1)');
  return {
...
export type D65LaunchPolicyFailure = (typeof D65_LAUNCH_POLICY_FAILURES)[number];

// ---- autopilot.subscription_probe.v1 ----------------------------------------

export const D65_SUBSCRIPTION_PROBE_SCHEMA = 'autopilot.subscription_probe.v1' as const;

export interface D65SubscriptionProbe {
  readonly schema_version: typeof D65_SUBSCRIPTION_PROBE_SCHEMA;
  readonly probe_id: string;
  readonly program_id: string;
  readonly probe_sequence: number;
  readonly prior_probe_sha256: `sha256:${string}` | null;
  readonly provider: string;
  readonly trigger_continuation_ref: string;
  readonly trigger_continuation_sha256: `sha256:${string}`;
  readonly repo_id: string;
  readonly workstream_run: string;
  readonly unit_id: string;
  readonly failed_attempt: number;
  readonly retry_ordinal: 1;
  readonly successor_attempt: number;
  ...
  readonly cooldown_completed: true;
...
export function parseD65SubscriptionProbe(value: unknown): D65SubscriptionProbe {
  ...
  if (probeSequence === 1 && priorProbe !== null) fail(label, 'probe_sequence 1 must have null prior_probe_sha256');
  if (probeSequence > 1 && priorProbe === null) fail(label, 'a chained probe must name the prior probe digest');
  const failedAttempt = integer(record, 'failed_attempt', label, 1);
  const successorAttempt = integer(record, 'successor_attempt', label, 2);
  if (successorAttempt !== failedAttempt + 1) fail(label, 'successor_attempt must equal failed_attempt + 1');
  if (record['retry_ordinal'] !== 1) fail(label, 'retry_ordinal must be exactly 1 (a second/exhausted failure has no probe-authorized successor)');
  if (record['healthy'] !== true) fail(label, 'healthy must be true');
  if (record['cooldown_completed'] !== true) fail(label, 'cooldown_completed must be true');
  ...
  if (notBefore !== cooldownUntil) fail(label, 'not_before must equal cooldown_until');
  if (!(Date.parse(notBefore) <= Date.parse(observedAt) && Date.parse(observedAt) <= Date.parse(issuedAt))) fail(label, 'timestamps must satisfy not_before <= observed_at <= issued_at');
  if (Date.parse(expiresAt) - Date.parse(issuedAt) !== 5 * 60 * 1000) fail(label, 'expires_at must be exactly issued_at + 5 minutes');
  const evidenceRefs = array(record['evidence_refs'], `${label}.evidence_refs`, 64).map((entry, index) => {
    if (typeof entry !== 'string') fail(label, `evidence_refs[${String(index)}] must be a repository-relative path`);
...
export const D65_HEARTBEAT_HIGH_WATER_SCHEMA = 'autopilot.heartbeat_high_water.v1' as const;

export interface D65HeartbeatHighWater {
  readonly schema_version: typeof D65_HEARTBEAT_HIGH_WATER_SCHEMA;
  readonly program_id: string;
  readonly repo_id: string;
  readonly workstream_run: string;
  readonly sequence: number;
  readonly heartbeat_sha256: `sha256:${string}`;
  readonly issued_at: string;
  readonly valid_until: string;
  readonly updated_at: string;
}

export function parseD65HeartbeatHighWater(value: unknown): D65HeartbeatHighWater {
...
export const HEARTBEAT_ADVANCE_MAX_RECORDS = 1024 as const;

// ---- autopilot.program_heartbeat_acceptance_result.v1 -----------------------

export const D65_HEARTBEAT_ACCEPTANCE_RESULT_SCHEMA = 'autopilot.program_heartbeat_acceptance_result.v1' as const;
export const D65_HEARTBEAT_ACCEPTANCE_KINDS = ['catch-up', 'governing'] as const;

export interface D65HeartbeatAcceptanceResult {
  readonly schema_version: typeof D65_HEARTBEAT_ACCEPTANCE_RESULT_SCHEMA;
  readonly program_id: string;
  readonly repo_id: string;
  readonly workstream_run: string;
  readonly sequence: number;
  readonly heartbeat_ref: string;
  readonly heartbeat_sha256: `sha256:${string}`;
  readonly acceptance_kind: 'catch-up' | 'governing';
  readonly prior_sha256: `sha256:${string}` | null;
  readonly issued_at: string;
  readonly valid_until: string;
  readonly coordinator_time: string;
}

export function parseD65HeartbeatAcceptanceResult(value: unknown): D65HeartbeatAcceptanceResult {
  ...
  if (sequence === 1 && prior !== null) fail(label, 'sequence 1 acceptance must have null prior_sha256');
  if (sequence > 1 && prior === null) fail(label, 'a later acceptance must name the prior heartbeat digest');
  return {
...
export const D65_PROGRAM_HEARTBEAT_SCHEMA = 'autopilot.program_heartbeat.v1' as const;
...
export type D65StopReason = (typeof D65_STOP_REASONS)[number];

export const D65_PROVIDER_STATES = ['healthy', 'blocked', 'retry-authorized', 'exhausted'] as const;
export const D65_ROW_STATES = ['planned', 'active', 'recovering', 'parked', 'blocked', 'closed'] as const;

function stopReasons(value: unknown, label: string): readonly D65StopReason[] {
  const entries = array(value, label, D65_STOP_REASONS.length);
  const out: D65StopReason[] = [];
  for (let index = 0; index < entries.length; index += 1) {
    const entry = entries[index];
    if (typeof entry !== 'string' || !D65_STOP_REASONS.includes(entry as D65StopReason)) fail(label, `entry ${String(index)} is not a known stop reason`);
    out.push(entry as D65StopReason);
  }
  // Decoded-byte-sorted unique.
  for (let index = 1; index < out.length; index += 1) if (!((out[index - 1] ?? '') < (out[index] ?? ''))) fail(label, 'stop reasons must be decoded-byte-sorted and unique');
  return Object.freeze(out);
}

export interface D65HeartbeatRow {
  readonly workstream: string;
  readonly workstream_run: string;
  readonly parent_session_file_sha256: `sha256:${string}` | null;
  readonly coordinator_session_lease_id: string | null;
  readonly accepted_graph_sequence: number | null;
  readonly accepted_graph_sha256: `sha256:${string}` | null;
  readonly status_sha256: `sha256:${string}` | null;
  readonly doctor_sha256: `sha256:${string}` | null;
  readonly session_lease_state: string | null;
  readonly child_lease_ids: readonly string[];
  readonly launch_policy_sha256: `sha256:${string}` | null;
  readonly last_progress_event_seq: number | null;
  readonly last_handoff_sha256: `sha256:${string}` | null;
  readonly row_state: (typeof D65_ROW_STATES)[number];
  readonly dispatch_allowed: boolean;
  readonly stop_reasons: readonly D65StopReason[];
}

function heartbeatRow(value: unknown, label: string): D65HeartbeatRow {
  const record = object(value, label, [
  ...
  const childLeaseIds = array(record['child_lease_ids'], `${label}.child_lease_ids`, 64).map((entry, index) => {
    if (typeof entry !== 'string' || entry.length === 0 || entry.length > 192) fail(label, `child_lease_ids[${String(index)}] must be a bounded identifier`);
    return entry;
  });
  ...
  if (dispatchAllowed && reasons.length > 0) fail(label, 'a dispatch-allowed row must have empty stop_reasons');
  if (!dispatchAllowed && reasons.length === 0) fail(label, 'a non-dispatch row must carry at least one stop reason');
...
export interface D65ProviderHealth {
  readonly provider: string;
  readonly state: (typeof D65_PROVIDER_STATES)[number];
  readonly observation_ref: string | null;
  readonly observation_sha256: `sha256:${string}` | null;
  readonly cooldown_until: string | null;
  readonly probe_workstream_run: string | null;
  readonly probe_ref: string | null;
  readonly probe_sha256: `sha256:${string}` | null;
  readonly consumption_event_seq: number | null;
}

function providerHealth(value: unknown, label: string): D65ProviderHealth {
  const record = object(value, label, [
...
export interface D65ProgramHeartbeat {
  readonly schema_version: typeof D65_PROGRAM_HEARTBEAT_SCHEMA;
  readonly program_id: string;
  readonly sequence: number;
  readonly prior_sha256: `sha256:${string}` | null;
  readonly issued_at: string;
  ...
  readonly base_commit: string;
  readonly base_tree: string;
  readonly rows: readonly D65HeartbeatRow[];
  readonly provider_health: readonly D65ProviderHealth[];
  readonly dispatch_allowed: boolean;
  readonly stop_reasons: readonly D65StopReason[];
  readonly trust_anchor_ref: string;
  readonly trust_anchor_sha256: `sha256:${string}`;
...
export function parseD65ProgramHeartbeat(value: unknown): D65ProgramHeartbeat {
  ...
  const sequence = integer(record, 'sequence', label, 1);
  const prior = nullableSha256Field(record, 'prior_sha256', label);
  if (sequence === 1 && prior !== null) fail(label, 'sequence 1 must have null prior_sha256');
  if (sequence > 1 && prior === null) fail(label, 'a later heartbeat must name the prior digest');
  const issuedAt = timestamp(record, 'issued_at', label);
  const validUntil = timestamp(record, 'valid_until', label);
  // valid_until = issued_at + 15m exactly.
  if (Date.parse(validUntil) - Date.parse(issuedAt) !== 15 * 60 * 1000) fail(label, 'valid_until must be exactly issued_at + 15 minutes');
  ...
  for (let index = 1; index < rowEntries.length; index += 1) if (!((rowEntries[index - 1]?.workstream ?? '') < (rowEntries[index]?.workstream ?? ''))) fail(label, 'rows must be identity-sorted by workstream with no duplicates');
  ...
  for (let index = 1; index < providerEntries.length; index += 1) if (!((providerEntries[index - 1]?.provider ?? '') < (providerEntries[index]?.provider ?? ''))) fail(label, 'provider_health must be provider-byte-sorted with no duplicates');
  const dispatchAllowed = boolean(record, 'dispatch_allowed', label);
  const globalReasons = stopReasons(record['stop_reasons'], `${label}.stop_reasons`);
  if (dispatchAllowed && globalReasons.length > 0) fail(label, 'global dispatch_allowed requires empty global stop_reasons');
  if (!dispatchAllowed && globalReasons.length === 0) fail(label, 'a false global dispatch value requires at least one global reason');
  return {
