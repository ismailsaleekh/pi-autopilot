...
import { COORDINATOR_BUSY_TIMEOUT_MS, ensureCoordinatorPrivateRoots, type CoordinatorRuntimePaths } from './runtime-paths.ts';

function createPrivatePlaceholder(path: string): void {
...
interface GuardFileIdentity { readonly dev: number; readonly ino: number }

function assertPrivateRegularSingleLink(path: string): GuardFileIdentity {
  assertPrivatePathNoAliases(path);
  const metadata = lstatSync(path);
  if (!metadata.isFile() || metadata.isSymbolicLink()) throw new CoordinationRuntimeError('system-fatal', 'writer guard must be a private regular non-symbolic file', [path]);
  if (metadata.nlink !== 1) throw new CoordinationRuntimeError('system-fatal', 'writer guard refuses hardlink aliases', [path, `link_count=${String(metadata.nlink)}`]);
  if (platform() !== 'win32') {
    const getuid = process.getuid;
    if (getuid !== undefined && metadata.uid !== getuid()) throw new CoordinationRuntimeError('system-fatal', 'writer guard owner differs from the coordinator process user', [path, `uid=${String(metadata.uid)}`]);
  }
...
export class CoordinatorWriterGuard {
  ...
  readonly #identity: GuardFileIdentity;
  #released = false;

  private constructor(path: string, database: DatabaseSync, identity: GuardFileIdentity) {
    this.path = path;
    this.#database = database;
    this.#identity = identity;
  }

  static async acquire(paths: CoordinatorRuntimePaths, timeoutMs = COORDINATOR_BUSY_TIMEOUT_MS): Promise<CoordinatorWriterGuard> {
    ...
    try {
      ...
      if (!database.isTransaction) throw new CoordinationRuntimeError('system-fatal', 'writer guard BEGIN EXCLUSIVE did not retain its transaction');
      const retainedIdentity = assertPrivateRegularSingleLink(paths.writerGuardPath);
      const opened = statSync(paths.writerGuardPath);
      if (!opened.isFile() || retainedIdentity.dev !== expectedIdentity.dev || retainedIdentity.ino !== expectedIdentity.ino) throw new CoordinationRuntimeError('system-fatal', 'writer guard identity changed during acquisition', [paths.writerGuardPath]);
