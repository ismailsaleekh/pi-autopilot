...
import {
  ...
  type CoordinationMailboxDeliveryReceipt,
  type CoordinationReconciliationDetail,
  type CoordinationReconciliationEvidence,
  type CoordinationReconciliationReceipt,
  type CoordinationResultDetail,
  type CoordinationResultReceipt,
...
function boundedResponseJsonValue(value: unknown, label: string, depth: number): unknown {
  if (depth > 8) fail(label, 'exceeds maximum JSON nesting depth');
  if (value === null || typeof value === 'boolean') return value;
  if (typeof value === 'string') {
    if (Buffer.byteLength(value, 'utf8') > 524_288) fail(label, 'contains a string larger than one page entity');
    return value;
  }
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) fail(label, 'contains a non-finite number');
...
function boundedResponseJsonObject(value: unknown, label: string): JsonObject {
...
export function parseCoordinationReconciliationReceipt(value: unknown): CoordinationReconciliationReceipt {
  ...
  if (Object.values(counts).reduce((total, count) => total + count, 0) !== detailCount) fail(label, 'detail_count does not equal the exact per-kind counts');
  const detailsSha256 = string(record, 'details_sha256', label, 71);
  if (!SHA256.test(detailsSha256)) fail(label, 'details_sha256 must use sha256:<64 lowercase hex>');
  return {
    ...
    details_sha256: detailsSha256 as `sha256:${string}`,
    counts,
    version: integer(record, 'version', label, 1),
  };
}

export function parseOptionalCoordinationReconciliationReceipt(value: unknown): CoordinationReconciliationReceipt | null {
  return value === undefined ? null : parseCoordinationReconciliationReceipt(value);
}

export function parseCoordinationReconciliationDetail(value: unknown): CoordinationReconciliationDetail {
...
export function parseCoordinationMailboxDeliveryReceipt(value: unknown): CoordinationMailboxDeliveryReceipt {
  ...
  if (!SHA256.test(digest)) fail(label, 'message_ids_sha256 must use sha256:<64 lowercase hex>');
  return {
    ...
    message_ids_sha256: digest as `sha256:${string}`,
    completed: boolean(record, 'completed', label),
    version: integer(record, 'version', label, 1),
  };
}

export function parseCoordinationResultReceipt(value: unknown): CoordinationResultReceipt {
  ...
  if (!isJsonObject(record['collections'])) fail(label, 'collections must be an object');
  const collectionEntries = Object.entries(record['collections']);
  if (collectionEntries.length < 1 || collectionEntries.length > 64) fail(label, 'collections must contain 1 through 64 named results');
  ...
  for (const [name, raw] of collectionEntries) {
    ...
    if (!SHA256.test(digest)) fail(label, `${name}.items_sha256 must use sha256:<64 lowercase hex>`);
    const count = integer(collection, 'item_count', `${label}.collections.${name}`);
    total += count;
    collections[name] = { item_count: count, items_sha256: digest as `sha256:${string}` };
  }
  const detailCount = integer(record, 'detail_count', label);
  if (total !== detailCount) fail(label, 'detail_count does not equal the exact collection counts');
  const detailsDigest = string(record, 'details_sha256', label, 71);
  if (!SHA256.test(detailsDigest)) fail(label, 'details_sha256 must use sha256:<64 lowercase hex>');
  return {
    ...
    committed_event_seq: integer(record, 'committed_event_seq', label, 1), detail_count: detailCount, details_sha256: detailsDigest as `sha256:${string}`,
    collections: Object.freeze(collections), version: integer(record, 'version', label, 1),
  };
}

export function parseCoordinationResultDetail(value: unknown): CoordinationResultDetail {
...
function parsePayload(value: unknown, action: CoordinatorQueryAction | CoordinatorMutationAction): JsonObject {
  ...
  for (const field of PAYLOAD_FIELDS[action]) {
    ...
    if (field === 'limit') {
      ...
      if (entry !== null && (typeof entry !== 'string' || entry.length === 0 || entry.length > 1024)) fail(label, `${field} must be null or bounded cursor text`);
    } else if (field === 'cursor' || field === 'scan_token') {
      if (entry !== null && (typeof entry !== 'string' || entry.length === 0 || entry.length > 2048)) fail(label, `${field} must be null or bounded opaque cursor text`);
      ...
      if (requested.length === 0) fail(label, 'requested_leases must not be empty');
      if (Buffer.byteLength(JSON.stringify(requested), 'utf8') > 524_288) fail(label, 'requested_leases make one durable acquisition group exceed the single-entity byte ceiling');
      assertRequestedLeaseSet(requested, label);
  ...
  if (action === 'drain-mailbox') {
    ...
    if (cursor !== undefined && cursor !== null && (typeof cursor !== 'string' || cursor.length === 0 || cursor.length > 2048)) fail(label, 'mailbox cursor must be null or bounded opaque text');
  }
  if (action === 'reconciliation-details') {
    const sessionFields = ['session_lease_id', 'session_token'] as const;
    const childFields = ['boot_id', 'child_lease_id', 'child_token', 'pid'] as const;
    ...
    if (!hasSession && !hasChild) fail(label, 'reconciliation details require exactly one complete session or child authority proof');
    if (hasSession) {
      if (typeof payload['session_lease_id'] !== 'string' || !IDENTIFIER.test(payload['session_lease_id']) || typeof payload['session_token'] !== 'string' || !CHILD_TOKEN.test(payload['session_token'])) fail(label, 'reconciliation detail session authority is malformed');
    } else if (typeof payload['child_lease_id'] !== 'string' || !IDENTIFIER.test(payload['child_lease_id']) || typeof payload['child_token'] !== 'string' || !CHILD_TOKEN.test(payload['child_token']) || typeof payload['pid'] !== 'number' || !Number.isSafeInteger(payload['pid']) || payload['pid'] < 1 || typeof payload['boot_id'] !== 'string' || payload['boot_id'].length === 0 || payload['boot_id'].length > 1024) fail(label, 'reconciliation detail child authority is malformed');
  }
...
export function parseCoordinatorProjectionPage(value: unknown, kind: 'status' | 'doctor'): Readonly<Record<string, unknown>> {
  ...
  if (record['schema_version'] !== schema) fail(label, `schema_version must equal ${schema}`);
  ...
  if (!isJsonObject(record['section_counts'])) fail(label, 'section_counts must be an object');
  const counts: Record<string, number> = {};
  for (const [name, count] of Object.entries(record['section_counts'])) {
    identifierValue(name, `${label}.section_counts key`);
    if (typeof count !== 'number' || !Number.isSafeInteger(count) || count < 0) fail(label, `${name} count must be a nonnegative integer`);
    counts[name] = count;
  }
  const observedAt = record['observed_at'];
  if (observedAt !== null && (typeof observedAt !== 'string' || !ISO_TIMESTAMP.test(observedAt))) fail(label, 'observed_at must be null or an ISO timestamp');
  const next = record['next_cursor'];
  if (next !== null && (typeof next !== 'string' || next.length === 0 || next.length > 2048)) fail(label, 'next_cursor must be null or bounded opaque text');
...
export function parseCoordinatorRunCatalogPage(value: unknown): Readonly<Record<string, unknown>> {
  ...
  if (runs.length !== resources.length || runs.some((run, index) => resources[index]?.workstream_run !== run.workstream_run)) fail(label, 'runs and run_resources must be in exact lockstep');
...
export function parseCoordinatorMigrationRecoveryPage(value: unknown): Readonly<Record<string, unknown>> {
  ...
  let next: Readonly<Record<string, string>> | null = null;
  if (nextValue !== null) {
    ...
    if (cursorRun !== cursorRecovery) fail(label, 'next cursor identities must match exactly');
...
export function parseCoordinatorReconciliationDetailPage(value: unknown): Readonly<Record<string, unknown>> {
...
export function parseCoordinatorResultDetailPage(value: unknown): Readonly<Record<string, unknown>> {
...
export function parseCoordinatorMailboxPage(value: unknown): Readonly<Record<string, unknown>> {
