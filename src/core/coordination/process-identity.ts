...
export function predecessorCompatibleBootEstimate(nowMs: number, uptimeSeconds: number, host: string): string {
  const bootSeconds = Math.floor((nowMs - uptimeSeconds * 1000) / 1000);
  return `boot-estimate:${host}:${String(bootSeconds)}`;
}

export function predecessorCompatibleBootId(): string {
  if (platform() === 'linux') {
    try {
      const value = readFileSync('/proc/sys/kernel/random/boot_id', 'utf8').trim();
  ...
  if (platform() === 'darwin') {
    const result = spawnSync('sysctl', ['-n', 'kern.boottime'], { encoding: 'utf8' });
    if (result.status === 0 && result.stdout.trim().length > 0) return `darwin:${createHash('sha256').update(result.stdout.trim(), 'utf8').digest('hex')}`;
  }
...
export function processStartIdentity(pid: number): string | null {
  ...
  if (platform() === 'linux') {
    try {
      ...
      if (startTicks !== undefined && /^\d+$/u.test(startTicks)) return `linux-start-ticks:${startTicks}`;
    } catch { return null; }
  }
  if (platform() === 'darwin') {
...
export function preflightProcessRetirementSupport(): void {
  ...
  if (!existsSync('/usr/bin/python3')) throw new Error('macOS coordinator retirement requires /usr/bin/python3; dependency preflight failed closed');
  if (processStartIdentity(process.pid) === null) throw new Error('macOS libproc process-birth identity preflight failed; coordinator retirement is unavailable');
}

export function isExactProcessAlive(pid: number, expectedStartIdentity: string): boolean {
 ...
 */
export function retireExactProcess(pid: number, expectedStartIdentity: string): void {
  preflightProcessRetirementSupport();
  const first = processStartIdentity(pid);
  const second = processStartIdentity(pid);
  if (first === null || first !== expectedStartIdentity || second !== expectedStartIdentity) throw new Error(`refusing to retire/signal pid ${String(pid)} because its process-creation identity changed or became ambiguous`);
