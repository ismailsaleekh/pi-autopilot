...
export function advanceD65GraphPublicationResidue(mainWorktreePath: string, next: D65GraphPublication): D65GraphPublication {
  ...
  const immutable: readonly (keyof D65GraphPublication)[] = [
  ...
  for (const key of immutable) {
    if (canonicalJson(parsed[key]) !== canonicalJson(current[key])) throw new CoordinationRuntimeError('invalid-state', `graph publication residue immutable field ${String(key)} changed across a stage advance`);
  }
  writeResidueAtomic(mainWorktreePath, parsed);
  return parsed;
}
