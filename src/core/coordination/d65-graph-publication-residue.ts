...
import {
  D65_GRAPH_PUBLICATION_FIELDS,
  D65_GRAPH_PUBLICATION_STAGES,
  parseD65GraphPublication,
  type D65GraphPublication,
  type D65GraphPublicationStage,
...
export const D65_GRAPH_PUBLICATION_RESIDUE_FILENAME = '_graph-publication.json' as const;
export const D65_GRAPH_PUBLICATION_RESIDUE_LOCK_FILENAME = '_graph-publication.lock' as const;
export const D65_GRAPH_PUBLICATION_RESIDUE_MAX_BYTES = 1_048_576 as const;

/** The residue path is the sibling of the run's main worktree. */
export function d65GraphPublicationResiduePath(mainWorktreePath: string): string {
...
export function d65GraphPublicationLockPath(mainWorktreePath: string): string {
 ...
 */
function fsyncResidueDirectory(mainWorktreePath: string): void {
 ...
 */
function withD65PublicationLock<T>(mainWorktreePath: string, action: () => T): T {
...
export function readD65GraphPublicationResidue(mainWorktreePath: string): D65GraphPublication | null {
...
function writeResidueAtomic(mainWorktreePath: string, residue: D65GraphPublication): void {
  const path = d65GraphPublicationResiduePath(mainWorktreePath);
  const serialized = `${canonicalJson(residue)}\n`;
  if (Buffer.byteLength(serialized, 'utf8') > D65_GRAPH_PUBLICATION_RESIDUE_MAX_BYTES) throw new CoordinationRuntimeError('invalid-request', 'graph publication residue exceeds its 1 MiB bound');
  ...
  try {
    ...
    let written = 0;
    while (written < buffer.length) written += writeSync(descriptor, buffer, written, buffer.length - written);
  ...
  if (persisted === null) throw new CoordinationRuntimeError('invalid-state', 'graph publication residue disappeared immediately after a durable write', [path]);
  if (`${canonicalJson(persisted)}\n` !== serialized) throw new CoordinationRuntimeError('invalid-state', 'graph publication residue post-write verification does not match the written bytes', [path]);
}
...
const STAGE_ADVANCE_MUTABLE_FIELDS = Object.freeze({
  ...
  prepared: Object.freeze([] as readonly (keyof D65GraphPublication)[]),
  // prepared -> authority-committed records only G and its tree.
  'authority-committed': Object.freeze(['authority_commit', 'authority_tree'] as readonly (keyof D65GraphPublication)[]),
  // authority-committed -> publication-committed records only H/tree/ref/hash/count.
  'publication-committed': Object.freeze(['publication_commit', 'publication_tree', 'graph_ref', 'graph_sha256', 'graph_byte_count'] as readonly (keyof D65GraphPublication)[]),
  // publication-committed -> registered records only R.
  registered: Object.freeze(['registration_event_seq'] as readonly (keyof D65GraphPublication)[]),
}) satisfies Readonly<Record<D65GraphPublicationStage, readonly (keyof D65GraphPublication)[]>>;
...
const ALL_RESIDUE_FIELDS: readonly (keyof D65GraphPublication)[] = D65_GRAPH_PUBLICATION_FIELDS;

/** Create the residue at the `prepared` stage. Rejects if one already exists. */
export function createD65GraphPublicationResidue(mainWorktreePath: string, residue: D65GraphPublication): D65GraphPublication {
  const parsed = parseD65GraphPublication(residue);
  if (parsed.stage !== 'prepared') throw new CoordinationRuntimeError('invalid-request', 'a new graph publication residue must begin at the prepared stage');
  return withD65PublicationLock(mainWorktreePath, () => {
    if (readD65GraphPublicationResidue(mainWorktreePath) !== null) throw new CoordinationRuntimeError('coordinator-contention', 'a graph publication residue already exists for this run');
 ...
 */
export function advanceD65GraphPublicationResidue(mainWorktreePath: string, next: D65GraphPublication): D65GraphPublication {
  const parsed = parseD65GraphPublication(next);
  return withD65PublicationLock(mainWorktreePath, () => {
    const current = readD65GraphPublicationResidue(mainWorktreePath);
    if (current === null) throw new CoordinationRuntimeError('invalid-state', 'cannot advance an absent graph publication residue');
    // Monotonic: exactly one stage forward.
    if (STAGE_ORDER[parsed.stage] !== STAGE_ORDER[current.stage] + 1) throw new CoordinationRuntimeError('invalid-state', `graph publication residue stage must advance exactly one step from ${current.stage}`, [current.stage, parsed.stage]);
    ...
    const mutable = new Set<keyof D65GraphPublication>([...STAGE_ADVANCE_MUTABLE_FIELDS[parsed.stage], 'stage', 'updated_at']);
    for (const key of ALL_RESIDUE_FIELDS) {
      if (mutable.has(key)) continue;
      if (canonicalJson(parsed[key]) !== canonicalJson(current[key])) throw new CoordinationRuntimeError('invalid-state', `graph publication residue field ${String(key)} changed outside the ${current.stage}->${parsed.stage} transition CAS`);
    }
    writeResidueAtomic(mainWorktreePath, parsed);
    return parsed;
  });
 ...
 */
export function cleanupD65GraphPublicationResidue(mainWorktreePath: string): void {
...
export function d65GraphPublicationPending(mainWorktreePath: string): boolean {
  const current = readD65GraphPublicationResidue(mainWorktreePath);
