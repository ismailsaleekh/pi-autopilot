...
export interface ReservationIntegrationPreparationResult {
  readonly integration_evidence_paths: readonly string[];
  readonly repair_route_paths: readonly string[];
  readonly stale_validation_paths: readonly string[];
}
 ...
 */
export async function preparePendingReservationIntegrations(active: ActiveAutopilotRow, env: ProcessEnvLike = process.env, now: Date = new Date()): Promise<ReservationIntegrationPreparationResult> {
  if (active.coordination_authority !== 'coordinator-edit-leases-v1') return { integration_evidence_paths: Object.freeze([]), repair_route_paths: Object.freeze([]), stale_validation_paths: Object.freeze([]) };
  const client = await ReservationCoordinationClient.fromEnvironment(env);
  const view = await client.view();
  ...
  for (const obligation of obligations) {
    if (obligation.predecessor_terminal_sha === null || obligation.predecessor_released_event_seq === null) throw new CoordinationRuntimeError('store-corrupt', 'integration-required reservation obligation lacks predecessor landing identity', [obligation.obligation_id]);
    const integrationPath = join(active.runtime_root, 'reservation-integration', `${obligation.obligation_id}.json`);
...
async function reconcileReservationIntegrationStaleness(active: ActiveAutopilotRow, obligation: CoordinationReservationObligation, integrationPath: string, now: Date): Promise<readonly string[]> {
  const parsed: unknown = JSON.parse(await readFile(integrationPath, 'utf8')) as unknown;
  if (!record(parsed) || parsed['schema_version'] !== 'autopilot.reservation_integration.v1' || parsed['repo_id'] !== obligation.repo_id || parsed['workstream_run'] !== obligation.workstream_run || parsed['obligation_id'] !== obligation.obligation_id || parsed['reservation_id'] !== obligation.reservation_id || parsed['predecessor_reservation_id'] !== obligation.predecessor_reservation_id || parsed['predecessor_released_event_seq'] !== obligation.predecessor_released_event_seq || parsed['predecessor_terminal_sha'] !== obligation.predecessor_terminal_sha) throw new CoordinationRuntimeError('invalid-state', 'persisted reservation integration evidence does not match its immutable obligation', [integrationPath, obligation.obligation_id]);
  const integrationHead = parsed['integration_head'];
  if (typeof integrationHead !== 'string' || !/^[a-f0-9]{40,64}$/u.test(integrationHead)) throw new CoordinationRuntimeError('invalid-state', 'persisted reservation integration evidence has an invalid integration head', [integrationPath]);
  ...
  if (classification === undefined && changed === undefined) return Object.freeze([]);
  if (classification === undefined || changed === undefined) throw new CoordinationRuntimeError('invalid-state', 'automatic reservation integration evidence is only partially populated', [integrationPath]);
  parseCoordinationIntegrationConflict(classification, 'reservation integration classification');
  if (!Array.isArray(changed) || !changed.every((path) => typeof path === 'string') || new Set(changed).size !== changed.length) throw new CoordinationRuntimeError('invalid-state', 'persisted reservation integration evidence has invalid changed paths', [integrationPath]);
  if (changed.length === 0) return Object.freeze([]);
  const changedPaths = changed as readonly string[];
  ...
  if (integrationRef.startsWith('../') || integrationRef.startsWith('/')) throw new CoordinationRuntimeError('invalid-state', 'reservation integration evidence escapes the authoritative runtime root', [integrationPath]);
...
async function writeReservationRepairRoute(active: ActiveAutopilotRow, obligation: CoordinationReservationObligation, classification: CoordinationIntegrationConflict, currentHead: string, now: Date): Promise<string> {
  ...
  if (existsSync(path)) {
    const existing: unknown = JSON.parse(await readFile(path, 'utf8')) as unknown;
    if (!record(existing) || existing['schema_version'] !== 'autopilot.reservation_repair.v1' || existing['obligation_id'] !== obligation.obligation_id || existing['current_head'] !== currentHead || existing['predecessor_terminal_sha'] !== obligation.predecessor_terminal_sha || !record(existing['classification']) || existing['classification']['classification_id'] !== classification.classification_id) throw new CoordinationRuntimeError('invalid-state', 'immutable reservation repair route differs on replay', [path, obligation.obligation_id]);
    return path;
  }
