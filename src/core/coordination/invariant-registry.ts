export type S1InvariantScope = 'global' | 'repository' | 'workstream_run' | 'entity';
export type S1InvariantCriticality = 'authority-critical' | 'progress-critical';

/** Implemented by the authority-owning store/generation integration. Keeping
 * methods closed makes every registry detector executable rather than a prose
 * symbol that can silently drift from production. */
export interface S1InvariantDetectorHost {
  detectPhysicalIntegrity(): void;
  detectStoreGeneration(): void;
  detectWriterGuard(): void;
  detectMigrationBoundary(): void;
  detectEventCounterBehind(): void;
  detectEventCounterAhead(): void;
  detectPayloadIndexAmbiguity(): void;
  detectCanonicalIdentity(): void;
  detectAliasOneHop(): void;
  detectSemanticUniqueness(): void;
  detectOperationCanonicalIndex(): void;
  detectIdentityRecovery(): void;
}

export type S1InvariantDetector = (host: S1InvariantDetectorHost) => void;

export interface S1InvariantDefinition {
  readonly id: string;
  readonly scope: S1InvariantScope;
  readonly criticality: S1InvariantCriticality;
  readonly detector_name: string;
  readonly detector: S1InvariantDetector;
  readonly mechanical_repair: string | null;
  readonly no_safe_repair_proof: string | null;
}

function invariant(input: S1InvariantDefinition): S1InvariantDefinition {
  if ((input.mechanical_repair === null) === (input.no_safe_repair_proof === null)) throw new Error(`S1 invariant ${input.id} must declare exactly one repair disposition`);
  return Object.freeze(input);
}

export const S1_INVARIANT_REGISTRY: readonly S1InvariantDefinition[] = Object.freeze([
...
const definitionsById: ReadonlyMap<string, S1InvariantDefinition> = new Map(S1_INVARIANT_REGISTRY.map((definition) => [definition.id, definition]));
if (definitionsById.size !== S1_INVARIANT_REGISTRY.length) throw new Error('S1 invariant registry contains duplicate IDs');

export function s1InvariantDefinition(id: string): S1InvariantDefinition {
  const definition = definitionsById.get(id);
  if (definition === undefined) throw new Error(`Unknown S1 invariant ${id}`);
  return definition;
}

export function runS1InvariantDetectors(host: S1InvariantDetectorHost, invariantIds: readonly string[]): void {
  const seen = new Set<string>();
  for (const id of invariantIds) {
    if (seen.has(id)) throw new Error(`S1 invariant detector ${id} was requested twice in one pass`);
    seen.add(id);
