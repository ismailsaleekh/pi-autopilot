...
function gitNulStrings(cwd: string, descriptor: GitQueryDescriptor): readonly string[] | null {
  const result = runGitQuery({ descriptor, cwd });
  ...
  while (cursor < result.stdout.length) {
    ...
    if (delimiter < 0) throw new CoordinationRuntimeError('invalid-state', 'NUL-delimited migration Git query ended with an unterminated record', [descriptor.kind, cwd]);
...
function inspectGit(rows: readonly ActiveAutopilotRow[], ledgerTerminalized: ReadonlySet<string>): readonly string[] {
  ...
  for (const row of rows) {
    ...
    if (top === null || common === null) { blockers.push(`source repository Git verification failed: ${row.source_repo}`); continue; }
    try {
