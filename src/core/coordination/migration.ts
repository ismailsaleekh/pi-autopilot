...
function object(value: unknown, label: string, fields: readonly string[]): Readonly<Record<string, unknown>> {
  ...
  for (const field of fields) if (!(field in row)) failure('invalid', `${label} is missing ${field}`);
  return row;
}
...
function readUnitMetadata(rows: readonly ActiveAutopilotRow[]): ParsedUnitMetadata {
  ...
  for (const active of rows) {
    ...
    if (!existsSync(taskInfoPath) && active.status === 'closed') continue;
    ...
    for (const candidate of indexed) {
      ...
      if (matching === undefined || stableJson(matching) !== stableJson(candidate)) failure('invalid', '_unit-index.json and _branches.json disagree', [key]);
      ...
      if (existsSync(unitInfoPath)) {
        ...
        if (stableJson(parsedUnitInfo) !== stableJson(candidate) || unitInfo['schema_version'] !== 'autopilot.unit_info.v1' || unitInfo['workstream_run'] !== active.workstream_run || unitInfo['autopilot_id'] !== active.autopilot_id || unitInfo['runtime_root'] !== active.runtime_root) failure('invalid', '_unit-info.json disagrees with run/unit index ownership', [unitInfoPath]);
      }
...
function inspectGit(rows: readonly ActiveAutopilotRow[]): readonly string[] {
  const blockers: string[] = [];
  for (const row of rows) {
    ...
    } else if (row.status !== 'closed') blockers.push(`live main worktree is missing and requires recovery: ${row.workstream_run}`);
  }
...
function assertReadOnlySchema(database: DatabaseSync): 6 | 7 | 8 | 9 {
  ...
  if (version !== 6 && version !== 7 && version !== 8 && version !== 9) failure('blocked', 'migration inspection supports only exact coordinator schema 6, 7, 8, or the current schema 9', [`schema=${String(version)}`]);
  ...
  for (let index = 0; index < migrationRows.length; index += 1) {
    ...
    if (row === undefined || sqlSafeInteger(row, 'version', 'schema migration') !== index + 1 || !checksumMatches) failure('blocked', 'coordinator schema migration journal is malformed, discontinuous, or not the exact locked schema-6/7/8/9 package lineage');
  }
  return version;
}
...
function copiedCoordinatorDatabase(paths: CoordinatorRuntimePaths, writerAuthorityAcquired: boolean): { readonly databasePath: string; readonly root: string; readonly source: readonly CoordinatorDatabaseSourceIdentity[] } {
  ...
  const wal = source[1];
  const shm = source[2];
  if (wal?.exists === true && wal.size > 0 && ((!coordinatorRunning(paths) && !writerAuthorityAcquired) || shm?.exists !== true)) failure('blocked', 'uncheckpointed coordinator WAL has no live fenced owner or durable migration writer authority; copied inspection is unsafe', [wal.path]);
...
function inspectLegacy(paths: CoordinatorRuntimePaths, repoKey: string, repository: MigrationRepositoryIdentity): LegacyInspection {
  ...
  return { rows, claims, unitMetadata, worktreeIndex: index, sourceEntries, gitSnapshot, audit, merges: mergeEvidence.merges, terminalEvidence, blockers: frozenBlockers, recovery: frozenRecovery, terminalLeakKeys, equivalentClaimKeys: new Set<string>(), reboundCount, totalBytes: sourceEntries.reduce((sum, entry) => sum + entry.size_bytes, 0) };
}
...
function coordinatorDrainBlockers(coordinator: ReadonlyCoordinatorInspection | null): readonly string[] {
  ...
  const blockers: string[] = [];
  for (const session of coordinator.sessions) if (session.status === 'attached' || session.status === 'handoff-pending') blockers.push(`coordinator session has not durably drained: ${session.workstream_run}:${session.session_lease_id}`);
  for (const child of coordinator.children) if (['preflight', 'starting', 'running', 'recovery-required'].includes(child.status)) blockers.push(`coordinator child has not durably drained: ${child.workstream_run}:${child.child_lease_id}`);
  for (const attempt of coordinator.attempts) if (attempt.critical_section !== null) blockers.push(`coordinator attempt remains in an incompatible critical section: ${attempt.owner.workstream_run}:${stableJson(attempt.critical_section)}`);
  for (const operation of coordinator.worktreeOperations) if (!['committed', 'compensated', 'failed'].includes(operation.stage)) blockers.push(`coordinator worktree operation critical section is incomplete: ${operation.owner.workstream_run}:${operation.operation_id}:${operation.stage}`);
  return Object.freeze(blockers.sort());
}

function runStatus(row: ActiveAutopilotRow, recoveryRuns: ReadonlySet<string>): CoordinationRunStatus {
...
function buildImportPlan(inspection: LegacyInspection, repositoryIdentity: MigrationRepositoryIdentity, repoKey: string, migrationId: string, snapshotSha: `sha256:${string}`, journalPath: string, report: CoordinationMigrationReport): CoordinationLegacyImportPlan {
  ...
  for (const [key, claims] of grouped) {
    ...
    const importedAttemptState: CoordinationUnitAttempt['state'] = metadata === undefined ? 'queued' : metadata.status === 'active' ? 'running' : metadata.status === 'merged' ? 'merged' : metadata.status === 'aborted' ? 'reset' : metadata.status === 'quarantined' ? 'quarantined' : 'superseded';
    ...
    for (const entry of claims) leases.push({ schema_version: 'autopilot.edit_lease.v1', edit_lease_id: entityId('migration-lease', claimKey(entry)), owner, acquisition_group_id: groupId, path: entry.path, mode: entry.claim_type, purpose: entry.reason, acquired_event_seq: 1, normal_release_condition: condition, version: 1 });
  }
  ...
  for (const row of inspection.rows) {
    const mainState: CoordinationWorktreeState = row.status === 'closed' ? 'terminal' : existsSync(row.main_worktree_path) ? 'active' : 'dirty';
...
async function acquireMigrationLock(stateRoot: string, path: string, afterBoundary?: (boundary: CoordinationMigrationCrashBoundary) => void | Promise<void>): Promise<{ readonly release: () => Promise<void> }> {
...
function coordinatorRunning(paths: CoordinatorRuntimePaths): boolean {
...
async function retireDrainedCoordinator(paths: CoordinatorRuntimePaths): Promise<readonly string[]> {
  ...
  while (Date.now() < deadline) {
    ...
    if (!isProcessAlive(lock.pid)) {
      ...
      try {
        ...
        let current;
        try { current = parseCurrentCoordinatorLock(JSON.parse(currentText) as unknown); }
...
function verifyCoordinatorDatabaseFileReadOnly(path: string): 6 | 7 | 8 | 9 {
...
export async function runCoordinationMigration(input: { readonly command: CoordinationMigrationCommand; readonly repoKey: string; readonly repoRoot?: string; readonly env?: ProcessEnvLike; readonly clock?: MigrationClock; readonly afterBoundary?: (boundary: CoordinationMigrationCrashBoundary) => void | Promise<void> }): Promise<CoordinationMigrationReport> {
  ...
  try {
    ...
    if (input.command === 'apply') {
      ...
      let writerBlockers: readonly string[] = coordinatorRunning(paths)
      ...
      if (coordinatorRunning(paths)) failure('blocked', 'coordinator lifecycle lock remains live after retirement');
