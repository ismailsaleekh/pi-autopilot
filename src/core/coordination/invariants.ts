  ...
  for (const group of snapshot.acquisition_groups) {
    ...
    if (group.state === 'granted') {
      ...
      if (leases.length + observations.length !== expectedActiveCount) {
        ...
        else findings.push(finding('granted-group-authority-incomplete', group.acquisition_group_id, `granted group has ${String(leases.length)} edit leases and ${String(observations.length)} durable observations for ${String(expectedActiveCount)} requested entries`));
      }
