...
function conditionSatisfied(snapshot: CoordinationSnapshot, repoId: string, workstreamRun: string, condition: CoordinationReleaseCondition): boolean {
  if (condition.condition_type === 'explicit-owner-release') return false;
  if (condition.condition_type === 'child-terminal') return snapshot.child_leases.some((child) => child.owner.repo_id === repoId && child.owner.workstream_run === workstreamRun && child.child_lease_id === condition.target_id && child.status === 'terminal');
  if (condition.condition_type === 'run-closed') return snapshot.runs.some((run) => run.repo_id === repoId && run.workstream_run === workstreamRun && condition.target_id === workstreamRun && (run.status === 'closed' || run.status === 'aborted'));
...
export function checkCoordinationInvariants(snapshot: CoordinationSnapshot): readonly CoordinationInvariantFinding[] {
  ...
  const currentWorktreeIdentity = (operation: CoordinationSnapshot['worktree_operations'][number]): string => {
  ...
  const retainedMigrationRecoveryLeaseIds = new Set(snapshot.migration_recovery_work.filter((work) => work.status === 'resolved' && work.resolution?.resolution_type === 'authority-retained' && typeof work.detail['edit_lease_id'] === 'string').map((work) => work.detail['edit_lease_id'] as string));
  ...
  for (const run of snapshot.runs) {
    ...
    if (cursors.length !== 1) findings.push(finding('run-mailbox-cursor-count', run.workstream_run, `run requires exactly one durable mailbox cursor, found ${String(cursors.length)}`));
  }
  ...
  for (const group of snapshot.acquisition_groups) {
    ...
    if (group.state === 'waiting' || group.state === 'grant-ready' || group.state === 'released' || group.state === 'cancelled' || group.state === 'superseded') {
      if (leases.length > 0) findings.push(finding('ungranted-group-holds-leases', group.acquisition_group_id, `${group.state} group holds ${String(leases.length)} active edit leases`));
      if (activeObservations.length > 0) findings.push(finding('ungranted-group-holds-observations', group.acquisition_group_id, `${group.state} group holds ${String(activeObservations.length)} active observations`));
    }
    ...
    if (group.state === 'granted') {
      ...
      if (unexpected.length > 0) findings.push(finding('acquisition-group-unrequested-lease', group.acquisition_group_id, 'active edit lease set contains authority outside the requested set'));
      if (unexpectedObservations.length > 0) findings.push(finding('acquisition-group-unrequested-observation', group.acquisition_group_id, 'observation set contains a dependency outside the requested set'));
      ...
      if (leases.length + observations.length !== expectedActiveCount) {
        ...
        if (group.acquisition_kind === 'legacy-unknown' && missing.length > 0 && missing.every((requestedLease) => requestedLease.mode === 'READ')) findings.push(finding('legacy-granted-group-read-revalidation-required', group.acquisition_group_id, 'historical unbound READ entries were audit-retired; the retained edit authority cannot dispatch until a new attempt revalidates observations', 'warning'));
        else if (!boundedCriticalSectionExited) findings.push(finding('granted-group-authority-incomplete', group.acquisition_group_id, `granted group has ${String(leases.length)} edit leases and ${String(observations.length)} durable observations for ${String(expectedActiveCount)} requested entries`));
      }
  ...
  for (const observation of snapshot.observations) {
    ...
    if (group === undefined) findings.push(finding('observation-group-missing', observation.observation_id, 'observation acquisition group does not exist'));
    else {
      if (ownerKey(group.owner) !== ownerKey(observation.owner)) findings.push(finding('observation-group-owner-mismatch', observation.observation_id, 'observation and acquisition group have different owners'));
      if (!group.requested_leases.some((requested) => requested.mode === 'READ' && requested.path === observation.path && requested.source_identity?.base_commit === observation.source_identity.base_commit && requested.source_identity.object_id === observation.source_identity.object_id && requested.source_identity.object_kind === observation.source_identity.object_kind)) findings.push(finding('observation-source-not-requested', observation.observation_id, 'observation source identity is absent from its immutable acquisition group'));
    }
    ...
    if (observation.execution_state === 'active' && child !== undefined && (child.status === 'terminal' || child.status === 'recovery-required')) findings.push(finding('terminal-child-retains-active-observation', observation.observation_id, `${child.status} child retains an active observation dependency`));
    if (observation.freshness === 'stale' && observation.stale_by_reservation_id !== null && !observation.stale_by_reservation_id.startsWith('legacy-observation-migration-') && !reservations.has(observation.stale_by_reservation_id)) findings.push(finding('stale-observation-reservation-missing', observation.observation_id, 'stale observation does not bind a landed change reservation'));
  }
  ...
  for (const lease of snapshot.edit_leases) {
    ...
    if (group === undefined) findings.push(finding('lease-group-missing', lease.edit_lease_id, 'acquisition group does not exist'));
      ...
      if (ownerKey(group.owner) !== ownerKey(lease.owner)) findings.push(finding('lease-group-owner-mismatch', lease.edit_lease_id, 'lease and acquisition group have different owners'));
      const requested = group.requested_leases.find((entry) => entry.mode === lease.mode && entry.path === lease.path);
      if (requested === undefined || JSON.stringify(requested.exclusive_operation) !== JSON.stringify(lease.exclusive_operation)) findings.push(finding('lease-operation-contract-mismatch', lease.edit_lease_id, 'lease does not preserve its immutable requested critical-operation contract'));
      if (lease.mode === 'EXCLUSIVE' && group.acquisition_kind !== 'legacy-unknown') {
        if (!snapshot.edit_leases.some((candidate) => candidate.acquisition_group_id === lease.acquisition_group_id && candidate.mode === 'WRITE' && candidate.path === lease.path && ownerKey(candidate.owner) === ownerKey(lease.owner))) findings.push(finding('exclusive-write-layer-missing', lease.edit_lease_id, 'new EXCLUSIVE authority requires a paired WRITE intention on the exact path'));
        if (attempt === undefined || attempt.critical_section !== lease.exclusive_operation?.critical_section || attempt.preemptible) findings.push(finding('exclusive-critical-section-mismatch', lease.edit_lease_id, 'active EXCLUSIVE authority requires the exact non-preemptible attempt critical section'));
      }
      if (group.acquisition_kind === 'legacy-unknown' && lease.normal_release_condition.condition_type === 'explicit-owner-release' && retainedMigrationRecoveryLeaseIds.has(lease.edit_lease_id) && !nonterminalChildOwners.has(ownerKey(lease.owner))) findings.push(finding('retained-legacy-explicit-owner-lease', lease.edit_lease_id, 'resolved authority-retained import has no resumable child and awaits evidence-backed terminal reconciliation or authenticated owner response through autopilot_respond_claim_request', 'warning'));
    }
  }
  ...
  for (const run of snapshot.runs) {
    ...
    if (activeObservations.length > 0) findings.push(finding('terminal-run-retains-active-observations', run.workstream_run, `${run.status} run retains ${String(activeObservations.length)} active observations`));
  ...
  for (const request of snapshot.claim_requests) {
    ...
    if (request.status === 'deferred' && request.release_condition !== null && conditionSatisfied(snapshot, request.owner.repo_id, request.owner.workstream_run, request.release_condition)) findings.push(finding('satisfied-deferred-request-not-released', request.request_id, 'satisfied typed release promise must release and notify automatically'));
  ...
  for (const cursor of snapshot.mailbox_cursors) {
    if (!runs.has(runKey(cursor.repo_id, cursor.workstream_run))) findings.push(finding('mailbox-cursor-run-missing', cursor.workstream_run, 'mailbox cursor owning run does not exist'));
    if (cursor.acknowledged_through_event_seq > cursor.delivered_through_event_seq) findings.push(finding('mailbox-cursor-order-invalid', cursor.workstream_run, 'acknowledgement cursor exceeds delivery cursor'));
  }

  const expectedConditionBySource: Readonly<Record<string, string>> = { 'child-process': 'child-terminal', 'unit-merge': 'unit-merged', 'attempt-reset': 'attempt-reset', 'quarantine-capture': 'quarantine-captured', 'run-close': 'run-closed', 'run-abort': 'run-closed' };
  for (const evidence of snapshot.reconciliation_evidence) {
    const run = runs.get(runKey(evidence.repo_id, evidence.workstream_run));
    if (run === undefined) findings.push(finding('reconciliation-evidence-run-missing', evidence.reconciliation_evidence_id, 'owning run does not exist'));
    else if (run.autopilot_id !== evidence.autopilot_id) findings.push(finding('reconciliation-evidence-owner-mismatch', evidence.reconciliation_evidence_id, 'evidence owner differs from durable run owner'));
    if (expectedConditionBySource[evidence.source] !== evidence.release_condition.condition_type) findings.push(finding('reconciliation-source-condition-mismatch', evidence.reconciliation_evidence_id, 'evidence source does not match its typed release condition'));
  }
  ...
  for (const message of snapshot.messages) {
    ...
    if (message.status !== 'pending' && cursor !== undefined && message.created_event_seq > cursor.delivered_through_event_seq) findings.push(finding('delivered-message-ahead-of-cursor', message.message_id, 'durable delivery cursor does not include the delivered message'));
  }

  for (const worktree of snapshot.worktrees) assertOwner(worktree.owner, worktree.worktree_id);
  const activeWorktreesByOwnerKind = new Map<string, typeof snapshot.worktrees>();
  ...
  for (let leftIndex = 0; leftIndex < activeWorktrees.length; leftIndex += 1) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < activeWorktrees.length; rightIndex += 1) {
      ...
      if (sameOwner || sameCanonicalPath) findings.push(finding('conflicting-active-worktree-authority', `${left.worktree_id},${right.worktree_id}`, sameOwner
  ...
  for (const operation of snapshot.worktree_operations) {
    ...
    if (current === undefined || operation.authority_version > current) maxAuthorityVersionByWorktree.set(currentWorktreeId, operation.authority_version);
  }
  ...
  for (const operation of snapshot.worktree_operations) {
    ...
    if (!['committed', 'compensated', 'failed'].includes(operation.stage)) {
      ...
      else incompleteByWorktree.set(currentWorktreeId, operation.operation_id);
    }
  ...
  for (const worktree of snapshot.worktrees) {
    if (worktree.state === 'removed' && !auditedProjectionRetirements.has(worktree.worktree_id) && !snapshot.worktree_operations.some((operation) => currentWorktreeIdentity(operation) === worktree.worktree_id && operation.operation_type === 'remove' && operation.stage === 'committed')) findings.push(finding('removed-worktree-without-commit', worktree.worktree_id, 'removed worktree state requires a committed remove operation or an audited exact-projection retirement'));
  }
