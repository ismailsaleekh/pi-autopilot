...
import { COORDINATOR_PACKAGE_VERSION } from './runtime-constants.ts';
...
export interface CoordinatorExecutableResolution {
  readonly packageRoot: string;
  readonly bootstrapPath: string;
  readonly coordinatorPath: string;
}

export interface ExtensionPackageExecutableResolution {
  readonly packageRoot: string;
...
function assertClosedPackagePath(packageRoot: string, target: string, label: string): void {
  ...
  if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink()) throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator package root is not a physical directory', [`package_root=${normalizedRoot}`]);
  ...
  for (const segment of relative(normalizedRoot, normalizedTarget).split(/[\\/]/u)) {
    ...
    if (info.isSymbolicLink()) throw new CoordinationRuntimeError('coordinator-unavailable', `${label} contains a symbolic link`, [`selected_compiled_entrypoint=${normalizedTarget}`, `symbolic_link=${cursor}`]);
  }
  const targetInfo = lstatSync(normalizedTarget);
  if (!targetInfo.isFile()) throw new CoordinationRuntimeError('coordinator-unavailable', `${label} is not a regular file`, [`selected_compiled_entrypoint=${normalizedTarget}`]);
...
function verifyPackageIdentity(packageRoot: string): Readonly<Record<string, unknown>> {
  ...
  catch (error) { throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator package manifest is unreadable', [`package_root=${packageRoot}`, `packaging_cause=${error instanceof Error ? error.message : String(error)}`]); }
  if (typeof manifest !== 'object' || manifest === null || Array.isArray(manifest)) throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator package manifest has invalid package identity', [`package_root=${packageRoot}`]);
...
function packageRootForKnownModule(input: {
  ...
  for (const relativePath of [input.sourceRelativePath, input.distRelativePath]) {
    ...
    for (const _segment of segments) candidateRoot = dirname(candidateRoot);
    if (modulePath === join(candidateRoot, relativePath)) return candidateRoot;
  }
...
function localModulePath(moduleUrl: string, label: string): string {
  try { return resolve(fileURLToPath(moduleUrl)); }
  catch (error) { throw new CoordinationRuntimeError('coordinator-unavailable', `${label} module URL is not a local package file`, [`packaging_cause=${error instanceof Error ? error.message : String(error)}`]); }
}

export function resolveCoordinatorExecutable(clientModuleUrl: string): CoordinatorExecutableResolution {
 ...
 */
export function resolveExtensionPackageExecutables(extensionModuleUrl: string): ExtensionPackageExecutableResolution {
  ...
  const binRecord = typeof bin === 'object' && bin !== null && !Array.isArray(bin) ? bin as Readonly<Record<string, unknown>> : null;
