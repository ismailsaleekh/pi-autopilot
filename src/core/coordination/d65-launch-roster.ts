...
export const D65_REQUIRED_ROSTER_QUALIFICATION_STATE = 'w4-certified-ready' as const;

/** The only roster generation source a D65 launch may consume. */
export const D65_REQUIRED_ROSTER_GENERATION_SOURCE = 'w4-certified-recipe' as const;

/** The only thinking levels a certified D65 assignment may carry. */
const D65_ALLOWED_THINKING = Object.freeze(['high', 'xhigh'] as const);
...
export function authenticateD65LaunchRoster(manifest: D65LaunchManifest): D65AuthenticatedRoster {
  ...
  if (roster.generation_source !== D65_REQUIRED_ROSTER_GENERATION_SOURCE) throw new CoordinationRuntimeError('invalid-state', 'sealed roster generation source is not W4-certified launch authority', [String(roster.generation_source), D65_REQUIRED_ROSTER_GENERATION_SOURCE]);
  if (typeof roster.certification_manifest_id !== 'string' || roster.certification_manifest_id.length === 0) throw new CoordinationRuntimeError('invalid-state', 'sealed roster carries no certification manifest id', [String(roster.certification_manifest_id)]);
  if (typeof roster.certification_manifest_sha256 !== 'string' || !/^sha256:[a-f0-9]{64}$/u.test(roster.certification_manifest_sha256)) throw new CoordinationRuntimeError('invalid-state', 'sealed roster carries no canonical certification manifest digest', [String(roster.certification_manifest_sha256)]);
  for (const role of D65_REQUIRED_ROSTER_ROLES) {
    const assignment = byRole.get(role);
    if (assignment === undefined) throw new CoordinationRuntimeError('invalid-state', 'sealed roster is missing a required role assignment', [role]);
    ...
    if (assignment.qualification_state !== D65_REQUIRED_ROSTER_QUALIFICATION_STATE) throw new CoordinationRuntimeError('invalid-state', `sealed roster role ${role} is not W4-certified launch authority`, [String(assignment.qualification_state), D65_REQUIRED_ROSTER_QUALIFICATION_STATE]);
    if (typeof assignment.model_id !== 'string' || assignment.model_id.length === 0) throw new CoordinationRuntimeError('invalid-state', `sealed roster role ${role} has no model id`, [String(assignment.model_id)]);
    if (!D65_ALLOWED_THINKING.includes(assignment.thinking as (typeof D65_ALLOWED_THINKING)[number])) throw new CoordinationRuntimeError('invalid-state', `sealed roster role ${role} thinking is out of range`, [String(assignment.thinking)]);
    if (assignment.model !== `${provider}/${assignment.model_id}`) throw new CoordinationRuntimeError('invalid-state', `sealed roster role ${role} model identifier is malformed`, [assignment.model]);
  ...
  for (const role of byRole.keys()) {
    if (!(D65_REQUIRED_ROSTER_ROLES as readonly string[]).includes(role)) throw new CoordinationRuntimeError('invalid-state', 'sealed roster contains an unexpected role', [role]);
  }
