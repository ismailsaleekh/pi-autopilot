...
export async function readCoordinatorUpgradeIntent(paths: CoordinatorRuntimePaths): Promise<CoordinatorUpgradeIntent | null> {
  try { return parseCoordinatorUpgradeIntent(JSON.parse(await readFile(coordinatorUpgradeIntentPath(paths), 'utf8')) as unknown); }
  catch (error) {
...
async function createVerifiedBackup(paths: CoordinatorRuntimePaths, upgradeId: string): Promise<CoordinatorUpgradeBackup> {
  ...
  const target = join(paths.backupsRoot, `coordinator.preflight.pre-upgrade-1.2-to-1.3.${upgradeId}.db`);
...
function finalUpgradeBackupPath(paths: CoordinatorRuntimePaths, upgradeId: string): string {
  return join(paths.backupsRoot, `coordinator.final.pre-upgrade-1.2-to-1.3.${upgradeId}.db`);
}
...
async function verifyMigrationOnCopy(paths: CoordinatorRuntimePaths, record: CoordinatorUpgradeBackup, upgradeId: string, retain = false): Promise<string> {
  ...
  try {
    ...
    if (status['package_build'] !== COORDINATOR_UPGRADE_PATH.target.package_build || status['protocol_version'] !== '1.3' || status['database_schema_version'] !== 9) throw new CoordinationRuntimeError('schema-mismatch', 'schema migration probe did not reach the locked schema-9 target');
  } finally { store?.close(); }
  const checkpoint = new DatabaseSync(probePaths.databasePath, { timeout: 5_000 });
  try {
    checkpoint.exec('PRAGMA wal_checkpoint(TRUNCATE)');
    if ((checkpoint.prepare('PRAGMA integrity_check').get() as JsonRecord | undefined)?.['integrity_check'] !== 'ok' || (checkpoint.prepare('PRAGMA user_version').get() as JsonRecord | undefined)?.['user_version'] !== 9) throw new CoordinationRuntimeError('store-corrupt', 'private migrated target failed final schema-9 verification');
...
function installedBarrierBackupDigest(database: DatabaseSync, upgradeId: string): `sha256:${string}` {
  ...
  if (rows.length !== 1 || row?.['upgrade_id'] !== upgradeId || row['source_schema'] !== 6 || row['target_schema'] !== 9 || typeof digest !== 'string' || !/^sha256:[a-f0-9]{64}$/u.test(digest)) throw new CoordinationRuntimeError('schema-mismatch', 'committed incompatible migration barrier identity does not match the durable upgrade', [upgradeId]);
...
function commitIncompatibleMigrationBarrier(database: DatabaseSync, upgradeId: string, backupRecord: CoordinatorUpgradeBackup): void {
  ...
  database.prepare('INSERT INTO coordinator_upgrade_barrier(upgrade_id, source_schema, target_schema, backup_sha256) VALUES(?, 6, 9, ?)').run(upgradeId, backupRecord.sha256);
...
async function publishMigratedPrivateDatabase(paths: CoordinatorRuntimePaths, backupRecord: CoordinatorUpgradeBackup, upgradeId: string): Promise<void> {
  ...
  if (current.version === 9 && current.integrity === 'ok') return;
  if (current.version !== COORDINATOR_UPGRADE_BARRIER_SCHEMA_VERSION || current.integrity !== 'ok') throw new CoordinationRuntimeError('schema-mismatch', 'schema-9 publication requires the exact incompatible migration barrier');
  const barrier = new DatabaseSync(paths.databasePath, { readOnly: true, timeout: 5_000 });
  try {
    if (installedBarrierBackupDigest(barrier, upgradeId) !== backupRecord.sha256) throw new CoordinationRuntimeError('store-corrupt', 'schema-9 publication backup does not match the committed barrier digest');
  ...
  if (published.version !== 9 || published.integrity !== 'ok') throw new CoordinationRuntimeError('store-corrupt', 'atomically published schema-9 database failed verification');
}
...
async function copyExactBackupForRestore(paths: CoordinatorRuntimePaths, backupRecord: CoordinatorUpgradeBackup, upgradeId: string): Promise<void> {
  ...
  if (stagedDigest !== expectedDigest) { await unlink(temporary).catch(() => undefined); throw new CoordinationRuntimeError('store-corrupt', 'staged rollback copy differs from the verified backup'); }
...
export async function resumeCoordinatorUpgrade(paths: CoordinatorRuntimePaths): Promise<CoordinatorUpgradeTransaction | null> {
  ...
  if (intent === null || intent.state === 'committed' || intent.state === 'refused') return null;
  ...
  try {
    ...
    } else if (observed.version !== 9) {
      throw new CoordinationRuntimeError('schema-mismatch', 'upgrade recovery found neither schema 6, the durable incompatibility barrier, nor schema 9');
    }
