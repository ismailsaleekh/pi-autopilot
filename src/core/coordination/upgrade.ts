...
async function verifyMigrationOnCopy(paths: CoordinatorRuntimePaths, record: CoordinatorUpgradeBackup, upgradeId: string, retain = false): Promise<string> {
  ...
  try {
    ...
    if (status['package_build'] !== COORDINATOR_UPGRADE_PATH.target.package_build || status['protocol_version'] !== '1.4' || status['database_schema_version'] !== 10) throw new CoordinationRuntimeError('schema-mismatch', 'schema migration probe did not reach the locked schema-10 target');
  } finally { store?.close(); }
  const checkpoint = new DatabaseSync(probePaths.databasePath, { timeout: 5_000 });
  try {
    checkpoint.exec('PRAGMA wal_checkpoint(TRUNCATE)');
    if ((checkpoint.prepare('PRAGMA integrity_check').get() as JsonRecord | undefined)?.['integrity_check'] !== 'ok' || (checkpoint.prepare('PRAGMA user_version').get() as JsonRecord | undefined)?.['user_version'] !== 10) throw new CoordinationRuntimeError('store-corrupt', 'private migrated target failed final schema-10 verification');
...
function installedBarrierBackupDigest(database: DatabaseSync, upgradeId: string): `sha256:${string}` {
  ...
  if (rows.length !== 1 || row?.['upgrade_id'] !== upgradeId || row['source_schema'] !== 6 || row['target_schema'] !== 10 || typeof digest !== 'string' || !/^sha256:[a-f0-9]{64}$/u.test(digest)) throw new CoordinationRuntimeError('schema-mismatch', 'committed incompatible migration barrier identity does not match the durable upgrade', [upgradeId]);
...
async function publishMigratedPrivateDatabase(paths: CoordinatorRuntimePaths, backupRecord: CoordinatorUpgradeBackup, upgradeId: string): Promise<void> {
  ...
  if (current.version === 10 && current.integrity === 'ok') return;
  if (current.version !== COORDINATOR_UPGRADE_BARRIER_SCHEMA_VERSION || current.integrity !== 'ok') throw new CoordinationRuntimeError('schema-mismatch', 'schema-10 publication requires the exact incompatible migration barrier');
  const barrier = new DatabaseSync(paths.databasePath, { readOnly: true, timeout: 5_000 });
  try {
    if (installedBarrierBackupDigest(barrier, upgradeId) !== backupRecord.sha256) throw new CoordinationRuntimeError('store-corrupt', 'schema-10 publication backup does not match the committed barrier digest');
  ...
  if (published.version !== 10 || published.integrity !== 'ok') throw new CoordinationRuntimeError('store-corrupt', 'atomically published schema-10 database failed verification');
}
