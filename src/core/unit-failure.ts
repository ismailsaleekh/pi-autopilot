...
export async function resetFailedUnit(input: UnitFailureInput): Promise<AutopilotUnitFailureRecord> {
  ...
  if (archiveRef !== null) await archiveFailureBranch(input, currentSha, archiveRef, 'unit reset preservation archive');
...
async function archiveFailureBranch(input: UnitFailureInput, sha: string, archiveRef: string, reason: string): Promise<void> {
...
async function captureDirtyBeforeDestructiveTransition(input: UnitFailureInput): Promise<AutopilotUnitFailureRecord | null> {
  if (!existsSync(input.unitWorktreePath) || readGitStatus(input.unitWorktreePath).changedPaths.length === 0) return null;
...
async function resetWorktreeForRecordedTransition(input: UnitFailureInput, authority: string, action: 'reset' | 'abort'): Promise<void> {
  if (!existsSync(input.unitWorktreePath)) return;
  const active = input.context.active;
...
async function captureQuarantineSnapshot(input: UnitFailureInput & { readonly action: AutopilotUnitFailureAction }, dirtyPaths: readonly string[]): Promise<string> {
  ...
  await executeOwnedWorktreeSaga({
    ...
    inspect,
    action: () => {
      if (readGitStatus(input.unitWorktreePath).changedPaths.length === 0) return;
      const env = {
    ...
    verify: () => {
      const inspected = inspect();
      if (inspected.outcome !== 'satisfied') throw new Error(`quarantine capture left mutable dirty paths: ${inspected.proof.join(', ')}`);
      return inspected.proof;
    },
  }, input.env ?? process.env);
