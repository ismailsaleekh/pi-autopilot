...
async function finishCommittedQuarantine(input: UnitFailureInput, operation: CoordinationWorktreeOperation): Promise<AutopilotUnitFailureRecord> {
  ...
  if ((ancestor.status ?? -1) !== 0 || (diff.status ?? -1) !== 0) throw new CoordinationRuntimeError('recovery-required', 'committed quarantine capture is not an inspectable descendant of its pre-capture head', [baseSha, facts.head, ancestor.stderr.trim(), diff.stderr.trim()]);
  const capturedPaths = diff.stdout.split('\0').filter((path) => path.length > 0).map((path) => path.replace(/\\/gu, '/'));
  const overlaps = (left: string, right: string): boolean => left === right || left.startsWith(`${right}/`) || right.startsWith(`${left}/`);
  const uncovered = operation.intent.paths.filter((path) => !capturedPaths.some((candidate) => overlaps(path, candidate)));
  if ((operation.intent.paths.length > 0 && capturedPaths.length === 0) || uncovered.length > 0) throw new CoordinationRuntimeError('recovery-required', 'committed quarantine capture does not preserve every originally mutable path', uncovered.length > 0 ? uncovered : operation.intent.paths);
...
async function archiveFailureBranch(input: UnitFailureInput, sha: string, archiveRef: string, reason: string, worktreeState: 'terminal' | 'quarantined' = 'terminal'): Promise<void> {
  ...
  await executeOwnedWorktreeSaga({
    ...
    verify: () => {
      ...
      if (inspected.outcome !== 'satisfied') throw new Error(`failure archive saga postcondition failed: ${inspected.proof.join(', ')}`);
...
async function resetWorktreeForRecordedTransition(input: UnitFailureInput, authority: string, action: 'reset' | 'abort'): Promise<void> {
  ...
  const inspect = (): WorktreeSagaInspection => {
    if (!existsSync(input.unitWorktreePath)) return { outcome: 'unsafe', proof: ['owned_worktree_missing_before_reset'] };
  ...
  await executeOwnedWorktreeSaga({
    ...
    verify: () => {
      const inspected = inspect();
      if (inspected.outcome !== 'satisfied' || gitHead(input.unitWorktreePath) !== target) throw new Error(`failed-unit ${action} saga did not restore clean head ${target}: ${inspected.proof.join(', ')}`);
      return inspected.proof;
    },
  }, input.env ?? process.env);
}
...
function configuredSubmodulePaths(cwd: string): readonly string[] {
  ...
  if ((result.status ?? -1) === 1) return Object.freeze([]);
  if ((result.status ?? -1) !== 0) throw new CoordinationRuntimeError('recovery-required', 'quarantine could not inspect submodule declarations', [result.stderr.trim()]);
...
async function failureBaselineHead(input: UnitFailureInput, beforeFacts: OwnedFailureWorktreeFacts): Promise<string> {
  ...
  if ((ancestor.status ?? -1) !== 0) throw new CoordinationRuntimeError('recovery-required', 'failed-unit pre-capture HEAD does not descend from its exact attempt baseline', [baseline, beforeFacts.head, ancestor.stderr.trim()]);
  return baseline;
}
...
async function captureQuarantineSnapshot(input: UnitFailureInput & { readonly action: AutopilotUnitFailureAction }, beforeFacts: OwnedFailureWorktreeFacts): Promise<string> {
  ...
  await executeOwnedWorktreeSaga({
    ...
    action: () => {
      ...
      if ((staged.status ?? -1) === 1) runGit(['commit', '--no-verify', '-m', `autopilot quarantine capture ${active.workstream_run} ${input.unitId} attempt ${String(input.attempt)}`], input.unitWorktreePath, env);
      else if ((staged.status ?? -1) !== 0) throw new CoordinationRuntimeError('recovery-required', 'quarantine could not verify its staged capture', [staged.stderr.trim()]);
    },
    verify: () => {
      const inspected = inspect();
      if (inspected.outcome !== 'satisfied') throw new CoordinationRuntimeError('recovery-required', 'quarantine capture left mutable or ignored paths', inspected.proof);
      return inspected.proof;
    },
  }, input.env ?? process.env);
