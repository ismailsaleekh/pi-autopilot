...
export type AutopilotObservedServiceTier = null | 'priority';
export type AutopilotObservedCachePolicy = 'provider-default' | 'none' | 'short' | 'long';
export type AutopilotObservedSystemPromptProfile = 'pi-default.v1' | 'anthropic-autopilot-sanitized.v1';

export interface AutopilotExecutionObservationModel {
  readonly provider: string | null;
  readonly id: string | null;
  readonly api: string | null;
}

export interface AutopilotExecutionObservationFinalAssistantMessage {
...
export interface AutopilotExecutionObservationProfile {
  readonly service_tier: AutopilotObservedServiceTier;
  readonly cache_policy: AutopilotObservedCachePolicy;
  readonly system_prompt_profile: AutopilotObservedSystemPromptProfile;
  readonly system_prompt_sha256: `sha256:${string}`;
  readonly route_policy_id: string;
  readonly route_policy_revision: number;
}

export interface AutopilotExecutionObservationV1 {
  readonly schema_version: typeof AUTOPILOT_EXECUTION_OBSERVATION_SCHEMA_VERSION;
  readonly source: 'pi-extension-events.v1' | 'fake-rpc-subprocess.v1';
  readonly observed_at: string;
  readonly mode: string | null;
  readonly active_model: AutopilotExecutionObservationModel | null;
  readonly final_assistant_message: AutopilotExecutionObservationFinalAssistantMessage | null;
  readonly execution_profile: AutopilotExecutionObservationProfile;
  readonly diagnostics: readonly string[];
}

interface MutableObservationState {
  mode: string | null;
  activeModel: AutopilotExecutionObservationModel | null;
  finalAssistantMessage: AutopilotExecutionObservationFinalAssistantMessage | null;
  serviceTier: AutopilotObservedServiceTier;
  cachePolicy: AutopilotObservedCachePolicy;
  systemPromptProfile: AutopilotObservedSystemPromptProfile | null;
  systemPromptSha256: `sha256:${string}` | null;
  routePolicyId: string | null;
  routePolicyRevision: number | null;
  diagnostics: string[];
}

interface PiObserverContextLike {
  readonly mode: string | null;
  readonly model: unknown;
}

interface PiObserverModelSelectEventLike {
  readonly model: unknown;
}

interface PiObserverBeforeAgentStartEventLike {
  readonly systemPrompt: string;
  readonly systemPromptOptions?: unknown;
}

interface PiObserverBeforeProviderRequestEventLike {
  readonly payload: unknown;
}

interface PiObserverMessageEndEventLike {
  readonly message: JsonRecord;
}

interface PiExecutionObserverExtensionHostLike {
  on(eventName: 'session_start', handler: (event: JsonRecord, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'model_select', handler: (event: PiObserverModelSelectEventLike, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'before_agent_start', handler: (event: PiObserverBeforeAgentStartEventLike, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'before_provider_request', handler: (event: PiObserverBeforeProviderRequestEventLike, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'message_end', handler: (event: PiObserverMessageEndEventLike, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'agent_settled', handler: (event: JsonRecord, ctx: PiObserverContextLike) => void): void;
  on(eventName: 'session_shutdown', handler: (event: JsonRecord, ctx: PiObserverContextLike) => void): void;
}

const SHA256_PATTERN = /^sha256:[a-f0-9]{64}$/u;

export function parseAutopilotExecutionObservation(value: unknown): AutopilotExecutionObservationV1 {
  ...
  if (!Number.isFinite(Date.parse(observedAt))) throw new Error('execution observation observed_at is invalid');
...
export function inferProviderRequestCachePolicy(payload: unknown): AutopilotObservedCachePolicy | 'unsupported-explicit-cache' {
  if (containsProviderCacheDirective(payload)) return 'unsupported-explicit-cache';
  return 'provider-default';
}

export function inferProviderRequestServiceTier(payload: unknown): AutopilotObservedServiceTier | 'unsupported-service-tier' {
...
export default function autopilotExecutionObserverExtension(pi: PiExecutionObserverExtensionHostLike): void {
  ...
  const writeObservation = (): void => {
    ...
    if (systemPromptSha256 === null) diagnostics.push('system prompt was not observed before provider request');
    if (systemPromptProfile === null) diagnostics.push('system prompt profile could not be classified from Pi prompt inputs');
    if (route === null) diagnostics.push('route policy could not be derived from observed provider/api');
    const record = {
      schema_version: AUTOPILOT_EXECUTION_OBSERVATION_SCHEMA_VERSION,
      source: 'pi-extension-events.v1' as const,
  ...
  pi.on('before_provider_request', (event, ctx) => {
    ...
    if (serviceTier === 'unsupported-service-tier') state.diagnostics.push('provider payload carried unsupported service tier');
    else state.serviceTier = serviceTier;
    const cachePolicy = inferProviderRequestCachePolicy(event.payload);
    if (cachePolicy === 'unsupported-explicit-cache') state.diagnostics.push('provider payload carried explicit cache controls unsupported by Autopilot W3 observer');
    else state.cachePolicy = cachePolicy;
    writeObservation();
  });
...
function parseExecutionProfile(value: unknown): AutopilotExecutionObservationProfile {
...
function classifySystemPromptProfile(options: unknown): AutopilotObservedSystemPromptProfile | null {
  ...
  if (!hasCustomPrompt && !hasAppend && !hasContextFiles && !hasSkills && !hasPromptTemplates && !hasThemes && processArgvProvesPiDefaultPromptProfile()) return 'pi-default.v1';
  if (hasCustomPrompt && typeof customPrompt === 'string' && /anthropic-autopilot-sanitized\.v1/u.test(customPrompt)) return 'anthropic-autopilot-sanitized.v1';
  return null;
}

function processArgvProvesPiDefaultPromptProfile(argv: readonly string[] = process.argv): boolean {
...
function projectModel(model: unknown): AutopilotExecutionObservationModel | null {
...
function nullableModel(value: unknown, label: string): AutopilotExecutionObservationModel | null {
...
function nullableFinalAssistant(value: unknown, label: string): AutopilotExecutionObservationFinalAssistantMessage | null {
  if (value === null) return null;
  const record = jsonRecord(value, label);
  return Object.freeze({
...
function findProviderServiceTier(value: unknown): unknown {
  if (!isRecord(value)) return undefined;
  if (Object.prototype.hasOwnProperty.call(value, 'service_tier')) return value['service_tier'];
  if (Object.prototype.hasOwnProperty.call(value, 'serviceTier')) return value['serviceTier'];
  return undefined;
}

function containsProviderCacheDirective(value: unknown, depth = 0): boolean {
  if (depth > 8) return false;
  if (Array.isArray(value)) return value.some((item) => containsProviderCacheDirective(item, depth + 1));
  if (!isRecord(value)) return false;
  for (const [key, entry] of Object.entries(value)) {
    if (key === 'cache_control' || key === 'prompt_cache_key' || key === 'prompt_cache_retention' || key === 'cachePolicy') return true;
    if (containsProviderCacheDirective(entry, depth + 1)) return true;
  }
  return false;
}

function cachePolicy(value: unknown): AutopilotObservedCachePolicy {
  if (value === 'provider-default' || value === 'none' || value === 'short' || value === 'long') return value;
  return fail('execution observation cache_policy is invalid');
}

function systemPromptProfile(value: unknown): AutopilotObservedSystemPromptProfile {
  if (value === 'pi-default.v1' || value === 'anthropic-autopilot-sanitized.v1') return value;
...
function shaField(record: JsonRecord, field: string): `sha256:${string}` {
  const value = stringField(record, field, 'execution observation profile');
  if (!SHA256_PATTERN.test(value)) throw new Error(`execution observation profile.${field} is not a sha256 digest`);
...
function arrayOfStrings(value: unknown, label: string): readonly string[] {
