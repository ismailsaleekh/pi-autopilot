...
import { parseCoordinationMigrationRecoveryWork } from '../core/coordination/contracts.ts';
import { CoordinationRuntimeError } from '../core/coordination/failures.ts';
import { coordinationMigrationCoordinatorRunning, retireCoordinationMigrationCoordinator } from '../core/coordination/migration.ts';
import { coordinatorRuntimePaths } from '../core/coordination/runtime-paths.ts';
import { DurableRunSupervisorClient, readMigrationRecoveryEvidenceFile, type RunSupervisorAttachment } from '../core/coordination/supervisor.ts';
...
type Command = 'list' | 'show' | 'doctor' | 'retain-authority' | 'release-with-evidence';
...
function parse(argv: readonly string[]): Args {
  ...
  if (command !== 'list' && command !== 'show' && command !== 'doctor' && command !== 'retain-authority' && command !== 'release-with-evidence') throw new Error(migrationRecoveryUsage());
...
function rows(payload: Readonly<Record<string, unknown>>): readonly CoordinationMigrationRecoveryWork[] {
  const value = payload['migration_recovery_work'];
  if (!Array.isArray(value)) throw new CoordinationRuntimeError('store-corrupt', 'coordinator status omitted migration recovery work');
...
export async function runMigrationRecoveryCli(argv: readonly string[], baseEnv: ProcessEnvLike = process.env): Promise<Readonly<Record<string, unknown>>> {
  ...
  try {
    ...
    if (!coordinatorWasRunning && coordinationMigrationCoordinatorRunning(paths)) {
      ...
      if (blockers.length > 0) throw new CoordinationRuntimeError('invalid-state', 'recovery CLI could not retire its temporary frozen coordinator', blockers);
    }
  }
}
