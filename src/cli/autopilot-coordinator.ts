...
interface CliArgs {
  ...
  readonly migrationCommand: CoordinationMigrationCommand | null;
}
...
function parseArgs(argv: readonly string[]): CliArgs {
  ...
  let migrationCommand: CoordinationMigrationCommand | null = command === 'verify' || command === 'rollback' || command === 'cutover' ? command : null;
  for (let index = 1; index < argv.length; index += 1) {
    const token = argv[index];
    const value = argv[index + 1];
    if (token === '--help' || token === '-h') throw new Error(usage());
    if (token === '--dry-run' || token === '--apply') {
      if (command !== 'migrate') throw new Error(`${token} is supported only by migrate`);
      if (migrationCommand !== null) throw new Error('migrate requires exactly one of --dry-run or --apply');
      migrationCommand = token === '--dry-run' ? 'dry-run' : 'apply';
      continue;
    }
    if (token !== '--state-root' && token !== '--repo-id' && token !== '--repo-key' && token !== '--repo-root' && token !== '--run' && token !== '--output' && token !== '--input' && token !== '--replay-id') throw new Error(`unknown option ${String(token)}\n${usage()}`);
    if (value === undefined || value.startsWith('--')) throw new Error(`${token} requires a value`);
    if (token === '--state-root') stateRoot = value;
    else if (token === '--repo-id') repoId = value;
    else if (token === '--repo-key') repoKey = value;
    else if (token === '--repo-root') repoRoot = value;
    else if (token === '--run') workstreamRun = value;
    else if (token === '--output') outputPath = value;
    else if (token === '--input') inputPath = value;
    else replayId = value;
    index += 1;
  }
  ...
  if (inputPath !== null && !isAbsolute(inputPath)) throw new Error('--input must be absolute');
  if (repoRoot !== null && !isAbsolute(repoRoot)) throw new Error('--repo-root must be absolute');
  ...
  if (command === 'replay' && (inputPath === null || replayId === null)) throw new Error('replay requires --input and --replay-id');
  if (command !== 'replay' && (inputPath !== null || replayId !== null)) throw new Error('--input and --replay-id are supported only by replay');
  if (!isMigration && (repoKey !== null || repoRoot !== null)) throw new Error('--repo-key and --repo-root are supported only by migration commands');
  if (repoKey !== null && repoRoot !== null) throw new Error('migration accepts only one of --repo-key or --repo-root');
  if (isMigration && migrationCommand === null) throw new Error(`migration command requires a mode\n${coordinationMigrationUsage()}`);
