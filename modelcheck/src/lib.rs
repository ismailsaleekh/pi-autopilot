...
fn parse_machine(n: &KdlNode, src: &str) -> Result<Machine, String> {
    ...
    for c in children(n, src)?.nodes() { match nn(c) {
        ...
        "transition" => { entries(c, 0, &["from", "to", "evidence", "verdict_kind", "verdict", "route", "outcome", "status", "doc"], src)?; opt_s(c, "doc", src)?; opt_s(c, "route", src)?; opt_s(c, "outcome", src)?; opt_s(c, "status", src)?; m.transitions.push(Transition { from: prop_s(c, "from", src)?.to_owned(), to: prop_s(c, "to", src)?.to_owned(), evidence: opt_s(c, "evidence", src)?.map(str::to_owned), verdict_kind: opt_s(c, "verdict_kind", src)?.map(str::to_owned), verdict: opt_s(c, "verdict", src)?.map(str::to_owned), line: line(c, src) }); no_child(c, src)?; }
...
fn c4(m: &Machine, v: &mut Vec<Violation>) { let terms: Vec<&String> = m.states.iter().filter_map(|(n, s)| s.terminal.then_some(n)).collect(); for (n, s) in &m.states { if s.terminal { continue; } let seen = reachable(m, n); if !terms.iter().any(|t| seen.contains(*t)) { add(v, "C4", &m.name, s.line, format!("no terminal state is reachable from non-terminal `{n}`")); } } }
