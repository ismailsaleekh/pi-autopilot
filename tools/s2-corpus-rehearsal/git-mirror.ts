...
import { assertNoSharedRegularFileIdentity, compareCodeUnits, digestBytes, inside, inventoryTree } from './inventory.ts';
...
export interface GitMirrorResult {
  ...
  readonly refs_sha256: Sha256Digest;
  readonly registrations_sha256: Sha256Digest;
  readonly registrations: readonly GitWorktreeFact[];
}

function gitEnvironment(): Readonly<Record<string, string | undefined>> {
  const env: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('GIT_') || /(?:SSH|CREDENTIAL|ASKPASS)/u.test(key)) continue;
    env[key] = value;
  }
...
async function runGit(cwd: string, args: readonly string[], accepted: readonly number[] = [0]): Promise<GitResult> {
  return await new Promise<GitResult>((resolveRun, rejectRun) => {
    ...
    child.once('close', (code, signal) => {
      ...
      if (code === null || !accepted.includes(code) || signal !== null || overflow) rejectRun(new Error(`S2-D Git command failed: code=${String(code)} signal=${String(signal)} overflow=${String(overflow)} stderr_bytes=${String(Buffer.byteLength(err, 'utf8'))}`));
      else resolveRun({ stdout: out, stderr: err });
    });
  });
}

export async function gitRefs(cwd: string, gitDir?: string): Promise<readonly string[]> {
...
async function deleteRemoteTrackingRefs(cwd: string): Promise<void> {
  const output = await runGit(cwd, ['for-each-ref', '--format=%(refname)', 'refs/remotes'], [0]);
  for (const ref of output.stdout.split('\n').filter((line) => line.length > 0).sort(compareCodeUnits)) await runGit(cwd, ['update-ref', '-d', ref], [0, 1]);
}

async function sanitizeGitConfig(gitCommonDir: string): Promise<void> {
  ...
  if (unsupported.length > 0) throw new Error(`S2-D Git mirror encountered unsupported extensions: ${unsupported.join(',')}`);
  ...
  if (objectFormat !== '' && objectFormat !== 'sha256') throw new Error('S2-D Git mirror encountered unsupported object format');
  if (refStorage !== '' && refStorage !== 'reftable') throw new Error('S2-D Git mirror encountered unsupported ref storage');
  const config = [
    '[core]',
...
function parseWorktreePorcelain(output: string): readonly GitWorktreeFact[] {
  const facts: GitWorktreeFact[] = [];
  ...
  const flush = (): void => {
    ...
    if (headSha === null || !/^[a-f0-9]{40,64}$/u.test(headSha)) throw new Error('S2-D Git worktree fact omits a head SHA');
  ...
  for (const line of output.split('\n')) {
    ...
    if (key === 'worktree') worktreePath = resolve(value);
    else if (key === 'HEAD') headSha = value;
...
export async function gitWorktreeFacts(gitCommonDir: string): Promise<readonly GitWorktreeFact[]> {
...
export async function verifyGitMirror(gitCommonDir: string, cloneRoot: string): Promise<readonly GitWorktreeFact[]> {
  ...
  if (forbidden.some(existsSync)) throw new Error('S2-D Git mirror retained alternates, grafts, shallow, worktree config, FETCH_HEAD, or logs');
  const packRoot = join(gitCommonDir, 'objects', 'pack');
  if (existsSync(packRoot) && (await readdir(packRoot)).some((name) => name.endsWith('.promisor'))) throw new Error('S2-D Git mirror retained promisor metadata');
  const config = readFileSync(join(gitCommonDir, 'config'), 'utf8');
  if (/\b(?:url|pushurl|insteadOf|include\.path|core\.hooksPath)\b/iu.test(config)) throw new Error('S2-D Git mirror retained remote, include, or hook routing config');
  const remotes = await runGit(gitCommonDir, ['--git-dir', gitCommonDir, 'remote'], [0]);
  if (remotes.stdout.trim() !== '') throw new Error('S2-D Git mirror retained a remote');
  if ((await readdir(join(gitCommonDir, 'hooks'))).length !== 0) throw new Error('S2-D Git mirror retained hooks');
  ...
  for (const fact of registrations) if (!inside(cloneRoot, fact.worktree_path)) throw new Error('S2-D Git mirror worktree registration escapes clone root');
  return registrations;
}

export async function buildWritableGitMirror(input: { readonly source_repository_root: string; readonly clone_root: string; readonly copy_repository_root: string }): Promise<GitMirrorResult> {
  ...
  if (!inside(cloneRoot, copyRepository) || !inside(cloneRoot, gitCommonDir)) throw new Error('S2-D Git mirror destination escapes clone root');
  if (existsSync(copyRepository) || existsSync(gitCommonDir)) throw new Error('S2-D Git mirror destinations must be absent');
  ...
  try {
    ...
    if (canonicalJson(sourceRefs) !== canonicalJson(copyRefs)) throw new Error('S2-D Git mirror refs differ from source refs');
...
export function hashGitWitness(facts: readonly GitWorktreeFact[], refsValue: readonly string[]): Sha256Digest {
  return digestBytes(canonicalJson({ facts, refs: refsValue }));
}

export function assertPhysicalGitRepository(path: string): void {
  const stat = lstatSync(path);
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error('S2-D Git repository path is not a physical directory');
}
