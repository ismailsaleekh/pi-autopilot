...
import { buildMutableClone, preflightCloneRequest, writeRehearsalResult } from './release-gate.ts';
...
interface GateStatus {
  readonly schema_version: 'autopilot.s2_d_corpus_gate_status.v1';
  readonly status: 'not_run';
  readonly reason: 'private_rehearsal_result_unavailable';
}
...
async function privateJson(path: string, requirePrivateMode: boolean): Promise<{ readonly bytes: Uint8Array; readonly value: unknown }> {
  ...
  if (input.size_bytes < 2 || input.identity.link_count !== 1) throw new Error('S2-D private input must be a bounded single-link regular file');
  if (requirePrivateMode && process.platform !== 'win32' && (input.mode & 0o077) !== 0) throw new Error('S2-D private request/result permissions must be 0600 or stricter');
  let value: unknown;
  try { value = JSON.parse(Buffer.from(input.bytes).toString('utf8')) as unknown; }
  catch { throw new Error('S2-D private input is not JSON'); }
...
async function main(args: readonly string[]): Promise<void> {
  ...
  if (command === 'status') {
    ...
    if (resultPath === undefined || resultPath.length === 0) {
      const status: GateStatus = { schema_version: 'autopilot.s2_d_corpus_gate_status.v1', status: 'not_run', reason: 'private_rehearsal_result_unavailable' };
