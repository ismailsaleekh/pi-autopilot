...
Every model assignment terminates through one parent-selected generated tool profile. `tool_execution_end.result.details`, correlated to `message_end(toolResult)` by opaque call id, is authoritative; assistant terminal text, mixed tool batches, duplicate results, identity drift, and stale unconsumed carrier files fail loudly. Planning carriers retain their planning boundary payload. Delivery and Validation use model-only v2 submissions wrapped in package-owned v2 results with immutable runner identity and a digest-bound tool audit. Same-session terminal continuation is recorded in the create-once attempt-event ledger; it never adds undeclared fields to a closed carrier, and a continued tool result follows the identical generated carrier parser as a first-turn result.

Delivery verification has two typed owners. `commands` are pre-package child evidence. Core projects each command to a deterministic package-owned id in `autopilot.delivery_assignment.v3`; delivery roles receive `autopilot_run_approved_command`, not general Bash, and can provide only that id. The pinned add-on resolves exact command bytes, unit, cwd, and assignment digest, executes through a hidden bounded shell backend with session-environment exposure disabled, and serializes execution with edit/write. A bounded typed ledger records command/result digests and a content/mode-sensitive approved-file snapshot. Before packaging, Core independently requires every command to have a success receipt on the final snapshot. It then mints package-commit-bound command evidence for the independent Validator, and every criterion must cite exactly its assigned command receipts.

`package_checks` remain closed Core-owned obligations. A unit carrying one is mechanically a closure unit and must declare the complete union of all work-map files, so an integrated repair cannot require authority invented after planning. `clean-exact-package-tip` is proved only after an admitted succeeded submission by Core's exact commit/tree, base-ancestry, strictly clean worktree (including nonignored untracked residue), and changed-path equality checks. Package checks never become child shell. Every check declares unique in-range 1-based `criterion_ordinals`; Core emits a digest-bound package-check receipt only onto those criteria in the fact-only validation context, and validation admission requires each affected criterion to cite that exact receipt. Missing or older assignment/audit fields fail closed rather than defaulting to child authority.

The child delivery policy records only unknown approved-command ids refused before execution in a bounded digest-only denial ledger. The ledger cannot be supplied by model parameters, carries `effected=false`, and saturates visibly after 32 entries. Edit/write/path/topology refusals are not relabeled repairable. Core revalidates both denial and execution ledgers against the pinned add-on, exact assignment, final source snapshot, and audit digests and separately inspects Git; a denied reference is therefore distinguishable from an effected mutation without trusting model prose.
