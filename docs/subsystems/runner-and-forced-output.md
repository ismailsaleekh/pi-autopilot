...
3. Core emits a `background_action` whose nested `bg_run` object is byte-exactly the public `pi-background-tasks@2.1.1` `run` payload. Every Autopilot-owned descriptor keeps `notifyOnCompletion: true` for durable operator notification and sets `triggerOnCompletion: false`, so completion is machine-consumed without waking the unrestricted parent model.
