...
| `validation_admission_diagnostic` | V3 Validator model shape or semantic value conflicts with the closed contract/authority (fields, generated enum values, citations, criterion/finding links, or line bounds) | Correct every represented row in the canonical diagnostic in the same session. Its root/field JSON pointers, actual values, path, digest, and complete pre-summary mismatch count are authoritative; a `diagnostic-row-summary` binds any rows beyond the 2048-row carrier limit by omitted count and digest. Core has already bound command/package receipts; the repair context remains receipt-redacted. The per-cycle RPC byte budget resets, but exactly three value attempts exist. |
| `fatal-authority` / v3 authority digest, source, diff, or receipt drift | Package-owned candidate/receipt authority is stale, malformed, cross-scoped, or tampered | Stop. This is not model repair and never routes to Recovery Engineer or fixer-integrator; preserve the named diagnostic and repair the package authority. |
| `rpc frame too large before allocation` or terminal-limit override refusal | A child emitted one JSONL record above the fixed 4 MiB ceiling, omitted its newline while growing past it, or configuration tried to raise/disable the ceiling | Treat as a protocol/authority failure. Do not enlarge the ceiling or retry model content; retain bounded diagnostics and fix the producer/configuration. |
| `git stdout exceeds` / `git stderr exceeds` or `bounded read oversized` | Package-state output or an authority artifact exceeded its pre-allocation descriptor/stream ceiling | Stop before admission. Narrow the package to declared bounds or repair corrupt/untrusted state; never switch back to `Command::output` or an unrestricted file read. |
...
| Closed carrier rejects `continuation_provenance` as unknown | Runner leaked attempt-ledger metadata into the result contract | Keep continuation class/directive/session evidence in create-once attempt events only; continued results must use the unchanged closed carrier shape. |
