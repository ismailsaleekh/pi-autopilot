...
| `delivery-recovery-inadmissible:Some(RequiresNewAuthority)` | No qualifying pre-effect command denial, no in-scope work, overflowed/malformed audit, path-policy denial, or a genuine authority gap | Preserve the carrier and audit. Do not widen tools or relabel from prose. New authority requires an operator decision; only exact mechanical reconciliation may issue the one Recovery Engineer. |
