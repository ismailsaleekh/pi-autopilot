...
import { parseCorpusCloneManifest, parseCorpusCloneRequest, parseCorpusRehearsalResult, S2_CORPUS_CLONE_MANIFEST_SCHEMA, S2_CORPUS_REHEARSAL_RESULT_SCHEMA, S2_D_DURABLE_RUN_ACTIONS, } from "./contracts.js";
import { buildWritableGitMirror, gitRefs, gitWorktreeFacts, hashGitWitness, verifyGitMirror } from "./git-mirror.js";
import { assertDisjointCanonicalRoots, assertNoSharedRegularFileIdentity, assertNoSymlinkSocketOrHardlinkRoute, compareCodeUnits, copyRegularFileNoFollow, copyTreeWithoutLinks, digestBytes, fileIdentity, hashRegularFile, inside, inventoryDigest, inventoryTree, readRegularFileNoFollow } from "./inventory.js";
import { assertMetadataCloneContained, rebaseCorpusPaths } from "./path-rebase.js";
import { isExactProcessAlive } from "../../src/core/coordination/process-identity.js";
...
function physicalDirectory(path, label) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink() || canonical !== resolve(path))
...
function physicalRegularFile(path, label) {
    const canonical = realpathSync(path);
    const stat = lstatSync(canonical);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || canonical !== resolve(path))
        throw new Error(`${label} must be a canonical single-link physical regular file`);
    return canonical;
}
function proof(value) {
    return Object.freeze({ passed: true, evidence_sha256: digestBytes(canonicalJson(value)) });
}
function sourcePathDigest(rehearsalId, path) {
    return digestBytes(`pi-autopilot/s2-d/source-path/v1\0${rehearsalId}\0${resolve(path)}`);
}
function cloneRelative(cloneRoot, path) {
    const rel = relative(cloneRoot, path).split(sep).join('/');
    if (rel === '' || rel === '..' || rel.startsWith('../') || rel.includes('\u0000'))
        throw new Error('S2-D clone-relative path escapes clone root');
    return rel;
}
async function databaseWitnesses(rehearsalId, corpusId, databasePath) {
    const roles = [
    ...
    for (const [role, path] of roles) {
        if (existsSync(path)) {
            ...
            if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1)
                throw new Error('S2-D database component is not a single-link physical file');
            ...
            rows.push(Object.freeze({ corpus_id: corpusId, role, present: false, path_sha256: sourcePathDigest(rehearsalId, path), identity: null, size_bytes: null, sha256: null }));
    }
    return Object.freeze(rows);
}
function quoteSqlIdentifier(value) {
    if (value.length === 0 || value.includes('\u0000'))
        throw new Error('S2-D SQLite identifier is invalid');
...
function logicalSqliteDigest(databasePath) {
    ...
    try {
        ...
        const tableRows = tables.map((table) => {
            const rowHashes = [];
            for (const row of database.prepare(`SELECT * FROM ${quoteSqlIdentifier(table)}`).iterate())
                rowHashes.push(digestBytes(canonicalJson(Object.keys(row).sort(compareCodeUnits).map((column) => ({ column, value: sqliteValue(row[column] ?? null) })))));
...
async function createCoherentDatabaseCopy(input) {
    if (existsSync(`${input.sourceDatabasePath}-journal`))
        throw new Error('S2-D SQLite source retained a rollback journal and has no coherent read-only boundary');
    ...
    if (existsSync(`${input.sourceDatabasePath}-wal`))
        await copyRegularFileNoFollow(`${input.sourceDatabasePath}-wal`, `${input.scratchDatabasePath}-wal`, 0o600);
    ...
    try {
        ...
        await backup(snapshot, input.copyDatabasePath);
    }
    finally {
        snapshot.close();
    }
    await chmod(input.copyDatabasePath, 0o600);
    if (existsSync(`${input.copyDatabasePath}-wal`) || existsSync(`${input.copyDatabasePath}-shm`) || existsSync(`${input.copyDatabasePath}-journal`))
        throw new Error('S2-D SQLite backup retained mutable sidecar authority in clone');
    ...
    if (sourceBefore !== sourceAfter || sourceBefore !== copyDigest)
        throw new Error('S2-D SQLite logical state is not coherent in mutable clone');
}
function sourceWitness(rehearsalId, corpusId, rootLabel, inventory) {
...
async function sourceGitWitness(corpusId, repositoryRoot) {
...
export async function preflightCloneRequest(request) {
    const parsed = parseCorpusCloneRequest(request);
    if (existsSync(parsed.destination_root))
        throw new Error('S2-D destination root must be absent');
    const parent = physicalDirectory(dirname(parsed.destination_root), 'S2-D destination parent');
    if (dirname(resolve(parsed.destination_root)) !== parent)
        throw new Error('S2-D destination root must have a canonical physical parent');
    const resultPath = resolve(parsed.result_path);
    if (!inside(parsed.destination_root, resultPath) || resultPath === parsed.destination_root)
        throw new Error('S2-D result path must be below destination root');
    const corpora = parsed.corpora.map((corpus) => {
        ...
        if (!inside(state, database) || !inside(state, capability))
            throw new Error('S2-D database and capability must be under the declared state root');
        ...
        if (!/^[a-f0-9]{64}$/u.test(capabilityText))
            throw new Error('S2-D source capability must use the current 64-hex format');
    ...
    for (const source of sourceRoots)
        if (inside(source, parsed.destination_root) || inside(parsed.destination_root, source))
            throw new Error('S2-D destination is not disjoint from source authority');
...
async function rotateCapability(sourceCapability, copyCapability) {
    ...
    if (fresh === source)
        throw new Error('S2-D capability rotation could not produce distinct authority');
    await writeFile(copyCapability, `${fresh}\n`, { encoding: 'utf8', mode: 0o600 });
    return digestBytes(Buffer.from(fresh, 'hex'));
}
async function buildCorpus(input) {
    const corpus = input.request.corpora[input.corpusIndex];
    if (corpus === undefined)
        throw new Error('S2-D corpus index disappeared');
    ...
    if (existsSync(copiedCoordinatorRoot)) {
        for (const entry of await readdir(copiedCoordinatorRoot))
            if (entry.endsWith('.sock') || entry.endsWith('.lock'))
                await rm(join(copiedCoordinatorRoot, entry), { force: true });
    }
...
function combineProofs(corpora, cloneRoot, request, sourceAfter) {
    for (const corpus of corpora) {
        ...
        for (const source of corpus.source_inventory)
            for (const copy of corpus.copy_inventory)
                assertNoSharedRegularFileIdentity(source, copy);
        for (const copy of corpus.copy_inventory) {
            assertNoSymlinkSocketOrHardlinkRoute(copy);
            if (copy.nodes.some((node) => /(?:^|\/)(?:[^/]+\.sock|[^/]+\.lock|LOCK)$/u.test(node.relative_path)))
                throw new Error('S2-D clone retained executable lock or socket authority');
        }
    }
    ...
    if (sourceBeforeDigest !== sourceAfterDigest)
        throw new Error('S2-D source changed during clone construction');
...
function sqlText(row, field, label) {
    const value = row[field];
    if (typeof value !== 'string' || value.length === 0 || value.includes('\u0000'))
        throw new Error(`${label}.${field} must be text`);
    return value;
}
function sqlInteger(row, field, label) {
    const value = row[field];
...
function optionalText(value) {
    if (value === null || value === undefined)
        return null;
    if (typeof value !== 'string')
        throw new Error('S2-D optional text field has invalid type');
    return value;
}
function activeFromDurableRow(input) {
...
async function discoverDurableRuns(input) {
    if (!existsSync(input.copyDatabasePath))
        throw new Error('S2-D cloned coordinator store is absent');
    const stat = lstatSync(input.copyDatabasePath);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1)
        throw new Error('S2-D cloned coordinator store is not single-link clone authority');
    ...
    try {
        ...
        for (const table of requiredTables)
            if (!tables.has(table))
                throw new Error(`S2-D cloned coordinator store missing authoritative table ${table}`);
        ...
        if (rows.length === 0)
            throw new Error('S2-D cloned coordinator store contains no durable runs to rehearse');
...
export async function buildMutableClone(input) {
    ...
    try {
        ...
        for (let index = 0; index < request.corpora.length; index += 1)
            corpora.push(await buildCorpus({ request, corpusIndex: index }));
        ...
        if (created)
            await rm(request.destination_root, { recursive: true, force: true });
        throw error;
    }
}
function assertObservedCoverage(manifest, actions) {
    const expected = new Set();
    for (const run of manifest.durable_runs)
        for (const action of run.required_actions)
            expected.add(`${run.corpus_id}\0${run.run_id_sha256}\0${action}`);
    const observed = new Set(actions.map((action) => `${action.corpus_id}\0${action.run_id_sha256}\0${action.action}`));
    if (expected.size === 0)
        throw new Error('S2-D rehearsal requires at least one discovered durable run');
    for (const key of expected)
        if (!observed.has(key))
            throw new Error(`S2-D candidate subprocess did not execute required action ${key}`);
    for (const key of observed)
        if (!expected.has(key))
            throw new Error(`S2-D candidate subprocess returned unknown action ${key}`);
}
export function rehearseManifest(manifest, observed, live) {
    const parsed = parseCorpusCloneManifest(manifest);
    if (observed === undefined || live === undefined)
        throw new Error('S2-D rehearsal requires observed candidate subprocess execution and live after-domain witnesses');
    if (!Object.values(parsed.isolation_proofs).every((entry) => entry.passed))
        throw new Error('S2-D release gate cannot rehearse without complete isolation proofs');
...
async function liveSourceWitnesses(clone) {
    const witnesses = [];
...
async function liveDatabaseWitnesses(clone) {
...
async function liveGitWitnesses(clone) {
...
function candidateWorkerPath() {
    return fileURLToPath(new URL('./candidate-worker.ts', import.meta.url));
}
function assertCloneCoordinatorReaped(stateRoot) {
    const lockPath = coordinatorRuntimePaths({ ...process.env, [AUTOPILOT_STATE_ROOT_ENV]: stateRoot }).lockPath;
    if (!existsSync(lockPath))
        return;
    let parsed;
    try {
        parsed = JSON.parse(Buffer.from(readRegularFileNoFollow(lockPath, 64 * 1024).bytes).toString('utf8'));
    }
    catch (error) {
        throw new Error(`S2-D clone coordinator leak assertion could not parse lifecycle lock: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))
        throw new Error('S2-D clone coordinator leak assertion found malformed lifecycle lock');
    const row = parsed;
    const pid = row['pid'];
    const start = row['process_start_identity'];
    if (typeof pid === 'number' && Number.isSafeInteger(pid) && typeof start === 'string' && isExactProcessAlive(pid, start))
        throw new Error(`S2-D candidate subprocess leaked clone coordinator pid=${String(pid)}`);
}
async function runCandidateSubprocess(input) {
    ...
    try {
        const output = await new Promise((resolveOutput, rejectOutput) => {
            ...
            child.once('close', (code, signal) => {
                ...
                if (code !== 0)
                    rejectOutput(new Error(`S2-D candidate subprocess failed code=${String(code)} signal=${String(signal)} stderr_sha256=${digestBytes(stderrText)}`));
                else
        ...
        assertCloneCoordinatorReaped(input.copy_state_root);
    }
}
export async function writeRehearsalResult(clone) {
    if (existsSync(clone.request.result_path))
        throw new Error('S2-D result path must remain absent until the gate is complete');
    const observedRows = [];
    for (const run of clone.corpora.flatMap((corpus) => corpus.durable_runs))
        observedRows.push(await runCandidateSubprocess(run));
