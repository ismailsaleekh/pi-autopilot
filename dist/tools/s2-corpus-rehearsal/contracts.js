...
function parseRunContract(value, label) {
    ...
    return Object.freeze({ corpus_id: identifier(row['corpus_id'], `${label}.corpus_id`), run_id_sha256: digest(row['run_id_sha256'], `${label}.run_id_sha256`), repo_id_sha256: digest(row['repo_id_sha256'], `${label}.repo_id_sha256`), required_actions: S2_D_DURABLE_RUN_ACTIONS, attachment_strategy: exactLiteral(row['attachment_strategy'], ['safe-attachment', 'owned-recovery'], `${label}.attachment_strategy`), terminal_attempt_lease: exactLiteral(row['terminal_attempt_lease'], ['no-retained-terminal-attempt-lease', 'retained-terminal-attempt-recovery-required', 'retained-terminal-attempt-reconciled'], `${label}.terminal_attempt_lease`), authority_version_mismatch: exactLiteral(row['authority_version_mismatch'], ['no-operation-authority-version-mismatch', 'operation-authority-version-mismatch-blocked', 'operation-authority-version-mismatch-recovered'], `${label}.authority_version_mismatch`), evidence_sha256: digest(row['evidence_sha256'], `${label}.evidence_sha256`) });
}
export function parseDurableRunContract(value, label = 'S2-D durable run contract') {
