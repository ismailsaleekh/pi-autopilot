...
import { compareCodeUnits, digestBytes, inside, readRegularFileNoFollow } from "./inventory.js";
...
function digestPrivatePath(rehearsalId, path) {
...
function normalizedMappings(mappings, cloneRoot) {
    if (mappings.length === 0)
        throw new Error('S2-D path rebase requires mappings');
    ...
    const normalized = mappings.map((mapping) => {
        ...
        if (sources.has(source))
            throw new Error('S2-D path rebase has duplicate source root');
        sources.add(source);
        if (!inside(cloneRoot, copy))
            throw new Error('S2-D mapped copy path escapes clone root');
...
function mappedPath(value, mappings) {
    if (!isAbsolute(value))
        return null;
    const path = resolve(value);
    for (const mapping of mappings) {
        if (!inside(mapping.source_path, path))
            continue;
...
function entry(input) {
    ...
    if (cloneRelative !== null && (cloneRelative === '' || cloneRelative === '..' || cloneRelative.startsWith('../') || isAbsolute(cloneRelative)))
        throw new Error('S2-D rebased path escapes clone root');
...
function transform(input) {
    if (typeof input.value === 'string')
        return transformString({ ...input, value: input.value });
    if (Array.isArray(input.value))
        return input.value.map((value, index) => transform({ ...input, value, pointer: `${input.pointer}/${String(index)}` }));
    ...
    for (const key of Object.keys(row).sort(compareCodeUnits)) {
        ...
        if (WRITABLE_REMOTE_FIELDS.has(key) && child !== null && child !== undefined) {
            if (typeof child !== 'string')
                throw new Error('S2-D writable remote field must be text before neutralization');
            const firstMapping = input.mappings[0];
            if (firstMapping === undefined)
                throw new Error('S2-D remote neutralization requires corpus authority');
...
function transformString(input) {
    ...
    if (mapped === null)
        return input.value;
    if (!inside(input.cloneRoot, mapped.path))
        throw new Error('S2-D transformed path escapes clone root');
...
async function metadataPaths(root) {
    const paths = [];
    const pending = [root];
    let visited = 0;
    while (pending.length > 0) {
        ...
        if (directory === undefined)
            throw new Error('S2-D metadata traversal underflow');
        const entries = (await readdir(directory, { withFileTypes: true })).sort((left, right) => compareCodeUnits(left.name, right.name));
        for (const dirent of entries) {
            visited += 1;
            if (visited > 1_000_000)
                throw new Error('S2-D metadata traversal exceeded bounded node limit');
            const path = join(directory, dirent.name);
            if (dirent.isDirectory())
                pending.push(path);
            else if (dirent.isFile() && (SCANNED_FILE_NAMES.has(dirent.name) || dirent.name.endsWith('.metadata.json') || dirent.name.endsWith('.jsonl')))
                paths.push(path);
        }
    }
    return Object.freeze(paths.sort(compareCodeUnits));
}
async function rebaseFile(input) {
    const stat = lstatSync(input.path);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || stat.size > 64 * 1024 * 1024)
        throw new Error('S2-D metadata file is not bounded single-link authority');
    ...
    const values = jsonl ? text.split('\n').filter((line) => line.length > 0).map((line) => JSON.parse(line)) : [JSON.parse(text)];
    ...
    if (canonicalJson(transformed) === before)
        return;
    ...
    try {
        ...
        if (current.dev !== Number(raw.identity.device) || current.ino !== Number(raw.identity.inode) || current.size !== raw.size_bytes || current.nlink !== 1)
            throw new Error('S2-D metadata changed during rebase');
...
function rebaseSqliteDatabase(input) {
    const stat = lstatSync(input.path);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || stat.size > 256 * 1024 * 1024)
        throw new Error('S2-D SQLite metadata database is not bounded single-link authority');
            ...
            for (const table of tables) {
                ...
                for (const column of columns) {
                    ...
                    for (const row of rows) {
                        ...
                        if (input.entries.length === beforeCount)
                            continue;
                        const keyValues = primaryKeys.length === 0 ? [sqlInput(row['s2_pk_0'])] : primaryKeys.map((_, index) => sqlInput(row[`s2_pk_${String(index)}`]));
...
export async function rebaseCorpusPaths(input) {
    ...
    if (!inside(cloneRoot, stateRoot) || !inside(cloneRoot, input.ledger_path))
        throw new Error('S2-D path rebase target escapes clone root');
    if (existsSync(input.ledger_path))
        throw new Error('S2-D path rebase ledger already exists');
    const mappings = normalizedMappings(input.mappings, cloneRoot);
    const entries = [];
    for (const path of await metadataPaths(stateRoot))
        await rebaseFile({ path, cloneRoot, rehearsalId: input.rehearsal_id, mappings, entries });
    for (const path of input.database_paths ?? []) {
        const databasePath = resolve(path);
        if (!inside(cloneRoot, databasePath))
            throw new Error('S2-D SQLite rebase database escapes clone root');
        if (existsSync(databasePath))
            rebaseSqliteDatabase({ path: databasePath, cloneRoot, rehearsalId: input.rehearsal_id, mappings, entries });
    }
    if (entries.length === 0)
        throw new Error('S2-D path rebase produced no audited transformations');
...
export function assertNoLiveActionablePaths(value, cloneRoot) {
    const visit = (candidate, pointer) => {
        if (typeof candidate === 'string') {
            if (isAbsolute(candidate) && !inside(cloneRoot, candidate))
                throw new Error(`S2-D live absolute path remains at ${pointer}`);
            return;
        }
        if (Array.isArray(candidate)) {
            candidate.forEach((entryValue, index) => visit(entryValue, `${pointer}/${String(index)}`));
            return;
        }
        ...
        for (const key of Object.keys(row).sort(compareCodeUnits)) {
            if (WRITABLE_REMOTE_FIELDS.has(key) && row[key] !== null && row[key] !== undefined)
                throw new Error(`S2-D writable remote route remains at ${pointer}/${pointerSegment(key)}`);
            visit(row[key], `${pointer}/${pointerSegment(key)}`);
        }
    };
    visit(value, '');
}
export async function assertMetadataCloneContained(stateRoot, cloneRoot, databasePaths = []) {
    for (const path of await metadataPaths(stateRoot)) {
        const text = await readFile(path, 'utf8');
        const values = path.endsWith('.jsonl') ? text.split('\n').filter((line) => line.length > 0).map((line) => JSON.parse(line)) : [JSON.parse(text)];
        for (const value of values)
            assertNoLiveActionablePaths(value, cloneRoot);
    }
    for (const path of databasePaths) {
        const databasePath = resolve(path);
        if (!inside(cloneRoot, databasePath) || !existsSync(databasePath))
            continue;
                ...
                for (const column of columns) {
                    ...
                    for (const row of rows) {
                        ...
                        if (parsed === null)
                            ...
                            assertNoLiveActionablePaths(parsed, cloneRoot);
                    }
                }
            }
        }
