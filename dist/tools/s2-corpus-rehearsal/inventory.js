...
export function readRegularFileNoFollow(path, maximumBytes) {
    const descriptor = openSync(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
    try {
        const before = fstatSync(descriptor);
        if (!before.isFile() || before.size < 0 || before.size > maximumBytes)
            throw new Error('S2-D input is not a bounded physical regular file');
        const bytes = readFileSync(descriptor);
        const after = fstatSync(descriptor);
        if (!sameStableFile(before, after) || bytes.byteLength !== before.size)
            throw new Error('S2-D input changed during bounded read');
...
export async function hashRegularFile(path) {
    ...
    try {
        const before = fstatSync(descriptor);
        if (!before.isFile())
            throw new Error('S2-D hash input is not a regular file');
        ...
        if (!sameStableFile(before, after))
            throw new Error('S2-D hash input changed during read');
...
export async function copyRegularFileNoFollow(source, destination, mode) {
    const descriptor = openSync(source, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
    let destinationCreated = false;
    try {
        const before = fstatSync(descriptor);
        if (!before.isFile())
            throw new Error('S2-D copy input is not a regular file');
            ...
            for await (const chunk of stream) {
                ...
                while (offset < chunk.byteLength) {
                    ...
                    if (written.bytesWritten < 1)
                        throw new Error('S2-D copy made no forward progress');
                    copied += written.bytesWritten;
                    offset += written.bytesWritten;
                }
            }
            if (copied !== before.size)
                throw new Error('S2-D copy byte count changed during read');
            await output.sync();
        }
        finally {
            await output.close();
        ...
        const after = fstatSync(descriptor);
        if (!sameStableFile(before, after))
            throw new Error('S2-D copy input changed during read');
        const copiedStat = lstatSync(destination);
        if (!copiedStat.isFile() || copiedStat.isSymbolicLink() || copiedStat.nlink !== 1 || (copiedStat.dev === before.dev && copiedStat.ino === before.ino))
            throw new Error('S2-D copy output does not have independent regular-file identity');
    }
    catch (error) {
...
function normalizedRelative(root, path) {
    ...
    if (rel === '' || isAbsolute(rel) || rel === '..' || rel.startsWith(`..${sep}`) || rel.split(sep).includes('..'))
        throw new Error('S2-D inventory path escaped root');
    return rel.split(sep).join('/');
}
async function nodeFor(root, path) {
    const stat = lstatSync(path);
    const kind = stat.isFile() ? 'regular' : stat.isDirectory() ? 'directory' : stat.isSymbolicLink() ? 'symlink' : stat.isSocket() ? 'socket' : (() => { throw new Error('S2-D inventory found unsupported filesystem node'); })();
...
export async function inventoryTree(rootPath) {
    ...
    if (!rootStat.isDirectory() || rootStat.isSymbolicLink())
        throw new Error('S2-D inventory root must be a physical directory');
    const pending = [canonicalRoot];
    const nodes = [];
    while (pending.length > 0) {
        const directory = pending.pop();
        if (directory === undefined)
            throw new Error('S2-D inventory traversal underflow');
        ...
        for (const entry of entries) {
            if (entry.name === '.' || entry.name === '..' || entry.name.includes('\u0000'))
                throw new Error('S2-D inventory found invalid entry name');
            const path = join(directory, entry.name);
            const node = await nodeFor(canonicalRoot, path);
            nodes.push(node);
            if (nodes.length > S2_CORPUS_MAX_INVENTORY_NODES)
                throw new Error('S2-D inventory exceeded bounded node limit');
...
export function assertDisjointCanonicalRoots(leftPath, rightPath) {
    const left = realpathSync(leftPath);
    const right = realpathSync(rightPath);
    if (inside(left, right) || inside(right, left))
        throw new Error('S2-D source and clone roots are not disjoint');
}
export function assertNoSharedRegularFileIdentity(source, copy) {
    ...
    for (const node of copy.nodes) {
        if (node.kind !== 'regular')
            continue;
        if (node.identity.link_count !== 1)
            throw new Error(`S2-D clone regular file is hardlinked: ${node.relative_path}`);
        if (sourceIds.has(`${node.identity.device}\0${node.identity.inode}`))
            throw new Error(`S2-D clone shares a regular-file identity with live source: ${node.relative_path}`);
    }
}
export function assertNoSymlinkSocketOrHardlinkRoute(inventory) {
    for (const node of inventory.nodes) {
        if (node.kind === 'symlink' || node.kind === 'socket')
            throw new Error(`S2-D clone contains a live-routable ${node.kind}: ${node.relative_path}`);
        if (node.kind === 'regular' && node.identity.link_count !== 1)
            throw new Error(`S2-D clone contains a hardlinked file: ${node.relative_path}`);
    }
}
export async function copyTreeWithoutLinks(sourceRoot, destinationRoot) {
    const source = realpathSync(sourceRoot);
    const destination = resolve(destinationRoot);
    if (inside(source, destination) || inside(destination, source))
        throw new Error('S2-D copy roots must be disjoint');
    const sourceStat = lstatSync(source);
    if (!sourceStat.isDirectory() || sourceStat.isSymbolicLink())
        throw new Error('S2-D copy source must be a physical directory');
    await mkdir(destination, { mode: sourceStat.mode & 0o777 });
    const pending = [source];
    while (pending.length > 0) {
        const directory = pending.pop();
        if (directory === undefined)
            throw new Error('S2-D copy traversal underflow');
        const entries = (await readdir(directory, { withFileTypes: true })).sort((left, right) => compareCodeUnits(left.name, right.name));
        for (const entry of entries) {
            if (entry.name === '.' || entry.name === '..' || entry.name.includes('\u0000'))
                throw new Error('S2-D copy found invalid entry name');
            const sourcePath = join(directory, entry.name);
            const sourceInfo = lstatSync(sourcePath);
            if (sourceInfo.isSymbolicLink() || sourceInfo.isSocket() || (sourceInfo.isFile() && sourceInfo.nlink !== 1))
                throw new Error('S2-D copy refuses live-routable symlink/socket/hardlink authority');
            const relativePath = relative(source, sourcePath);
            const destinationPath = resolve(destination, relativePath);
            if (!inside(destination, destinationPath))
                throw new Error('S2-D copy destination escaped root');
            if (sourceInfo.isDirectory()) {
                ...
                throw new Error('S2-D copy found unsupported filesystem node');
        }
    }
