...
import { buildAutopilotWorkstreamRun, isValidWorkstreamRun, isValidWorkstreamSlug } from "./paths.js";
...
async function assertDiskGateOrRecordS2Pressure(input) {
    try {
        ...
        if (error instanceof AutopilotDiskGateError)
            await recordS2WorktreeDiskPressure({ worktreeRoot: input.worktreeRoot, workstreamRun: input.workstreamRun, error, now: input.now });
        throw error;
    }
}
...
async function verifyPhase37RosterPrepareSelection(input) {
    ...
    if (input.selection === undefined) {
        if (input.required)
            fail('phase37-roster-launch-fence-required', 'Phase 37 activation requires a readback-authenticated roster selection before runtime mutation.');
        return null;
    }
    ...
    if (input.selection.mode === 'new-run') {
        ...
        if (fence.schema_version !== 'autopilot.run_selection_launch_fence.v1' ||
...
function phase37SelectionsEqual(left, right) {
            ...
            if (existsSync(bootstrapResidue)) {
                ...
                if (missing.length > 0) {
                    if (cutover && input.coordinationSessionId !== undefined) {
                        ...
                        if (recovered !== null) {
                            if (requestedWorkstreamRun !== null && recovered.active.workstream_run !== requestedWorkstreamRun)
                                fail('phase37-selection-workstream-run-mismatch', 'Recovered Phase 37 bootstrap run does not match roster selection.', [requestedWorkstreamRun, recovered.active.workstream_run]);
                            return recovered;
                        }
                    }
    ...
    return await withAutopilotFileLock(lockPath, `unit-worktree:${input.active.autopilot_id}:${input.unitId}:${String(input.attempt)}`, async () => {
        ...
        if (found !== undefined) {
            ...
            if (pending.length === 0)
                return { unitInfo: info, created: false, resumed: true };
            if (pending.length !== 1)
                throw new CoordinationRuntimeError('store-corrupt', 'unit index owner has multiple incomplete durable create operations', pending.map((operation) => operation.operation_id));
        ...
        if (pendingCreates.length > 1)
            throw new CoordinationRuntimeError('store-corrupt', 'unit worktree preparation found multiple durable create operations for one owner', pendingCreates.map((operation) => operation.operation_id));
        ...
        if (pendingCreate !== undefined) {
            ...
            if (!('worktree_path' in intent) || !('branch' in intent) || !('base_sha' in intent) || !('reason' in intent))
                throw new CoordinationRuntimeError('store-corrupt', 'unit worktree create operation has a metadata-reconcile intent', [pendingCreate.operation_id]);
            if (intent.base_sha === null)
                throw new CoordinationRuntimeError('store-corrupt', 'unit worktree create operation lost its immutable base SHA');
            if (intent.worktree_path !== unitWorktreePath || intent.branch !== branch || intent.git_common_dir !== input.active.git_common_dir || intent.reason !== 'prepare source-changing unit worktree')
                throw new CoordinationRuntimeError('idempotency-conflict', 'durable unit worktree create operation differs from the requested deterministic owner identity', [pendingCreate.operation_id]);
            baseSha = intent.base_sha;
        }
        const unitInfo = resumableUnitInfo ?? {
        ...
        const createPhysicalWorktree = async () => {
        ...
        const finalizeUnitWorktree = async () => {
...
export async function updateUnitBranchStatus(input) {
    const env = input.env ?? process.env;
    if (input.recovery === 'unit-recovery')
        await assertD65RecoveryBoundaryFromEnvironment('unit-recovery', { attached_session_current: true, policy_trust_current: true, no_pending_publication: true, terminal_prepared_cancellable: false, terminal_after_commit: false, accepted_continuation_reason: null, covered_semantic_reason: null, attach_terminal_recovery: false }, env);
    else
...
async function createNewWorkstream(input) {
    ...
    if (!isValidWorkstreamRun(workstreamRun))
        fail('invalid-workstream-run', `Invalid Autopilot workstream_run: ${workstreamRun}`);
...
async function upsertUnitBranchInfo(taskRoot, branchInfo) {
    ...
    if (!existsSync(path))
        fail('unit-branch-authority-missing', 'unit branch projection requires the durable run-main branch authority.', [path]);
