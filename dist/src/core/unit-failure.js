...
import { COORDINATOR_IMPLEMENTATION_BUILD } from "./coordination/runtime-constants.js";
...
export function latestCommittedQuarantineOperationForWorktree(worktree, operations) {
    ...
    if (worktree.worktree_id !== canonicalWorktreeId)
        throw new CoordinationRuntimeError('invalid-state', 'retained failed-unit reconciliation requires the current canonical worktree projection', [worktree.worktree_id, canonicalWorktreeId]);
...
async function resumeRemovedResetFailure(input) {
    if (input.context.active.coordination_authority !== 'coordinator-edit-leases-v1')
        throw new CoordinationRuntimeError('recovery-required', 'missing failed-unit worktree has no coordinator recovery authority', [input.unitWorktreePath]);
    const evidencePath = await failureEvidencePublicationPath(input, 'reset');
    if (!existsSync(evidencePath))
        throw new CoordinationRuntimeError('recovery-required', 'missing failed-unit worktree has no immutable reset evidence', [input.unitWorktreePath, evidencePath]);
    ...
    if (facts.action !== 'reset' || resolve(facts.unitWorktreePath) !== resolve(input.unitWorktreePath) || realpathSync(facts.gitCommonDir) !== realpathSync(input.context.active.git_common_dir) || facts.branch !== `autopilot/unit/${input.context.active.workstream_run}/${input.unitId}/attempt-${String(input.attempt)}` || facts.gitHeadBefore !== facts.gitHeadAfter)
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset evidence differs from its exact clean owner identity', [evidencePath]);
    let value;
    try {
        value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(evidenceBytes));
    }
    catch (error) {
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset evidence is unreadable', [evidencePath, error instanceof Error ? error.message : String(error)]);
    }
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset evidence is not an object', [evidencePath]);
    const field = (name) => Reflect.get(value, name);
    const dirtyPaths = field('dirty_paths');
    const summary = field('summary');
    const createdAt = field('created_at');
    if (!Array.isArray(dirtyPaths) || dirtyPaths.some((entry) => typeof entry !== 'string') || dirtyPaths.length !== 0 || typeof summary !== 'string' || summary.length === 0 || typeof createdAt !== 'string' || !Number.isFinite(Date.parse(createdAt)))
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset evidence has invalid bounded publication fields', [evidencePath]);
    const contextPath = (input.env ?? process.env)[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        throw new CoordinationRuntimeError('unauthorized-client', 'removed unit reset recovery requires its durable coordinator session');
    const session = await readCoordinatorSessionContext(contextPath);
    if (session.repo_id !== input.context.active.repo_key || session.autopilot_id !== input.context.active.autopilot_id || session.workstream_run !== input.context.active.workstream_run)
        throw new CoordinationRuntimeError('unauthorized-client', 'removed unit reset recovery session differs from its exact owner');
    ...
    const array = (candidate, label) => {
        if (!Array.isArray(candidate))
            throw new CoordinationRuntimeError('invalid-state', `removed unit reset recovery status omitted ${label}`);
        return candidate;
    };
    ...
    if (resetOperations.length !== 1 || removeOperations.length !== 1)
        throw new CoordinationRuntimeError('recovery-required', 'missing failed-unit worktree lacks one committed reset and one durable remove operation', [...resetOperations.map((operation) => operation.operation_id), ...removeOperations.map((operation) => operation.operation_id)]);
    const resetOperation = resetOperations[0];
    const removeOperation = removeOperations[0];
    if (resetOperation === undefined || removeOperation === undefined || !('worktree_path' in resetOperation.intent) || !('worktree_path' in removeOperation.intent) || resolve(resetOperation.intent.worktree_path) !== resolve(input.unitWorktreePath) || resolve(removeOperation.intent.worktree_path) !== resolve(input.unitWorktreePath) || resetOperation.intent.branch !== facts.branch || removeOperation.intent.branch !== facts.branch || resetOperation.intent.base_sha !== facts.gitHeadAfter || resetOperation.intent.target_sha !== facts.gitHeadAfter || resetOperation.intent.paths.length !== 0 || removeOperation.intent.target_sha !== facts.gitHeadAfter || removeOperation.intent_event_seq <= resetOperation.intent_event_seq)
        throw new CoordinationRuntimeError('invalid-state', 'durable reset/remove operations differ from immutable removed-unit evidence', [evidencePath]);
    ...
    if (attempts.length !== 1 || attempts[0]?.state !== 'reset')
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset recovery lacks the exact released attempt state', attempts.map((attempt) => attempt.state));
    ...
    if (worktrees.length !== 1 || (worktrees[0]?.state !== 'terminal' && worktrees[0]?.state !== 'removed') || resolve(worktrees[0].canonical_path) !== resolve(input.unitWorktreePath))
        throw new CoordinationRuntimeError('invalid-state', 'removed unit reset recovery lacks the exact terminal canonical worktree projection');
...
export async function resetFailedUnit(input) {
    if (!existsSync(input.unitWorktreePath))
        return await resumeRemovedResetFailure(input);
...
async function commitD65FailureEvidence(input, publication) {
    const env = input.env ?? process.env;
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        return;
    ...
    if (!Array.isArray(artifactsValue) || !Array.isArray(resourcesValue) || resourcesValue.length !== 1 || resourcesValue[0] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 unit failure evidence commit lacks one exact artifact/resource projection');
    ...
    if (!bootstrap) {
        if (graphs.length > 0)
            throw new CoordinationRuntimeError('invalid-state', 'complete graph exists without D65 bootstrap authority');
        return;
    }
    ...
    if (evidenceRef.length === 0 || evidenceRef === '..' || evidenceRef.startsWith('../') || isAbsolute(evidenceRef))
        throw new CoordinationRuntimeError('unauthorized-client', 'unit failure evidence path escapes run-main authority', [publication.evidencePath]);
    ...
    if (head === graph.git_commit) {
        ...
        if (!acceptedBytes.negative) {
            ...
            if (acceptedDigest !== expectedDigest)
                throw new CoordinationRuntimeError('idempotency-conflict', 'accepted graph contains different unit failure evidence bytes', [evidenceRef, expectedDigest, acceptedDigest]);
            return;
        }
        if (runGitQuery({ descriptor: { kind: 'staged-clean' }, cwd: resource.main_worktree_path }).negative)
            throw new CoordinationRuntimeError('invalid-state', 'unit failure evidence refuses to commit over a nonempty shared index', [evidenceRef]);
        const gitEnv = { ...env, GIT_AUTHOR_NAME: 'autopilot-runtime', GIT_AUTHOR_EMAIL: 'autopilot-runtime@example.invalid', GIT_COMMITTER_NAME: 'autopilot-runtime', GIT_COMMITTER_EMAIL: 'autopilot-runtime@example.invalid', GIT_AUTHOR_DATE: publication.record.created_at, GIT_COMMITTER_DATE: publication.record.created_at };
        ...
        if (staged.kind !== 'reported' || staged.exitCode !== 0)
            throw new CoordinationRuntimeError('invalid-state', 'unit failure evidence staging failed', [staged.kind === 'reported' ? staged.diagnostic : staged.reason]);
        ...
        if (committed.kind !== 'reported' || committed.exitCode !== 0)
            throw new CoordinationRuntimeError('invalid-state', 'unit failure evidence commit failed', [committed.kind === 'reported' ? committed.diagnostic : committed.reason]);
    ...
    if (parents.length !== 2 || parents[1] !== graph.git_commit || paths.length !== 1 || paths[0] !== evidenceRef || committedDigest !== expectedDigest)
        throw new CoordinationRuntimeError('idempotency-conflict', 'unit failure evidence commit differs from its exact one-parent one-ref stage', [head, graph.git_commit, evidenceRef, expectedDigest, committedDigest]);
}
async function recordFailureEvidence(input, publication, source) {
    if (input.context.active.coordination_authority !== 'coordinator-edit-leases-v1')
        return;
...
async function failureEvidencePublicationPath(input, action) {
    ...
    if (classifyHistoricalUnitFailureEvidenceGeneration(bytes) === null)
        return path;
    const candidate = parseHistoricalUnitFailureRegenerationCandidate(bytes, failureEvidenceIdentity(input, action));
    if (candidate.action !== action)
        throw new CoordinationRuntimeError('invalid-state', 'historical unit failure evidence action differs from current regeneration action', [path, candidate.action, action]);
...
function currentFailureRecordBytes(record) {
    return `${JSON.stringify(record, null, 2)}\n`;
}
async function persistCurrentFailureRecord(path, record, expected) {
    ...
    const acceptExisting = async () => {
        ...
        let value;
        try {
            value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(existingBytes));
        }
        catch (error) {
            throw new CoordinationRuntimeError('invalid-state', 'existing current unit failure evidence is unreadable', [path, error instanceof Error ? error.message : String(error)]);
        }
        if (typeof value !== 'object' || value === null || Array.isArray(value))
            throw new CoordinationRuntimeError('invalid-state', 'existing current unit failure evidence is not an object', [path]);
        const existing = value;
        if (typeof existing.created_at !== 'string' || !Number.isFinite(Date.parse(existing.created_at)))
            throw new CoordinationRuntimeError('invalid-state', 'existing current unit failure evidence created_at is invalid', [path]);
        const expectedBytes = currentFailureRecordBytes({ ...record, created_at: existing.created_at });
        if (new TextDecoder().decode(existingBytes) !== expectedBytes)
            throw new CoordinationRuntimeError('idempotency-conflict', 'existing current unit failure evidence differs from re-derived worktree facts', [path]);
        return existing;
    };
    if (existsSync(path))
