...
function providerHealth(value, label) {
    ...
    if (observationRef === null || observationSha === null)
        fail(label, 'provider health must cite one exact launch, continuation, probe, or consumption observation');
    ...
    if (state === 'healthy' && (cooldownUntil !== null || !((probeTripleAbsent && consumption === null) || (probeTriplePresent && consumption !== null))))
        fail(label, 'healthy provider must be initial-null or exact post-consume probe authority');
    if (state === 'blocked' && (cooldownUntil === null || !probeTripleAbsent || consumption !== null))
        fail(label, 'blocked provider must carry cooldown and no probe/consumption tuple');
    if (state === 'retry-authorized' && (cooldownUntil === null || !probeTriplePresent || consumption !== null))
        fail(label, 'retry-authorized provider must carry cooldown plus one unconsumed probe triple');
    if (state === 'exhausted' && (cooldownUntil !== null || !probeTripleAbsent || consumption !== null))
        fail(label, 'exhausted provider must have null cooldown/probe/consumption tuple');
    return {
        provider: identifier(record, 'provider', label),
        state,
