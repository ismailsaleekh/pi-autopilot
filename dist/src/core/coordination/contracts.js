 ...
 */
export function coordinatorSessionIdIssue(value) {
    if (typeof value !== 'string')
        return 'must be a coordinator session identity string';
    if (value.length < 1 || value.length > 192)
        return 'must contain 1..192 characters';
    if (!IDENTIFIER.test(value))
        return 'must match the bounded coordinator session-identity grammar';
    return null;
}
...
export function parseCoordinationExclusiveOperation(value, label = 'CoordinationExclusiveOperation') {
    ...
    if (criticalSection !== operationKind)
        fail(label, 'critical_section must equal the closed operation_kind');
    const expectedDuration = integer(record, 'expected_duration_ms', label, 1);
    if (expectedDuration > COORDINATION_EXCLUSIVE_MAX_EXPECTED_DURATION_MS)
        fail(label, `expected_duration_ms must be <= ${String(COORDINATION_EXCLUSIVE_MAX_EXPECTED_DURATION_MS)}`);
    return {
...
export function parseCoordinationRequestedLease(value, label = 'CoordinationRequestedLease') {
    ...
    if (mode === 'EXCLUSIVE' && exclusiveOperation === undefined)
        fail(label, 'EXCLUSIVE authority requires a closed critical-operation contract');
    if (mode !== 'EXCLUSIVE' && exclusiveOperation !== undefined)
        fail(label, 'only EXCLUSIVE authority may carry a critical-operation contract');
    const path = repoPath(record, 'path', label);
    if (mode === 'EXCLUSIVE' && exclusiveOperation?.operation_kind !== 'legacy-migration-exclusive' && (path.endsWith('/**') || path.endsWith('/')))
        fail(label, 'new EXCLUSIVE authority requires one exact repository path');
    return {
        path,
        mode,
...
function assertRequestedLeaseSet(leases, label) {
    ...
    for (let leftIndex = 0; leftIndex < leases.length; leftIndex += 1) {
        ...
        for (let rightIndex = leftIndex + 1; rightIndex < leases.length; rightIndex += 1) {
            ...
            if (right !== undefined && coordinationPathsOverlap(left.path, right.path) && claimModesConflict(left.mode, right.mode) && !isLayeredWriteExclusivePair(left, right))
                fail(label, `requested_leases contain internally incompatible authority: ${left.mode} ${left.path} and ${right.mode} ${right.path}`);
        }
    }
}
function isLayeredWriteExclusivePair(left, right) {
    return left.path === right.path && ((left.mode === 'WRITE' && right.mode === 'EXCLUSIVE') || (left.mode === 'EXCLUSIVE' && right.mode === 'WRITE'));
}
function assertExclusiveGroupPolicy(group, label) {
    const exclusives = group.requested_leases.filter((lease) => lease.mode === 'EXCLUSIVE');
    if (group.acquisition_kind !== 'legacy-unknown' && exclusives.length > 1)
        fail(label, 'one new acquisition group may contain at most one EXCLUSIVE critical operation');
    if (group.acquisition_kind === 'materialization-read-expansion' && exclusives.length > 0)
        fail(label, 'materialization expansion cannot acquire EXCLUSIVE authority');
    for (const exclusive of exclusives) {
        const operation = exclusive.exclusive_operation;
        if (operation === undefined)
            fail(label, 'EXCLUSIVE authority lacks its critical-operation contract');
        if (group.acquisition_kind === 'legacy-unknown') {
            if (operation.operation_kind !== 'legacy-migration-exclusive')
                fail(label, 'legacy acquisition may retain only explicitly classified migration EXCLUSIVE authority');
            continue;
        }
        if (operation.operation_kind === 'legacy-migration-exclusive')
            fail(label, 'new acquisition cannot create legacy EXCLUSIVE authority');
        if (!group.requested_leases.some((lease) => lease.mode === 'WRITE' && lease.path === exclusive.path))
            fail(label, 'EXCLUSIVE authority must layer over an exact WRITE intention');
        if (group.normal_release_condition.condition_type !== 'unit-merged')
            fail(label, 'source EXCLUSIVE authority requires exact unit-merge/reset/quarantine lifecycle ownership');
    }
}
...
export function parseCoordinationAcquisitionGroup(value) {
    ...
    const acquisitionKind = oneOf(record, 'acquisition_kind', ['initial', 'materialization-read-expansion', 'legacy-unknown'], label);
...
export function parseCoordinationEditLease(value) {
    ...
    const mode = oneOf(record, 'mode', ['WRITE', 'EXCLUSIVE'], label);
    const exclusiveOperation = record['exclusive_operation'] === undefined ? undefined : parseCoordinationExclusiveOperation(record['exclusive_operation'], `${label}.exclusive_operation`);
    if (mode === 'EXCLUSIVE' && exclusiveOperation === undefined)
        fail(label, 'EXCLUSIVE lease requires its closed critical-operation contract');
    if (mode !== 'EXCLUSIVE' && exclusiveOperation !== undefined)
        fail(label, 'WRITE lease cannot carry an EXCLUSIVE critical-operation contract');
    const path = repoPath(record, 'path', label);
    if (mode === 'EXCLUSIVE' && exclusiveOperation?.operation_kind !== 'legacy-migration-exclusive' && (path.endsWith('/**') || path.endsWith('/')))
        fail(label, 'new EXCLUSIVE lease requires one exact repository path');
    return {
...
function parsePayload(value, action) {
    ...
    if (action === 'prepare-run-terminal') {
        const d65Fields = ['intent_attempt', 'prior_terminal_intent_id', 'prior_terminal_intent_sha256', 'terminal_effect_sets'];
        const present = d65Fields.filter((field) => payload[field] !== undefined);
        if (present.length !== 0 && present.length !== d65Fields.length)
            fail(label, 'D65 prepare-run-terminal requires all of intent_attempt, prior_terminal_intent_id, prior_terminal_intent_sha256, terminal_effect_sets together or none');
        if (present.length === d65Fields.length) {
            ...
            if (typeof intentAttempt !== 'number' || !Number.isSafeInteger(intentAttempt) || intentAttempt < 1)
                fail(label, 'intent_attempt must be a positive safe integer');
            const priorId = payload['prior_terminal_intent_id'];
            if (priorId !== null && (typeof priorId !== 'string' || priorId.length === 0 || priorId.length > 192))
                fail(label, 'prior_terminal_intent_id must be null or a bounded identifier');
            const priorSha = payload['prior_terminal_intent_sha256'];
            if (priorSha !== null && (typeof priorSha !== 'string' || !SHA256.test(priorSha)))
                fail(label, 'prior_terminal_intent_sha256 must be null or sha256:<64 lowercase hex>');
