...
const QUERY_ACTIONS = ['status', 'doctor'];
const MUTATION_ACTIONS = ['attach-run', 'attach-session', 'detach-session', 'heartbeat', 'acquire-group', 'acknowledge-grant', 'respond-claim-request', 'cancel-claim-request', 'acknowledge-message', 'transition-operation'];
...
export function parseCoordinationSnapshot(value) {
    ...
    const fields = ['acquisition_groups', 'change_reservations', 'child_leases', 'claim_requests', 'edit_leases', 'escalations', 'events', 'messages', 'repositories', 'repository_event_seq', 'runs', 'schema_version', 'session_leases', 'unit_attempts', 'worktree_operations', 'worktrees'];
    ...
    for (const field of PAYLOAD_FIELDS[action]) {
        ...
        if (field === 'pid') {
            if (typeof entry !== 'number' || !Number.isSafeInteger(entry) || entry < 1)
                fail('CoordinatorRequestEnvelope.payload', 'pid must be a positive safe integer');
        }
        else if (field === 'lease_expires_at') {
            timestamp(payload, field, 'CoordinatorRequestEnvelope.payload');
        }
        else if (field === 'response') {
            oneOf(payload, field, ['release-now', 'deferred'], 'CoordinatorRequestEnvelope.payload');
        }
        else if (field === 'stage') {
