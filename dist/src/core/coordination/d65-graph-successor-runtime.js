...
import { parseCoordinationAuthoritativeArtifact, parseCoordinationRun, parseCoordinationRunResource } from "./contracts.js";
import { parseD65ContinuationEvent, parseD65ParentLoss } from "./d65-continuation.js";
import { parseD65SubscriptionProbe } from "./d65-launch-policy.js";
import { readImmutableFileBytes } from "./immutable-file.js";
import { assertD65RecoveryBoundaryFromEnvironment, ensureD65ProgramHeartbeatForGraphFromEnvironment } from "./d65-runtime-dispatch.js";
import { canonicalSha256 } from "./d65-semantic-graph.js";
import { prepareD65CoordinatorOnlySuccessor, prepareD65FirstCompleteGraphPublication } from "./d65-graph-successor.js";
import { createD65GraphGitOps, createD65GraphPublicationStoreGateway } from "./d65-graph-runtime.js";
import { publishD65CompleteGraph } from "./d65-graph-publisher.js";
...
function gitText(cwd, descriptor, label) {
    const result = runGitQuery({ cwd, descriptor });
    const value = new TextDecoder('utf-8', { fatal: true }).decode(result.stdout).trim();
    if (!/^[a-f0-9]{40}$/u.test(value))
        throw new CoordinationRuntimeError('invalid-state', `${label} did not resolve to one full 40-hex Git object id`, [value]);
    return value;
}
async function cleanupIsolatedIndex(path) {
    if (!existsSync(path))
        return;
    const metadata = lstatSync(path);
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 graph isolated index cleanup refuses a non-regular or aliased path', [path]);
    await unlink(path);
    if (existsSync(path))
        throw new CoordinationRuntimeError('invalid-state', 'D65 graph isolated index remained after cleanup', [path]);
}
const D65_FIRST_GRAPH_CORE_FILENAMES = Object.freeze(['mission.md', 'master-plan.json', 'state.json', 'decision-log.jsonl', 'events.jsonl']);
function parseD65RuntimeJsonObjectBytes(bytes, label) {
    let document;
    try {
        document = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes));
    }
    catch (error) {
        throw new CoordinationRuntimeError('invalid-request', `${label} is not strict UTF-8 JSON`, [error instanceof Error ? error.message : String(error)]);
    }
    if (document === null || typeof document !== 'object' || Array.isArray(document))
        throw new CoordinationRuntimeError('invalid-request', `${label} must be one JSON object`);
    return document;
}
function parseD65RuntimeUnitSpecBytes(bytes, label) {
...
function parseD65RuntimeReceiptBytes(bytes, label) {
    const document = parseD65RuntimeJsonObjectBytes(bytes, label);
    try {
...
function standardBase64(pathText) {
 ...
 */
export async function publishD65FirstCompleteGraphFromEnvironment(input = {}) {
    const env = input.env ?? process.env;
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 first graph publication requires a durable coordinator session context');
    ...
    try {
        ...
        const manifestRows = D65_FIRST_GRAPH_CORE_FILENAMES.map((name) => {
            ...
            try {
                ...
                throw new CoordinationRuntimeError('invalid-state', 'first graph publication requires the exact parent-planning core postimage file', [ref, error instanceof Error ? error.message : String(error)]);
            }
...
async function persistExactPrivateAuthorityFile(authorityRoot, path, bytes, label) {
    ...
    if (rel.length === 0 || rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel))
        throw new CoordinationRuntimeError('unauthorized-client', `${label} path escapes run-main authority`, [path, authorityRoot]);
    ...
    if (parentReal !== resolve(rootReal, dirname(rel)))
        throw new CoordinationRuntimeError('unauthorized-client', `${label} parent path contains an aliased or escaping segment`, [path, parentReal]);
    if (!existsSync(path)) {
        try {
            ...
            if (code !== 'EEXIST')
                throw new CoordinationRuntimeError('invalid-state', `${label} could not be exclusively created`, [path, error instanceof Error ? error.message : String(error)]);
        }
    }
    const metadata = lstatSync(path);
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1 || (metadata.mode & 0o777) !== 0o600)
        throw new CoordinationRuntimeError('invalid-state', `${label} must remain a one-link no-follow regular mode-0600 file`, [path, `mode=${(metadata.mode & 0o777).toString(8)}`, `nlink=${String(metadata.nlink)}`]);
    const existing = readImmutableFileBytes({ path, maximumBytes: 1_048_576, label });
    if (!equalBytes(existing, bytes))
        throw new CoordinationRuntimeError('idempotency-conflict', `${label} already exists with different immutable bytes`, [path]);
}
function persistExactMutableAuthorityFile(authorityRoot, path, bytes, label) {
    const lexicalRoot = resolve(authorityRoot);
    const lexicalPath = resolve(path);
    const rel = relative(lexicalRoot, lexicalPath);
    if (rel.length === 0 || rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel))
    ...
    if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1 || ((before.mode & 0o777) !== 0o600 && (before.mode & 0o777) !== 0o644))
        throw new CoordinationRuntimeError('invalid-state', `${label} must be a one-link no-follow regular authority file`, [path]);
    const temporary = `${lexicalPath}.d65-recovery.tmp`;
    if (existsSync(temporary)) {
        const residue = lstatSync(temporary);
        if (!residue.isFile() || residue.isSymbolicLink() || residue.nlink !== 1)
            throw new CoordinationRuntimeError('invalid-state', `${label} temporary residue is unsafe`, [temporary]);
        const residueBytes = readImmutableFileBytes({ path: temporary, maximumBytes: 1_048_576, label: `${label} temporary residue` });
        if (!equalBytes(residueBytes, bytes))
            throw new CoordinationRuntimeError('idempotency-conflict', `${label} temporary residue contains different bytes`, [temporary]);
    ...
    try {
        ...
        if (!written.isFile() || written.nlink !== 1 || written.size !== bytes.byteLength)
            throw new CoordinationRuntimeError('invalid-state', `${label} temporary write postcondition failed`, [temporary]);
    }
    finally {
        if (descriptor !== null)
    ...
    if (!equalBytes(persisted, bytes))
        throw new CoordinationRuntimeError('invalid-state', `${label} persisted bytes differ after atomic replacement`, [path]);
}
async function commitAndRegisterD65RecoveryArtifact(input) {
    ...
    if (!Array.isArray(rawArtifacts) || !Array.isArray(rawRuns) || rawRuns.length !== 1 || rawRuns[0] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact stage lacks exact run/artifact projections');
    ...
    if (artifacts.length > 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact identity is duplicated', [input.artifactId]);
    ...
    if (new Set(refs).size !== refs.length)
        throw new CoordinationRuntimeError('invalid-request', 'D65 recovery artifact bound file refs must be unique', refs);
    const accepted = artifacts[0];
    if (accepted !== undefined) {
        if (accepted.source_type !== 'task' || accepted.source_scope !== 'run-main' || accepted.document_schema_version !== input.schema || accepted.evidence.ref !== input.ref || accepted.evidence.sha256 !== digest)
            throw new CoordinationRuntimeError('idempotency-conflict', 'accepted D65 recovery artifact differs from the resumed exact stage', [input.artifactId]);
        for (const file of allFiles) {
            ...
            if (!equalBytes(committedBytes, file.bytes))
                throw new CoordinationRuntimeError('invalid-state', 'accepted D65 recovery artifact Git bytes differ from its resumed exact bound stage', [input.artifactId, file.ref]);
        }
        return;
    }
    for (const file of allFiles) {
        ...
        if (file.mutable)
            persistExactMutableAuthorityFile(input.resource.main_worktree_path, absolute, file.bytes, `${input.schema} mutable bound authority`);
        else if (existsSync(absolute)) {
            const existing = readImmutableFileBytes({ path: absolute, maximumBytes: 1_048_576, label: `${input.schema} bound authority` });
            if (!equalBytes(existing, file.bytes))
                throw new CoordinationRuntimeError('idempotency-conflict', 'D65 immutable bound authority file already exists with different bytes', [file.ref]);
        }
        else
            await persistExactPrivateAuthorityFile(input.resource.main_worktree_path, absolute, file.bytes, input.schema);
    }
    ...
    if (changedRefs.length > 0) {
        if (!changedRefs.includes(input.ref))
            throw new CoordinationRuntimeError('idempotency-conflict', 'D65 recovery bound files changed without the immutable primary artifact path', changedRefs);
        if (runGitQuery({ cwd: input.resource.main_worktree_path, descriptor: { kind: 'staged-clean' } }).negative)
            throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact refuses to commit over a nonempty shared index', refs);
        const gitEnv = { ...input.env, GIT_AUTHOR_NAME: 'autopilot-runtime', GIT_AUTHOR_EMAIL: 'autopilot-runtime@example.invalid', GIT_COMMITTER_NAME: 'autopilot-runtime', GIT_COMMITTER_EMAIL: 'autopilot-runtime@example.invalid', GIT_AUTHOR_DATE: input.commitDate, GIT_COMMITTER_DATE: input.commitDate };
        ...
        if (staged.kind !== 'reported' || staged.exitCode !== 0)
            throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact staging failed', [staged.kind === 'reported' ? staged.diagnostic : staged.reason]);
        ...
        if (committed.kind !== 'reported' || committed.exitCode !== 0)
            throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact commit failed', [committed.kind === 'reported' ? committed.diagnostic : committed.reason]);
        head = gitText(input.resource.main_worktree_path, { kind: 'head' }, `${input.schema} committed HEAD`);
    }
    for (const file of allFiles) {
        const tracked = runGitQuery({ cwd: input.resource.main_worktree_path, descriptor: { kind: 'show-file', revision: head, path: file.ref, allowAbsent: true } });
        if (tracked.negative || !equalBytes(tracked.stdout, file.bytes))
            throw new CoordinationRuntimeError('idempotency-conflict', 'current HEAD does not contain exact D65 recovery bound bytes', [file.ref]);
    }
    ...
    if (registered.artifact_id !== input.artifactId || registered.git_commit !== head || registered.evidence.ref !== input.ref || registered.evidence.sha256 !== digest)
        throw new CoordinationRuntimeError('invalid-state', 'D65 recovery artifact registration response differs from the exact committed stage', [input.artifactId]);
}
/**
 ...
 */
export async function driveD65ParentLossRecoveryFromEnvironment(input) {
    const env = input.env ?? process.env;
    const sequence = input.continuationSequence ?? 1;
    if (!Number.isSafeInteger(sequence) || sequence < 1)
        throw new CoordinationRuntimeError('invalid-request', 'D65 parent-loss continuation sequence must be a positive safe integer');
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 parent-loss recovery requires a durable coordinator session context');
    const session = await readCoordinatorSessionContext(contextPath);
    const client = new CoordinatorClient({ env });
    ...
    const rawArtifacts = status.payload['authoritative_artifacts'];
    if (!Array.isArray(resources) || resources.length !== 1 || resources[0] === undefined || !Array.isArray(rawArtifacts))
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss recovery lacks one exact run resource/artifact projection');
    ...
    if (policyArtifact === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss recovery requires the accepted launch policy');
    ...
    if (typeof evidenceRoot !== 'string')
        throw new CoordinationRuntimeError('invalid-state', 'accepted policy lacks its program evidence root');
    ...
    if (candidateSha256 !== input.expectedCandidateSha256)
        throw new CoordinationRuntimeError('idempotency-conflict', 'sealed parent-loss candidate no longer equals the digest consumed by attach-session', [input.expectedCandidateSha256, candidateSha256]);
    const candidate = parseD65ParentLoss(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(candidateBytes)));
    if (candidate.repo_id !== session.repo_id || candidate.workstream_run !== session.workstream_run || candidate.successor_session_id !== session.session_id || candidate.successor_session_lease_id !== session.session_lease_id || candidate.successor_generation !== session.session_generation || candidate.successor_pid !== session.pid || candidate.successor_boot_id !== session.boot_id)
        throw new CoordinationRuntimeError('invalid-state', 'sealed parent-loss candidate does not name the attached recovery session');
    ...
    if (runtimePrefix === null)
        throw new CoordinationRuntimeError('invalid-state', 'run runtime root is not a descendant of its main worktree');
    ...
    if (attachGraph.semanticEventType !== null && attachGraph.semanticEventType !== 'session-attached' && !allowedArtifactEvent)
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss recovery settled a foreign semantic event', [attachGraph.semanticEventType]);
    ...
    if (artifactGraph.semanticEventType !== null && artifactGraph.semanticEventType !== 'authoritative-artifact-registered')
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss artifact graph covered a foreign semantic event', [artifactGraph.semanticEventType]);
    ...
    if (!Array.isArray(continuationArtifactsValue))
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss continuation recovery lacks artifact projection');
    ...
    if (existingContinuations.length > 1)
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss continuation artifact identity is duplicated', [continuationArtifactId]);
    if (existingContinuation !== undefined) {
        if (existingContinuation.document_schema_version !== 'autopilot.continuation_event.v1')
            throw new CoordinationRuntimeError('idempotency-conflict', 'parent-loss continuation artifact id was accepted under a different schema', [continuationArtifactId]);
        ...
        if (existingDigest !== existingContinuation.evidence.sha256 || existingEvent.event_id !== candidate.event_id || existingEvent.event_sequence !== sequence || existingEvent.trigger !== 'parent-loss' || existingEvent.class !== 'parent-recovering' || existingEvent.session_lease_id !== session.session_lease_id || existingEvent.successor_id !== session.session_lease_id || existingEvent.evidence_refs.length !== 1 || existingEvent.evidence_refs[0]?.ref !== parentLossRef || existingEvent.evidence_refs[0]?.sha256 !== candidateSha256)
            throw new CoordinationRuntimeError('idempotency-conflict', 'accepted parent-loss continuation differs from the resumed incident authority', [continuationArtifactId]);
        const settled = await publishD65CoordinatorOnlySuccessorFromEnvironment({ env, createdAt: candidate.issued_at });
        if (settled.graphSequence !== existingEvent.result_graph_sequence)
            throw new CoordinationRuntimeError('invalid-state', 'accepted parent-loss continuation result graph sequence differs from its settled graph', [String(existingEvent.result_graph_sequence), String(settled.graphSequence)]);
    ...
    if (continuationGraph.semanticEventType !== null && continuationGraph.semanticEventType !== 'authoritative-artifact-registered')
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss continuation graph covered a foreign semantic event', [continuationGraph.semanticEventType]);
    if (continuationGraph.graphSequence !== continuationEvent.result_graph_sequence)
        throw new CoordinationRuntimeError('invalid-state', 'parent-loss continuation result graph sequence differs from its immutable event', [String(continuationGraph.graphSequence), String(continuationEvent.result_graph_sequence)]);
...
function compareBase64Paths(left, right) {
    const decode = (value) => {
        const bytes = decodeUnpaddedBase64Url(value.replace(/\+/gu, '-').replace(/\//gu, '_'));
        if (bytes === null)
            throw new CoordinationRuntimeError('invalid-state', 'first graph manifest path_b64 is not canonical base64', [value]);
        return bytes;
    };
 ...
 */
export async function ensureD65WorktreeStageCadenceFromEnvironment(env = process.env) {
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        return; // explicit legacy classification
    const session = await readCoordinatorSessionContext(contextPath);
    const client = new CoordinatorClient({ env });
    const status = await client.query('status', session.repo_id, session.workstream_run);
    const rawArtifacts = status.payload['authoritative_artifacts'];
    if (!Array.isArray(rawArtifacts))
        throw new CoordinationRuntimeError('invalid-state', 'D65 worktree cadence classification lacks committed artifact projection');
    ...
    if (!bootstrapPresent) {
        if (completePresent)
            throw new CoordinationRuntimeError('invalid-state', 'D65 complete graph exists without the deterministic bootstrap artifact');
        return; // legacy non-D65 run
    }
    if (!completePresent)
        return; // bootstrap charter owns pre-first-graph cadence
 ...
 */
export async function publishD65CoordinatorOnlySuccessorFromEnvironment(input = {}) {
    const env = input.env ?? process.env;
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 successor publication requires a durable coordinator session context');
    const session = await readCoordinatorSessionContext(contextPath);
    const client = new CoordinatorClient({ env });
    const status = await client.query('status', session.repo_id, session.workstream_run);
    const resources = status.payload['run_resources'];
    if (!Array.isArray(resources) || resources.length !== 1 || resources[0] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 successor publication requires one exact run resource');
    ...
    if (prepared.mainWorktreePath !== mainWorktreePath)
        throw new CoordinationRuntimeError('invalid-state', 'D65 successor export resource changed across preparation', [prepared.mainWorktreePath, mainWorktreePath]);
    if (prepared.state === 'already-current')
        return Object.freeze({ graphSha256: prepared.priorGraphSha256, publicationCommit: prepared.priorPublicationCommit, registrationEventSeq: prepared.priorRegistrationEventSeq, graphSequence: prepared.graphSequence, semanticEventType: null });
...
export async function driveD65SubscriptionFailureRecoveryFromEnvironment(input) {
    ...
    if (!Number.isSafeInteger(continuationSequence) || continuationSequence < 1)
        throw new CoordinationRuntimeError('invalid-request', 'D65 subscription-failure continuation sequence must be a positive safe integer');
    const parseCanonical = (bytes, label, parser) => {
        let value;
        try {
            value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes));
        }
        catch (error) {
            throw new CoordinationRuntimeError('invalid-request', `${label} is not canonical UTF-8 JSON`, [error instanceof Error ? error.message : String(error)]);
        }
        const parsed = parser(value);
        const canonical = new TextEncoder().encode(`${canonicalJson(parsed)}\n`);
        if (!equalBytes(bytes, canonical))
            throw new CoordinationRuntimeError('invalid-request', `${label} bytes are not exact RFC-8785-plus-LF encoding`);
        return parsed;
    };
    ...
    if (continuation.trigger !== 'subscription-failure' || continuation.class !== 'provider-capacity-blocked' || continuation.provider === null || continuation.failed_spec_ref === null || continuation.failed_receipt_ref === null || continuation.unit_id === null || continuation.attempt === null || continuation.cooldown_until === null || continuation.retry_ordinal !== 1)
        throw new CoordinationRuntimeError('invalid-request', 'D65 subscription-failure recovery requires the exact first provider-capacity failure tuple');
    if (continuation.event_sequence !== continuationSequence)
        throw new CoordinationRuntimeError('invalid-state', 'subscription-failure continuation event_sequence differs from the expected sequence', [String(continuation.event_sequence), String(continuationSequence)]);
    const resultGraphSequence = continuation.result_graph_sequence;
    if (resultGraphSequence === null)
        throw new CoordinationRuntimeError('invalid-request', 'subscription-failure continuation requires its exact result graph sequence');
    if (Date.parse(continuation.cooldown_until) !== Date.parse(continuation.observed_at) + 15 * 60 * 1000)
        throw new CoordinationRuntimeError('invalid-request', 'subscription-failure cooldown must equal observed_at plus exactly 15 minutes');
    const continuationDigest = `sha256:${createHash('sha256').update(continuationBytes).digest('hex')}`;
    if (probe.provider !== continuation.provider || probe.unit_id !== continuation.unit_id || probe.failed_attempt !== continuation.attempt || probe.retry_ordinal !== continuation.retry_ordinal || probe.cooldown_until !== continuation.cooldown_until || probe.trigger_continuation_sha256 !== continuationDigest)
        throw new CoordinationRuntimeError('invalid-state', 'subscription probe differs from the exact continuation failure tuple');
    const contextPath = env[AUTOPILOT_COORDINATOR_SESSION_CONTEXT_ENV];
    if (contextPath === undefined || contextPath.trim().length === 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 subscription-failure recovery requires a durable coordinator session context');
    const session = await readCoordinatorSessionContext(contextPath);
    const client = new CoordinatorClient({ env });
    if (continuation.repo_id !== session.repo_id || continuation.workstream_run !== session.workstream_run || probe.repo_id !== session.repo_id || probe.workstream_run !== session.workstream_run)
        throw new CoordinationRuntimeError('invalid-state', 'subscription recovery inputs do not bind the current session run');
    ...
    if (!Array.isArray(resources) || resources.length !== 1 || resources[0] === undefined || !Array.isArray(rawArtifacts))
        throw new CoordinationRuntimeError('invalid-state', 'subscription-failure recovery lacks one exact run resource/artifact projection');
    const resource = parseCoordinationRunResource(resources[0]);
    const runtimePrefix = resource.runtime_root.startsWith(`${resource.main_worktree_path}/`) ? resource.runtime_root.slice(resource.main_worktree_path.length + 1) : null;
    if (runtimePrefix === null)
        throw new CoordinationRuntimeError('invalid-state', 'run runtime root is not a descendant of its main worktree');
    ...
    if (probe.trigger_continuation_ref !== continuationRef)
        throw new CoordinationRuntimeError('invalid-request', 'subscription probe trigger ref is not the exact continuation artifact path', [probe.trigger_continuation_ref, continuationRef]);
    const supplied = new Map();
    for (const file of boundAuthorityFiles) {
        if (supplied.has(file.ref))
            throw new CoordinationRuntimeError('invalid-request', 'subscription recovery bound authority refs must be unique', [file.ref]);
    ...
    if (new Set(descriptors.map((entry) => entry.ref)).size !== descriptors.length)
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation contains duplicate bound authority refs');
    for (const ref of supplied.keys())
        if (!descriptors.some((entry) => entry.ref === ref))
            throw new CoordinationRuntimeError('invalid-request', 'subscription recovery supplied an unbound authority file', [ref]);
    ...
    const resolvedFiles = descriptors.map((descriptor) => {
        ...
        if (digest !== descriptor.sha256 || bytes.byteLength !== descriptor.byte_count)
            throw new CoordinationRuntimeError('invalid-request', 'subscription continuation bound authority bytes differ from their exact descriptor', [descriptor.ref, descriptor.sha256, digest]);
    ...
    if (stateFiles.length !== 1 || stateFiles[0] === undefined || !stateFiles[0].supplied)
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation must supply exactly one updated runtime state authority', [stateRef]);
    const successorSpecs = resolvedFiles.map((entry) => {
        try {
            return { entry, spec: parseD65RuntimeUnitSpecBytes(entry.bytes, 'subscription successor spec') };
        }
        catch {
            return null;
        }
    }).filter((entry) => entry !== null && entry.spec.unit_id === probe.unit_id && entry.spec.attempt === probe.successor_attempt);
    if (successorSpecs.length !== 1 || successorSpecs[0] === undefined || !successorSpecs[0].entry.supplied)
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation must supply exactly one successor unit spec authority');
    ...
    if (failedSpecEntry === undefined || failedReceiptEntry === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'subscription continuation failed authority resolution is incomplete');
    ...
    if (d65RuntimeSpecGeneration(failedSpec) !== d65RuntimeReceiptGeneration(failedReceipt) || d65RuntimeSpecGeneration(successorSpec) !== d65RuntimeSpecGeneration(failedSpec))
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation cannot mix runtime unit_spec/receipt generations');
    if (failedSpec.unit_id !== probe.unit_id || failedSpec.attempt !== probe.failed_attempt || failedReceipt.unit_id !== probe.unit_id || failedReceipt.attempt !== probe.failed_attempt || (failedSpecProviderId !== null && failedSpecProviderId !== probe.provider) || failedReceipt.provider_identity.provider_id !== probe.provider || failedReceipt.provider_identity.requested_model_id !== failedProviderModelId || failedReceipt.provider_identity.executed_model_id !== failedProviderModelId || !d65RuntimeSpecsPreserveRetryAuthority(failedSpec, successorSpec))
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation spec/receipt/provider/model authority is not exact');
    ...
    if (stateUnit === undefined || stateUnit.attempt !== probe.successor_attempt || stateUnit.spec_ref !== successorStateRef || workItems.length !== 1)
        throw new CoordinationRuntimeError('invalid-request', 'subscription continuation updated state does not authorize exactly one successor spec/work-item tuple');
    ...
    if (probeAccepted || probeCommitPending) {
        ...
        if (artifact === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'subscription probe stage lacks the immutable continuation result graph');
        ...
        if (priorGraph === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'subscription recovery requires an accepted complete prior graph');
        const priorSequence = Number(priorGraph.artifact_id.slice(-20));
        if (!Number.isSafeInteger(priorSequence) || continuation.prior_graph_sha256 !== priorGraph.evidence.sha256 || resultGraphSequence !== priorSequence + 1)
            throw new CoordinationRuntimeError('invalid-state', 'subscription continuation prior/result graph tuple is not exact', [String(resultGraphSequence), String(priorSequence)]);
        ...
        if (!continuationCommitPending) {
            ...
            if (settled.semanticEventType !== null)
                throw new CoordinationRuntimeError('invalid-state', 'subscription recovery began before prior failed-attempt recovery cadence was settled', [settled.semanticEventType]);
            await ensureD65ProgramHeartbeatForGraphFromEnvironment({ graphSequence: settled.graphSequence, graphSha256: settled.graphSha256, env });
            if (settled.graphSha256 !== priorGraph.evidence.sha256 || settled.graphSequence !== priorSequence)
                throw new CoordinationRuntimeError('invalid-state', 'subscription recovery settled graph differs from the immutable continuation prior', [String(settled.graphSequence), String(priorSequence)]);
        }
        ...
        if (graph.semanticEventType !== 'authoritative-artifact-registered' || graph.graphSequence !== resultGraphSequence)
            throw new CoordinationRuntimeError('invalid-state', 'subscription continuation graph did not cover exactly its registration event', [String(graph.semanticEventType), String(graph.graphSequence)]);
        ...
        const graph = await publishD65CoordinatorOnlySuccessorFromEnvironment({ env, createdAt: continuation.observed_at });
        if (graph.semanticEventType !== null && graph.semanticEventType !== 'authoritative-artifact-registered')
            throw new CoordinationRuntimeError('invalid-state', 'subscription continuation resume encountered a foreign semantic event', [graph.semanticEventType]);
        if (graph.graphSequence !== resultGraphSequence)
            throw new CoordinationRuntimeError('invalid-state', 'resumed subscription continuation graph sequence differs from immutable result_graph_sequence');
        await ensureD65ProgramHeartbeatForGraphFromEnvironment({ graphSequence: graph.graphSequence, graphSha256: graph.graphSha256, env });
        continuationGraph = graph;
    }
    let probeGraph;
    if (!probeAccepted) {
        ...
        if (graph.semanticEventType !== 'authoritative-artifact-registered')
            throw new CoordinationRuntimeError('invalid-state', 'subscription probe graph did not cover exactly its registration event', [String(graph.semanticEventType)]);
        await ensureD65ProgramHeartbeatForGraphFromEnvironment({ graphSequence: graph.graphSequence, graphSha256: graph.graphSha256, env });
        probeGraph = graph;
    }
    else {
        const graph = await publishD65CoordinatorOnlySuccessorFromEnvironment({ env, createdAt: probe.issued_at });
        if (graph.semanticEventType !== null && graph.semanticEventType !== 'authoritative-artifact-registered')
            throw new CoordinationRuntimeError('invalid-state', 'subscription probe resume encountered a foreign semantic event', [graph.semanticEventType]);
        await ensureD65ProgramHeartbeatForGraphFromEnvironment({ graphSequence: graph.graphSequence, graphSha256: graph.graphSha256, env });
        probeGraph = graph;
    }
