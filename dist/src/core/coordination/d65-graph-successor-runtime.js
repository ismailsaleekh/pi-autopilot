...
export async function driveD65SubscriptionFailureRecoveryFromEnvironment(input) {
    ...
    const successorSpecs = resolvedFiles.map((entry) => {
        try {
            return { entry, spec: parseAutopilotUnitSpec(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(entry.bytes))) };
        }
        catch {
            return null;
        }
    ...
    const state = parseAutopilotState(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(stateFiles[0].bytes)));
