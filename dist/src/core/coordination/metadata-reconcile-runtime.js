...
function assertInside(root, path, label) {
    ...
    if (rel === '' || rel.startsWith('..') || isAbsolute(rel) || rel.split(sep).includes('..'))
        throw new CoordinationRuntimeError('unauthorized-client', `${label} escapes its package-owned evidence root`, [path, root]);
}
...
function readPreservedRefs(intent, env) {
    return Object.freeze(intent.preserved_refs.map((expected) => {
        ...
        if (query.negative)
            throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation preserved ref is absent', [expected.ref]);
        const actual = new TextDecoder('utf-8', { fatal: true }).decode(query.stdout).trim();
        if (actual !== expected.sha)
            throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation preserved ref moved', [expected.ref, `expected=${expected.sha}`, `actual=${actual}`]);
        return Object.freeze({ ref: expected.ref, sha: actual });
    }));
}
function inspectApproval(approval, env) {
...
async function prepareEvidencePath(evidenceRoot, canonicalWorktreeId) {
    const root = resolve(evidenceRoot);
    await mkdir(root, { recursive: true, mode: 0o700 });
    const rootInfo = lstatSync(root);
    if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink())
        throw new CoordinationRuntimeError('unauthorized-client', 'metadata reconciliation evidence root must be a real directory', [root]);
    const directory = resolve(root, 'metadata-reconcile');
    if (existsSync(directory)) {
        const info = lstatSync(directory);
        if (!info.isDirectory() || info.isSymbolicLink())
            throw new CoordinationRuntimeError('unauthorized-client', 'metadata reconciliation evidence directory is a symbolic or non-directory entry', [directory]);
    }
    else
    ...
    if (existsSync(path)) {
        ...
        if (!info.isFile() || info.isSymbolicLink())
            throw new CoordinationRuntimeError('unauthorized-client', 'metadata reconciliation evidence destination is not a regular non-symbolic file', [path]);
    }
    return path;
}
...
export async function reconcileApprovedMissingWorktreeMetadata(input) {
    if (input.approvals.length === 0)
        throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation requires at least one approved row');
    if ((input.evidence_root === undefined) === (input.evidence_roots === undefined))
        throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation requires exactly one evidence-root authority form');
    const approvals = input.approvals.map((approval) => ({ ...approval, intent: parseMetadataReconcileIntent(approval.intent) }));
    const first = approvals[0];
    if (first === undefined)
        throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation approval set disappeared');
    const evidenceRoots = new Map();
    if (input.evidence_root !== undefined) {
        for (const approval of approvals)
            evidenceRoots.set(approval.intent.canonical_worktree_id, input.evidence_root);
    }
    else {
        for (const entry of input.evidence_roots ?? []) {
            if (evidenceRoots.has(entry.canonical_worktree_id))
                throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation evidence-root map contains a duplicate canonical worktree', [entry.canonical_worktree_id]);
        ...
        if (missing.length > 0 || unexpected.length > 0)
            throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation evidence-root map differs from the exact approval set', [...missing.map((entry) => `missing=${entry}`), ...unexpected.map((entry) => `unexpected=${entry}`)]);
    }
    for (const approval of approvals) {
        ...
        if (canonicalId !== approval.intent.canonical_worktree_id || approval.semantic_identity.repo_id !== approval.intent.repo_id)
            throw new CoordinationRuntimeError('invalid-state', 'metadata reconciliation approval canonical owner differs from immutable intent', [approval.intent.canonical_worktree_id, canonicalId]);
        if (approval.intent.git_common_dir !== first.intent.git_common_dir || approval.intent.repo_id !== first.intent.repo_id)
            throw new CoordinationRuntimeError('invalid-request', 'metadata reconciliation batch crosses repository authority');
        if (!exactEqual(approval.intent.approved_before_registrations, first.intent.approved_before_registrations)
            ...
            || !exactEqual(approval.intent.preserved_refs, first.intent.preserved_refs))
            throw new CoordinationRuntimeError('invalid-state', 'metadata reconciliation rows disagree on complete batch before/after/ref facts', [approval.intent.canonical_worktree_id]);
        if (pathEntryExists(approval.intent.target_registration_path))
            throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation target path has a physical or symbolic filesystem entry; metadata-only prune is forbidden', [approval.intent.target_registration_path]);
        ...
        if (evidenceRoot === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'metadata reconciliation approval lost its exact evidence-root authority', [approval.intent.canonical_worktree_id]);
        await prepareEvidencePath(evidenceRoot, approval.intent.canonical_worktree_id);
    }
    ...
    if (!exactEqual(targets, approved))
        throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation rows do not cover the complete approved prunable set', [...targets.map((path) => `row=${path}`), ...approved.map((path) => `approved=${path}`)]);
    ...
    if (initialInspection.outcome === 'unsafe')
        throw new CoordinationRuntimeError('recovery-required', 'canonical metadata reconciliation probe rejected the approved batch before action', initialInspection.proof);
    const initiallyApplied = initialInspection.outcome === 'satisfied';
    if (!initiallyApplied && !exactEqual(before, first.intent.approved_before_registrations))
        throw new CoordinationRuntimeError('recovery-required', 'Git worktree registration set drifted before metadata reconciliation proof', [`observed=${String(before.length)}`, `approved=${String(first.intent.approved_before_registrations.length)}`]);
    ...
    if (!exactEqual(finalPreservedBefore, preservedBefore))
        throw new CoordinationRuntimeError('recovery-required', 'preserved Git refs changed between metadata reconciliation proof and action', first.intent.preserved_refs.map((entry) => entry.ref));
    ...
    if (!alreadySatisfied && !exactEqual(finalBefore, first.intent.approved_before_registrations))
        throw new CoordinationRuntimeError('recovery-required', 'Git worktree registration set changed between proof and action; refusing prune', [`observed=${String(finalBefore.length)}`, `approved=${String(first.intent.approved_before_registrations.length)}`]);
    if (!alreadySatisfied) {
        ...
        if (!exactEqual(actualPrunablePaths, approved))
            throw new CoordinationRuntimeError('recovery-required', 'global worktree prune is forbidden unless every currently prunable registration has one approved row', [...actualPrunablePaths.map((path) => `actual_prunable=${path}`), ...approved.map((path) => `approved_prunable=${path}`)]);
    }
    ...
    if (afterInspection.outcome !== 'satisfied' || !exactEqual(after, first.intent.expected_after_registrations))
        throw new CoordinationRuntimeError('recovery-required', 'metadata reconciliation canonical postcondition is not satisfied', [`before=${String(finalBefore.length)}`, `after=${String(after.length)}`, `expected_after=${String(first.intent.expected_after_registrations.length)}`, ...afterInspection.proof, ...(mutation === null ? [] : [`mutation_report=${mutation.kind}`, `mutation_diagnostic=${mutation.diagnostic}`])]);
