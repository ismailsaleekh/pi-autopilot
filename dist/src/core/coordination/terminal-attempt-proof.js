...
import { parseAutopilotEventRow, parseAutopilotExecutionAudit, parseAutopilotReceipt, parseAutopilotReceiptV2, parseAutopilotStatusEntry, parseAutopilotUnitSpec, parseAutopilotUnitSpecV2 } from "../contracts/index.js";
...
import { assertAutopilotChildTerminalAcceptanceChain, autopilotAuditProvesZeroSourceChange, parseAutopilotChildTerminalAcceptance } from "./terminal-acceptance.js";
...
function parseTerminalProofUnitSpec(bytes) {
...
function parseTerminalProofReceipt(bytes, spec, statusPath) {
    ...
    if (isJsonRecord(parsed) && parsed['schema_version'] === 'autopilot.receipt.v2') {
        if (spec.schema_version !== 'autopilot.unit_spec.v2')
            throw new Error('receipt.v2 requires unit_spec.v2 terminal proof bytes');
        return parseAutopilotReceiptV2(parsed);
    }
    if (spec.schema_version === 'autopilot.unit_spec.v2')
        throw new Error('unit_spec.v2 terminal proof requires native receipt.v2 bytes');
    return parseAutopilotReceipt(parsed, { statusOutputPath: statusPath });
}
function isJsonRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function artifact(authorityRoot, path, label) {
    ...
    if (lexical.length === 0 || lexical === '..' || lexical.startsWith(`..${sep}`) || isAbsolute(lexical))
        throw new Error(`${label} escapes the run main worktree`);
    ...
    if (physical.length === 0 || physical === '..' || physical.startsWith(`..${sep}`) || isAbsolute(physical))
        throw new Error(`${label} physically escapes the run main worktree`);
    const descriptor = openSync(lexicalPath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0));
    try {
        const opened = fstatSync(descriptor);
        if (!opened.isFile() || opened.dev !== before.dev || opened.ino !== before.ino || opened.size !== before.size || opened.mtimeMs !== before.mtimeMs || opened.ctimeMs !== before.ctimeMs)
...
function underRoot(root, path, label) {
...
export function proveStructuredAttemptTerminal(input) {
    ...
    try {
        ...
        if (specArtifact.sha256 !== input.spec.sha256)
            return unproven('unit spec hash differs from the coordinator attempt identity', inspectedPaths);
        const parsedSpec = parseTerminalProofUnitSpec(specArtifact.bytes);
        const spec = parsedSpec.lowered;
        if (parsedSpec.original.workstream !== input.workstream || parsedSpec.original.unit_id !== input.unitId || parsedSpec.original.attempt !== input.attempt)
            return unproven('unit spec identity differs from the durable attempt', inspectedPaths);
        ...
        if (status.workstream !== input.workstream || status.unit_id !== input.unitId || status.role !== spec.role || status.attempt !== input.attempt || audit.workstream !== input.workstream || audit.unit_id !== input.unitId || audit.role !== spec.role || audit.attempt !== input.attempt || receipt.workstream !== input.workstream || receipt.unit_id !== input.unitId || receipt.role !== spec.role || receipt.attempt !== input.attempt)
            return unproven('status, receipt, audit, and spec identities disagree', inspectedPaths);
        const sourceChanging = spec.role === 'implement' || spec.role === 'fix';
        if (audit.truncated_path_sets.length > 0 || audit.baseline_head === null || audit.post_run_head === null || audit.head_change_kind === 'rewrite' || audit.head_change_kind === 'unavailable' || audit.dirty_baseline !== false)
            return unproven('execution audit does not completely account for a stable clean-baseline Git transition', inspectedPaths);
        if (!sourceChanging && !autopilotAuditProvesZeroSourceChange(audit))
            return unproven('non-source terminal repair requires a mechanically clean zero-source-change audit', inspectedPaths);
        ...
        if (existsSync(acceptancePath)) {
            ...
            if (receipt.schema_version === 'autopilot.receipt.v2') {
                ...
                if (!validation.ok)
                    return unproven(`receipt.v2 terminal compatibility failed: ${validation.diagnostics.map((diagnostic) => diagnostic.code).join(', ')}`, inspectedPaths);
            }
            if (acceptance.spec.ref !== specArtifact.ref || acceptance.status.ref !== statusArtifact.ref || acceptance.receipt.ref !== receiptArtifact.ref || acceptance.audit.ref !== auditArtifact.ref)
                return unproven('terminal acceptance artifact refs differ from the durable attempt artifact paths', inspectedPaths);
            ...
            const matching = new TextDecoder('utf-8', { fatal: true }).decode(eventsArtifact.bytes).split(/\r?\n/u).filter((line) => line.length > 0).map((line) => parseAutopilotEventRow(JSON.parse(line))).filter((event) => event.event === 'agent_completed' && event.workstream === input.workstream && event.unit_id === input.unitId && event.role === spec.role && event.verdict === status.verdict && event.status_ref === statusRef && event.receipt_ref === receiptRef);
            if (matching.length !== 1)
                return unproven(`historical terminal repair requires exactly one parent agent_completed carrier-acceptance fact, found ${String(matching.length)}`, inspectedPaths);
            const acceptedEvent = matching[0];
            if (acceptedEvent === undefined || Date.parse(acceptedEvent.ts) < Date.parse(receipt.emitted_at) || Date.parse(acceptedEvent.ts) < Date.parse(audit.audited_at))
                return unproven('parent agent_completed fact predates its receipt or audit', inspectedPaths);
