 ...
 */
export function d65SemanticGraphSequenceFromArtifactId(artifactId) {
    const prefix = 'semantic-graph:';
    if (!artifactId.startsWith(prefix))
        throw new CoordinationRuntimeError('invalid-request', 'semantic graph artifact id lacks the deterministic prefix', [artifactId]);
    const suffix = artifactId.slice(prefix.length);
    if (!/^[0-9]{20}$/u.test(suffix))
        throw new CoordinationRuntimeError('invalid-request', 'semantic graph artifact id suffix is not exactly 20 digits', [artifactId]);
    const sequence = Number(suffix);
    if (!Number.isSafeInteger(sequence) || sequence < 2)
        throw new CoordinationRuntimeError('invalid-request', 'semantic graph artifact id sequence is out of range', [artifactId]);
    if (d65SemanticGraphArtifactId(sequence) !== artifactId)
        throw new CoordinationRuntimeError('invalid-request', 'semantic graph artifact id is not the canonical padded form', [artifactId]);
    return sequence;
}
