...
import { chmod, copyFile, mkdir, open, readdir, rename, rm } from 'node:fs/promises';
...
import { inspectWorktreePostcondition } from "./worktree-postconditions.js";
...
function boundedString(value, label, maximum = 2048) {
    if (typeof value !== 'string' || value.length === 0 || value.length > maximum || value.includes('\u0000'))
        failure('invalid', `${label} must be a bounded non-empty string`);
    return value;
}
function integer(value, label, minimum = 0) {
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < minimum)
        failure('invalid', `${label} must be a safe integer >= ${String(minimum)}`);
    return value;
}
...
function readBounded(path, maximum = COORDINATION_MIGRATION_MAX_FILE_BYTES) {
    try {
        return readImmutableFileBytes({ path, maximumBytes: maximum, minimumBytes: 0, label: 'migration input' });
    }
    catch (error) {
        failure('invalid', `migration input is not immutable and bounded: ${error instanceof Error ? error.message : String(error)}`, [path]);
    }
}
function readBoundedDatabase(path) {
    try {
        return readImmutableFileBytes({ path, maximumBytes: COORDINATION_MIGRATION_MAX_DATABASE_COMPONENT_BYTES, minimumBytes: 0, label: 'migration database component' });
    }
    catch (error) {
        failure('blocked', `migration database component is not immutable and bounded: ${error instanceof Error ? error.message : String(error)}`, [path]);
    }
}
function assertNoDuplicateJsonKeys(text, label) {
    ...
    const parseStringToken = () => {
        if (text[index] !== '"')
            failure('invalid', `${label} contains invalid JSON string`);
        const start = index;
        index += 1;
        let escaped = false;
        while (index < text.length) {
            ...
            if (char === '"') {
                ...
                try {
                    ...
                    failure('invalid', `${label} contains invalid JSON string escaping`);
                }
            }
            if (char !== undefined && char.charCodeAt(0) < 0x20)
                failure('invalid', `${label} contains an unescaped control character`);
        }
    ...
    const parseValue = () => {
        ...
        if (char === '{') {
            ...
            while (true) {
                ...
                if (keys.has(key))
                    failure('invalid', `${label} contains duplicate JSON object key`, [key]);
                keys.add(key);
                whitespace();
                if (text[index] !== ':')
                    failure('invalid', `${label} contains invalid JSON object syntax`);
                index += 1;
                parseValue();
                whitespace();
                if (text[index] === '}') {
                ...
                if (text[index] !== ',')
                    failure('invalid', `${label} contains invalid JSON object separator`);
                index += 1;
            }
        }
        if (char === '[') {
            ...
            while (true) {
                ...
                if (text[index] !== ',')
                    failure('invalid', `${label} contains invalid JSON array separator`);
                index += 1;
            }
        }
        ...
        if (token === undefined)
            failure('invalid', `${label} contains an invalid JSON value`);
        index += token.length;
    };
    parseValue();
    whitespace();
    if (index !== text.length)
        failure('invalid', `${label} contains trailing JSON content`);
}
function parseJsonFile(path, missing) {
    if (!existsSync(path))
        return missing;
...
function validateBoundedJson(value, label, depth = 0) {
    if (depth > 12)
        failure('invalid', `${label} exceeds maximum JSON depth`);
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            failure('invalid', `${label} contains a non-finite number`);
        return;
    }
    if (Array.isArray(value)) {
        if (value.length > 4096)
            failure('invalid', `${label} contains an oversized array`);
        value.forEach((entry, index) => validateBoundedJson(entry, `${label}[${String(index)}]`, depth + 1));
        return;
    }
    if (typeof value !== 'object')
        failure('invalid', `${label} contains a non-JSON value`);
    const row = value;
    if (Object.keys(row).length > 128)
        failure('invalid', `${label} contains too many fields`);
...
function stringArray(value, label, maximum = 4096) {
    if (!Array.isArray(value) || value.length > maximum || value.some((entry) => typeof entry !== 'string' || entry.length === 0 || entry.length > 2048))
        failure('invalid', `${label} must be a bounded string array`);
    return value;
}
function validateLegacyJsonlPayload(payload, schema, label) {
    if (schema === 'autopilot.claim_event.v1') {
        ...
        if (!['acquire', 'release', 'upgrade', 'expand', 'rejected'].includes(event))
            failure('invalid', `${label}.event is invalid`);
        integer(payload['active_run_epoch'], `${label}.active_run_epoch`, 1);
        for (const field of ['autopilot_id', 'reason', 'repo_key', 'ts', 'workstream', 'workstream_run'])
            boundedString(payload[field], `${label}.${field}`);
        if (payload['path'] !== undefined)
            normalizeRepoRelativePath(boundedString(payload['path'], `${label}.path`));
        if (payload['attempt'] !== undefined)
            integer(payload['attempt'], `${label}.attempt`, 1);
        return;
    }
    if (schema === 'autopilot.merge_event.v1') {
        requiredPayloadFields(payload, LEGACY_JSONL_FIELDS[schema] ?? [], label);
        for (const field of ['autopilot_id', 'branch', 'merge_id', 'merged_at', 'repo_key', 'target_after', 'target_before', 'target_branch', 'workstream', 'workstream_after', 'workstream_before', 'workstream_run'])
            boundedString(payload[field], `${label}.${field}`);
        for (const changedPath of stringArray(payload['changed_paths'], `${label}.changed_paths`))
            normalizeRepoRelativePath(changedPath);
        if (payload['integration_commit_sha'] !== null)
            boundedString(payload['integration_commit_sha'], `${label}.integration_commit_sha`, 128);
        return;
    }
    if (schema === 'autopilot.foreign_merge_ack.v1') {
        requiredPayloadFields(payload, LEGACY_JSONL_FIELDS[schema] ?? [], label);
        for (const field of ['ack_id', 'acked_at', 'acknowledging_autopilot_id', 'acknowledging_workstream_run', 'foreign_autopilot_id', 'foreign_workstream_run', 'merge_id', 'repo_key'])
            boundedString(payload[field], `${label}.${field}`);
        if (payload['action'] !== 'non-intersecting')
            failure('invalid', `${label}.action is invalid`);
        for (const changedPath of stringArray(payload['intersection_paths'], `${label}.intersection_paths`))
            normalizeRepoRelativePath(changedPath);
        return;
    }
...
function parseJsonl(path, sourceKind, expectedSchema) {
    ...
    if (lines.length > COORDINATION_MIGRATION_MAX_JSONL_ROWS + 1)
        failure('invalid', 'migration JSONL row bound exceeded', [path]);
    const rows = [];
    for (let index = 0; index < lines.length; index += 1) {
        const line = lines[index];
        if (line === undefined || line.length === 0)
            continue;
        if (Buffer.byteLength(line, 'utf8') > COORDINATION_MIGRATION_MAX_JSONL_LINE_BYTES)
            failure('invalid', 'migration JSONL line bound exceeded', [path, String(index + 1)]);
        let parsed;
        try {
            assertNoDuplicateJsonKeys(line, `${path}:${String(index + 1)}`);
            parsed = JSON.parse(line);
        }
        catch {
            failure('invalid', 'migration JSONL line contains invalid JSON', [path, String(index + 1)]);
        }
        validateBoundedJson(parsed, `${path}:${String(index + 1)}`);
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed) || parsed['schema_version'] !== expectedSchema)
            failure('invalid', `migration JSONL line must use ${expectedSchema}`, [path, String(index + 1)]);
        const payload = parsed;
        const allowed = LEGACY_JSONL_FIELDS[expectedSchema];
        if (allowed === undefined)
            failure('invalid', 'migration JSONL schema has no closed field contract', [expectedSchema]);
        const unknownFields = Object.keys(payload).filter((field) => !allowed.includes(field));
        if (unknownFields.length > 0)
            failure('invalid', 'migration JSONL line has unknown fields', [path, String(index + 1), ...unknownFields.sort()]);
...
function attemptKey(run, unit, attempt) {
...
function parseUnitBranch(value, label) {
    ...
    if (!['active', 'merged', 'aborted', 'quarantined', 'superseded'].includes(statusValue))
        failure('invalid', `${label}.status is invalid`);
    const worktreePath = boundedString(row['worktree_path'], `${label}.worktree_path`);
    if (!isAbsolute(worktreePath))
        failure('invalid', `${label}.worktree_path must be absolute`);
    const archive = row['archive_ref'];
    if (archive !== null && typeof archive !== 'string')
        failure('invalid', `${label}.archive_ref must be nullable text`);
    return {
        ...
        archive_ref: archive, status: statusValue,
    };
}
function readUnitMetadata(rows) {
    ...
    for (const active of rows) {
        ...
        if (unit['schema_version'] !== 'autopilot.unit_index.v1' || !Array.isArray(unit['units']) || unit['units'].length > 10_000)
            failure('invalid', '_unit-index.json has an invalid schema or bound', [unitPath]);
        ...
        if (branches['schema_version'] !== 'autopilot.branches.v1' || !Array.isArray(branches['unit_branches']) || branches['unit_branches'].length > 10_000)
            failure('invalid', '_branches.json has an invalid schema or bound', [branchesPath]);
...
function readLegacyMergeEvidence(rows) {
    const merges = [];
    const blockers = [];
    ...
    for (const row of rows) {
        ...
        if (files.length > 10_000)
            failure('invalid', 'unit-merge evidence file count exceeds bound', [root]);
        for (const file of files) {
            const path = join(root, file);
            const raw = parseJsonFile(path, null);
            object(raw, path, fields);
            const merge = parseAutopilotUnitMerge(raw);
            if (merge.workstream_run !== row.workstream_run || merge.autopilot_id !== row.autopilot_id || merge.workstream !== row.workstream || merge.main_branch !== row.branch)
                failure('invalid', 'unit-merge evidence disagrees with durable run identity', [path]);
            const normalizedPaths = merge.changed_paths.map((changedPath) => normalizeRepoRelativePath(changedPath)).sort();
            if (new Set(normalizedPaths).size !== normalizedPaths.length || stableJson(normalizedPaths) !== stableJson([...merge.changed_paths].sort()))
                failure('invalid', 'unit-merge changed paths must be unique and normalized', [path]);
            if (!existsSync(row.main_worktree_path)) {
                blockers.push(`unit-merge Git proof unavailable because main worktree is missing: ${path}`);
                continue;
            }
...
function acceptedAttemptProof(row, metadata, record, path, action) {
    if (metadata === undefined || resolve(boundedString(record['unit_worktree_path'], `${path}.unit_worktree_path`)) !== resolve(metadata.worktree_path))
        return null;
    ...
    if (metadata.status !== 'quarantined' || typeof captureValue !== 'string' || typeof captureRefValue !== 'string' || captureRefValue !== metadata.archive_ref || typeof preCaptureValue !== 'string' || !Array.isArray(dirtyValue) || dirtyValue.some((entry) => typeof entry !== 'string'))
        return null;
    const capture = exactCommit(row.source_repo, captureValue);
    const preCapture = exactCommit(row.source_repo, preCaptureValue);
    if (capture === null || capture !== current || preCapture === null || record['git_head_after'] !== capture || record['branch'] !== metadata.branch || resolve(String(record['git_common_dir'])) !== resolve(row.git_common_dir))
        return null;
    ...
    if (inspection.outcome !== 'satisfied' || inspection.capture_sha !== capture)
        return null;
    if (existsSync(metadata.worktree_path)) {
        if (inspection.proof_source !== 'physical-worktree' || branchRef !== capture || archiveRef !== capture)
            return null;
    ...
    if (inspection.proof_source !== 'owned-git-ref' || selectedSha !== capture || gitWorktreeContains(row.source_repo, metadata.worktree_path))
        return null;
    const exactGitObjects = Object.freeze([base, preCapture, capture]);
...
function acceptedRunProof(row, index, path, outcome, terminalValue) {
        ...
        for (const runtimeRoot of runtimeRoots) {
            ...
            if (existsSync(quarantineRoot)) {
                ...
                for (const path of files) {
                    ...
                    if (!['reset', 'abort', 'quarantine', 'preserve'].includes(action))
                        continue;
                    ...
                    if (accepted !== null && (attemptProof.get(key)?.source !== 'attempt-reset' || accepted.source === 'attempt-reset'))
                        attemptProof.set(key, accepted);
                }
            }
            const closeRoot = join(runtimeRoot, 'close');
            for (const outcome of ['closed', 'aborted']) {
                const path = join(closeRoot, `_run-terminal.${outcome}.json`);
                if (!existsSync(path))
                    continue;
                paths.push(path);
                const terminalValue = parseJsonFile(path, null);
                const terminalMap = typeof terminalValue === 'object' && terminalValue !== null && !Array.isArray(terminalValue) ? terminalValue : null;
                ...
                if (record['schema_version'] !== 'autopilot.run_terminal.v1' || record['repo_key'] !== row.repo_key || record['autopilot_id'] !== row.autopilot_id || record['workstream'] !== row.workstream || record['workstream_run'] !== row.workstream_run || record['outcome'] !== outcome || typeof record['terminal_sha'] !== 'string' || !/^[a-f0-9]{7,64}$/u.test(record['terminal_sha']))
                    failure('invalid', 'run terminal evidence identity is invalid', [path]);
                if (hasCleanupBinding && (record['cleanup_manifest_ref'] !== 'close/_terminal-cleanup.json' || typeof record['cleanup_manifest_sha256'] !== 'string' || !DIGEST.test(record['cleanup_manifest_sha256'])))
                    failure('invalid', 'run terminal cleanup binding is invalid', [path]);
                const accepted = acceptedRunProof(row, index, path, outcome, record['terminal_sha']);
                if (accepted !== null)
                    runProof.set(row.workstream_run, accepted);
            }
        }
    }
    for (const row of rows) {
...
function parseWorktreeIndex(path) {
    ...
    if (root['schema_version'] !== 'autopilot.worktree_index.v1' || !Array.isArray(root['active']) || !Array.isArray(root['archive']) || root['active'].length + root['archive'].length > 100_000)
        failure('invalid', 'worktree index has invalid schema or bound', [path]);
    return Object.freeze([...root['active'], ...root['archive']].map((entry, index) => {
        ...
        if (statusValue !== 'active' && statusValue !== 'archived')
            failure('invalid', 'worktree index status is invalid');
        const mainPath = boundedString(row['main_path'], 'worktree index main_path');
        if (!isAbsolute(mainPath))
            failure('invalid', 'worktree index main_path must be absolute');
        return { workstream: boundedString(row['workstream'], 'worktree index workstream', 192), workstream_run: boundedString(row['workstream_run'], 'worktree index run', 192), autopilot_id: boundedString(row['autopilot_id'], 'worktree index autopilot', 192), started_at: boundedString(row['started_at'], 'worktree index started_at', 64), main_path: resolve(mainPath), branch: boundedString(row['branch'], 'worktree index branch', 512), status: statusValue };
    }));
}
function sourcePaths(stateRoot, repoKey, rows, unitMetadata, merges, terminalPaths) {
...
function physicalContainmentRoot(path) {
    let current = resolve(path);
    while (!existsSync(current)) {
        const parent = dirname(current);
        if (parent === current)
            failure('invalid', 'migration input has no existing physical ancestor', [path]);
        current = parent;
    }
    return realpathSync(current);
}
function snapshotEntries(stateRoot, paths) {
    if (paths.length > COORDINATION_MIGRATION_MAX_FILES)
        failure('invalid', 'migration input file count exceeds bound');
    const physicalStateRoot = physicalContainmentRoot(stateRoot);
    let total = 0;
    return Object.freeze(paths.map((path) => {
        if (!isInside(stateRoot, path) || !isInside(physicalStateRoot, physicalContainmentRoot(path)))
            failure('invalid', 'migration input path escapes isolated state root physically or lexically', [path]);
        const relativePath = relative(stateRoot, path).split(sep).join('/');
        if (!existsSync(path))
            return { source_path: path, relative_path: relativePath, exists: false, size_bytes: 0, sha256: digest(new TextEncoder().encode('<missing>')) };
        const bytes = readBounded(path);
        total += bytes.byteLength;
        if (total > COORDINATION_MIGRATION_MAX_TOTAL_BYTES)
            failure('invalid', 'migration aggregate input bytes exceed bound');
        return { source_path: path, relative_path: relativePath, exists: true, size_bytes: bytes.byteLength, sha256: digest(bytes) };
    }));
}
function aggregateDigest(entries, gitSnapshot) {
...
function recheckGitSnapshot(expected, rows, repository) {
...
function inspectGit(rows, ledgerTerminalized) {
    ...
    for (const row of rows) {
        ...
        try {
            if (realpathSync(top) !== realpathSync(row.source_repo))
                blockers.push(`source repository canonical root mismatch: ${row.workstream_run}`);
            const resolvedCommon = isAbsolute(common) ? common : resolve(row.source_repo, common);
            if (realpathSync(resolvedCommon) !== realpathSync(row.git_common_dir))
                blockers.push(`Git common-dir mismatch: ${row.workstream_run}`);
        }
        catch {
...
function sqlText(row, field, label, maximum = 2048) {
    ...
    if (typeof value !== 'string' || value.length === 0 || value.length > maximum || value.includes('\u0000'))
        failure('blocked', `${label}.${field} is not bounded text`);
    return value;
}
function sqlSafeInteger(row, field, label) {
    const value = row[field];
    if (typeof value !== 'number' || !Number.isSafeInteger(value))
        failure('blocked', `${label}.${field} is not a safe integer`);
    return value;
}
function boundedDatabaseRows(database, table, sql, parameters = []) {
    const countRow = database.prepare(`SELECT COUNT(*) AS row_count FROM ${table}`).get();
    if (countRow === undefined || sqlSafeInteger(countRow, 'row_count', table) > COORDINATION_MIGRATION_MAX_DATABASE_ROWS)
        failure('blocked', `coordinator table ${table} exceeds the migration inspection row bound`);
    const rows = database.prepare(sql).all(...parameters);
    if (rows.length > COORDINATION_MIGRATION_MAX_DATABASE_ROWS)
        failure('blocked', `coordinator query for ${table} exceeds the migration inspection row bound`);
    return Object.freeze(rows);
}
function parseDatabasePayload(row, field, label) {
    const text = sqlText(row, field, label, COORDINATION_MIGRATION_MAX_DATABASE_JSON_BYTES);
    if (Buffer.byteLength(text, 'utf8') > COORDINATION_MIGRATION_MAX_DATABASE_JSON_BYTES)
        failure('blocked', `${label}.${field} exceeds the migration JSON byte bound`);
    let parsed;
    try {
        assertNoDuplicateJsonKeys(text, `${label}.${field}`);
    ...
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))
        failure('blocked', `${label}.${field} must be a JSON object`);
    return parsed;
}
function assertExactTableColumns(database, table, expected) {
    const rows = database.prepare(`PRAGMA table_info(${table})`).all();
    if (rows.length > 64)
        failure('blocked', `coordinator table ${table} has too many columns`);
    const actual = rows.map((row) => sqlText(row, 'name', `${table} column`, 128));
    if (stableJson(actual) !== stableJson(expected))
        failure('blocked', `coordinator table ${table} does not match its exact schema profile`, [`expected=${stableJson(expected)}`, `actual=${stableJson(actual)}`]);
}
function assertReadOnlySchema(database) {
    ...
    const schemaRows = database.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all();
    ...
    if (version >= 10)
        assertExactTableColumns(database, 'observations', ['entity_id', 'repo_id', 'workstream_run', 'acquisition_group_id', 'payload_json', 'execution_state', 'freshness', 'version']);
    if (version >= 12) {
...
function coordinatorDatabaseSourceIdentity(path) {
    if (!existsSync(path))
        return { path, exists: false, dev: 0, ino: 0, size: 0, mtime_ms: 0, ctime_ms: 0, sha256: digest(new TextEncoder().encode('<missing>')) };
    ...
    if (bytes.byteLength !== info.size || !sameFileIdentity(info, after) || after.size !== info.size || after.mtimeMs !== info.mtimeMs || after.ctimeMs !== info.ctimeMs)
        failure('blocked', 'coordinator database snapshot source identity changed during hashing', [path]);
...
function coordinatorDatabaseSourceIdentities(databasePath) {
    ...
    if (total > COORDINATION_MIGRATION_MAX_TOTAL_BYTES)
        failure('blocked', 'coordinator database snapshot exceeds its aggregate byte ceiling', [String(total)]);
    return identities;
}
function assertCoordinatorDatabaseSourceStable(expected) {
    ...
    if (stableJson(actual) !== stableJson(expected))
        failure('blocked', 'coordinator database/WAL/SHM identity or bytes changed during copied inspection', [`expected=${stableJson(expected)}`, `actual=${stableJson(actual)}`]);
}
/**
    ...
    try {
        for (const entry of source) {
            ...
            if (copied.size !== entry.size || copied.sha256 !== entry.sha256)
                failure('blocked', 'coordinator database snapshot copy does not match its source bytes', [entry.path, target]);
        }
...
function openImmutableCoordinatorDatabase(path) {
    const walPath = `${path}-wal`;
    if (existsSync(walPath) && statSync(walPath).size > 0)
        failure('blocked', 'immutable coordinator inspection refuses an uncheckpointed WAL', [walPath]);
...
function inspectCoordinatorReadOnly(paths, repoKey, writerAuthorityAcquired = false) {
    ...
    let database;
    try {
        database = new DatabaseSync(copied.databasePath, { readOnly: true, timeout: 1_000, enableForeignKeyConstraints: false });
    }
    catch (error) {
    ...
    try {
        ...
        if (schemaVersion >= 7) {
            ...
            if (migrationRows.length > 1)
                failure('blocked', 'coordinator migration record cardinality is invalid');
            ...
            if (migrationRow !== undefined) {
                ...
                if (!DIGEST.test(snapshot))
                    failure('blocked', 'coordinator migration snapshot digest is invalid');
                const migrationState = sqlText(migrationRow, 'state', 'coordination migration', 32);
                if (!['imported', 'verified', 'cutover-ready', 'cutover-committed', 'legacy-archived'].includes(migrationState))
                    failure('blocked', 'coordinator migration state is invalid');
                migration = { migration_id: sqlText(migrationRow, 'migration_id', 'coordination migration', 192), snapshot_sha256: snapshot, state: migrationState, report: parseDatabasePayload(migrationRow, 'report_json', 'coordination migration') };
            }
        }
        ...
        catch { /* The disposable read-only connection is being closed fail-closed. */ }
        throw error;
    }
    finally {
        try {
...
function repositoryIdentityFromRoot(repoRoot, repoKey) {
    if (!isAbsolute(repoRoot) || !existsSync(repoRoot) || lstatSync(repoRoot).isSymbolicLink())
        failure('blocked', 'migration repository root must be an existing canonical non-symbolic directory', [repoRoot]);
    const identity = resolveRepoIdentity(repoRoot);
    if (identity.repoKey !== repoKey)
        failure('blocked', 'canonical --repo-root does not match the requested repository key', [repoRoot, `expected=${repoKey}`, `actual=${identity.repoKey}`]);
    return { canonical_root: realpathSync(identity.repoRoot), git_common_dir: realpathSync(identity.gitCommonDir) };
}
function resolveMigrationRepositoryIdentity(paths, repoKey, repoRoot, coordinator, rows) {
    const candidates = [];
    if (repoRoot !== undefined)
        candidates.push(repositoryIdentityFromRoot(repoRoot, repoKey));
    ...
    if (legacyRows.length > 0) {
        ...
        if (first === undefined)
            failure('blocked', 'legacy repository identity is unavailable');
    ...
    for (const row of coordinator?.repositories ?? []) {
        if (row.repo_id !== repoKey || row.repo_key !== repoKey)
            failure('blocked', 'coordinator repository identity does not match migration key');
        candidates.push({ canonical_root: realpathSync(row.canonical_root), git_common_dir: realpathSync(row.git_common_dir) });
    }
    const first = candidates[0];
    if (first === undefined)
        failure('blocked', 'empty legacy migration requires canonical --repo-root or an existing coordinator repository identity');
    for (const candidate of candidates.slice(1))
        if (candidate.canonical_root !== first.canonical_root || candidate.git_common_dir !== first.git_common_dir)
            failure('blocked', 'legacy, explicit, and coordinator repository identities disagree', [stableJson(first), stableJson(candidate)]);
    const verified = repositoryIdentityFromRoot(first.canonical_root, repoKey);
    if (verified.git_common_dir !== first.git_common_dir)
        failure('blocked', 'migration repository Git common-dir identity changed', [first.git_common_dir, verified.git_common_dir]);
    return verified;
}
function inspectLegacy(paths, repoKey, repository) {
    ...
    for (const claim of claims) {
        ...
        if (claim.active_run_epoch !== owner.active_run_epoch)
            reboundCount += 1;
        ...
        let mergeExactGitObjects = [];
        let mergeFilesystemPostconditions = [];
        if (merge !== undefined) {
...
function reconcileMixedCoordinatorAuthority(coordinator, inspection) {
    ...
    for (const claim of inspection.claims) {
        if (inspection.terminalLeakKeys.has(claimKey(claim)))
            continue;
        ...
        if (conflicting.length > 0)
            blockers.push(`legacy claim conflicts with non-equivalent coordinator authority and cannot be discarded: ${claim.workstream_run}:${claim.unit_id}:${String(claim.attempt)}:${claim.path}`);
    }
...
function readFreezeAcknowledgements(stateRoot, migrationPaths) {
    assertMigrationPathSafe(stateRoot, migrationPaths.freezeAckRoot, 'migration freeze acknowledgement root');
    if (!existsSync(migrationPaths.freezeAckRoot))
        return [];
    ...
    for (const entry of readdirSync(migrationPaths.freezeAckRoot, { withFileTypes: true })) {
        ...
        if (entry.isSymbolicLink() || !entry.isFile())
            failure('blocked', 'freeze acknowledgement root contains a non-regular entry', [path]);
        ...
        if (row['schema_version'] !== COORDINATION_FREEZE_ACK_SCHEMA || row['client_kind'] !== 'legacy-package-client' || row['drain_state'] !== 'dispatch-stopped' || row['critical_section'] !== null || typeof row['pid'] !== 'number' || !Number.isSafeInteger(row['pid']) || typeof row['boot_id'] !== 'string' || typeof row['acknowledged_at'] !== 'string')
            failure('blocked', 'freeze acknowledgement has an invalid closed contract', [path]);
...
function legacyDrainBlockers(paths, migrationPaths, journal, rows) {
    const acknowledgements = journal === null ? [] : readFreezeAcknowledgements(paths.stateRoot, migrationPaths);
    const blockers = [];
    for (const row of rows) {
        if (row.status === 'closed' || row.boot_id !== currentBootId() || !isProcessAlive(row.pid))
            continue;
        if (journal === null) {
            blockers.push(`reachable legacy client requires bounded freeze acknowledgement and drain: ${row.workstream_run} pid=${String(row.pid)}`);
            continue;
        }
        ...
        if (exact === undefined) {
            blockers.push(`reachable legacy client has not durably acknowledged the bounded migration freeze: ${row.workstream_run} pid=${String(row.pid)}`);
            continue;
        }
        if (exact.package_build !== COORDINATOR_PACKAGE_BUILD || exact.protocol_version !== AUTOPILOT_COORDINATOR_PROTOCOL_VERSION || exact.database_schema_version !== COORDINATOR_DATABASE_SCHEMA_VERSION || exact.drain_state !== 'dispatch-stopped' || exact.critical_section !== null)
            blockers.push(`reachable legacy client is incompatible with migration freeze/drain protocol: ${row.workstream_run} pid=${String(row.pid)}`);
    }
    return Object.freeze(blockers.sort());
}
...
function buildImportPlan(inspection, repositoryIdentity, repoKey, migrationId, snapshotSha, journalPath, report) {
    ...
    const terminalReleases = inspection.claims.filter((claim) => inspection.terminalLeakKeys.has(claimKey(claim))).map((claim) => {
        ...
        if (row === undefined || audit === undefined || typeof audit.payload['evidence_ref'] !== 'string' || typeof audit.payload['evidence_sha256'] !== 'string' || !DIGEST.test(audit.payload['evidence_sha256']))
            failure('blocked', 'terminal claim release lacks exact retained migration evidence identity', [claimKey(claim)]);
        return { owner: { repo_id: repoKey, autopilot_id: row.autopilot_id, workstream_run: row.workstream_run, unit_id: claim.unit_id, attempt: claim.attempt }, path: claim.path, mode: claim.claim_type, evidence_ref: audit.payload['evidence_ref'], evidence_sha256: audit.payload['evidence_sha256'] };
    });
    ...
    for (const [key, claims] of grouped) {
        ...
        if (row === undefined)
            failure('blocked', 'live migration claim has no valid durable owner', [key]);
...
function baseReport(command, repoKey, inspection, now, state, migrationId, snapshot, backupPath, markerPath) {
    const liveClaims = inspection.claims.filter((claim) => !inspection.terminalLeakKeys.has(claimKey(claim)));
...
function parseMigrationReport(value) {
    ...
    const commands = ['dry-run', 'apply', 'verify', 'rollback', 'cutover'];
    const states = ['planned', 'frozen', 'snapshotted', 'imported', 'verified', 'cutover-ready', 'cutover-committed', 'legacy-archived', 'rollback-restoring', 'rollback-restored', 'rollback-unfreezing', 'rolled-back'];
    if (row['schema_version'] !== 'autopilot.coordination_migration_report.v1' || !commands.includes(command) || !states.includes(state) || typeof row['dry_run'] !== 'boolean')
        failure('invalid', 'migration report discriminator is invalid');
    const nullableText = (field) => row[field] === null ? null : boundedString(row[field], `migration report ${field}`);
    const snapshot = row['snapshot_sha256'];
    if (snapshot !== null && (typeof snapshot !== 'string' || !DIGEST.test(snapshot)))
        failure('invalid', 'migration report snapshot digest is invalid');
    const blockers = stringArray(row['blockers'], 'migration report blockers', 100_000);
    if (!Array.isArray(row['recovery']) || row['recovery'].length > 100_000)
        failure('invalid', 'migration report recovery must be a bounded array');
...
function journalRecord(value, path) {
    ...
    if (row['schema_version'] !== COORDINATION_MIGRATION_JOURNAL_SCHEMA)
        failure('invalid', 'migration journal schema is invalid', [path]);
    const state = boundedString(row['state'], 'migration journal state', 32);
    const states = ['planned', 'frozen', 'snapshotted', 'imported', 'verified', 'cutover-ready', 'cutover-committed', 'legacy-archived', 'rollback-restoring', 'rollback-restored', 'rollback-unfreezing', 'rolled-back'];
    if (!states.includes(state))
        failure('invalid', 'migration journal state is invalid');
    const snapshot = row['snapshot_sha256'];
    const backupSha = row['backup_sha256'];
    if (snapshot !== null && (typeof snapshot !== 'string' || !DIGEST.test(snapshot)))
        failure('invalid', 'migration journal snapshot digest is invalid');
    if (backupSha !== null && (typeof backupSha !== 'string' || !DIGEST.test(backupSha)))
        failure('invalid', 'migration journal backup digest is invalid');
    if (!Array.isArray(row['snapshot_entries']) || row['snapshot_entries'].length > COORDINATION_MIGRATION_MAX_FILES || !Array.isArray(row['completed_effects']) || row['completed_effects'].some((entry) => typeof entry !== 'string'))
        failure('invalid', 'migration journal arrays are invalid');
    ...
    if (!ID.test(migrationId) || !ID.test(repoKey) || !/^[a-f0-9]{64}$/u.test(freezeToken))
        failure('invalid', 'migration journal identity or freeze token is invalid');
    if (report.migration_id !== migrationId || report.repo_key !== repoKey || report.state !== state || report.snapshot_sha256 !== snapshot || report.backup_path !== (row['backup_path'] === null ? null : row['backup_path']))
        failure('invalid', 'migration journal report disagrees with its durable envelope');
    const frozenEntries = Object.freeze(row['snapshot_entries'].map((entry, index) => parseSnapshotEntry(entry, index)));
    if (!Array.isArray(row['git_snapshot']) || row['git_snapshot'].length > 100_000)
        failure('invalid', 'migration journal Git snapshot is invalid');
    ...
    for (const entry of frozenEntries) {
        ...
        if (isAbsolute(normalizedRelative) || normalizedRelative === '..' || normalizedRelative.startsWith(`..${sep}`) || relativePaths.has(entry.relative_path))
            failure('invalid', 'migration journal snapshot path is escaped or duplicated', [entry.relative_path]);
        relativePaths.add(entry.relative_path);
        const derived = resolve(stateRoot, normalizedRelative);
        if (resolve(entry.source_path) !== derived || !isInside(stateRoot, derived))
            failure('invalid', 'migration journal source path is not derived from its state-root relative path', [entry.source_path, entry.relative_path]);
    }
    if (snapshot !== null && aggregateDigest(frozenEntries, gitSnapshot) !== snapshot)
        failure('invalid', 'migration journal snapshot digest does not bind its parsed entries and Git snapshot');
    ...
    if (!isAbsolute(repositoryRoot) || !isAbsolute(repositoryGitCommonDir))
        failure('invalid', 'migration journal repository identity must be absolute');
    return { schema_version: COORDINATION_MIGRATION_JOURNAL_SCHEMA, migration_id: migrationId, repo_key: repoKey, state: state, freeze_token: freezeToken, created_at: boundedString(row['created_at'], 'created_at', 64), updated_at: boundedString(row['updated_at'], 'updated_at', 64), snapshot_sha256: snapshot, snapshot_entries: frozenEntries, git_snapshot: gitSnapshot, backup_path: row['backup_path'] === null ? null : boundedString(row['backup_path'], 'backup path'), backup_sha256: backupSha, database_existed_before: typeof row['database_existed_before'] === 'boolean' ? row['database_existed_before'] : failure('invalid', 'database_existed_before must be boolean'), repository_root: resolve(repositoryRoot), repository_git_common_dir: resolve(repositoryGitCommonDir), completed_effects: frozenEffects, report };
}
function parseSnapshotEntry(value, index) {
    ...
    if (!DIGEST.test(sha) || typeof row['exists'] !== 'boolean')
        failure('invalid', 'snapshot entry digest or existence flag is invalid');
    return { source_path: boundedString(row['source_path'], 'snapshot source path'), relative_path: boundedString(row['relative_path'], 'snapshot relative path'), exists: row['exists'], size_bytes: integer(row['size_bytes'], 'snapshot size'), sha256: sha };
}
...
async function atomicJson(stateRoot, path, value, mode = 0o600) {
    ...
    if (existsSync(path) && lstatSync(path).isSymbolicLink())
        failure('blocked', 'migration atomic JSON destination is symbolic', [path]);
...
async function readJournal(stateRoot, path) {
    assertMigrationPathSafe(stateRoot, path, 'migration journal');
    if (!existsSync(path))
        return null;
    let parsed;
    try {
        parsed = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(readBounded(path, COORDINATION_MIGRATION_MAX_JSONL_LINE_BYTES)));
    }
    catch {
        failure('invalid', 'migration journal is unreadable', [path]);
    }
    return journalRecord(parsed, path);
}
async function writeJournal(stateRoot, path, journal) { await atomicJson(stateRoot, path, journal); }
function sameFileIdentity(left, right) {
    return left.dev === right.dev && left.ino === right.ino;
}
function parseMigrationLockBytes(bytes, label) {
    let parsed;
    try {
        const text = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
    ...
    if (row['schema_version'] !== 'autopilot.coordination_migration_lock.v1' || typeof row['pid'] !== 'number' || !Number.isSafeInteger(row['pid']) || row['pid'] < 1 || typeof row['boot_id'] !== 'string' || typeof row['token'] !== 'string' || !/^[a-f0-9]{48}$/u.test(row['token']) || typeof row['created_at'] !== 'string')
        failure('blocked', `${label} has an invalid closed identity; reclamation is refused`);
    return { pid: row['pid'], bootId: row['boot_id'], token: row['token'] };
}
function readMigrationLockBytes(path, label) {
    try {
        return readImmutableFileBytes({ path, maximumBytes: 65_536, label, errorCode: 'coordinator-contention', allowMultipleLinks: true });
    }
    catch (error) {
        failure('blocked', `${label} is not immutable and bounded: ${error instanceof Error ? error.message : String(error)}`, [path]);
    }
}
...
function assertRegularMigrationLockResidue(stateRoot, path, label) {
    assertMigrationPathSafe(stateRoot, path, label);
    const info = lstatSync(path);
    if (!info.isFile() || info.isSymbolicLink())
        failure('blocked', `${label} must be a regular non-symbolic file`, [path]);
    return info;
}
function unlinkExactMigrationLockResidue(path, identity, label) {
    if (!existsSync(path) || !sameFileIdentity(lstatSync(path), identity))
        failure('blocked', `${label} identity changed before cleanup`, [path]);
    unlinkSync(path);
}
/** Completes only identity-proved lock operations left by a dead process. */
function recoverMigrationLockResidues(stateRoot, path) {
    ...
    if (existsSync(reclaimPath)) {
        ...
        if (mainIdentity === null) {
            if (migrationLockOwnerAlive(reclaimOwner))
                failure('blocked', 'live migration reclamation residue lost its elected pathname', [reclaimPath]);
            for (const entry of readdirSync(directory, { withFileTypes: true })) {
                if (!entry.name.startsWith(`${name}.stale-${reclaimOwner.token}-`))
                    continue;
                ...
                if (!sameFileIdentity(staleIdentity, reclaimIdentity))
                    failure('blocked', 'stale migration quarantine residue disagrees with its reclaim identity', [stalePath, reclaimPath]);
            ...
            if (migrationLockOwnerAlive(reclaimOwner))
                failure('blocked', 'live migration reclaim residue conflicts with the elected lock identity', [path, reclaimPath]);
            unlinkExactMigrationLockResidue(reclaimPath, reclaimIdentity, 'stale migration reclaim residue');
            changed = true;
        }
    }
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
        if (!entry.name.startsWith(`${name}.release-`) && !entry.name.startsWith(`${name}.released-`) && !entry.name.startsWith(`${name}.candidate-`) && !entry.name.startsWith(`${name}.stale-`))
            continue;
        const residue = join(directory, entry.name);
        if (entry.isSymbolicLink() || !entry.isFile())
            failure('blocked', 'migration lock residue has an unsafe type', [residue]);
        ...
        if (entry.name.startsWith(`${name}.candidate-`)) {
            ...
            if (candidatePid === null || !Number.isSafeInteger(candidatePid) || candidatePid < 1 || !/^[a-f0-9]{48}$/u.test(candidateToken))
                failure('blocked', 'migration lock candidate residue name is invalid', [residue]);
        ...
        if (migrationLockOwnerAlive(owner)) {
            if (mainIdentity === null)
                failure('blocked', 'live migration lock operation lost its elected pathname', [residue]);
            if (!sameFileIdentity(mainIdentity, identity) && entry.name.startsWith(`${name}.release-`))
                failure('blocked', 'live migration release residue disagrees with the elected lock identity', [residue]);
            continue;
        }
        ...
        try {
            ...
            return { path, pid: process.pid, bootId: currentBootId(), token, release: async () => {
                    ...
                    if (!existsSync(path)) {
                        if (!existsSync(releasePath))
                            failure('blocked', 'migration lock elected pathname disappeared before release', [path]);
                        const residue = parseMigrationLockBytes(readMigrationLockBytes(releasePath, 'migration release fence'), 'migration release fence');
                        if (residue.token !== token)
                            failure('blocked', 'migration release fence belongs to another lock identity', [releasePath]);
                    ...
                    if (owner.token !== token)
                        failure('blocked', 'migration lock ownership changed before release; no lock was removed', [path]);
                    try {
                        linkSync(path, releasePath);
                    }
                    catch (error) {
                    ...
                    if (!sameFileIdentity(lockIdentity, releaseIdentity) || parseMigrationLockBytes(readMigrationLockBytes(releasePath, 'migration release fence'), 'migration release fence').token !== token)
                        failure('blocked', 'migration lock identity changed while release was fenced; no lock was removed', [path, releasePath]);
            ...
            if (existsSync(reclaimPath)) {
                ...
                if (!sameFileIdentity(lockIdentity, reclaimIdentity)) {
                    ...
                    if (migrationLockOwnerAlive(residueOwner))
                        failure('blocked', 'migration stale-lock reclamation is owned by a live process with a different identity', [reclaimPath]);
                    unlinkExactMigrationLockResidue(reclaimPath, reclaimIdentity, 'stale migration reclaim residue');
                    fsyncParentDirectory(path);
                    continue;
                }
                ...
                try {
                    ...
                    if (linkError instanceof Error && 'code' in linkError && (linkError.code === 'EEXIST' || linkError.code === 'ENOENT'))
                        continue;
                    throw linkError;
                }
                reclaimIdentity = assertRegularMigrationLockResidue(stateRoot, reclaimPath, 'repository migration lock reclamation fence');
            ...
            const lockIdentity = assertRegularMigrationLockResidue(stateRoot, path, 'repository migration lock before reclamation');
            if (!sameFileIdentity(lockIdentity, reclaimIdentity))
                failure('blocked', 'migration lock identity changed before stale-lock reclamation; no lock was removed', [path]);
            ...
            if (!sameFileIdentity(quarantinedIdentity, reclaimIdentity))
                failure('blocked', 'migration lock identity changed during stale-lock reclamation; quarantined identities were preserved', [path]);
...
async function createFreeze(stateRoot, path, repoKey, migrationId, token, now) {
    ...
    if (existsSync(path)) {
        ...
        try {
            ...
            failure('blocked', 'existing migration freeze is unreadable', [path]);
        }
        ...
        if (row['schema_version'] !== COORDINATION_FREEZE_SCHEMA || row['repo_key'] !== repoKey || row['migration_id'] !== migrationId || row['freeze_token'] !== token || row['required_package_build'] !== COORDINATOR_PACKAGE_BUILD || row['required_protocol_version'] !== AUTOPILOT_COORDINATOR_PROTOCOL_VERSION || row['required_database_schema_version'] !== COORDINATOR_DATABASE_SCHEMA_VERSION || row['dispatch'] !== 'stopped' || row['writer_policy'] !== 'fail-loudly')
            failure('blocked', 'existing migration freeze identity does not match the durable journal', [path]);
        return;
    }
...
async function copySnapshot(stateRoot, entries, root) {
    ...
    for (const entry of entries) {
        ...
        if (!isInside(root, target))
            failure('invalid', 'snapshot target escapes snapshot root', [target]);
        ...
        if (digest(readBounded(target)) !== entry.sha256)
            failure('blocked', 'snapshot copy hash verification failed', [entry.source_path]);
        if (platform() !== 'win32')
...
function recheckEntries(entries) {
    const drift = [];
    for (const entry of entries) {
        if (existsSync(entry.source_path) !== entry.exists) {
            drift.push(`${entry.relative_path}: existence changed`);
            continue;
        }
        if (!entry.exists)
            continue;
        const info = statSync(entry.source_path);
        if (!info.isFile() || info.size !== entry.size_bytes || digest(readBounded(entry.source_path)) !== entry.sha256)
            drift.push(`${entry.relative_path}: content hash changed`);
    }
...
export async function retireCoordinationMigrationCoordinator(paths, expectedIdentity) {
    ...
    try {
        ...
        return Object.freeze([`coordinator process-retirement dependency preflight failed: ${error instanceof Error ? error.message : String(error)}`]);
    }
    let lock;
    try {
        ...
        if (lockText === null)
            return [];
        const value = JSON.parse(lockText);
    ...
    try {
        ...
        return Object.freeze([`drained coordinator could not be retired: ${error instanceof Error ? error.message : String(error)}`]);
    }
    ...
    while (Date.now() < deadline) {
        ...
        if (!isProcessAlive(lock.pid)) {
            let election;
            try {
                election = acquireSerializedProcessGuard(paths.lifecycleElectionPath, 1_000, 'migration coordinator retirement');
            }
            catch (error) {
                return Object.freeze([`coordinator lifecycle retirement could not acquire serialized election: ${error instanceof Error ? error.message : String(error)}`]);
            }
            try {
                ...
                if (current === null || current.pid !== lock.pid || current.boot_id !== lock.boot_id || current.process_start_identity !== lock.process_start_identity || current.token !== lock.token || current.instance_id !== lock.instance_id || current.started_at !== lock.started_at)
                    return Object.freeze(['coordinator lifecycle identity changed while retiring for migration']);
 ...
 */
async function acquireMigrationStoreAuthority(paths, label) {
    ...
    try {
        ...
        if (currentText !== null) {
            ...
            if (current === null)
                failure('blocked', `${label} found an unreadable current coordinator lifecycle lock`, [paths.lockPath]);
            if (isProcessAlive(current.pid))
                failure('blocked', `${label} refuses direct store access while current coordinator pid ${String(current.pid)} is live`, [paths.lockPath]);
        ...
        if (predecessorText !== null) {
            ...
            if (predecessor === null)
                failure('blocked', `${label} found an unreadable predecessor coordinator lifecycle lock`, [paths.predecessorLockPath]);
            if (isProcessAlive(predecessor.pid))
                failure('blocked', `${label} refuses direct store access while predecessor coordinator pid ${String(predecessor.pid)} is live`, [paths.predecessorLockPath]);
        ...
        const releaseElection = () => { if (!electionReleased) {
        ...
        return {
            ...
            release: async () => {
                ...
                if (text === null)
                    failure('blocked', `${label} predecessor exclusion fence disappeared before release`, [paths.predecessorLockPath]);
                const observed = parsePredecessorCoordinatorLock(JSON.parse(text));
                if (observed === null || observed.pid !== fence.pid || observed.token !== fence.token || observed.started_at !== fence.started_at)
                    failure('blocked', `${label} predecessor exclusion fence changed ownership before release`, [paths.predecessorLockPath]);
        ...
        try {
            if (fenceCreated) {
                ...
                if (text !== null) {
                    const observed = parsePredecessorCoordinatorLock(JSON.parse(text));
                    if (observed === null || observed.pid !== fence.pid || observed.token !== fence.token || observed.started_at !== fence.started_at)
                        throw new CoordinationRuntimeError('system-fatal', `${label} failed and its predecessor exclusion fence changed ownership`, [paths.predecessorLockPath]);
                    await discardLockTombstone(await quarantineExactLock(paths.predecessorLockPath, text, `failed ${label} predecessor fence`));
                }
            }
            if (predecessorTombstone !== null)
                await restoreLockTombstone(paths.predecessorLockPath, predecessorTombstone, `pre-${label} predecessor lock`);
            if (currentTombstone !== null)
                await restoreLockTombstone(paths.lockPath, currentTombstone, `pre-${label} current lock`);
        }
        finally {
            election.release();
        }
...
async function withMigrationStoreAuthority(paths, label, operation) {
...
function verifyCoordinatorDatabaseFileReadOnly(path) {
    const database = openImmutableCoordinatorDatabase(path);
    try {
        return assertReadOnlySchema(database);
    }
    finally {
        database.close();
    }
...
async function restoreBackup(paths, journal) {
    ...
    if (!existsSync(journal.backup_path) || lstatSync(journal.backup_path).isSymbolicLink())
        failure('blocked', 'verified migration backup is unavailable');
    if (digest(readBoundedDatabase(journal.backup_path)) !== journal.backup_sha256)
    ...
    if (digest(readBoundedDatabase(temporary)) !== journal.backup_sha256)
    ...
    if (digest(readBoundedDatabase(paths.databasePath)) !== journal.backup_sha256)
        failure('blocked', 'migration rollback did not restore the byte-exact verified backup', [paths.databasePath]);
...
async function archiveLegacy(stateRoot, entries, migrationPaths) {
    const mutable = new Set([...TOP_LEVEL_COORDINATION_FILES.map((name) => `coordination/${name}`), ...TOP_LEVEL_WORKTREE_FILES.map((name) => `worktrees/${name}`)]);
    const archived = [];
    for (const entry of entries) {
        ...
        if (!mutable.has(key) || !entry.exists)
            continue;
        ...
        if (existsSync(entry.source_path)) {
            if (digest(readBounded(entry.source_path)) !== entry.sha256)
                failure('blocked', 'legacy source drifted during archive', [entry.source_path]);
            await rename(entry.source_path, target);
        }
        if (!existsSync(target) || digest(readBounded(target)) !== entry.sha256)
            failure('blocked', 'legacy archive verification failed', [target]);
        if (platform() !== 'win32')
            await chmod(target, 0o400);
        else
            await enforcePrivateAuthorityPath(target, false);
        if (platform() === 'win32') {
            ...
            if (readonly.status !== 0)
                failure('blocked', 'failed to set Windows read-only archive attribute', [target, readonly.stderr]);
        }
    ...
    if (existsSync(archiveManifestPath)) {
        const existing = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(readBounded(archiveManifestPath, COORDINATION_MIGRATION_MAX_JSONL_LINE_BYTES)));
        if (stableJson(existing) !== stableJson(archiveManifest))
            failure('blocked', 'legacy archive manifest disagrees with archived bytes', [archiveManifestPath]);
    }
    else
        await atomicJson(stateRoot, archiveManifestPath, archiveManifest, 0o400);
    if (platform() === 'win32') {
        const readonly = spawnSync('attrib', ['+R', archiveManifestPath], { encoding: 'utf8' });
        if (readonly.status !== 0)
            failure('blocked', 'failed to set Windows read-only archive manifest attribute', [archiveManifestPath, readonly.stderr]);
    }
    if (platform() !== 'win32' && existsSync(migrationPaths.archiveRoot))
        await sealArchiveDirectories(migrationPaths.archiveRoot);
}
async function sealArchiveDirectories(root) {
    const entries = await readdir(root, { withFileTypes: true });
    for (const entry of entries)
        if (entry.isDirectory())
            await sealArchiveDirectories(join(root, entry.name));
    await chmod(root, 0o500);
}
async function archiveRolledBackGeneration(stateRoot, migrationPaths, journal) {
    ...
    if (existsSync(migrationPaths.snapshotRoot) && !existsSync(historicSnapshot))
        await rename(migrationPaths.snapshotRoot, historicSnapshot);
    const historicJournal = join(historyRoot, 'journal.json');
    if (!existsSync(historicJournal))
        await rename(migrationPaths.journalPath, historicJournal);
    await rm(migrationPaths.freezePath, { force: true });
}
function nextJournal(journal, state, report, now, effects, patch = {}) {
...
export async function runCoordinationMigration(input) {
    if (!ID.test(input.repoKey))
        failure('invalid', 'migration repo key is invalid');
    ...
    if (input.command === 'dry-run') {
        ...
        try {
            ...
            if (drift.length > 0)
                failure('blocked', 'legacy source changed during dry-run inspection; rerun against a stable source', drift);
    ...
    try {
        ...
        for (const existingAuthorityRoot of [join(paths.stateRoot, 'coordination'), join(paths.stateRoot, 'worktrees')])
            if (existsSync(existingAuthorityRoot)) {
                if (platform() === 'win32')
                    enforceWindowsPrivateTree(existingAuthorityRoot, input.env ?? process.env);
                else
        ...
        if (marker !== null && (journal === null || journal.migration_id !== marker.migration_id || journal.snapshot_sha256 !== marker.snapshot_sha256))
            failure('blocked', 'cutover marker identity does not match the durable migration journal', [migrationPaths.cutoverMarkerPath]);
        if (marker !== null) {
            ...
            else if (migration.state !== 'cutover-committed' && migration.state !== 'legacy-archived')
                failure('blocked', 'cutover marker has an impossible forward database state', [migration.state]);
        }
        if (marker !== null && input.command === 'rollback')
            failure('blocked', 'rollback is forbidden after the one-way cutover marker; repair is forward-only', [migrationPaths.cutoverMarkerPath]);
        if (marker !== null && input.command !== 'cutover')
            failure('blocked', 'repository coordination cutover is already committed; legacy migration commands are no longer valid', [migrationPaths.cutoverMarkerPath]);
        if (journal === null) {
            if (input.command !== 'apply')
                failure('blocked', `${input.command} requires a prior migrate --apply journal`);
            const coordinator = inspectCoordinatorReadOnly(paths, input.repoKey);
            const repository = resolveMigrationRepositoryIdentity(paths, input.repoKey, input.repoRoot, coordinator);
            let inspection = inspectLegacy(paths, input.repoKey, repository);
            inspection = reconcileMixedCoordinatorAuthority(coordinator, inspection);
        ...
        if (journal.repo_key !== input.repoKey)
            failure('blocked', 'migration journal repository identity mismatch');
        const repository = { canonical_root: journal.repository_root, git_common_dir: journal.repository_git_common_dir };
        ...
        if (input.command === 'apply') {
            ...
            if (journal.state === 'rollback-restoring' || journal.state === 'rollback-restored' || journal.state === 'rollback-unfreezing')
                failure('blocked', 'apply is forbidden while durable rollback is incomplete; resume rollback first');
            if (journal.state === 'cutover-committed' || journal.state === 'legacy-archived')
                failure('blocked', 'apply is forbidden after cutover');
            ...
            journal = nextJournal(journal, 'frozen', report, now, ['freeze-written-before-store-authority']);
            await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
            await input.afterBoundary?.('after-freeze');
            if (blockers.length > 0)
                return report;
            ...
            journal = nextJournal(journal, 'frozen', report, now, ['freeze-drain-complete', 'single-writer-authority-acquired']);
            await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
            await input.afterBoundary?.('after-writer-authority');
            if (blockers.length > 0)
                return report;
            if (journal.snapshot_sha256 !== null && journal.backup_path !== null && journal.backup_sha256 !== null && coordinator?.migration !== null && coordinator?.migration !== undefined) {
                ...
                if (imported.migration_id !== journal.migration_id || imported.snapshot_sha256 !== journal.snapshot_sha256)
                    failure('blocked', 'database migration effect disagrees with the durable journal');
                const importedReport = { ...parseMigrationReport(imported.report), command: 'apply', state: 'imported', dry_run: false };
            ...
            journal = nextJournal(journal, 'snapshotted', report, now, ['snapshot-copied-and-hashed'], { snapshot_sha256: snapshotSha, snapshot_entries: inspection.sourceEntries, git_snapshot: inspection.gitSnapshot });
            await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
            await input.afterBoundary?.('after-snapshot');
            const drift = recheckEntries(journal.snapshot_entries);
            if (drift.length > 0)
                failure('blocked', 'legacy source drifted after snapshot; migration remains frozen', drift);
            ...
            const backupResult = await withMigrationStoreAuthority(paths, 'migration apply backup', async () => {
                if (backupJournal.backup_path !== null && backupJournal.backup_sha256 !== null) {
                    if (backupJournal.backup_path !== backupPath || !existsSync(backupPath) || digest(readBoundedDatabase(backupPath)) !== backupJournal.backup_sha256)
                        failure('blocked', 'durable pre-import backup is missing or changed', [backupPath]);
            ...
            journal = nextJournal(journal, 'snapshotted', report, now, ['database-backup-verified'], { backup_path: backupResult.path, backup_sha256: backupResult.sha256 });
            await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
            await input.afterBoundary?.('after-backup');
            ...
            if (secondDrift.length > 0 || aggregateDigest(inspection.sourceEntries, inspection.gitSnapshot) !== snapshotSha)
                failure('blocked', 'legacy source drifted before transactional import; migration remains frozen', secondDrift);
            ...
            journal = nextJournal(journal, 'imported', committedReport, now, ['transactional-import-committed'], { snapshot_sha256: snapshotSha, snapshot_entries: inspection.sourceEntries, git_snapshot: inspection.gitSnapshot, backup_path: backupResult.path, backup_sha256: backupResult.sha256 });
            await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
            await input.afterBoundary?.('after-import');
        ...
        if (input.command === 'rollback') {
            if (journal.state === 'rolled-back')
                return journal.report;
            if (journal.state !== 'rollback-restoring' && journal.state !== 'rollback-restored' && journal.state !== 'rollback-unfreezing') {
                const restoringReport = { ...journal.report, command: 'rollback', state: 'rollback-restoring', dry_run: false, created_at: now.toISOString() };
            ...
            if (journal.state === 'rollback-restoring') {
                ...
                if (restoringJournal.backup_path !== null)
                    await withMigrationStoreAuthority(paths, 'migration rollback restore', async () => { await restorePreImportBoundary(paths, restoringJournal); });
                await input.afterBoundary?.('after-rollback-restore-before-journal');
                const restoredReport = { ...journal.report, command: 'rollback', state: 'rollback-restored', dry_run: false, created_at: now.toISOString() };
            ...
            if (journal.state === 'rollback-restored') {
                ...
                if (drift.length > 0)
                    failure('blocked', 'database boundary is restored but legacy or Git state drifted while frozen; freeze remains active', drift);
                const unfreezingReport = { ...journal.report, command: 'rollback', state: 'rollback-unfreezing', dry_run: false, created_at: now.toISOString() };
            ...
            const report = { ...journal.report, command: 'rollback', state: 'rolled-back', dry_run: false, created_at: now.toISOString() };
        ...
        if (input.command === 'verify') {
            if (journal.state !== 'imported' && journal.state !== 'verified' && journal.state !== 'cutover-ready')
                failure('blocked', 'verify requires a completed transactional import');
            ...
            if (drift.length > 0) {
                const restoreIntentReport = { ...journal.report, command: 'verify', state: 'frozen', created_at: now.toISOString() };
                ...
                const report = { ...journal.report, command: 'verify', state: 'frozen', snapshot_sha256: null, created_at: now.toISOString() };
            ...
            if (fsBlockers.length > 0)
                failure('blocked', 'filesystem/Git verification failed', fsBlockers);
            ...
            return await withMigrationStoreAuthority(paths, 'migration verify', async () => {
                ...
                try {
                    ...
                    const verifiedReport = { ...verifyJournal.report, command: 'verify', state: 'verified', dry_run: false, created_at: now.toISOString() };
                    ...
                    await writeJournal(paths.stateRoot, migrationPaths.journalPath, verifiedJournal);
                    await input.afterBoundary?.('after-verified');
                    const readyReport = { ...verifiedReport, state: 'cutover-ready' };
                    ...
                    await writeJournal(paths.stateRoot, migrationPaths.journalPath, readyJournal);
                    await input.afterBoundary?.('after-cutover-ready');
                    return readyReport;
                }
                finally {
                    store.close();
        ...
        if (journal.state !== 'cutover-ready' && journal.state !== 'cutover-committed')
            failure('blocked', 'cutover requires verified cutover-ready state');
        ...
        try {
            if (marker === null) {
                ...
                const drift = Object.freeze([...recheckEntries(journal.snapshot_entries), ...recheckGitSnapshot(journal.git_snapshot, driftInspection.rows, repository)]);
                if (drift.length > 0)
                    failure('blocked', 'legacy source or Git state drift rejected cutover', drift);
                const store = await CoordinatorStore.open(paths, clock, { allowExistingSchemaMigration: true });
                let databaseSha;
                try {
                    store.verifyMigrationImport(input.repoKey, journal.migration_id);
                    databaseSha = store.databaseDigest();
                }
                finally {
                    store.close();
                }
                if (journal.snapshot_sha256 === null)
                    failure('blocked', 'cutover-ready journal lacks snapshot digest');
                const cutoverMarker = { schema_version: COORDINATION_CUTOVER_MARKER_SCHEMA, repo_key: input.repoKey, snapshot_sha256: journal.snapshot_sha256, database_sha256: databaseSha, committed_at: now.toISOString(), migration_id: journal.migration_id };
                ...
                const committedReport = { ...journal.report, command: 'cutover', state: 'cutover-committed', dry_run: false, cutover_marker_path: migrationPaths.cutoverMarkerPath, created_at: now.toISOString() };
                journal = nextJournal(journal, 'cutover-committed', committedReport, now, ['source-hashes-rechecked-before-cutover', 'cutover-marker-committed']);
                await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
                await input.afterBoundary?.('after-cutover-marker');
                const recordStore = await CoordinatorStore.open(paths, clock, { allowExistingSchemaMigration: true });
                try {
                    recordStore.updateMigrationState(input.repoKey, journal.migration_id, 'cutover-committed', committedReport);
                }
                finally {
                    recordStore.close();
                }
        ...
        try {
            ...
            if (doctor.payload['integrity'] !== 'ok' || doctor.payload['healthy'] !== true)
                failure('blocked', 'post-cutover coordinator/client health verification failed');
            ...
            if (!Array.isArray(repositories) || repositories.length !== 1 || !Array.isArray(runs) || !Array.isArray(resources) || resources.length !== runs.length)
                failure('blocked', 'post-cutover client cannot observe exact repository/run-resource identity');
        }
        finally {
            await healthServer.close();
        }
        journal = nextJournal(journal, 'legacy-archived', finalReport, now, ['runtime-projections-rebound', 'legacy-files-archived-read-only', 'post-cutover-health-verified', 'migration-freeze-removal-pending']);
        await writeJournal(paths.stateRoot, migrationPaths.journalPath, journal);
        await input.afterBoundary?.('after-legacy-archive');
        await rm(migrationPaths.freezePath, { force: true });
        fsyncParentDirectory(migrationPaths.freezePath);
