...
export class OwnedWorktreeSagaClient {
    ...
    async prepare(spec) {
        ...
        if (spec.operationId === undefined && exactExisting === undefined && semanticExisting.length > 1)
        ...
        if (existing !== undefined) {
            ...
            if (!sameOwner(existing.owner, owner) || existingProjection === undefined || existingProjection.kind !== spec.kind || !sameWorktreeAuthority(existingProjection, proposed) || existing.operation_type !== spec.operationType || canonicalJson(existing.intent) !== canonicalJson(spec.intent))
                throw new CoordinationRuntimeError('idempotency-conflict', 'worktree operation key was reused with different canonical authority or immutable intent');
            return { operation: existing, worktree, replayed: true };
        }
        if (spec.operationId !== undefined)
            throw new CoordinationRuntimeError('recovery-required', 'an explicit historical operation ID has no durable operation to recover; S1 will not mint caller-selected operation identity', [spec.operationId, canonicalOperationId]);
        const operation = {
