...
export class OwnedWorktreeSagaClient {
    ...
    async prepare(spec) {
        ...
        if (existing !== undefined && operationWorktree === undefined)
            throw new CoordinationRuntimeError('store-corrupt', 'existing worktree operation has no exact historical projection', [existing.operation_id, existing.worktree_id]);
