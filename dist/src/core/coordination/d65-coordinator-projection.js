...
import { parseCoordinationAcquisitionGroup, parseCoordinationAuthoritativeArtifact, parseCoordinationChangeReservation, parseCoordinationEditLease, parseCoordinationOwnerIdentity, parseCoordinationRun, parseCoordinationRunResource, parseCoordinationRunTerminalIntent, parseCoordinationUnitAttempt, parseCoordinationWorktree, parseCoordinationWorktreeOperation, } from "./contracts.js";
import { AUTOPILOT_RUN_SCOPED_FAULT_SCHEMA, parseRunScopedLogicalFault } from "./logical-faults.js";
import { array, integer, nullableSha256Field, object, sha256Field, str, } from "./d65-semantic-graph.js";
...
export function d65AttemptEntityId(owner) {
...
export function d65CoordinatorProjectionIdentity(kind, naturalIdentity) {
    const identity = SINGLETON_KINDS.has(kind)
        ? (() => {
            if (naturalIdentity !== undefined)
                fail('a singleton coordinator projection identity cannot carry a natural identity', [kind]);
            return `cp:${kind}`;
        })()
        : (() => {
            if (naturalIdentity === undefined || !IDENTIFIER.test(naturalIdentity))
                fail('a row coordinator projection requires one complete bounded natural identity', [kind, String(naturalIdentity)]);
            return `cp:${kind}:${sha256Hex(naturalIdentity)}`;
        })();
    if (!IDENTIFIER.test(identity) || Buffer.byteLength(identity, 'utf8') > 192)
        fail('encoded coordinator projection identity exceeds the frozen identifier grammar', [kind, identity]);
    return identity;
}
function semanticVersion(rawVersion, pureLeaseEventCount, label) {
    if (!Number.isSafeInteger(rawVersion) || rawVersion < 1 || !Number.isSafeInteger(pureLeaseEventCount) || pureLeaseEventCount < 0)
        fail(`${label} semantic version operands must be nonnegative safe integers`);
    const projected = rawVersion - pureLeaseEventCount;
    if (!Number.isSafeInteger(projected) || projected < 0)
        fail(`${label} semantic_version subtraction underflowed`, [`raw=${String(rawVersion)}`, `pure=${String(pureLeaseEventCount)}`]);
    return projected;
}
export function projectD65SessionLease(session, pureLeaseEventCount) {
...
export function projectD65ChildLease(child, pureLeaseEventCount) {
    return Object.freeze({
...
export function buildD65CoordinatorProjectionMembers(snapshot) {
    if (snapshot.run.repo_id !== snapshot.resource.repo_id || snapshot.run.workstream_run !== snapshot.resource.workstream_run)
        fail('run and resource singleton identities disagree');
    if (snapshot.run.version !== snapshot.run_version)
        fail('run_version singleton does not equal the run row version');
    ...
    if (terminalCurrent.length > 1)
        fail('more than one prepared/committed terminal intent exists in the snapshot');
    const derivedCurrent = terminalCurrent[0]?.terminal_intent_id ?? null;
    if (snapshot.current_terminal_intent_id !== derivedCurrent)
        fail('current_terminal_intent_id does not select the sole prepared/committed terminal intent', [String(snapshot.current_terminal_intent_id), String(derivedCurrent)]);
    const members = [
    ...
    for (let index = 1; index < members.length; index += 1)
        if (members[index - 1]?.identity === members[index]?.identity)
            fail('duplicate logical coordinator projection identity', [members[index]?.identity ?? '']);
    return Object.freeze(members);
}
function exactString(record, field, label) {
    const value = str(record, field, label, 192);
    if (!IDENTIFIER.test(value))
        fail(`${label}.${field} is not a bounded identifier`);
    return value;
}
function parseSemanticSession(value) {
    ...
    if (record['schema_version'] !== 'autopilot.session_lease.v2')
        fail(`${label}.schema_version is invalid`);
    const attachment = record['attachment_kind'];
    if (attachment !== 'dispatch' && attachment !== 'terminal-recovery' && attachment !== 'migration-recovery')
        fail(`${label}.attachment_kind is invalid`);
    const status = record['status'];
    if (status !== 'attached' && status !== 'handoff-pending' && status !== 'detached' && status !== 'fenced' && status !== 'expired')
...
function parseTerminalEvidence(value, label) {
    if (value === null)
        return null;
...
function parseSemanticChild(value) {
    ...
    if (record['schema_version'] !== 'autopilot.child_lease.v1')
        fail(`${label}.schema_version is invalid`);
    const status = record['status'];
    if (status !== 'running' && status !== 'terminal' && status !== 'recovery-required')
        fail(`${label}.status is invalid`);
...
function parseConsumedProbe(value) {
    if (value === null)
        return null;
...
function parseIntent(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        fail('terminal-intent value must be an object');
    const schema = value['schema_version'];
    if (schema === 'autopilot.run_terminal_intent.v2')
        return parseD65RunTerminalIntentV2(value);
    return parseCoordinationRunTerminalIntent(value);
}
function ensureRunScope(repoId, workstreamRun, actualRepo, actualRun, label) {
    if (actualRepo !== repoId || actualRun !== workstreamRun)
        fail(`${label} belongs to a different graph run`, [actualRepo, actualRun, repoId, workstreamRun]);
}
/** Parse, identity-bind, group, and reconstruct the exact frozen object. */
export function reconstructD65CoordinatorProjection(members, expected) {
    const byKind = new Map();
    const identities = new Set();
    for (const raw of members) {
        if (!D65_COORDINATOR_PROJECTION_KINDS.includes(raw.kind))
            fail('coordinator projection entry has an unknown kind', [raw.kind, raw.identity]);
        const kind = raw.kind;
        if (identities.has(raw.identity))
            fail('coordinator projection has a duplicate entry identity', [raw.identity]);
        identities.add(raw.identity);
        const list = byKind.get(kind) ?? [];
        list.push({ identity: raw.identity, kind, value: raw.value });
        byKind.set(kind, list);
    }
    const singleton = (kind) => {
        const list = byKind.get(kind) ?? [];
        if (list.length !== 1)
            fail('coordinator projection singleton cardinality is not exactly one', [kind, `count=${String(list.length)}`]);
        const member = list[0];
        if (member === undefined || member.identity !== d65CoordinatorProjectionIdentity(kind))
            fail('coordinator projection singleton has a wrong/aliased identity', [kind, member?.identity ?? 'missing']);
        return member;
    };
    const rows = (kind, parse, natural) => {
        const parsed = (byKind.get(kind) ?? []).map((member) => {
            ...
            if (member.identity !== encoded)
                fail('coordinator projection row identity does not bind its complete embedded natural identity', [kind, member.identity, encoded, naturalIdentity]);
        ...
        for (let index = 1; index < parsed.length; index += 1)
            if (parsed[index - 1]?.naturalIdentity === parsed[index]?.naturalIdentity)
                fail('duplicate logical coordinator projection natural identity', [kind, parsed[index]?.naturalIdentity ?? '']);
    ...
    for (const row of sessions)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.repo_id, row.workstream_run, 'session');
    const children = rows('child', parseSemanticChild, (row) => row.child_lease_id);
    for (const row of children)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.owner.repo_id, row.owner.workstream_run, 'child');
    const attempts = rows('attempt', parseAttempt, (row) => d65AttemptEntityId(row.attempt.owner));
    for (const row of attempts)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.attempt.owner.repo_id, row.attempt.owner.workstream_run, 'attempt');
    const faults = rows('fault', parseRunScopedLogicalFault, (row) => row.fault_id);
    for (const row of faults)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.repo_id, row.workstream_run, 'fault');
    const reservations = rows('reservation', parseCoordinationChangeReservation, (row) => row.reservation_id);
    for (const row of reservations)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.repo_id, row.workstream_run, 'reservation');
    const editLeases = rows('edit-lease', parseCoordinationEditLease, (row) => row.edit_lease_id);
    for (const row of editLeases)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.owner.repo_id, row.owner.workstream_run, 'edit lease');
    const acquisitionGroups = rows('acquisition-group', parseCoordinationAcquisitionGroup, (row) => row.acquisition_group_id);
    for (const row of acquisitionGroups)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.owner.repo_id, row.owner.workstream_run, 'acquisition group');
    const worktrees = rows('worktree', parseCoordinationWorktree, (row) => row.worktree_id);
    for (const row of worktrees)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.owner.repo_id, row.owner.workstream_run, 'worktree');
    const operations = rows('operation', parseCoordinationWorktreeOperation, (row) => row.operation_id);
    for (const row of operations)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.owner.repo_id, row.owner.workstream_run, 'operation');
    const terminalIntents = rows('terminal-intent', parseIntent, (row) => row.terminal_intent_id);
    for (const row of terminalIntents)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.repo_id, row.workstream_run, 'terminal intent');
    const artifacts = rows('artifact', parseCoordinationAuthoritativeArtifact, (row) => row.artifact_id);
    for (const row of artifacts)
        ensureRunScope(expected.repoId, expected.workstreamRun, row.repo_id, row.source_run, 'artifact');
    ...
    if (currentIntents.length > 1 || pointer !== (currentIntents[0]?.terminal_intent_id ?? null))
        fail('current terminal intent pointer is not the sole prepared/committed intent', [String(pointer)]);
    ...
    if (covered !== expected.coveredEventSeq)
        fail('covered_event_seq singleton does not equal the graph root covered_event_seq', [String(covered), String(expected.coveredEventSeq)]);
    ...
    if (runVersion !== run.version)
        fail('run_version singleton does not equal the projected run row version', [String(runVersion), String(run.version)]);
...
export function applyD65GraphRegistrationBaseline(input) {
    ...
    if (artifact.document_schema_version !== 'autopilot.semantic_graph.v1')
        fail('B(N) artifact is not a complete semantic graph');
    if (artifact.repo_id !== prior.run.repo_id || artifact.source_run !== prior.run.workstream_run)
        fail('B(N) artifact belongs to a different run');
    if (artifact.registered_event_seq !== prior.covered_event_seq + 1)
        fail('B(N) registration R is not prior E+1', [String(artifact.registered_event_seq), String(prior.covered_event_seq)]);
    if (prior.authoritative_artifacts.some((row) => row.artifact_id === artifact.artifact_id))
        fail('B(N) prior projection already contains its own future artifact row', [artifact.artifact_id]);
...
export function assertD65CoordinatorProjectionEqual(actual, expected, canonicalize) {
    const actualText = canonicalize(actual);
    const expectedText = canonicalize(expected);
    if (actualText !== expectedText)
        fail('loaded coordinator projection does not equal the committed coordinator state at E');
}
