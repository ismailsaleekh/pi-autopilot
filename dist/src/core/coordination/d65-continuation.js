...
export const D65_CONTINUATION_EVENT_SCHEMA = 'autopilot.continuation_event.v1';
...
export function parseD65ContinuationEvent(value) {
    ...
    if (trigger === 'other' && klass !== 'continuation-unclassified')
        fail(label, 'trigger `other` maps only to class `continuation-unclassified`');
    ...
    if (!isSubscription && (provider !== null || failedSpec !== null || failedReceipt !== null))
        fail(label, 'provider/failed_spec_ref/failed_receipt_ref are only present for subscription failures');
    if (isSubscription && (provider === null || failedSpec === null || failedReceipt === null))
        fail(label, 'a subscription failure requires provider and failed spec/receipt refs');
    ...
    if (!unitFailure && (unitId !== null || attempt !== null || childLeaseId !== null))
        fail(label, 'unit/attempt/child fields are only present for unit failures');
    ...
    if (record['operator_decision_ref'] !== null)
        fail(label, 'operator_decision_ref must be null under D65');
    return {
...
export const D65_PARENT_LOSS_SCHEMA = 'autopilot.parent_loss.v1';
function boundedIdentityObject(value, label) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        fail(label, 'must be a closed identity object');
    const record = value;
    const keys = Object.keys(record);
    if (keys.length === 0 || keys.length > 32)
        fail(label, 'identity object field count is out of range');
    for (const key of keys) {
        ...
        if (typeof entry === 'string') {
            if (entry.length > 4096)
                fail(label, `identity field ${key} is too long`);
            continue;
        }
        if (typeof entry === 'number') {
            if (!Number.isSafeInteger(entry))
                fail(label, `identity field ${key} must be a safe integer`);
            continue;
        }
...
export function parseD65ParentLoss(value) {
    ...
    if (record['successor_budget'] !== 1)
        fail(label, 'successor_budget must be exactly 1');
    if (record['operator_decision_ref'] !== null)
        fail(label, 'operator_decision_ref must be null under D65');
    const signature = str(record, 'signature', label, 128);
    if (!/^[A-Za-z0-9_-]+$/u.test(signature))
        fail(label, 'signature must be unpadded base64url');
    return {
