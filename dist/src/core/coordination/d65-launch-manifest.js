...
import { isValidAutopilotRepoId, isValidWorkstreamRun } from "../paths.js";
...
export const D65_LAUNCH_MANIFEST_SCHEMA = 'autopilot.launch_manifest.v1';
/** Absolute normalized filesystem path (no NUL, no `/../`, no trailing slash). */
function absolutePathField(record, field, label, maxLength = 4096) {
    const value = str(record, field, label, maxLength);
    if (!isAbsolute(value) || value.includes('\u0000') || normalize(value) !== value || (value.length > 1 && value.endsWith('/')))
...
function parentModel(record, label) {
    ...
    if (slash <= 0 || slash === value.length - 1 || value.includes(' '))
        fail(label, 'parent_model must be a provider/model identifier');
    return value;
}
function parentThinking(record, label) {
...
function trustAnchor(value, label) {
...
export const D65_LAUNCH_MANIFEST_MAX_BYTES = 262_144;
/**
 ...
 */
export function parseD65LaunchManifest(value) {
    ...
    if (record['coordination_authority'] !== 'coordinator-edit-leases-v1')
        fail(label, 'coordination_authority must be exactly coordinator-edit-leases-v1');
    const manifest = {
    ...
    if (manifest.b0_commit === manifest.content_result_commit)
        fail(label, 'b0_commit and content_result_commit must be distinct');
    ...
    if (manifest.bootstrap_overlay.overlay_commit === manifest.b0_commit || manifest.bootstrap_overlay.overlay_commit === manifest.content_result_commit)
        fail(label, 'bootstrap overlay commit must be a sibling, never B0 or the content-result base');
    if (manifest.launch_seal.launch_commit !== manifest.bootstrap_overlay.overlay_commit)
        fail(label, 'launch_seal.launch_commit must equal the bootstrap overlay commit');
    if (manifest.launch_seal.launch_tree !== manifest.bootstrap_overlay.overlay_tree)
        fail(label, 'launch_seal.launch_tree must equal the bootstrap overlay tree');
...
function closedRunIdentity(record, field, label) {
    const value = str(record, field, label, 120);
    if (!isValidWorkstreamRun(value))
        fail(label, `${field} must match the closed 120-byte ASCII alphanumeric-or-hyphen workstream-run grammar`);
    return value;
}
function closedRepoIdentity(record, field, label) {
    const value = str(record, field, label, 120);
    if (!isValidAutopilotRepoId(value))
        fail(label, `${field} must match the closed lowercase 120-byte ASCII alphanumeric-or-hyphen repo-id grammar`);
    return value;
}
...
function requireProspectiveRun(manifest) {
    const label = `${D65_LAUNCH_MANIFEST_SCHEMA}.prospective_run`;
    const run = manifest.prospective_run;
    if (stringField(run, 'schema_version', label) !== 'autopilot.coordination_run.v1')
        fail(label, 'prospective_run schema must be autopilot.coordination_run.v1');
    if (stringField(run, 'repo_id', label) !== manifest.repo_id)
        fail(label, 'prospective_run repo_id differs from the sealed repo_id');
    if (stringField(run, 'workstream_run', label) !== manifest.workstream_run)
        fail(label, 'prospective_run workstream_run differs from the sealed workstream_run');
    if (stringField(run, 'workstream', label) !== manifest.workstream)
        fail(label, 'prospective_run workstream differs from the sealed workstream');
    if (stringField(run, 'autopilot_id', label) !== manifest.autopilot_id)
        fail(label, 'prospective_run autopilot_id differs from the sealed autopilot_id');
    if (stringField(run, 'coordination_authority', label) !== manifest.coordination_authority)
        fail(label, 'prospective_run coordination_authority differs from the sealed authority');
    if (run['status'] !== 'active' || run['active_session_generation'] !== 0 || run['created_event_seq'] !== 1 || run['version'] !== 1)
        fail(label, 'prospective_run must be the exact fresh active generation-0 version-1 row');
}
function requireProspectiveResource(manifest) {
    const label = `${D65_LAUNCH_MANIFEST_SCHEMA}.prospective_resource`;
    const resource = manifest.prospective_resource;
    if (stringField(resource, 'schema_version', label) !== 'autopilot.coordination_run_resource.v1')
        fail(label, 'prospective_resource schema must be autopilot.coordination_run_resource.v1');
    if (stringField(resource, 'repo_id', label) !== manifest.repo_id)
        fail(label, 'prospective_resource repo_id differs from the sealed repo_id');
    if (stringField(resource, 'workstream_run', label) !== manifest.workstream_run)
        fail(label, 'prospective_resource workstream_run differs from the sealed workstream_run');
    if (stringField(resource, 'source_repo', label) !== manifest.canonical_root)
        fail(label, 'prospective_resource source_repo differs from the sealed canonical_root');
    if (stringField(resource, 'git_common_dir', label) !== manifest.git_common_dir)
        fail(label, 'prospective_resource git_common_dir differs from the sealed git_common_dir');
    if (stringField(resource, 'worktree_root', label) !== manifest.worktree_root)
        fail(label, 'prospective_resource worktree_root differs from the sealed worktree_root');
    if (stringField(resource, 'main_worktree_path', label) !== manifest.main_worktree_path)
        fail(label, 'prospective_resource main_worktree_path differs from the sealed main_worktree_path');
    if (stringField(resource, 'runtime_root', label) !== manifest.runtime_root)
        fail(label, 'prospective_resource runtime_root differs from the sealed runtime_root');
    if (stringField(resource, 'branch', label) !== manifest.run_branch)
        fail(label, 'prospective_resource branch differs from the sealed run_branch');
    if (stringField(resource, 'target_branch', label) !== manifest.target_branch)
        fail(label, 'prospective_resource target_branch differs from the sealed target_branch');
    if (stringField(resource, 'target_base_sha', label) !== manifest.content_result_commit)
        fail(label, 'prospective_resource target_base_sha must be the content-result commit (never B0 or the launch overlay)');
    if (resource['version'] !== 1)
        fail(label, 'prospective_resource version must be exactly 1');
}
function requireAuthorityDistinctRoots(manifest) {
    const label = D65_LAUNCH_MANIFEST_SCHEMA;
    ...
    for (const [name, root] of [['state_root', manifest.state_root], ['session_root', manifest.session_root], ['program_evidence_root', manifest.program_evidence_root]]) {
        if (isInside(root, clone))
            fail(label, `${name} must live outside the source clone`);
    }
    if (!isInside(manifest.canonical_root, clone) && manifest.canonical_root !== clone)
        fail(label, 'canonical_root must be the source clone or a descendant of it');
    for (const other of [manifest.source_clone, manifest.state_root, manifest.session_root, manifest.worktree_root, manifest.main_worktree_path]) {
        if (isInside(manifest.program_evidence_root, other) || isInside(other, manifest.program_evidence_root))
            fail(label, 'program_evidence_root must be authority-distinct from every clone/state/session/worktree root');
    }
}
/** The exact SHA-256 of a manifest's canonical UTF-8 bytes (as loaded). */
export function launchManifestBytesSha256(bytes) {
