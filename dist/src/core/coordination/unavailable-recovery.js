import { isProcessAlive, preflightProcessRetirementSupport, processStartIdentity, retireExactProcess } from "./process-identity.js";
...
async function exactCurrentLock(paths, expectedText, expected) {
    const text = await readExactLockText(paths.lockPath);
    if (text !== expectedText)
        throw new CoordinationRuntimeError('coordinator-contention', 'coordinator lifecycle identity changed during unavailable-endpoint recovery');
    let parsed = null;
    try {
        parsed = parseKnownCompatibleCurrentCoordinatorLock(JSON.parse(text));
    }
    catch { /* fail below */ }
    if (parsed === null || !sameCurrentOwner(parsed, expected))
        throw new CoordinationRuntimeError('protocol-mismatch', 'coordinator lifecycle identity became unknown during unavailable-endpoint recovery');
}
async function exactFence(paths, expected) {
    const text = await readExactLockText(paths.predecessorLockPath);
    let parsed = null;
    try {
        parsed = text === null ? null : parsePredecessorCoordinatorLock(JSON.parse(text));
    }
    catch { /* fail below */ }
    if (parsed === null || !sameFenceOwner(parsed, expected))
        throw new CoordinationRuntimeError('coordinator-contention', 'predecessor safety fence changed during unavailable-endpoint recovery');
}
function exactProcessState(owner) {
    if (!isProcessAlive(owner.pid))
        return 'absent';
    const observed = processStartIdentity(owner.pid);
    if (observed === null)
        throw new CoordinationRuntimeError('recovery-required', 'live coordinator process-creation identity is unavailable; automatic retirement is forbidden', [`pid=${String(owner.pid)}`, `instance_id=${owner.instance_id}`]);
    return observed === owner.process_start_identity ? 'exact-live' : 'absent';
}
function recoveryGuard(paths, timeoutMs) {
 ...
 */
export async function recoverUnavailableKnownCoordinator(paths, probeEndpoint, options = {}) {
    ...
    if (!Number.isSafeInteger(attestationTimeoutMs) || attestationTimeoutMs < 100 || attestationTimeoutMs > 30_000 || !Number.isSafeInteger(retirementTimeoutMs) || retirementTimeoutMs < 100 || retirementTimeoutMs > 30_000)
        throw new CoordinationRuntimeError('invalid-request', 'unavailable coordinator recovery timeouts are outside their bounded contract');
    const lockText = await readExactLockText(paths.lockPath);
    if (lockText === null)
        throw new CoordinationRuntimeError('coordinator-unavailable', 'unavailable coordinator lifecycle lock disappeared before recovery');
    let owner = null;
    try {
        owner = parseKnownCompatibleCurrentCoordinatorLock(JSON.parse(lockText));
    }
    catch { /* fail below */ }
    if (owner === null)
        throw new CoordinationRuntimeError('protocol-mismatch', 'unavailable coordinator recovery requires an exact known wire-compatible lifecycle owner');
    if (owner.pid === process.pid)
        throw new CoordinationRuntimeError('recovery-required', 'client process is recorded as coordinator owner; automatic self-retirement is forbidden', [`pid=${String(owner.pid)}`]);
    const fenceText = await readExactLockText(paths.predecessorLockPath);
    let fence = null;
    try {
        fence = fenceText === null ? null : parsePredecessorCoordinatorLock(JSON.parse(fenceText));
    }
    catch { /* fail below */ }
    if (fence === null || fence.pid !== owner.pid || fence.started_at !== owner.started_at)
        throw new CoordinationRuntimeError('recovery-required', 'known coordinator predecessor fence does not bind the exact lifecycle owner', [`pid=${String(owner.pid)}`, `instance_id=${owner.instance_id}`]);
    if (exactProcessState(owner) === 'absent')
        return report(owner, 'owner-absent', 0);
    ...
    do {
        ...
        if (exactProcessState(owner) === 'absent')
            return report(owner, 'owner-absent', endpointProbeCount);
        endpointProbeCount += 1;
        if (await probeEndpoint())
            return report(owner, 'endpoint-recovered', endpointProbeCount);
        if (Date.now() < deadline)
            await sleep(Math.min(UNAVAILABLE_PROBE_INTERVAL_MS, Math.max(1, deadline - Date.now())));
    ...
    try {
        ...
        while (Date.now() < retirementDeadline) {
            if (!isProcessAlive(owner.pid))
                return report(owner, 'owner-retired', endpointProbeCount);
            ...
            if (observed === null) {
                ...
                if (observed === null)
                    throw new CoordinationRuntimeError('recovery-required', 'coordinator process identity became ambiguous after exact retirement signal', [`pid=${String(owner.pid)}`]);
            }
