...
export function repoIdField(record, field, label) {
...
export function workstreamRunField(record, field, label) {
    const value = str(record, field, label, 120);
...
export function parseD65RunTerminalIntentV2(value) {
    ...
    if (intentAttempt === TERMINAL_INTENT_CANCELLATION_MAX + 1 && state === 'cancelled')
        fail(label, 'the mandatory fourth abort intent is noncancellable');
