...
export function checkCoordinationInvariants(snapshot) {
    ...
    for (const reservation of snapshot.change_reservations) {
        ...
        else if (run.autopilot_id !== reservation.autopilot_id)
    ...
    for (const request of snapshot.claim_requests) {
        ...
        if ((request.status === 'granted' || request.status === 'requester-notified' || request.status === 'resolved') && request.grant_event_seq === null)
    ...
    for (const operation of snapshot.worktree_operations) {
        ...
        else if (ownerKey(worktree.owner) !== ownerKey(operation.owner))
            findings.push(finding('foreign-worktree-operation', operation.operation_id, 'operation owner differs from worktree owner'));
        if ((operation.stage === 'verified' || operation.stage === 'committed') && operation.verification_evidence === null)
        ...
        if (operation.operation_type === 'remove' && operation.stage === 'committed' && worktree?.state === 'dirty')
            findings.push(finding('dirty-worktree-removed', operation.operation_id, 'dirty worktree cannot be removed without quarantine capture'));
    }
