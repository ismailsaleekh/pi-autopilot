...
export function checkCoordinationInvariants(snapshot) {
    ...
    for (const work of snapshot.migration_recovery_work) {
        ...
        if (work.status === 'pending') {
            if (run.status !== 'recovering' && run.status !== 'closed' && run.status !== 'aborted')
                findings.push(finding('pending-migration-recovery-run-dispatchable', work.recovery_id, `${run.status} run must be fenced as recovering or remain terminal`));
            ...
            if (dispatchSessions.length > 0)
                findings.push(finding('pending-migration-recovery-dispatch-session', work.recovery_id, 'pending migration recovery must not coexist with ordinary attached dispatch'));
            const leaseId = work.detail['edit_lease_id'];
            if (work.recovery_type === 'ambiguous-live-claim' && (typeof leaseId !== 'string' || !snapshot.edit_leases.some((lease) => lease.edit_lease_id === leaseId)))
                findings.push(finding('pending-migration-recovery-lease-missing', work.recovery_id, 'ambiguous authority recovery must bind one exact retained edit lease'));
        }
    }
    ...
    for (const lease of snapshot.edit_leases) {
        ...
        if (attempt !== undefined && ['merged', 'failed', 'reset', 'quarantined', 'superseded'].includes(attempt.state) && !pendingMigrationRecoveryLeaseIds.has(lease.edit_lease_id))
    ...
    for (const obligation of snapshot.reservation_obligations) {
        ...
        if (reservation !== undefined && predecessor !== undefined) {
            ...
            if (!obligation.integration_conflict.overlapping_paths.some((path) => coordinationPathsOverlap(path, reservation.path) && coordinationPathsOverlap(path, predecessor.path)))
                findings.push(finding('obligation-conflict-path-mismatch', obligation.obligation_id, 'integration classification does not bind the actual reservation overlap'));
        }
        ...
        if (repairKinds.has(obligation.integration_conflict.kind) !== (obligation.integration_conflict.disposition === 'repair-required'))
            findings.push(finding('obligation-conflict-disposition-mismatch', obligation.obligation_id, `${obligation.integration_conflict.kind} has ${obligation.integration_conflict.disposition}`));
    ...
    for (const request of snapshot.claim_requests) {
        ...
        for (const leaseId of request.blocking_lease_ids) {
            ...
            if (lease === undefined) {
                if (!TERMINAL_REQUEST_STATES.has(request.status) && request.status !== 'released' && request.status !== 'requester-notified' && request.status !== 'grant-ready' && request.status !== 'granted')
                    findings.push(finding('request-blocking-lease-missing', request.request_id, `${request.status} request blocking lease ${leaseId} is absent before release evidence`));
            }
    ...
    for (const operation of snapshot.worktree_operations) {
        ...
        if (operation.operation_type === 'remove' && operation.stage === 'committed' && worktree?.state !== 'removed') {
            ...
            if (!laterCommittedRecreate)
                findings.push(finding('worktree-remove-state-mismatch', operation.operation_id, 'committed remove operation requires removed worktree state unless a later committed recreate superseded its exact effect'));
        }
    }
