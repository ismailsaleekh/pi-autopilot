import { AUTOPILOT_RESPOND_CLAIM_REQUEST_TOOL_NAME } from "../names.js";
const COORDINATION_IDENTIFIER = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,191}$/u;
const AUTOMATIC_DEFER_CONDITIONS = ['child-terminal', 'unit-merged', 'attempt-reset', 'quarantine-captured', 'run-closed'];
function requiredText(value, field, maximum) {
    if (typeof value !== 'string' || value.trim().length === 0 || value.length > maximum || value.includes('\u0000'))
        throw new Error(`${field} must be a non-empty string of at most ${String(maximum)} characters`);
    return value.trim();
}
...
function identifier(value, field) {
    ...
    if (!COORDINATION_IDENTIFIER.test(text))
        throw new Error(`${field} must be a bounded coordinator identifier`);
    return text;
}
function releaseCondition(params) {
    ...
    if (params.response === 'release-now') {
        if (conditionType !== undefined || targetId !== undefined || evidenceRef !== undefined || evidenceSha256 !== undefined)
            throw new Error('release-now must not include deferred release-condition fields');
        return null;
    }
    if (conditionType === undefined || !AUTOMATIC_DEFER_CONDITIONS.includes(conditionType))
        throw new Error(`deferred requires condition_type=${AUTOMATIC_DEFER_CONDITIONS.join('|')}`);
    if (targetId === undefined)
        throw new Error('deferred requires target_id');
    if ((evidenceRef === undefined) !== (evidenceSha256 === undefined))
        throw new Error('deferred evidence_ref and evidence_sha256 must be supplied together');
    if (evidenceSha256 !== undefined && !/^sha256:[a-f0-9]{64}$/u.test(evidenceSha256))
        throw new Error('evidence_sha256 must be a lowercase sha256 digest');
    const evidence = evidenceRef === undefined || evidenceSha256 === undefined ? null : { ref: evidenceRef, sha256: evidenceSha256 };
    return { condition_type: conditionType, target_id: targetId, evidence };
}
export function createClaimResponseTool(responder) {
    return {
        ...
        async execute(_toolCallId, params, signal) {
            if (signal?.aborted === true)
                throw new Error('Autopilot claim response was aborted before coordinator mutation');
            const current = responder();
            if (current === null)
                throw new Error('No authenticated active Autopilot run is attached in this parent session');
            ...
            if (params.response !== 'release-now' && params.response !== 'deferred')
                throw new Error('response must be release-now or deferred');
