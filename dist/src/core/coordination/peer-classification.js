...
function bounded(value, field, maximum = 512) {
    if (typeof value !== 'string' || value.length < 1 || [...value].length > maximum || /[\u0000-\u001f\u007f]/u.test(value))
        fail(`${field} must be a bounded non-control string`);
    return value;
}
export function parseCoordinatorLegacyFacadeHandshake(value, identity) {
    ...
    if (actual.length !== expected.length || actual.some((field, index) => field !== expected[index]))
        fail(`field set must be exactly ${expected.join(',')}`);
    if (record['schema_version'] !== 'autopilot.coordinator_handshake.v1')
        fail('schema_version is not the frozen legacy handshake schema');
    if (record['package_build'] !== identity.legacyFacadeBuild)
        fail('package_build is not the frozen cf50 façade');
    if (record['protocol_version'] !== AUTOPILOT_COORDINATOR_PROTOCOL_VERSION)
        fail('protocol_version is not 1.6');
    if (record['database_schema_version'] !== identity.apiSchemaVersion)
        fail('database_schema_version is not the frozen API schema');
    if (record['lifecycle_lock_schema'] !== 'autopilot.coordinator_lock.v2')
        fail('lifecycle_lock_schema is not the frozen cf50 shape');
    const pid = record['lifecycle_pid'];
    if (typeof pid !== 'number' || !Number.isSafeInteger(pid) || pid < 1)
        fail('lifecycle_pid must be a positive safe integer');
...
export function classifyCoordinatorInitialPeer(value, identity) {
    const handshake = parseCoordinatorLegacyFacadeHandshake(value, identity);
    if (handshake.admission_upgrade === undefined)
        return Object.freeze({ kind: 'known-cf50-predecessor', handshake });
