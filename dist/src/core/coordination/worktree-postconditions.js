...
function result(outcome, proof, source = 'repository-facts', captureSha = null, effectApplied = outcome === 'satisfied') {
...
function ordinaryRequest(request) {
    if (request.operationType === 'metadata-reconcile')
        throw new Error('metadata reconciliation does not use an ordinary worktree intent');
    return request;
}
function metadataRequest(request) {
    if (request.operationType !== 'metadata-reconcile')
        throw new Error('ordinary worktree operation does not use metadata reconciliation intent');
    return request;
}
function assertCanonicalIdentity(request) {
    const expected = deterministicWorktreeId(request.owner, request.kind);
    if (request.canonicalWorktreeId !== expected)
        return result('unsafe', [`actual_canonical_worktree_id=${request.canonicalWorktreeId}`, `expected_canonical_worktree_id=${expected}`]);
    if (request.operationType === 'metadata-reconcile') {
        const intent = parseMetadataReconcileIntent(request.intent);
        if (intent.canonical_worktree_id !== expected || intent.repo_id !== request.owner.repo_id)
            return result('unsafe', ['metadata_reconcile_canonical_owner_mismatch']);
        return null;
    }
    ...
    if (request.intent.branch !== expectedBranch)
        return result('unsafe', [`actual_branch_authority=${request.intent.branch}`, `expected_branch_authority=${expectedBranch}`]);
    ...
    if (invalidMetadataRefs.length > 0)
        return result('unsafe', invalidMetadataRefs.map((ref) => `unbounded_metadata_ref=${ref}`));
    return null;
}
export function gitWorktreeRegistrationFacts(repoRoot, env) {
    ...
    const flush = () => {
        if (path !== null)
            registrations.push(Object.freeze({ worktree_path: canonicalPath(path), head_sha: head, branch_ref: branch, prunable }));
        path = null;
        head = '';
        branch = null;
        prunable = false;
...
function registrationFor(intent, env) {
...
function metadataMissing(request) {
    if (request.intent.metadata_refs.length === 0)
    ...
    return Object.freeze(request.intent.metadata_refs.filter((ref) => {
        if (request.operationType === 'remove' && request.intent.reason.startsWith(AUTOPILOT_PREFLIGHT_ROLLBACK_REASON_PREFIX) && ref === '_ledger.jsonl')
            return false;
...
function physicalAuthority(request) {
    const intent = request.intent;
    const present = existsSync(intent.worktree_path);
    const registrations = registrationFor(intent, request.env);
    if (registrations.exact.length > 1)
        return result('unsafe', registrations.exact.map((entry) => `duplicate_registration=${entry.worktree_path}:${entry.head_sha}`));
    if (!present)
        return null;
    if (registrations.exact.length !== 1)
        return result('unsafe', ['git_registration_absent', 'physical_path_present']);
    const entry = registrations.exact[0];
    if (entry === undefined || entry.branch_ref !== `refs/heads/${intent.branch}`)
        return result('unsafe', [`actual_registration_branch=${String(entry?.branch_ref ?? null)}`, `expected_registration_branch=refs/heads/${intent.branch}`]);
    ...
    if (top !== canonicalPath(intent.worktree_path))
        return result('unsafe', [`actual_toplevel=${top}`, `expected_toplevel=${canonicalPath(intent.worktree_path)}`]);
    ...
    if (common !== canonicalPath(intent.git_common_dir))
        return result('unsafe', [`actual_git_common_dir=${common}`, `expected_git_common_dir=${canonicalPath(intent.git_common_dir)}`]);
    ...
    if (branch !== intent.branch)
        return result('unsafe', [`actual_physical_branch=${String(branch)}`, `expected_physical_branch=${intent.branch}`]);
    return null;
}
...
function mutableWorktreeFacts(request, includeIgnored) {
    ...
    for (let index = 0; index < records.length; index += 1) {
        ...
        if (record.length < 4 || record[2] !== ' ')
            throw new Error('Git porcelain status record is malformed');
        ...
        if (status.includes('R') || status.includes('C')) {
            ...
            if (second === undefined || second.length === 0)
                throw new Error('Git porcelain rename/copy record lacks its second path');
...
function clean(request, includeIgnored) {
    return Object.freeze(mutableWorktreeFacts(request, includeIgnored).map((fact) => `mutable=${fact.status} ${fact.path}`));
}
function effectWithMetadata(request, proof, source = 'repository-facts', captureSha = null) {
...
function exactDiffPaths(request, from, to) {
...
function samePaths(expectedInput, actualInput) {
...
function createPostcondition(requestInput) {
    ...
    if (!present) {
        if (registration.length > 0)
            return result('unsafe', ['physical_path_absent', 'git_registration_present']);
        ...
        if (branch.negative)
            return result('not-applied', ['branch_absent', 'git_registration_absent', 'physical_path_absent']);
        if (intent.base_sha === null)
            return result('unsafe', ['create_base_sha_absent']);
    ...
    if (currentHead !== intent.base_sha && !historicalAdvance)
        return result('unsafe', [`actual_head=${currentHead}`, `expected_head=${intent.base_sha}`]);
    if (intent.checkout_mode !== null && intent.checkout_mode !== 'full') {
        ...
        if (sparse.negative || new TextDecoder().decode(sparse.stdout).trim() !== 'true')
            return result('not-applied', ['sparse_configuration_incomplete', 'worktree_registered']);
    }
    const missing = metadataMissing(request);
    return missing.length === 0
...
function futureOwnedParentPresent(worktreePath, path) {
...
function materializePostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const authority = physicalAuthority(request);
    if (authority !== null)
        return authority;
    if (!existsSync(request.intent.worktree_path))
        return result('unsafe', ['owned_worktree_missing']);
    ...
    if (sparse.negative || new TextDecoder().decode(sparse.stdout).trim() !== 'true')
        return result('unsafe', ['sparse_checkout_disabled']);
    const missing = [];
    const lfs = [];
    const unsupported = [];
    for (const path of request.intent.paths) {
        ...
        if (records.length === 0) {
            if (!futureOwnedParentPresent(request.intent.worktree_path, path))
                missing.push(path);
            continue;
        }
        if (records.some((entry) => entry.startsWith('S '))) {
            missing.push(path);
            continue;
        }
    ...
    if (unsupported.length > 0)
        return result('unsafe', unsupported.map((path) => `unsupported_materialized_entry=${path}`));
    if (lfs.length > 0)
        return result('unsafe', lfs.map((path) => `lfs_pointer=${path}`));
...
function commitPostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const authority = physicalAuthority(request);
    if (authority !== null)
        return authority;
    if (!existsSync(request.intent.worktree_path) || request.intent.base_sha === null)
        return result('unsafe', ['commit_authority_incomplete']);
    const dirty = clean(request, false);
    if (dirty.length > 0)
        return result('not-applied', dirty);
    const currentHead = head(request);
    if (request.intent.target_sha !== null && currentHead !== request.intent.target_sha)
        return result('unsafe', [`actual_head=${currentHead}`, `expected_target_head=${request.intent.target_sha}`]);
    if (currentHead === request.intent.base_sha)
        return result('not-applied', [`head=${currentHead}`, 'worktree_clean']);
    if (runGitQuery({ descriptor: { kind: 'is-ancestor', ancestor: request.intent.base_sha, descendant: currentHead }, cwd: request.intent.repo_root, ...(request.env === undefined ? {} : { env: request.env }) }).negative)
        return result('unsafe', [`actual_head=${currentHead}`, `expected_ancestor=${request.intent.base_sha}`]);
    const paths = exactDiffPaths(request, request.intent.base_sha, currentHead);
    if (!samePaths(request.intent.paths, paths))
        return result('unsafe', [...request.intent.paths.map((path) => `expected_path=${path}`), ...paths.map((path) => `actual_path=${path}`)]);
    const metadata = metadataMissing(request);
...
function quarantinePostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const intent = request.intent;
    if (intent.base_sha === null)
    ...
    if (existsSync(intent.worktree_path)) {
        ...
        if (mutable.length > 0) {
            ...
            if (!samePaths(intent.paths, actualPaths))
                return result('unsafe', [...intent.paths.map((path) => `expected_mutable_path=${path}`), ...actualPaths.map((path) => `actual_mutable_path=${path}`)], 'physical-worktree');
            return result('not-applied', mutable.map((fact) => `mutable=${fact.status} ${fact.path}`), 'physical-worktree');
        }
        const capture = head(request);
        if (capture === intent.base_sha)
        ...
        if (parents.length !== 1 || parents[0] !== intent.base_sha || !samePaths(intent.paths, paths))
            return result('unsafe', [`capture_sha=${capture}`, ...parents.map((parent) => `actual_parent=${parent}`), ...intent.paths.map((path) => `expected_path=${path}`), ...paths.map((path) => `actual_path=${path}`)], 'physical-worktree');
...
function resetPostcondition(requestInput) {
    ...
    if (authority !== null)
        return authority;
    if (!existsSync(request.intent.worktree_path) || request.intent.target_sha === null)
        return result('unsafe', ['reset_authority_incomplete']);
    const dirty = clean(request, true);
    if (dirty.length > 0)
        return result('unsafe', dirty);
    const current = head(request);
    if (current === request.intent.target_sha)
        return effectWithMetadata(request, [`head=${current}`, 'worktree_clean_including_ignored']);
    if (request.intent.base_sha !== null && current === request.intent.base_sha)
        return result('not-applied', [`head=${current}`, `pending_target_head=${request.intent.target_sha}`, 'worktree_clean_including_ignored']);
...
function mergePostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const intent = request.intent;
    if (intent.base_sha === null || intent.target_sha === null)
        return result('unsafe', ['merge_intent_incomplete']);
    ...
    if (intent.archive_ref === null) {
        ...
        if (authority !== null)
            return authority;
        if (!existsSync(cwd))
            return result('unsafe', ['merge_worktree_absent']);
        const mergePath = gitQueryText({ descriptor: { kind: 'git-path', name: 'MERGE_HEAD' }, cwd, ...(request.env === undefined ? {} : { env: request.env }) }).trim();
        if (existsSync(isAbsolute(mergePath) ? mergePath : resolve(cwd, mergePath)))
            return head(request) === intent.base_sha
    ...
    if (intent.archive_ref !== null) {
        ...
        const branch = branchQuery.negative ? null : new TextDecoder('utf-8', { fatal: true }).decode(branchQuery.stdout).trim();
        if (branch !== intent.archive_ref)
            return result('unsafe', [`actual_integration_branch=${String(branch)}`, `expected_integration_branch=${intent.archive_ref}`]);
        if (current === intent.target_sha)
            return effectWithMetadata(request, [`head=${current}`, `merged_source=${intent.target_sha}`]);
    ...
    if (!runGitQuery({ descriptor: { kind: 'is-ancestor', ancestor: intent.target_sha, descendant: current }, cwd: intent.repo_root, ...(request.env === undefined ? {} : { env: request.env }) }).negative)
        return effectWithMetadata(request, [`head=${current}`, `merged_source=${intent.target_sha}`]);
    if (current === intent.base_sha)
        return result('not-applied', [`head=${current}`]);
...
function archivePostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const intent = request.intent;
    if (intent.archive_ref === null || intent.target_sha === null)
        return result('unsafe', ['archive_intent_incomplete']);
    ...
    if (query.negative)
        return result('not-applied', [`archive_ref_absent=${ref}`]);
...
function removePostcondition(requestInput) {
    const request = ordinaryRequest(requestInput);
    const intent = request.intent;
    const present = existsSync(intent.worktree_path);
    const registration = registrationFor(intent, request.env).exact;
    ...
    if (present) {
        ...
        if (authority !== null)
            return authority;
        if (branch.negative || intent.target_sha === null)
            return result('unsafe', ['owned_branch_or_expected_sha_absent_before_remove']);
        const actual = gitQueryText({ descriptor: { kind: 'resolve-revision', revision: `refs/heads/${intent.branch}`, verify: true }, cwd: intent.repo_root, ...(request.env === undefined ? {} : { env: request.env }) }).trim();
        if (actual !== intent.target_sha)
            return result('unsafe', [`actual_branch_sha=${actual}`, `expected_branch_sha=${intent.target_sha}`]);
        const dirty = clean(request, true);
...
function metadataReconcilePostcondition(requestInput) {
    const request = metadataRequest(requestInput);
    const intent = parseMetadataReconcileIntent(request.intent);
    if (pathEntryExists(intent.target_registration_path))
        return result('unsafe', ['metadata_reconcile_target_path_present'], 'metadata-registration-set');
    ...
    if (refMismatch.length > 0)
        return result('unsafe', refMismatch.map((entry) => `preserved_ref_mismatch=${entry}`), 'metadata-registration-set');
    if (observedJson === afterJson)
        return result('satisfied', ['approved_registration_metadata_absent', ...refs.map((entry) => `preserved_ref=${entry}`)], 'metadata-registration-set');
    if (observedJson === beforeJson)
        return result('not-applied', ['exact_approved_prunable_set_present', ...refs.map((entry) => `preserved_ref=${entry}`)], 'metadata-registration-set');
...
export const WORKTREE_POSTCONDITION_REGISTRY = Object.freeze({
...
export function inspectWorktreePostcondition(request) {
    const identity = assertCanonicalIdentity(request);
    if (identity !== null)
        return identity;
    try {
        return WORKTREE_POSTCONDITION_REGISTRY[request.operationType](request);
    }
    catch (error) {
