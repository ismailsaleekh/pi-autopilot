...
import { parseCoordinationWorktree } from "./contracts.js";
import { CoordinationRuntimeError } from "./failures.js";
import { AUTOPILOT_IDENTITY_FAULT_RESOLUTION_EVIDENCE_SCHEMA, parseIdentityFaultResolutionEvidence, } from "./identity-fault-resolution-contract.js";
import { parseRunScopedLogicalFault } from "./logical-faults.js";
import { deterministicWorktreeId, sameWorktreeAuthority } from "./worktree-identity.js";
...
function exactObject(value, fields, label) {
    ...
    if (canonicalJson(Object.keys(record).sort()) !== canonicalJson([...fields].sort()))
        throw new CoordinationRuntimeError('invalid-state', `${label} fields are not the exact negotiated contract`, Object.keys(record));
    return record;
}
function sortedStrings(value, label) {
    if (!Array.isArray(value) || !value.every((entry) => typeof entry === 'string'))
        throw new CoordinationRuntimeError('invalid-state', `${label} must be a string array`);
    const sorted = [...value].sort();
    if (canonicalJson(value) !== canonicalJson(sorted) || new Set(value).size !== value.length)
        throw new CoordinationRuntimeError('invalid-state', `${label} must be sorted and unique`);
    return Object.freeze(sorted);
}
function parseCandidateOperations(value) {
    if (!Array.isArray(value))
        throw new CoordinationRuntimeError('invalid-state', 'identity recovery operation projection must be an array');
    const output = value.map((entry, index) => {
        const row = exactObject(entry, ['operation_ids', 'worktree_id'], `identity recovery operation row ${String(index)}`);
        if (typeof row['worktree_id'] !== 'string')
            throw new CoordinationRuntimeError('invalid-state', 'identity recovery operation row lacks a worktree ID');
    ...
    if (canonicalJson(output) !== canonicalJson(sorted) || new Set(output.map((entry) => entry.worktree_id)).size !== output.length)
        throw new CoordinationRuntimeError('invalid-state', 'identity recovery operation rows must be sorted and candidate-unique');
    return Object.freeze(sorted);
}
function parseProjection(value) {
    ...
    for (const field of ['fault_id', 'canonical_worktree_id', 'selected_current_worktree_id'])
        if (typeof record[field] !== 'string')
            throw new CoordinationRuntimeError('invalid-state', `identity recovery projection ${field} must be text`);
    const candidateIds = sortedStrings(record['candidate_worktree_ids'], 'identity recovery candidate IDs');
    if (!Array.isArray(record['candidate_worktrees']))
        throw new CoordinationRuntimeError('invalid-state', 'identity recovery candidate worktrees must be an array');
    ...
    if (canonicalJson(worktrees.map((entry) => entry.worktree_id)) !== canonicalJson(candidateIds)
        || canonicalJson(operations.map((entry) => entry.worktree_id)) !== canonicalJson(candidateIds))
        throw new CoordinationRuntimeError('invalid-state', 'identity recovery projection does not cover its exact candidate set');
    return Object.freeze({
        fault: parseRunScopedLogicalFault(record['fault']),
        fault_id: record['fault_id'],
        canonical_worktree_id: record['canonical_worktree_id'],
        selected_current_worktree_id: record['selected_current_worktree_id'],
        candidate_worktree_ids: candidateIds,
        candidate_worktrees: worktrees,
        candidate_operation_ids: operations,
    });
}
function exactRunProjection(payload, faultId) {
    const values = payload['negotiated_identity_recovery'];
    const activeFaults = payload['run_scoped_logical_faults'];
    if (!Array.isArray(values) || !Array.isArray(activeFaults))
        throw new CoordinationRuntimeError('unauthorized-client', 'identity fault resolution requires negotiated canonical-alias and scoped-fault vocabulary');
    const matches = values.map(parseProjection).filter((entry) => entry.fault_id === faultId);
    if (matches.length !== 1 || matches[0] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'identity fault resolution requires exactly one matching negotiated projection', [faultId]);
    ...
    if (projection.fault.status === 'active' && (currentActive.length !== 1 || canonicalJson(currentActive[0]) !== canonicalJson(projection.fault)))
        throw new CoordinationRuntimeError('store-corrupt', 'active identity fault projections disagree across negotiated surfaces', [faultId]);
    if (projection.fault.status === 'resolved' && currentActive.length !== 0)
        throw new CoordinationRuntimeError('store-corrupt', 'resolved identity fault remains in the active negotiated fault surface', [faultId]);
    return projection;
}
function assertInside(root, path, label) {
    const rel = relative(resolve(root), resolve(path));
    if (rel.length === 0 || rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel))
        throw new CoordinationRuntimeError('unauthorized-client', `${label} escapes its package-owned root`, [path, root]);
}
async function publishEvidence(root, evidence) {
    ...
    if (existsSync(path)) {
        ...
        if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1 || await readFile(path, 'utf8') !== bytes)
            throw new CoordinationRuntimeError('idempotency-conflict', 'immutable identity fault resolution evidence differs from exact replay', [path]);
    ...
    try {
        ...
        const info = lstatSync(path);
        if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1 || await readFile(path, 'utf8') !== bytes)
            throw new CoordinationRuntimeError('idempotency-conflict', 'concurrent identity fault resolution evidence differs from exact replay', [path]);
    }
    finally {
...
function branchSha(gitCommonDir, branch, env) {
    ...
    if (query.negative)
        throw new CoordinationRuntimeError('recovery-required', 'identity fault selected branch ref is absent', [branch]);
    let value;
    try {
        value = new TextDecoder('utf-8', { fatal: true }).decode(query.stdout).trim();
    }
    catch {
        throw new CoordinationRuntimeError('recovery-required', 'identity fault selected branch ref output is not valid UTF-8', [branch]);
    }
    if (!/^[a-f0-9]{40}(?:[a-f0-9]{24})?$/u.test(value))
        throw new CoordinationRuntimeError('recovery-required', 'identity fault selected branch did not resolve to an exact object ID', [branch, value]);
    return value;
}
/**
...
export async function resolveCanonicalIdentityFault(input) {
    ...
    if (fault.invariant_id !== 'F3-SEMANTIC-UNIQUENESS' || fault.fault_code !== 'identity-recovery-pending' || fault.entity_type !== 'worktree'
        || fault.repo_id !== input.session.repo_id || fault.workstream_run !== input.session.workstream_run || fault.entity_id !== projection.canonical_worktree_id)
        throw new CoordinationRuntimeError('invalid-state', 'negotiated identity recovery projection differs from its exact fault authority', [fault.fault_id]);
    ...
    if (selected === undefined || deterministicWorktreeId(selected.owner, selected.kind) !== projection.canonical_worktree_id
        || projection.candidate_worktrees.some((candidate) => !sameWorktreeAuthority(candidate, selected)))
        throw new CoordinationRuntimeError('recovery-required', 'identity fault candidates do not prove one exact canonical routing authority', [fault.fault_id]);
    ...
    if (fault.status === 'active') {
        ...
        if (registration === undefined || registration.head_sha !== sha)
            throw new CoordinationRuntimeError('recovery-required', 'identity fault Git registration and selected branch ref do not agree exactly', [selected.canonical_path, selected.branch]);
        ...
        if (fault.resolved_event_seq === null || fault.version < 2)
            throw new CoordinationRuntimeError('store-corrupt', 'resolved identity fault lacks exact resolution authority', [fault.fault_id]);
        ...
        try {
            ...
            throw new CoordinationRuntimeError('recovery-required', 'committed identity fault replay evidence is unavailable', [path, error instanceof Error ? error.message : String(error)]);
        }
        ...
        if (evidence.fault_id !== fault.fault_id || evidence.canonical_worktree_id !== projection.canonical_worktree_id
            ...
            || canonicalJson(evidence.candidate_operation_ids) !== canonicalJson(projection.candidate_operation_ids))
            throw new CoordinationRuntimeError('store-corrupt', 'committed identity fault evidence differs from durable canonical routing projections', [fault.fault_id]);
    }
    ...
    if (resolved.fault_id !== fault.fault_id || resolved.status !== 'resolved' || resolved.resolved_event_seq !== response.committed_event_seq)
        throw new CoordinationRuntimeError('store-corrupt', 'identity fault resolution response lacks exact committed audit authority', [fault.fault_id]);
