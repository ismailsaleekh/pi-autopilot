    ...
    lookupCommittedGraphRegistration(repoId, workstreamRun, input) {
        ...
        if (artifactRow === undefined) {
            ...
            if (eventRow !== undefined)
                throw new CoordinationRuntimeError('invalid-state', 'graph registration event exists without its committed authoritative artifact row', [input.artifactId]);
            return null;
        }
        ...
        if (artifact.document_schema_version !== 'autopilot.semantic_graph.v1')
            throw new CoordinationRuntimeError('invalid-state', 'committed registration lookup found a non-graph artifact under the graph artifact id', [input.artifactId, artifact.document_schema_version]);
        if (artifact.source_run !== workstreamRun)
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration source run does not match the recovering run', [artifact.source_run, workstreamRun]);
        ...
        if (eventRow === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration artifact exists without its immutable registration event', [input.artifactId]);
        const eventSeq = sqlInteger(eventRow, 'event_seq');
        if (eventSeq !== expectedR)
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration event is not at R=E+1', [`event_seq=${String(eventSeq)}`, `expected_R=${String(expectedR)}`]);
        ...
        if (resultRow === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration event exists without its immutable idempotency result', [input.artifactId, idempotencyKey]);
        if (sqlInteger(resultRow, 'committed_event_seq') !== expectedR)
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration idempotency result does not commit exactly R=E+1', [`committed_event_seq=${String(sqlInteger(resultRow, 'committed_event_seq'))}`, `expected_R=${String(expectedR)}`]);
        if (sqlString(resultRow, 'request_sha256') !== sqlString(eventRow, 'request_sha256'))
            throw new CoordinationRuntimeError('invalid-state', 'committed graph registration event and idempotency result bind different request digests', [input.artifactId]);
