...
import { COORDINATOR_API_SCHEMA_VERSION, COORDINATOR_IMPLEMENTATION_BUILD, COORDINATOR_LEGACY_FACADE_BUILD, COORDINATOR_STORE_SCHEMA_VERSION, COORDINATOR_WIRE_LINEAGE, enforcePrivateAuthorityPath } from "./runtime-paths.js";
export const COORDINATOR_RUNTIME_IDENTITY_SCHEMA = 'autopilot.coordinator_runtime_identity.v1';
const SHA256_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const GENERATION_PATTERN = /^generation-[a-f0-9]{32}$/u;
function exactRecord(value, fields) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity must be an object');
    const record = value;
    const actual = Object.keys(record).sort();
    const expected = [...fields].sort();
    if (actual.length !== expected.length || actual.some((field, index) => field !== expected[index]))
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity fields are closed', actual);
    return record;
}
function text(record, field) {
    const value = record[field];
    if (typeof value !== 'string' || value.length === 0 || value.includes('\0'))
        throw new CoordinationRuntimeError('store-corrupt', `runtime identity ${field} is invalid`);
    return value;
}
function canonicalTimestamp(value) {
    if (typeof value !== 'string' || value.length !== 24)
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity published_at is invalid');
    try {
...
export function parseCoordinatorRuntimeIdentity(value) {
    ...
    if (record['schema_version'] !== COORDINATOR_RUNTIME_IDENTITY_SCHEMA
        ...
        || record['legacy_facade_build'] !== COORDINATOR_LEGACY_FACADE_BUILD)
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity build/wire/API/store split is invalid');
    ...
    if (typeof pid !== 'number' || !Number.isSafeInteger(pid) || pid < 1)
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity lifecycle_pid is invalid');
    if (typeof generation !== 'string' || !GENERATION_PATTERN.test(generation))
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity store_generation_id is invalid');
    if (typeof pointerDigest !== 'string' || !SHA256_PATTERN.test(pointerDigest))
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity pointer digest is invalid');
    return Object.freeze({
        ...
        current_store_pointer_sha256: pointerDigest,
...
export async function publishCoordinatorRuntimeIdentity(paths, generation, lifecycle, writerGuard, now = new Date()) {
    ...
    for (const name of await readdir(paths.coordinatorRoot)) {
        if (!/^\.runtime-identity\.\d+\.[a-f0-9]{16}\.tmp$/u.test(name))
            continue;
        const stale = resolve(paths.coordinatorRoot, name);
        assertRegularSingleLink(stale);
        await unlink(stale);
    }
    if (existsSync(paths.runtimeIdentityPath))
    ...
    try {
        ...
        try {
            ...
            if (written.bytesWritten !== bytes.byteLength)
                throw new CoordinationRuntimeError('system-fatal', 'runtime identity publication made a short write');
            await handle.sync();
        }
        finally {
            await handle.close();
...
export function readAndVerifyCoordinatorRuntimeIdentity(paths, generation, lifecycle) {
    assertRegularSingleLink(paths.runtimeIdentityPath);
    const bytes = readFileSync(paths.runtimeIdentityPath);
    const identity = parseCoordinatorRuntimeIdentity(JSON.parse(Buffer.from(bytes).toString('utf8')));
    const canonicalBytes = Buffer.from(`${canonicalJson(identity)}\n`, 'utf8');
    if (bytes.byteLength !== canonicalBytes.byteLength || bytes.some((byte, index) => canonicalBytes[index] !== byte))
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity bytes are not exact canonical publication bytes');
    if (identity.store_generation_id !== generation.pointer.generation_id
        ...
        || identity.lifecycle_instance_id !== lifecycle.instance_id)
        throw new CoordinationRuntimeError('store-corrupt', 'runtime identity is not bound to the current pointer and lifecycle identity');
    return Object.freeze({ identity, sha256: sha256(bytes), bytes });
}
