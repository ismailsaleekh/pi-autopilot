...
export class AutopilotChildLeaseHandle {
    ...
    get preemptionSignal() {
        return this.#preemption.signal;
    }
    async checkpoint(checkpointOrdinal, criticalSection, preemptible) {
    ...
    async completeAdjudication(assignmentId, adjudicationPath, terminalEvidence) {
        await this.#enqueue(async () => {
            if (this.#terminal)
                throw new CoordinationRuntimeError('invalid-state', 'terminal child cannot complete adjudication');
            this.assertHealthy();
            this.#stopHeartbeat();
    ...
    async markRecoveryRequired() {
        ...
        if (this.#terminal || this.#terminalCompletionUncertain)
            return;
                ...
                try {
                    ...
                    try {
                        ...
                        if (!Array.isArray(childValues))
                            throw new CoordinationRuntimeError('invalid-state', 'coordinator status child_leases is not an array');
                        ...
                        if (status === 'terminal')
                            this.#terminalCompletionUncertain = true;
                        throw error;
                    }
                    if (observed === null)
                        throw new CoordinationRuntimeError('invalid-state', 'child lease disappeared while terminal completion acknowledgement was reconciled', [this.#child.child_lease_id]);
                    if (observed.status === status) {
                        ...
                        if (!exactEvidence)
                            throw new CoordinationRuntimeError('idempotency-conflict', 'child reached the requested terminal status with different evidence', [this.#child.child_lease_id]);
                        this.#child = observed;
                        this.#terminal = true;
                        return;
                    }
                    if (observed.status !== 'running')
                        throw new CoordinationRuntimeError('invalid-state', `child completion observed contradictory ${observed.status} state`, [this.#child.child_lease_id]);
                    this.#child = observed;
                }
            }
