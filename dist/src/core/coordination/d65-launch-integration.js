...
import { AUTOPILOT_CHECKOUT_PROFILE_SNAPSHOT_FILE, checkoutProfileSnapshotFromResolved, resolveAutopilotFullCheckoutProfile } from "../checkout-profile.js";
...
import { AUTOPILOT_RUNTIME_ENV, AUTOPILOT_RUNTIME_VALUE, BRANCHES_FILE, TASK_INFO_FILE, UNIT_INDEX_FILE, writeJsonAtomic, } from "../parallel-runtime.js";
import { CoordinatorClient } from "./client.js";
import { coordinatorRuntimePaths } from "./runtime-paths.js";
import { coordinatorSessionIdIssue, parseCoordinationAuthoritativeArtifact, parseCoordinationRun, parseCoordinationSessionLease } from "./contracts.js";
import { opaqueToolCallIdIssue } from "../tool-call-id.js";
import { CoordinationRuntimeError } from "./failures.js";
import { readImmutableFileBytes } from "./immutable-file.js";
import { DurableRunSupervisorClient, readCoordinatorSessionContext, } from "./supervisor.js";
import { executeOwnedWorktreeSaga } from "./worktree-saga.js";
import { canonicalJson } from "./canonical-json.js";
import { parseD65SemanticGraphBootstrap } from "./d65-semantic-graph.js";
import { publishD65FirstCompleteGraphFromEnvironment } from "./d65-graph-successor-runtime.js";
import { ensureD65ProgramHeartbeatForGraphFromEnvironment } from "./d65-runtime-dispatch.js";
import { authenticateD65LaunchRoster } from "./d65-launch-roster.js";
import { publishRuntimeRosterSnapshot } from "../roster/snapshot.js";
import { preRunSelectionPath, resolveRosterScopePaths, rosterRevisionPath } from "../roster/paths.js";
import { ensurePrivateDirectory, publishCreateOnlyAtomic } from "../roster/transaction.js";
...
import { parseCanonicalPreRunSelectionBytes } from "../roster/run-selection.js";
...
export function activeRowFromLaunchManifest(manifest) {
    return {
...
export function repoIdentityFromLaunchManifest(manifest) {
    return {
...
function bootstrapGraphPayload(manifest) {
    return {
 ...
 */
export function verifyLaunchManifestAgainstClone(manifest, options = {}) {
    ...
    if (resolvedContent !== manifest.content_result_commit)
        throw new CoordinationRuntimeError('invalid-state', 'sealed content-result commit is not resolvable in the source clone');
    ...
    if (parents.length !== 2 || parents[1] !== manifest.b0_commit)
        throw new CoordinationRuntimeError('invalid-state', 'sealed content-result commit must have exactly one parent equal to B0', parents.slice(1));
    const contentTree = gitText(repoRoot, { kind: 'resolve-tree', revision: manifest.content_result_commit }, 'content-result tree');
    if (contentTree !== manifest.content_result_tree)
        throw new CoordinationRuntimeError('invalid-state', 'sealed content-result tree diverges from the source clone');
    const b0Tree = gitText(repoRoot, { kind: 'resolve-tree', revision: manifest.b0_commit }, 'B0 tree');
    if (b0Tree !== manifest.b0_tree)
        throw new CoordinationRuntimeError('invalid-state', 'sealed B0 tree diverges from the source clone');
    ...
    if (overlayParents.length !== 2 || overlayParents[1] !== manifest.content_result_commit)
        throw new CoordinationRuntimeError('invalid-state', 'sealed bootstrap overlay commit must have exactly one parent equal to the content-result commit', overlayParents.slice(1));
    ...
    if (overlayTree !== manifest.bootstrap_overlay.overlay_tree)
        throw new CoordinationRuntimeError('invalid-state', 'sealed bootstrap overlay tree diverges from the committed overlay commit', [overlayTree, manifest.bootstrap_overlay.overlay_tree]);
    ...
    if (overlayDiff.length !== 2 || overlayDiff[0] !== expectedOverlayPaths[0] || overlayDiff[1] !== expectedOverlayPaths[1])
        throw new CoordinationRuntimeError('invalid-state', 'sealed overlay must change exactly the bootstrap and trust authority paths', [overlayDiff.join(','), expectedOverlayPaths.join(',')]);
    ...
    if (overlayRefCommit !== manifest.bootstrap_overlay.overlay_commit)
        throw new CoordinationRuntimeError('invalid-state', 'sealed overlay ref does not resolve to the sealed overlay commit', [overlayRefCommit, manifest.bootstrap_overlay.overlay_commit]);
    ...
    if (bootstrapBlob.bytes.byteLength !== manifest.bootstrap_overlay.bootstrap_byte_count)
        throw new CoordinationRuntimeError('invalid-state', 'sealed bootstrap byte_count diverges from the committed overlay blob');
    if (bytesSha256(bootstrapBlob.bytes) !== manifest.bootstrap_overlay.bootstrap_sha256)
        throw new CoordinationRuntimeError('invalid-state', 'sealed bootstrap digest diverges from the committed overlay blob');
    ...
    if (bootstrapEnvelope.program_id !== manifest.program_id || bootstrapEnvelope.repo_id !== manifest.repo_id || bootstrapEnvelope.workstream !== manifest.workstream || bootstrapEnvelope.workstream_run !== manifest.workstream_run || bootstrapEnvelope.autopilot_id !== manifest.autopilot_id)
        throw new CoordinationRuntimeError('invalid-state', 'committed bootstrap envelope identity diverges from the sealed manifest');
    if (bootstrapEnvelope.run_timestamp !== manifest.run_timestamp || bootstrapEnvelope.run_nonce !== manifest.run_nonce)
        throw new CoordinationRuntimeError('invalid-state', 'committed bootstrap envelope run timestamp/nonce diverges from the sealed manifest', [bootstrapEnvelope.run_timestamp, manifest.run_timestamp, bootstrapEnvelope.run_nonce, manifest.run_nonce]);
    if (bootstrapEnvelope.content_commit !== manifest.content_result_commit || bootstrapEnvelope.content_tree !== manifest.content_result_tree || bootstrapEnvelope.package_commit !== manifest.package_commit || bootstrapEnvelope.package_tree !== manifest.package_tree)
        throw new CoordinationRuntimeError('invalid-state', 'committed bootstrap envelope content/package facts diverge from the sealed manifest');
    if (bootstrapEnvelope.trust_anchor_ref !== manifest.trust_anchor.trust_anchor_ref || bootstrapEnvelope.trust_anchor_sha256 !== manifest.trust_anchor.trust_anchor_sha256)
        throw new CoordinationRuntimeError('invalid-state', 'committed bootstrap envelope trust binding diverges from the sealed manifest');
    if (canonicalJson(bootstrapEnvelope.prospective_run) !== canonicalJson(manifest.prospective_run) || canonicalJson(bootstrapEnvelope.prospective_resource) !== canonicalJson(manifest.prospective_resource))
        throw new CoordinationRuntimeError('invalid-state', 'committed bootstrap envelope prospective rows diverge from the sealed manifest');
    ...
    if (trustBlob.bytes.byteLength !== 44)
        throw new CoordinationRuntimeError('invalid-state', 'sealed trust anchor must be exactly 44 SPKI bytes in the committed overlay');
    if (bytesSha256(trustBlob.bytes) !== manifest.trust_anchor.trust_anchor_sha256)
        throw new CoordinationRuntimeError('invalid-state', 'sealed trust anchor digest diverges from the committed overlay blob');
    if (trustBlob.oid !== manifest.trust_anchor.trust_anchor_blob_oid)
        throw new CoordinationRuntimeError('invalid-state', 'sealed trust anchor blob OID diverges from the committed overlay blob', [trustBlob.oid, manifest.trust_anchor.trust_anchor_blob_oid]);
    ...
    if (manifest.attach_run_idempotency_key !== `attach-run:${manifest.repo_id}:${manifest.workstream_run}`)
        throw new CoordinationRuntimeError('invalid-state', 'sealed attach-run idempotency key is not the exact frozen form');
    if (manifest.attach_session_idempotency_key !== `attach-session:${manifest.repo_id}:${manifest.workstream_run}`)
        throw new CoordinationRuntimeError('invalid-state', 'sealed attach-session idempotency key is not the exact frozen form');
    if (manifest.policy_candidate.registration_idempotency_key !== `register-launch-policy:${manifest.workstream_run}:${manifest.policy_candidate.policy_id}`)
        throw new CoordinationRuntimeError('invalid-state', 'sealed policy registration idempotency key is not the exact frozen form');
    ...
    if (!evidenceStat.isDirectory() || (evidenceStat.mode & 0o777) !== 0o700)
        throw new CoordinationRuntimeError('invalid-state', 'sealed program_evidence_root must be a mode-0700 directory', [manifest.program_evidence_root]);
    if (realpathSync(manifest.program_evidence_root) !== manifest.program_evidence_root)
        throw new CoordinationRuntimeError('invalid-state', 'sealed program_evidence_root is not its own canonical real path', [manifest.program_evidence_root]);
    ...
    if (options.loadedManifestSha256 !== undefined)
        persistD65LaunchManifestReceipt(manifest, options.loadedManifestSha256);
}
/**
 ...
 */
function requireCoordinatorPathBinding(manifest, env) {
    const paths = coordinatorRuntimePaths({ ...env, [AUTOPILOT_STATE_ROOT_ENV]: manifest.state_root });
    if (paths.stateRoot !== manifest.state_root)
        throw new CoordinationRuntimeError('invalid-state', 'coordinator state root does not equal the sealed manifest state_root', [paths.stateRoot, manifest.state_root]);
    const withSep = (root) => (root.endsWith('/') ? root : `${root}/`);
    const isInside = (child, parent) => child === parent || child.startsWith(withSep(parent));
    ...
    if (!isInside(paths.sessionsRoot, manifest.state_root))
        throw new CoordinationRuntimeError('invalid-state', 'coordinator sessions root is not inside the sealed state root', [paths.sessionsRoot, manifest.state_root]);
    if (isInside(manifest.state_root, manifest.session_root) || isInside(manifest.session_root, manifest.state_root))
        throw new CoordinationRuntimeError('invalid-state', 'sealed Pi session root must be authority-distinct from the coordinator state root', [manifest.state_root, manifest.session_root]);
}
const D65_LAUNCH_MANIFEST_RECEIPT_SCHEMA = 'autopilot.d65_launch_manifest_receipt.v1';
/**
 ...
 */
function persistD65LaunchManifestReceipt(manifest, loadedManifestSha256) {
    ...
    if (existsSync(receiptPath)) {
        ...
        if (bytesSha256(existing) !== bytesSha256(bytes))
            throw new CoordinationRuntimeError('invalid-state', 'a divergent launch-manifest receipt already exists for this run; the sealed manifest cannot be swapped between attempts', [receiptPath]);
        return;
    }
...
function requireSealedEvidence(absolutePath, expectedSha256, label) {
    ...
    if (bytesSha256(bytes) !== expectedSha256)
        throw new CoordinationRuntimeError('invalid-state', `sealed ${label} bytes diverge from the sealed digest`, [absolutePath]);
}
function readD65OverlayBlob(repoRoot, commit, path) {
    ...
    if (listing.length !== 1)
        throw new CoordinationRuntimeError('invalid-state', 'sealed overlay path did not resolve to exactly one tracked blob', [path]);
    const match = /^([0-7]{6}) (blob) ([a-f0-9]{40})\t/u.exec(listing[0] ?? '');
    if (match === null || match[1] !== '100644' || match[3] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'sealed overlay entry must be a mode-100644 blob', [path, listing[0] ?? '']);
...
export async function publishD65RuntimeRosterSnapshot(input) {
    ...
    if (selectionPublish.status === 'conflict')
        throw new CoordinationRuntimeError('invalid-state', 'a divergent external pre-run selection already exists for the sealed run', [selectionPath]);
    ...
    if (rosterPublish.status === 'conflict')
        throw new CoordinationRuntimeError('invalid-state', 'a divergent roster authority revision already exists for the sealed run', [rosterPath]);
    ...
    if (!snapshot.ok)
        throw new CoordinationRuntimeError('invalid-state', 'D65 runtime roster snapshot publication failed closed', snapshot.diagnostics.map((diagnostic) => diagnostic.code));
}
/**
 ...
 */
function excludeD65RuntimeRosterSnapshot(manifest, env) {
    ...
    if (runtimePrefix === null)
        throw new CoordinationRuntimeError('invalid-state', 'runtime root is not a descendant of the main worktree');
    ...
    if (excludeRelative.length === 0)
        throw new CoordinationRuntimeError('invalid-state', 'could not resolve the worktree-local git exclude path');
    ...
    if (lines.includes(excludePattern))
        return;
...
export async function registerD65LaunchPolicyAndInitialHeartbeat(input) {
    ...
    if (acceptedPolicy !== undefined) {
        ...
        if (acceptedPolicy.artifact_id !== policyArtifactId || acceptedPolicy.evidence.ref !== manifest.policy_candidate.policy_ref || acceptedPolicy.evidence.sha256 !== manifest.policy_candidate.policy_sha256)
            throw new CoordinationRuntimeError('invalid-state', 'accepted launch policy differs from the sealed manifest policy identity/ref/digest', [acceptedPolicy.artifact_id, acceptedPolicy.evidence.ref, acceptedPolicy.evidence.sha256]);
    }
    else {
        ...
        if (!alreadyCommitted) {
            ...
            if (signed.sha256 !== manifest.policy_candidate.policy_sha256)
                throw new CoordinationRuntimeError('invalid-state', 'signed launch policy digest differs from the sealed manifest policy digest', [signed.sha256, manifest.policy_candidate.policy_sha256]);
        ...
        if (registered.evidence.sha256 !== manifest.policy_candidate.policy_sha256)
            throw new CoordinationRuntimeError('invalid-state', 'accepted launch policy digest differs from the signed policy');
    }
...
function bootstrapDigest(statusPayload, workstreamRun) {
    ...
    if (bootstrap === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch policy stage lacks the accepted bootstrap artifact');
    return bootstrap.evidence.sha256;
}
/** Commit the operator-signed policy blob at run-main from content-result HEAD. */
async function commitD65PolicyBlob(input) {
    ...
    if (bytesSha256(signedBytes) !== input.manifest.policy_candidate.policy_sha256)
        throw new CoordinationRuntimeError('invalid-state', 'signed policy candidate bytes diverge from the sealed digest before commit');
    await mkdir(dirname(policyPath), { recursive: true });
    await writeFile(policyPath, signedBytes);
    const gitEnv = { ...input.env, GIT_AUTHOR_NAME: 'autopilot-launch', GIT_AUTHOR_EMAIL: 'autopilot-launch@example.invalid', GIT_COMMITTER_NAME: 'autopilot-launch', GIT_COMMITTER_EMAIL: 'autopilot-launch@example.invalid', GIT_AUTHOR_DATE: input.manifest.run_timestamp, GIT_COMMITTER_DATE: input.manifest.run_timestamp };
    const { runGitMutation } = await import("../git-process.js");
    ...
    if (staged.kind !== 'reported' || staged.exitCode !== 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch policy staging failed', [staged.kind === 'reported' ? staged.diagnostic : staged.reason]);
    ...
    if (committed.kind !== 'reported' || committed.exitCode !== 0)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch policy commit failed', [committed.kind === 'reported' ? committed.diagnostic : committed.reason]);
}
/**
...
async function acceptSignedGraphHeartbeat(input) {
    ...
    const acceptedSequenceRaw = typeof head === 'object' && head !== null ? head['sequence'] : null;
    ...
    if (input.heartbeatSequence === 1) {
        if (!alreadyAccepted)
            await acceptInitialGoverningHeartbeat({ client, session, manifest: input.manifest });
    }
    else {
 ...
 */
async function acceptInitialGoverningHeartbeat(input) {
    ...
    if (response.committed_event_seq === null)
        throw new CoordinationRuntimeError('invalid-state', 'initial governing heartbeat acceptance did not commit');
}
function readEvidenceHeartbeat(programEvidenceRoot, ref) {
    const path = join(programEvidenceRoot, ...ref.split('/'));
    const before = lstatSync(path);
    if ((before.mode & 0o777) !== 0o600)
        throw new CoordinationRuntimeError('invalid-state', 'initial heartbeat candidate must be mode 0600', [path]);
...
export async function publishD65FirstGraphAndSuccessorHeartbeat(input) {
    ...
    if (existingGraph === undefined) {
        ...
        if (published.graphSequence !== 2)
            throw new CoordinationRuntimeError('invalid-state', 'first complete graph publication did not produce graph sequence 2', [String(published.graphSequence)]);
...
export async function resolveD65LaunchPhase(input) {
    ...
    if (catalogRuns.length !== 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch phase resolution found more than one durable run for the sealed identity');
    ...
    if (bootstraps.length !== 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch phase requires exactly one accepted bootstrap artifact', [String(bootstraps.length)]);
    ...
    if (policies.length > 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch phase found more than one accepted launch policy');
    ...
    if (graphs.length > 1)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch phase found more than one accepted complete graph before ordinary dispatch');
    const graph = graphs[0];
    let graphSha256 = null;
    if (graph !== undefined) {
        if (graph.artifact_id !== `semantic-graph:${String(2).padStart(20, '0')}`)
            throw new CoordinationRuntimeError('invalid-state', 'the first accepted complete graph is not exact sequence-2 artifact identity', [graph.artifact_id]);
    ...
    if (graph === undefined) {
        ...
        if (!charterComplete)
            return Object.freeze({ kind: 'bootstrap-plan-required', graphSha256: null, acceptedHeartbeatSequence });
...
function acceptedHeartbeatSequenceOf(statusPayload) {
...
function requireSessionLeases(statusPayload) {
    const value = statusPayload['session_leases'];
    if (!Array.isArray(value))
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch status lacks the session-lease projection');
 ...
 */
function detectD65CharterCompleteQuiet(manifest) {
    for (const root of D65_CHARTER_ROOTS) {
        if (!existsSync(join(manifest.runtime_root, root)))
            return false;
    }
    return detectD65CharterComplete(manifest);
}
/** The exact five previously-absent charter roots the bootstrap turn writes. */
export const D65_CHARTER_ROOTS = Object.freeze(['mission.md', 'master-plan.json', 'state.json', 'decision-log.jsonl', 'events.jsonl']);
const D65_CONTEXT_BUDGET_RECEIPT_SCHEMA = 'autopilot.d65_context_budget_receipt.v1';
...
function requireD65ContextBudgetIdentities(toolCallId, sessionId) {
    const toolCallIssue = opaqueToolCallIdIssue(toolCallId);
    if (toolCallIssue !== null)
        throw new CoordinationRuntimeError('invalid-state', `D65 bootstrap context_budget tool_call_id is not a bounded opaque tool-call ID: it ${toolCallIssue}`);
    const sessionIssue = coordinatorSessionIdIssue(sessionId);
    if (sessionIssue !== null)
        throw new CoordinationRuntimeError('invalid-state', `D65 bootstrap context_budget session_id is not a bounded coordinator session identity: it ${sessionIssue}`);
}
/**
...
export function requireD65ContextBudgetReceipt(manifest, expectedSessionId) {
    ...
    if (!exactFields || record['schema_version'] !== D65_CONTEXT_BUDGET_RECEIPT_SCHEMA || record['program_id'] !== manifest.program_id || record['workstream_run'] !== manifest.workstream_run || record['gate'] !== 'ok' || typeof percent !== 'number' || !Number.isFinite(percent) || percent < 0 || percent > 100 || !identitiesValid || record['session_id'] !== expectedSessionId)
        throw new CoordinationRuntimeError('invalid-state', 'D65 context_budget receipt is malformed or belongs to another run/session', [receiptPath]);
}
/**
 ...
 */
export function detectD65CharterComplete(manifest) {
    const runtimeRoot = manifest.runtime_root;
    for (const root of D65_CHARTER_ROOTS) {
        if (!existsSync(join(runtimeRoot, root)))
            return false;
    }
    ...
    const hasStatusPrefix = (record) => record.length >= 4 && STATUS_CODES.has(record.charAt(0)) && STATUS_CODES.has(record.charAt(1)) && record.charAt(2) === ' ';
    const withinRuntime = (path) => path === runtimePrefix || path === runtimeDir || path.startsWith(runtimeDir);
    for (const record of records) {
        const path = hasStatusPrefix(record) ? record.slice(3) : record;
        if (!withinRuntime(path))
            throw new CoordinationRuntimeError('invalid-state', 'semantic-graph-bootstrap-operation-denied: parent planning touched a path outside the package-owned runtime charter scope', [path]);
    }
    return true;
}
function requireArtifacts(statusPayload) {
    const value = statusPayload['authoritative_artifacts'];
    if (!Array.isArray(value))
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch status lacks committed artifact projection');
    return value.map(parseCoordinationAuthoritativeArtifact);
}
function requireOneRun(statusPayload) {
    const value = statusPayload['runs'];
    if (!Array.isArray(value) || value.length !== 1 || value[0] === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 launch status lacks exactly one run');
