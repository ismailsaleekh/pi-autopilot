...
async function createVerifiedLockedBoundaryBackup(paths, upgradeId, database) {
    ...
    try {
        ...
        while (offset < bytes.byteLength)
            offset += (await handle.write(bytes.subarray(offset))).bytesWritten;
        await handle.sync();
    }
    finally {
        await handle.close();
...
async function verifyMigrationOnCopy(paths, record, upgradeId, retain = false) {
    ...
    try {
        ...
        if (status['package_build'] !== COORDINATOR_UPGRADE_PATH.target.package_build || status['protocol_version'] !== '1.3' || status['database_schema_version'] !== 9)
            throw new CoordinationRuntimeError('schema-mismatch', 'schema migration probe did not reach the locked schema-9 target');
    }
    finally {
        store?.close();
    }
    ...
    try {
        ...
        if (checkpoint.prepare('PRAGMA integrity_check').get()?.['integrity_check'] !== 'ok' || checkpoint.prepare('PRAGMA user_version').get()?.['user_version'] !== 9)
            throw new CoordinationRuntimeError('store-corrupt', 'private migrated target failed final schema-9 verification');
    }
    finally {
        checkpoint.close();
    }
...
function installedBarrierBackupDigest(database, upgradeId) {
    ...
    if (rows.length !== 1 || row?.['upgrade_id'] !== upgradeId || row['source_schema'] !== 6 || row['target_schema'] !== 9 || typeof digest !== 'string' || !/^sha256:[a-f0-9]{64}$/u.test(digest))
...
async function publishMigratedPrivateDatabase(paths, backupRecord, upgradeId) {
    ...
    if (current.version === 9 && current.integrity === 'ok')
        return;
    if (current.version !== COORDINATOR_UPGRADE_BARRIER_SCHEMA_VERSION || current.integrity !== 'ok')
        throw new CoordinationRuntimeError('schema-mismatch', 'schema-9 publication requires the exact incompatible migration barrier');
    ...
    try {
        if (installedBarrierBackupDigest(barrier, upgradeId) !== backupRecord.sha256)
            throw new CoordinationRuntimeError('store-corrupt', 'schema-9 publication backup does not match the committed barrier digest');
    }
    finally {
        barrier.close();
    }
    ...
    if (published.version !== 9 || published.integrity !== 'ok')
        throw new CoordinationRuntimeError('store-corrupt', 'atomically published schema-9 database failed verification');
}
