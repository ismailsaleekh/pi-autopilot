...
import { parseCentralVersionedUnitFailureIngress } from "./unit-failure-ingress.js";
...
function schemaVersionLiteral(schema, label) {
    ...
    if (typeof schemaVersion['const'] === 'string' && schemaVersion['const'].length > 0)
        return schemaVersion['const'];
    const enumValues = schemaVersion['enum'];
    if (Array.isArray(enumValues) && enumValues.length === 1 && typeof enumValues[0] === 'string' && enumValues[0].length > 0)
        return enumValues[0];
    throw new CoordinationRuntimeError('invalid-state', `${label} lacks an exact schema_version const or singleton enum`);
}
function assertSchemaVersionLiteralIfPresent(schema, expected, label) {
    const properties = asRecord(schema['properties'], `${label}.properties`);
    if (!Object.hasOwn(properties, 'schema_version'))
        return;
    const actual = schemaVersionLiteral(schema, label);
    if (actual !== expected)
        throw new CoordinationRuntimeError('invalid-state', `${label} schema_version literal differs from its catalog family`, [expected, actual]);
}
function packageContractSchema(schemaVersion) {
    ...
    if (directMatches.length > 1)
        throw new CoordinationRuntimeError('invalid-state', 'package contract schema has ambiguous direct registrations', [schemaVersion]);
    ...
    if (rosterRecord !== null)
        assertSchemaVersionLiteralIfPresent(rosterRecord, schemaVersion, `package roster contract schema ${schemaVersion}`);
...
function coordinationSchemaVersion(name, schema) {
    ...
    if (isRecord(properties)) {
        ...
        if (isRecord(schemaVersion) && typeof schemaVersion['const'] === 'string')
            return schemaVersion['const'];
    }
    if (name === 'coordinator_mailbox_page')
        return 'autopilot.coordinator_mailbox_page.v1';
    throw new CoordinationRuntimeError('invalid-state', 'coordination schema lacks an inventoried schema_version const', [name]);
}
const coordinationFamilies = Object.entries(AUTOPILOT_COORDINATION_JSON_SCHEMAS).map(([name, schema]) => currentOnlyFamily(coordinationSchemaVersion(name, schema), name.includes('request') || name.includes('response') || name.includes('page') ? 'transport-or-page' : 'coordination-store', fieldNamesFromJsonSchema(schema), requiredFieldNamesFromJsonSchema(schema), `coordination schema inventory key ${name}`));
...
function sourceAnchoredExtraArtifactRequiredFields(schemaVersion) {
    return SOURCE_ANCHORED_EXTRA_PERSISTED_ARTIFACT_REQUIRED_FIELDS[schemaVersion] ?? sourceAnchoredExtraArtifactFields(schemaVersion);
}
...
export function assertPersistedArtifactFamilyRegistryWellFormed(registry = VERSIONED_PERSISTED_ARTIFACT_FAMILY_REGISTRY) {
    ...
    for (const family of registry) {
        if (familyIds.has(family.family))
            throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry has a duplicate family', [family.family]);
        familyIds.add(family.family);
        if (family.schema_version.length === 0 || family.producer_ranges.length === 0)
            throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry family is incomplete', [family.family]);
        ...
        for (const range of ranges) {
            if (!Number.isSafeInteger(range.first_generation) || !Number.isSafeInteger(range.last_generation) || range.first_generation < 1 || range.last_generation < range.first_generation)
                throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry has an invalid producer generation range', [family.family, JSON.stringify(range)]);
            if (range.first_generation !== expectedFirst)
                throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry has a producer generation gap or overlap', [family.family, `expected=${String(expectedFirst)}`, `actual=${String(range.first_generation)}-${String(range.last_generation)}`]);
            expectedFirst = range.last_generation + 1;
            if (producerBuilds.has(range.producer_build))
                throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry has an ambiguous producer_build', [family.family, range.producer_build]);
            producerBuilds.add(range.producer_build);
            if (range.current && range.producer_build !== COORDINATOR_IMPLEMENTATION_BUILD)
                throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry current range does not use the exact package implementation build', [family.family, range.producer_build]);
            const exactFields = sortedUnique(range.exact_fields);
            if (exactFields.length !== range.exact_fields.length || exactFields.some((field, index) => field !== range.exact_fields[index]))
                throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry fields must be sorted and unique', [family.family, range.producer_build]);
            for (const required of range.required_fields) {
                if (!range.exact_fields.includes(required))
                    throw new CoordinationRuntimeError('invalid-state', 'versioned ingress registry required field is outside exact field inventory', [family.family, required]);
            }
        }
    }
}
export function persistedArtifactFamily(family, registry = VERSIONED_PERSISTED_ARTIFACT_FAMILY_REGISTRY) {
    ...
    if (matches.length !== 1 || matches[0] === undefined)
        throw new CoordinationRuntimeError('invalid-request', 'unsupported persisted artifact family', [family]);
    return matches[0];
}
export function selectVersionedIngressProducer(input) {
    ...
    if (buildMatches.length === 0)
        throw new CoordinationRuntimeError('protocol-mismatch', 'unsupported persisted artifact producer_build; compatibility is not inferred from semver or shape', [family.family, input.producer_build]);
    if (buildMatches.length > 1)
        throw new CoordinationRuntimeError('protocol-mismatch', 'ambiguous persisted artifact producer_build', [family.family, input.producer_build]);
    const range = buildMatches[0];
    if (range === undefined)
        throw new CoordinationRuntimeError('protocol-mismatch', 'unsupported persisted artifact producer_build', [family.family, input.producer_build]);
    const generation = input.producer_generation;
    if (!Number.isSafeInteger(generation))
        throw new CoordinationRuntimeError('protocol-mismatch', 'persisted artifact producer generation is required explicitly and is never inferred', [family.family, input.producer_build]);
    if (generation < range.first_generation || generation > range.last_generation)
        throw new CoordinationRuntimeError('protocol-mismatch', 'persisted artifact producer generation is outside its exact producer_build fence', [family.family, input.producer_build, String(generation)]);
...
export function parseVersionedPersistedArtifact(input) {
    ...
    if (selection.range.exact_fields.includes('schema_version') && stringField(document, 'schema_version', selection.family.family, 192) !== selection.family.schema_version)
        throw new CoordinationRuntimeError('schema-mismatch', 'persisted artifact schema_version does not match its selected family', [selection.family.family]);
    if (Object.hasOwn(document, 'producer_build') && stringField(document, 'producer_build', selection.family.family, 192) !== input.producer_build)
        throw new CoordinationRuntimeError('protocol-mismatch', 'persisted artifact producer_build field differs from selected provenance', [selection.family.family]);
    if (Object.hasOwn(document, 'producer_generation') && integerField(document, 'producer_generation', selection.family.family) !== input.producer_generation)
        throw new CoordinationRuntimeError('protocol-mismatch', 'persisted artifact producer_generation field differs from selected provenance', [selection.family.family]);
    const unknownFields = fields.filter((field) => !selection.range.exact_fields.includes(field));
    if (selection.range.unknown_field_policy === 'reject' && unknownFields.length > 0)
        throw new CoordinationRuntimeError('schema-mismatch', 'persisted artifact has unknown fields for its exact producer generation', [selection.family.family, ...unknownFields]);
    for (const field of selection.range.required_fields) {
        if (!fields.includes(field))
            throw new CoordinationRuntimeError('schema-mismatch', 'persisted artifact is missing a required field for its exact producer generation', [selection.family.family, field]);
    }
...
function stringArrayField(document, field) {
    ...
    if (!Array.isArray(value) || value.length > 4096 || value.some((entry) => typeof entry !== 'string' || entry.length === 0 || entry.length > 1024))
        throw new CoordinationRuntimeError('invalid-state', `unit failure evidence ${field} must be a bounded string array`);
...
export function parseVersionedUnitFailureIngress(input) {
    return parseCentralVersionedUnitFailureIngress(input);
}
