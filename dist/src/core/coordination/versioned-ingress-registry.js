...
function schemaConst(schema, label) {
    ...
    if (typeof schemaVersion !== 'string' || schemaVersion.length === 0)
        throw new CoordinationRuntimeError('invalid-state', `${label} lacks an exact schema_version const`);
...
export function parseVersionedPersistedArtifact(input) {
    ...
    if (stringField(document, 'schema_version', selection.family.family, 192) !== selection.family.schema_version)
