...
import { gitHead, readGitStatus, runGit, writeJsonAtomic } from "../parallel-runtime.js";
import { executeOwnedWorktreeSaga, WorktreeSagaCompensatedError } from "./worktree-saga.js";
    ...
    for (const obligation of obligations) {
        ...
        if (!predecessorAlreadyIntegrated) {
            const inspect = () => {
                ...
                if (gitAncestor(active.main_worktree_path, obligation.predecessor_terminal_sha ?? '', head))
                    return { outcome: 'satisfied', proof: [`predecessor=${obligation.predecessor_terminal_sha}`, `head=${head}`] };
            ...
            try {
                await executeOwnedWorktreeSaga({
                    ...
                    action: () => {
                        try {
                            runGit(['merge', '--no-ff', '--no-edit', '-m', `autopilot reservation integration ${obligation.obligation_id}`, obligation.predecessor_terminal_sha ?? ''], active.main_worktree_path, reservationGitEnv(env));
                        }
                        catch (error) {
                            const abort = spawnSync('git', ['merge', '--abort'], { cwd: active.main_worktree_path, encoding: 'utf8', env: reservationGitEnv(env) });
                            if ((abort.status ?? -1) !== 0 && readGitStatus(active.main_worktree_path).changedPaths.length > 0)
                                throw new CoordinationRuntimeError('recovery-required', 'reservation merge conflicted and abort failed', [obligation.obligation_id, abort.stderr.trim()]);
                    ...
                    verify: () => { const result = inspect(); if (result.outcome !== 'satisfied')
                        throw new CoordinationRuntimeError('invalid-state', 'reservation integration postcondition failed', result.proof); return result.proof; },
                }, env);
            }
            catch (error) {
...
export async function reservationCloseBlockers(active, env = process.env) {
    ...
    for (const obligation of resolvedObligations) {
        ...
        if (currentHead === null)
            ...
            if ((ancestry.status ?? -1) !== 0 || (diff.status ?? -1) !== 0 || invalidating.length > 0)
