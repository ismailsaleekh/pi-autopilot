...
import { coordinationPathsOverlap, parseCoordinationAuthoritativeArtifact, parseCoordinationChangeReservation, parseCoordinationEditLease, parseCoordinationIntegrationConflict, parseCoordinationReservationObligation, parseCoordinationRun, parseCoordinationRunResource, parseCoordinationRunTerminalIntent } from "./contracts.js";
import { canonicalJson } from "./canonical-json.js";
import { loadD65CompleteGraph } from "./d65-graph-loader.js";
import { canonicalSha256, parseD65CompleteGraph, parseD65RunTerminalIntentV2 } from "./d65-semantic-graph.js";
import { computeD65ObligationPartition, d65TerminalIntentId } from "./d65-terminal-intent.js";
...
function terminalIntentCompatible(value) {
    if (typeof value === 'object' && value !== null && !Array.isArray(value) && value['schema_version'] === 'autopilot.run_terminal_intent.v2') {
        return d65TerminalIntentV1Projection(parseD65RunTerminalIntentV2(value));
    }
    return parseCoordinationRunTerminalIntent(value);
}
function d65TerminalIntentV1Projection(intent) {
...
function parseJsonBytes(bytes, label) {
    ...
    try {
        ...
        throw new CoordinationRuntimeError('invalid-state', `${label} is not valid UTF-8`, [error instanceof Error ? error.message : String(error)]);
    }
    try {
        return JSON.parse(text);
    }
    catch (error) {
        throw new CoordinationRuntimeError('invalid-state', `${label} is not valid JSON`, [error instanceof Error ? error.message : String(error)]);
    }
}
function loadAcceptedD65TerminalHistory(status, repoId, workstreamRun) {
    ...
    if (latest === undefined)
        throw new CoordinationRuntimeError('invalid-state', 'D65 terminal preparation requires an accepted complete graph');
    const resources = parseArray(status['run_resources'], 'status.run_resources', parseCoordinationRunResource);
    if (resources.length !== 1 || resources[0]?.repo_id !== repoId || resources[0].workstream_run !== workstreamRun)
        throw new CoordinationRuntimeError('invalid-state', 'D65 terminal preparation requires exactly one matching run resource');
    ...
    if (actualRootSha !== latest.evidence.sha256)
        throw new CoordinationRuntimeError('invalid-state', 'accepted D65 graph root bytes diverge from the committed artifact digest', [latest.artifact_id, actualRootSha, latest.evidence.sha256]);
    const graph = parseD65CompleteGraph(parseJsonBytes(rootBytes, 'accepted D65 graph root'));
    if (graph.repo_id !== repoId || graph.workstream_run !== workstreamRun || d65SemanticGraphSequenceFromArtifactId(latest.artifact_id) !== graph.graph_sequence)
        throw new CoordinationRuntimeError('invalid-state', 'accepted D65 graph identity differs from terminal preparation scope');
    ...
    const attempts = loaded.coordinatorProjection.terminal_intents.map((intent) => {
        if (intent.schema_version !== 'autopilot.run_terminal_intent.v2')
            throw new CoordinationRuntimeError('invalid-state', 'D65 graph terminal history contains a legacy v1 intent');
    ...
    if (statusRows.length !== attempts.length)
        throw new CoordinationRuntimeError('invalid-state', 'accepted D65 graph does not cover the complete current terminal-intent history');
    for (let index = 0; index < attempts.length; index += 1) {
        const attempt = attempts[index];
        const statusRow = statusRows[index];
        if (attempt === undefined || statusRow === undefined)
            throw new CoordinationRuntimeError('invalid-state', 'D65 terminal history cardinality changed after exact length comparison');
        const projected = d65TerminalIntentV1Projection(attempt);
        if (canonicalJson(projected) !== canonicalJson(statusRow))
            throw new CoordinationRuntimeError('invalid-state', 'accepted D65 graph terminal-intent history differs from current coordinator state', [projected.terminal_intent_id]);
    }
...
export class ReservationCoordinationClient {
    ...
    #contextPath;
    #env;
    constructor(client, session, contextPath = null, env = null) {
        this.#client = client;
    ...
    async prepareRunTerminal(outcome) {
        ...
        let terminalIntentId;
        let d65Payload = {};
        const runtimeEnv = this.#env;
        if (d65) {
            if (runtimeEnv === null)
                throw new CoordinationRuntimeError('unauthorized-client', 'D65 terminal preparation requires the production environment-backed dispatch authority');
            ...
            if (latest !== undefined && latest.state !== 'cancelled')
                throw new CoordinationRuntimeError('invalid-state', `D65 terminal preparation cannot follow a ${latest.state} terminal intent`);
            ...
            terminalIntentId = `terminal-${this.#session.workstream_run}-${randomUUID()}`;
        if (d65) {
            if (runtimeEnv === null)
                throw new CoordinationRuntimeError('unauthorized-client', 'D65 terminal preparation lost its production dispatch authority');
    ...
    async cancelRunTerminal(intent, reason) {
        ...
        if (intent.terminal_intent_id.startsWith(`terminal-intent:${this.#session.workstream_run}:`)) {
            if (runtimeEnv === null)
                throw new CoordinationRuntimeError('unauthorized-client', 'D65 terminal cancellation requires production recovery authority');
