...
function nullableString(record, key) {
    const value = record[key];
    if (value === null)
        return null;
    if (typeof value !== 'string' || value.length === 0)
...
function nullableSeq(record, key) {
    const value = record[key];
    if (value === null)
        return null;
...
function parseRunLaneStatus(value) {
    if (value === 'running' || value === 'paused')
        return value;
    throw new Error('new_worktree_creation is invalid');
}
function parsePublicationLaneStatus(value) {
    if (value === 'open' || value === 'blocked')
        return value;
    throw new Error('publication lane status is invalid');
}
function parseProgressModel(bytes) {
    const parsed = JSON.parse(new TextDecoder().decode(bytes));
    if (!isJsonRecord(parsed))
        throw new Error('s2 pressure state must be a JSON object');
    if (requiredString(parsed, 'schema_version') !== 'autopilot.s2_retention.progress_model.v1')
        throw new Error('s2 pressure state schema_version is invalid');
    const lanesValue = parsed['lanes'];
    if (!Array.isArray(lanesValue))
        throw new Error('s2 pressure state lanes must be an array');
    const lanes = lanesValue.map((value) => {
        if (!isJsonRecord(value))
            throw new Error('s2 pressure state lane must be a JSON object');
        return {
...
async function atomicWriteUtf8(path, contents) {
    await mkdir(dirname(path), { recursive: true });
    const parentStat = await lstat(dirname(path));
    if (!parentStat.isDirectory() || parentStat.isSymbolicLink())
        throw new Error('s2 pressure state parent must be a non-symbolic directory');
...
export function createS2RetentionProgressModel(runs) {
...
export function applyS2RetentionDiskPressure(model, event) {
    if (!Number.isSafeInteger(event.eventSeq) || event.eventSeq < 0)
        throw new Error('disk pressure eventSeq must be a non-negative safe integer');
    if (event.offendingRun.length === 0)
        throw new Error('offendingRun must be non-empty');
    const lanes = new Map();
    for (const lane of model.lanes)
        lanes.set(lane.workstream_run, lane);
...
export function clearS2RetentionDiskPressure(model, run, observedEventSeq) {
    if (!Number.isSafeInteger(observedEventSeq) || observedEventSeq < 0)
        throw new Error('observedEventSeq must be a non-negative safe integer');
    return {
        schema_version: 'autopilot.s2_retention.progress_model.v1',
        lanes: model.lanes.map((lane) => {
            if (lane.workstream_run !== run)
                return lane;
            if (lane.disk_pressure_event_seq !== observedEventSeq)
                return lane;
            return {
                ...lane,
                new_worktree_creation: 'running',
...
export function s2RetentionRunsPausedForWorktreeCreation(model) {
    return model.lanes.filter((lane) => lane.new_worktree_creation === 'paused').map((lane) => lane.workstream_run);
}
export function s2RetentionPressureStatePath(retentionRoot) {
    return join(resolve(retentionRoot), S2_PRESSURE_STATE_FILE);
}
export async function readS2RetentionProgressState(path) {
    try {
        await lstat(path);
    }
    catch (error) {
        if (error instanceof Error && 'code' in error && error.code === 'ENOENT')
            return createS2RetentionProgressModel([]);
        throw error;
    }
...
export async function writeS2RetentionProgressState(path, model) {
    const bytes = canonicalJson(model);
    if (Buffer.byteLength(bytes, 'utf8') > S2_PRESSURE_STATE_MAX_BYTES)
        throw new Error('s2 retention pressure state exceeds byte limit');
    parseProgressModel(new TextEncoder().encode(bytes));
    await atomicWriteUtf8(path, bytes);
}
export async function recordS2RetentionDiskPressure(path, event) {
...
export async function clearDurableS2RetentionDiskPressure(path, run, observedEventSeq) {
