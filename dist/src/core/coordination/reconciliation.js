...
function supersededIntentRoot(active) {
...
export async function readStableRegularFile(path, label, maximumBytes) {
    let descriptor = null;
    try {
        ...
        if (!opened.isFile() || opened.size > maximumBytes)
            throw new CoordinationRuntimeError('invalid-request', `${label} must be a regular file no larger than ${String(maximumBytes)} bytes`, [path, `size=${String(opened.size)}`]);
        if (opened.dev !== before.dev || opened.ino !== before.ino)
            throw new CoordinationRuntimeError('unauthorized-client', `${label} changed while its identity was being established`, [path]);
        ...
        if (bytes.byteLength !== opened.size || afterDescriptor.size !== opened.size || afterDescriptor.dev !== opened.dev || afterDescriptor.ino !== opened.ino || afterPath.dev !== opened.dev || afterPath.ino !== opened.ino || canonicalAfter !== canonicalBefore)
            throw new CoordinationRuntimeError('unauthorized-client', `${label} changed during its atomic read`, [path]);
        return bytes;
    }
    catch (error) {
        if (error instanceof CoordinationRuntimeError)
...
async function readOwnedEvidenceBytes(active, evidencePath) {
    ...
    if (physicalRef.length === 0 || physicalRef === '..' || physicalRef.startsWith(`..${sep}`) || isAbsolute(physicalRef))
        throw new CoordinationRuntimeError('unauthorized-client', 'reconciliation evidence physically escapes the run-owned main worktree', [evidencePath]);
    return await readStableRegularFile(evidencePath, 'reconciliation evidence', MAX_COORDINATION_EVIDENCE_BYTES);
}
async function persistImmutableSupersession(path, supersession) {
    ...
    if (existsSync(path)) {
        ...
        if (existing !== bytes)
            throw new CoordinationRuntimeError('idempotency-conflict', 'historical pending reconciliation intent supersession differs from its immutable replay identity', [path]);
        return;
    }
    ...
    try {
        try {
            ...
            const existing = new TextDecoder('utf-8', { fatal: true }).decode(await readStableRegularFile(path, 'pending reconciliation intent supersession', MAX_PENDING_RECONCILIATION_INTENT_BYTES));
            if (existing !== bytes)
                throw new CoordinationRuntimeError('idempotency-conflict', 'concurrent historical pending reconciliation supersession differs from its immutable replay identity', [path]);
        }
    }
    finally {
...
function evidenceIdentity(active, intent) {
...
async function supersedeHistoricalPendingIntent(input) {
    if (input.intent.source !== 'attempt-reset' && input.intent.source !== 'quarantine-capture')
        return false;
    if (classifyHistoricalUnitFailureEvidenceGeneration(input.evidenceBytes) === null)
        return false;
    ...
    if (input.intent.source !== 'attempt-reset')
        throw new CoordinationRuntimeError('invalid-state', 'historical reset/abort pending evidence cannot satisfy a quarantine-capture intent', [input.intentPath]);
    const evidenceSha256 = `sha256:${createHash('sha256').update(input.evidenceBytes).digest('hex')}`;
    if (candidate.originalSha256 !== input.intent.evidence_sha256 || evidenceSha256 !== input.intent.evidence_sha256)
        throw new CoordinationRuntimeError('invalid-state', 'historical pending reconciliation evidence digest differs from its immutable intent', [input.intentPath]);
    const supersession = {
...
export async function replayPendingCoordinatorReconciliation(input) {
    ...
    for (const name of names) {
        ...
        try {
            ...
            value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(intentBytes));
        }
        catch (error) {
            if (error instanceof CoordinationRuntimeError)
                throw error;
        ...
        if (await supersedeHistoricalPendingIntent({ active: input.active, intentPath: path, intentBytes, intent, evidenceBytes: currentBytes }))
            continue;
