...
import { AUTOPILOT_CLOSURE_GATE_STATUS_VALUES, AUTOPILOT_EXCEPTION_STATE_VALUES, AUTOPILOT_WORK_ITEM_STATE_VALUES, parseAutopilotStatusEntry, } from "../contracts/index.js";
import { array, boolean, isJsonObject, object, str, } from "./d65-semantic-graph.js";
import { CoordinationRuntimeError } from "./failures.js";
const PROJECTION_ID = /^gp:(?:work-item|bughunt|exception):[a-f0-9]{64}$/u;
const UNIT_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u;
const WORK_ITEM_STATES = new Set(AUTOPILOT_WORK_ITEM_STATE_VALUES);
const EXCEPTION_STATES = new Set(AUTOPILOT_EXCEPTION_STATE_VALUES);
const CLOSURE_STATES = new Set(AUTOPILOT_CLOSURE_GATE_STATUS_VALUES);
...
function boundedText(record, field, label, maximum = 4096) {
    return str(record, field, label, maximum);
}
function nullableText(record, field, label, maximum = 4096) {
    if (record[field] === null)
        return null;
    return boundedText(record, field, label, maximum);
}
function stringArray(value, label, maximumItems = 100_000) {
...
function nullableUnitId(record, field, label) {
    const value = nullableText(record, field, label, 128);
    if (value !== null && !UNIT_ID.test(value))
        fail(`${label}.${field} is not a bounded unit id`);
    return value;
}
function sortedMembers(members) {
    ...
    for (let index = 1; index < sorted.length; index += 1) {
        if (sorted[index - 1]?.identity === sorted[index]?.identity)
            fail('projection contains a duplicate encoded identity', [sorted[index]?.identity ?? '']);
    }
    return Object.freeze(sorted);
}
/** Exact projection-only encoded identity. Natural bytes are never truncated. */
export function d65GraphProjectionIdentity(namespace, naturalIdentity) {
    if (naturalIdentity.length === 0 || (namespace === 'work-item' && naturalIdentity.includes('\u0000')))
        fail('projection natural identity is invalid', [namespace]);
    const identity = `gp:${namespace}:${sha256Hex(naturalIdentity)}`;
    if (!PROJECTION_ID.test(identity))
        fail('projection identity encoding is invalid', [identity]);
    return identity;
}
export function parseD65WorkItemProjection(value) {
    ...
    if (!WORK_ITEM_STATES.has(state))
        fail(`${label}.state is invalid`, [state]);
...
export function parseD65BughuntProjection(value) {
    ...
    if (record['role'] !== 'bughunt')
        fail(`${label}.role must be bughunt`);
    const coveredWitnessIds = record['covered_witness_ids'];
    if (coveredWitnessIds !== null)
        stringArray(coveredWitnessIds, `${label}.covered_witness_ids`);
    const packageValue = { ...record };
    if (coveredWitnessIds === null)
        delete packageValue['covered_witness_ids'];
    const parsed = parseAutopilotStatusEntry(packageValue);
    if (parsed.role !== 'bughunt')
        fail(`${label}.role must be bughunt`);
    return Object.freeze({ ...parsed, covered_witness_ids: parsed.covered_witness_ids ?? null });
}
export function parseD65ExceptionProjection(value) {
    ...
    if (exceptionKind === 'scope') {
        ...
        if (!UNIT_ID.test(unitId))
            fail(`${label}.unit_id is invalid`, [unitId]);
        return Object.freeze({
...
export function parseD65ClosureProjection(value) {
    if (value === null)
        return null;
    ...
    if (!CLOSURE_STATES.has(status))
        fail(`${label}.status is invalid`, [status]);
    return Object.freeze({
        status,
...
export function assertD65ProjectionMembersClosed(kind, members) {
    if (kind !== 'work_items' && kind !== 'bughunt' && kind !== 'exceptions')
        return;
    const seenNatural = new Set();
    for (const member of members) {
        if (member.kind !== kind)
            fail('projection member kind differs from its index', [kind, member.kind, member.identity]);
        let expected;
        let natural;
        if (kind === 'work_items') {
        ...
        if (seenNatural.has(natural))
            fail('projection contains a duplicate natural identity', [kind, natural]);
        seenNatural.add(natural);
        if (member.identity !== expected)
            fail('projection identity does not bind its complete natural identity', [kind, member.identity, expected]);
    }
}
/** Derive all non-coordinator graph projections from independently parsed package authority. */
export function normalizeD65NonCoordinatorProjections(masterPlan, state, discoveredStatuses) {
    if (masterPlan.workstream !== state.workstream)
        fail('master plan and state workstream disagree');
    const planUnitIds = Object.keys(masterPlan.units).sort();
    const stateUnitIds = Object.keys(state.units).sort();
    if (planUnitIds.length !== stateUnitIds.length || planUnitIds.some((unitId, index) => unitId !== stateUnitIds[index]))
        fail('state unit set does not exactly equal the master-plan unit set');
    const unitIds = new Set(planUnitIds);
    const workItemMembers = [];
    for (const [mapKey, workItem] of Object.entries(state.work_items ?? {})) {
        if (mapKey !== workItem.work_item_id)
            fail('work-item map key differs from embedded work_item_id', [mapKey, workItem.work_item_id]);
        for (const unitId of workItem.unit_ids)
            if (!unitIds.has(unitId))
                fail('work-item names a unit outside the exact plan/state unit set', [mapKey, unitId]);
        if (workItem.implementation_unit_id !== undefined && !unitIds.has(workItem.implementation_unit_id))
            fail('work-item implementation unit is outside the exact unit set', [mapKey, workItem.implementation_unit_id]);
        if (workItem.validation_unit_id !== undefined && !unitIds.has(workItem.validation_unit_id))
            fail('work-item validation unit is outside the exact unit set', [mapKey, workItem.validation_unit_id]);
    ...
    const bughuntMembers = [];
    const bughuntByRef = new Map();
    for (const discovered of discoveredStatuses) {
        if (statusRefs.has(discovered.runtime_ref))
            fail('two discovered status authorities use the same runtime ref', [discovered.runtime_ref]);
        statusRefs.add(discovered.runtime_ref);
        if (discovered.status.workstream !== state.workstream)
            fail('discovered status belongs to a different workstream', [discovered.runtime_ref]);
        if (discovered.status.role !== 'bughunt')
            continue;
        ...
        const member = Object.freeze({ identity: d65GraphProjectionIdentity('bughunt', natural), kind: 'bughunt', value });
        if (bughuntByRef.has(discovered.runtime_ref))
            fail('bughunt status ref is ambiguous', [discovered.runtime_ref]);
    ...
    for (const exception of state.scope_exceptions ?? []) {
        if (exceptionIds.has(exception.exception_id))
            fail('duplicate operator-facing exception_id across namespaces', [exception.exception_id]);
    ...
    if (state.closure_gate !== undefined) {
        ...
        if (bughuntRef !== null && !bughuntByRef.has(bughuntRef))
            fail('closure bughunt_status_ref does not resolve to exactly one discovered bughunt status', [bughuntRef]);
