...
import { CoordinationContractError } from "./contracts.js";
import { CoordinationRuntimeError } from "./failures.js";
import { AUTOPILOT_COORDINATOR_PROTOCOL_VERSION } from "./types.js";
export const COORDINATOR_ADMISSION_OFFER_SCHEMA = 'autopilot.coordinator_admission_offer.v1';
export const COORDINATOR_ADMISSION_REQUEST_SCHEMA = 'autopilot.coordinator_admission_request.v1';
export const COORDINATOR_ADMISSION_RESPONSE_SCHEMA = 'autopilot.coordinator_admission_response.v1';
export const COORDINATOR_ADMISSION_ACTION = 'negotiate-admission';
export const COORDINATOR_ADMISSION_ALGORITHM = 'hmac-sha256';
export const COORDINATOR_ADMISSION_DOMAIN = 'pi-autopilot/admission/v1\0';
...
function boundedString(record, key, label, maximum = 512) {
    const value = record[key];
    if (typeof value !== 'string' || value.length < 1 || [...value].length > maximum || /[\u0000-\u001f\u007f]/u.test(value))
        fail(label, `${key} must be a bounded non-control string`);
    return value;
}
function safeInteger(record, key, label, minimum = 0) {
    const value = record[key];
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < minimum)
        fail(label, `${key} must be a safe integer >= ${String(minimum)}`);
    return value;
}
function stringArray(record, key, label, allowEmpty) {
    const value = record[key];
    if (!Array.isArray(value) || (!allowEmpty && value.length === 0) || value.length > COORDINATOR_ADMISSION_VOCABULARY.length + 16)
        fail(label, `${key} must be a bounded${allowEmpty ? '' : ' non-empty'} array`);
    const entries = [];
    for (const entry of value) {
        if (typeof entry !== 'string' || !VOCABULARY_IDENTIFIER.test(entry))
            fail(label, `${key} entries must be bounded vocabulary identifiers`);
        entries.push(entry);
    }
...
function parseIdentity(identity) {
    if (!BUILD_IDENTIFIER.test(identity.implementationBuild))
        fail('CoordinatorAdmissionIdentity', 'implementationBuild is malformed');
    if (identity.wireLineage.length < 1 || identity.wireLineage.length > 192)
        fail('CoordinatorAdmissionIdentity', 'wireLineage is malformed');
    if (!Number.isSafeInteger(identity.apiSchemaVersion) || identity.apiSchemaVersion < 1)
        fail('CoordinatorAdmissionIdentity', 'apiSchemaVersion is malformed');
    if (!Number.isSafeInteger(identity.storeSchemaVersion) || identity.storeSchemaVersion < 1)
        fail('CoordinatorAdmissionIdentity', 'storeSchemaVersion is malformed');
    if (identity.knownClientBuilds.length < 1)
        fail('CoordinatorAdmissionIdentity', 'knownClientBuilds must be non-empty');
    const known = [...identity.knownClientBuilds];
    if (known.some((build) => !BUILD_IDENTIFIER.test(build)))
        fail('CoordinatorAdmissionIdentity', 'knownClientBuilds contains a malformed build');
    const sorted = [...new Set(known)].sort();
    if (sorted.length !== known.length)
        fail('CoordinatorAdmissionIdentity', 'knownClientBuilds must be unique');
    return Object.freeze({ ...identity, knownClientBuilds: Object.freeze(known) });
}
function parseEndpointFacts(value, label) {
    const record = {
    ...
    if (!SHA256.test(legacyLockSha256))
        fail(label, 'legacy_lock_sha256 must be sha256:<64-lowercase-hex>');
    if (!SHA256.test(runtimeIdentitySha256))
        fail(label, 'runtime_identity_sha256 must be sha256:<64-lowercase-hex>');
    if (!GENERATION_IDENTIFIER.test(storeGenerationId))
        fail(label, 'store_generation_id is malformed');
...
function parseRequestedVocabulary(record, key, label) {
    return stringArray(record, key, label, false);
}
function parseGrantedVocabulary(record, label) {
    const entries = stringArray(record, 'granted_vocabulary', label, true);
    const granted = [];
    for (const entry of entries) {
        if (!isCoordinatorAdmissionVocabulary(entry))
            fail(label, 'granted_vocabulary contains an unknown value');
        granted.push(entry);
    }
    return Object.freeze(granted);
}
export function isCoordinatorAdmissionVocabulary(value) {
    return COORDINATOR_ADMISSION_VOCABULARY.some((entry) => entry === value);
}
export function parseCoordinatorAdmissionOffer(value, expectedWireLineage) {
    const label = 'CoordinatorAdmissionOffer';
    const record = object(value, label, OFFER_FIELDS);
    return Object.freeze({
        schema_version: exactString(record, 'schema_version', COORDINATOR_ADMISSION_OFFER_SCHEMA, label),
        action: exactString(record, 'action', COORDINATOR_ADMISSION_ACTION, label),
        algorithm: exactString(record, 'algorithm', COORDINATOR_ADMISSION_ALGORITHM, label),
        wire_lineage: exactString(record, 'wire_lineage', expectedWireLineage, label),
    });
}
export function createCoordinatorAdmissionOffer(identityValue) {
...
export function parseCoordinatorAdmissionRequest(value, identityValue) {
    const identity = parseIdentity(identityValue);
    ...
    if (!BUILD_IDENTIFIER.test(clientBuild))
        fail(label, 'client_build is malformed');
    const nonce = boundedString(record, 'nonce', label, 64);
    if (!LOWER_HEX_64.test(nonce))
        fail(label, 'nonce must be exactly 32 random bytes encoded as 64 lowercase hex');
    return Object.freeze({
        schema_version: exactString(record, 'schema_version', COORDINATOR_ADMISSION_REQUEST_SCHEMA, label),
        ...
        api_schema_version: (() => {
            ...
            if (version !== identity.apiSchemaVersion)
                fail(label, `api_schema_version must equal ${String(identity.apiSchemaVersion)}`);
            return version;
        })(),
...
export function parseCoordinatorAdmissionRequestEnvelope(value, identityValue) {
    ...
    if (!REQUEST_IDENTIFIER.test(requestId))
        fail(label, 'request_id is malformed');
    if (record['idempotency_key'] !== null || record['workstream_run'] !== null || record['session_id'] !== null || record['fencing_generation'] !== null || record['expected_version'] !== null)
        fail(label, 'admission envelope authority fields must be null');
    return Object.freeze({
        schema_version: exactString(record, 'schema_version', 'autopilot.coordinator_request.v1', label),
        protocol_version: exactString(record, 'protocol_version', AUTOPILOT_COORDINATOR_PROTOCOL_VERSION, label),
        request_id: requestId,
        action: exactString(record, 'action', COORDINATOR_ADMISSION_ACTION, label),
        idempotency_key: null,
        repo_id: exactString(record, 'repo_id', 'global', label),
        workstream_run: null,
        session_id: null,
...
function unsignedAdmissionResponse(response) {
    return Object.freeze({
...
function decodeCapability(capability) {
    if (!LOWER_HEX_64.test(capability))
    ...
    if (key.byteLength !== 32)
        throw new CoordinationRuntimeError('unauthorized-client', 'coordinator capability proof has an invalid decoded length');
    return key;
}
function admissionHmac(unsigned, capability) {
...
export function createCoordinatorAdmissionResponse(input) {
    ...
    const granted = admitted
        ? Object.freeze(request.requested_vocabulary.map((entry) => {
            if (!isCoordinatorAdmissionVocabulary(entry))
                throw new CoordinationRuntimeError('system-fatal', 'admitted vocabulary escaped its closed registry');
            return entry;
        }))
        : Object.freeze([]);
...
export function parseCoordinatorAdmissionResponse(value, identityValue) {
    const identity = parseIdentity(identityValue);
    ...
    if (!BUILD_IDENTIFIER.test(actualClientBuild))
        fail(label, 'actual_client_build is malformed');
    ...
    if (typeof admitted !== 'boolean')
        fail(label, 'admitted must be boolean');
    const result = record['result'];
    if (result !== 'admitted' && result !== 'rejected')
        fail(label, 'result must be admitted or rejected');
    if (admitted !== (result === 'admitted'))
        fail(label, 'admitted and result disagree');
    if (admitted) {
        if (requested.some((entry) => !isCoordinatorAdmissionVocabulary(entry)))
            fail(label, 'an admitted response requests unknown vocabulary');
        if (requested.length !== granted.length || requested.some((entry, index) => entry !== granted[index]))
            fail(label, 'an admitted response must grant exactly the requested vocabulary');
    }
    else if (granted.length !== 0)
        fail(label, 'a rejected response must grant an empty vocabulary');
    const nonce = boundedString(record, 'nonce', label, 64);
    const attestation = boundedString(record, 'attestation', label, 64);
    if (!LOWER_HEX_64.test(nonce))
        fail(label, 'nonce must be 64 lowercase hex');
    if (!LOWER_HEX_64.test(attestation))
        fail(label, 'attestation must be 64 lowercase hex');
    ...
    if (apiSchemaVersion !== identity.apiSchemaVersion)
        fail(label, `api_schema_version must equal ${String(identity.apiSchemaVersion)}`);
    if (storeSchemaVersion !== identity.storeSchemaVersion)
        fail(label, `store_schema_version must equal ${String(identity.storeSchemaVersion)}`);
    return Object.freeze({
        schema_version: exactString(record, 'schema_version', COORDINATOR_ADMISSION_RESPONSE_SCHEMA, label),
        admission_mode: exactString(record, 'admission_mode', 'negotiated-s1', label),
        admitted,
...
export function verifyCoordinatorAdmissionResponse(input) {
    ...
    if (actualHmac.byteLength !== 32)
        throw new CoordinationRuntimeError('unauthorized-client', 'coordinator admission attestation has an invalid decoded length');
    ...
    if (expectedHmac.byteLength !== 32 || !timingSafeEqual(actualHmac, expectedHmac))
        throw new CoordinationRuntimeError('unauthorized-client', 'coordinator admission attestation was rejected');
    const expectedEndpoint = parseEndpointFacts(input.expected, 'CoordinatorAdmissionVerificationExpectation');
    if (response.actual_client_build !== input.expected.actualClientBuild)
        throw new CoordinationRuntimeError('protocol-mismatch', 'coordinator admission changed the actual client build');
    if (response.nonce !== input.expected.nonce)
        throw new CoordinationRuntimeError('unauthorized-client', 'coordinator admission nonce does not match this socket');
    if (!equalStringArrays(response.requested_vocabulary, input.expected.requestedVocabulary))
        throw new CoordinationRuntimeError('protocol-mismatch', 'coordinator admission changed the requested vocabulary');
    if (response.admitted !== input.expected.admitted)
        throw new CoordinationRuntimeError('protocol-mismatch', 'coordinator admission result was not the expected policy outcome');
    ...
    if (response.legacy_lock_sha256 !== expectedEndpoint.legacy_lock_sha256)
        throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator admission legacy-lock digest changed');
    if (response.runtime_identity_sha256 !== expectedEndpoint.runtime_identity_sha256)
        throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator admission runtime-identity digest changed');
    if (response.store_generation_id !== expectedEndpoint.store_generation_id)
        throw new CoordinationRuntimeError('coordinator-unavailable', 'coordinator admission store generation changed');
    return response;
}
export function sha256CoordinatorAuthorityBytes(bytes) {
