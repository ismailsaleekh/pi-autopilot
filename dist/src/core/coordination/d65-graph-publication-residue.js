...
function withD65PublicationLock(mainWorktreePath, action) {
    ...
    try {
        try {
            writeSync(descriptor, Buffer.from(`${String(process.pid)}\n`, 'utf8'), 0, Buffer.byteLength(`${String(process.pid)}\n`, 'utf8'), null);
            fsyncSync(descriptor);
        }
        catch { /* lock content is advisory; presence is authority */ }
        return action();
    }
    finally {
        ...
        catch { /* best-effort release; a stale lock fails closed on next acquire */ }
    }
}
