...
import { executeOwnedWorktreeSaga, inspectOwnedWorktreeSpecPostcondition, WorktreeSagaCompensatedError } from "./coordination/worktree-saga.js";
            ...
            await executeOwnedWorktreeSaga(mergeSpec, {
                action: async () => {
                    ...
                    if (postcondition.outcome === 'unsafe')
                        throw new CoordinationRuntimeError('recovery-required', 'unit merge canonical probe found unsafe repository state', postcondition.proof);
                    if (!postcondition.proof.includes('interrupted_merge'))
                        throw new CoordinationRuntimeError('recovery-required', 'unit merge did not apply and left no canonically compensable merge state', [...postcondition.proof, `mutation_report=${merge.kind}`, `mutation_diagnostic=${merge.diagnostic}`]);
                    ...
                    if (restored.outcome !== 'not-applied' || restored.effect_applied || restored.proof.includes('interrupted_merge'))
                        fail('merge-abort-failed', 'unit merge conflicted and canonical abort probe did not restore the pre-merge state.', [abort.diagnostic, conflictPath, ...restored.proof]);
...
async function activeMainObservationBlockers(active, changedPaths, env) {
    ...
    if (contextPath === undefined || contextPath.trim().length === 0)
        fail('coordinator-session-missing', 'unit merge requires the current owner session before mutating the shared run main worktree.');
    const session = await readCoordinatorSessionContext(contextPath);
    if (session.repo_id !== active.repo_key || session.autopilot_id !== active.autopilot_id || session.workstream_run !== active.workstream_run)
