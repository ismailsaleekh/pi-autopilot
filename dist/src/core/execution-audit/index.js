...
function readGitStatusSnapshot(cwd) {
    try {
        ...
        if (gitHead === null)
            return Object.freeze({ available: false, gitHead: null, changedPaths: [], summary: 'git status succeeded but HEAD was unavailable' });
        return Object.freeze({
            available: true,
            gitHead,
...
function readGitHead(cwd) {
    let result;
    try {
        result = runGitQuery({ descriptor: { kind: 'head' }, cwd });
    }
    catch (error) {
        if (error instanceof GitQueryError)
            return null;
