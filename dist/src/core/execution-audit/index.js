...
export function buildAutopilotExecutionAudit(input) {
    ...
    const actualChangedPaths = auditAvailable
        ? sortedDifference(input.postRun.changedPaths, input.baseline.dirtyPaths).filter((path) => !matchesPathPattern(path, runtimeRoot))
        : [];
...
function classifyAudit(input) {
    ...
    if (!input.auditAvailable || input.baselineDirty)
...
function auditSummary(classification, auditAvailable, baselineDirty) {
    if (!auditAvailable)
        return 'Execution audit could not read git status; semantic closure requires review.';
    if (baselineDirty)
        return 'Execution audit found a dirty baseline; semantic closure requires review.';
