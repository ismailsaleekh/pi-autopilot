 ...
 */
export function opaqueToolCallIdIssue(value) {
    if (typeof value !== 'string')
        return 'must be an opaque string';
    const codePoints = Array.from(value).length;
    if (codePoints < 1 || codePoints > AUTOPILOT_TOOL_CALL_ID_MAX_CODE_POINTS)
        return `must contain 1..${String(AUTOPILOT_TOOL_CALL_ID_MAX_CODE_POINTS)} Unicode code points`;
    if (value.includes('\u0000'))
        return 'must not contain NUL';
    return null;
}
/** Closed schema shared verbatim by receipt and terminal-acceptance contracts. */
export function opaqueToolCallIdJsonSchema() {
