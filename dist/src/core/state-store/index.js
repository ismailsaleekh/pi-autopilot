...
export async function appendAutopilotEventRow(input) {
    ...
    if (event.id !== expectedId) {
        throw new AutopilotStateStoreError('event-id-not-monotonic', `event id ${String(event.id)} must equal next monotonic id ${String(expectedId)}`);
    }
...
export async function readAutopilotResumeSnapshot(input) {
    ...
    if (newestEvent !== undefined && newestEvent.id !== state.last_event_id) {
        throw new AutopilotStateStoreError('state-event-id-mismatch', `state.last_event_id ${String(state.last_event_id)} does not match events tail id ${String(newestEvent.id)}`);
    }
    if (events.length === 0 && state.last_event_id !== 0) {
        throw new AutopilotStateStoreError('state-event-id-mismatch', `state.last_event_id ${String(state.last_event_id)} is non-zero but events.jsonl is empty/missing`);
    }
    const emptyStatuses = {};
    const emptyReceipts = {};
    const emptySpecs = {};
    const refs = input.validateReferences === false
    ...
    for (const unit of Object.values(state.units)) {
        ...
        if (unit.spec_ref !== undefined) {
            ...
            if (spec.unit_id !== unit.unit_id || spec.role !== unit.role || spec.attempt !== unit.attempt) {
                throw new AutopilotStateStoreError('spec-ref-mismatch', `${unit.spec_ref} identity does not match state unit ${unit.unit_id}`);
            }
        ...
        if (unit.status_ref !== undefined) {
            ...
            if (status.workstream !== state.workstream ||
                ...
                throw new AutopilotStateStoreError('status-ref-mismatch', `${unit.status_ref} identity does not match state unit ${unit.unit_id}`);
            }
 ...
 */
export async function readAutopilotEventsIfPresent(eventsPath) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        try {
            ...
            throw new AutopilotStateStoreError('corrupt-events-jsonl', `events.jsonl line ${String(index + 1)} is not valid JSON: ${errorMessage(error)}`);
        }
        let event;
        try {
            ...
            throw new AutopilotStateStoreError('corrupt-events-jsonl', `events.jsonl line ${String(index + 1)} failed Autopilot event validation: ${errorMessage(error)}`);
        }
        const previous = lastEvent(events);
        const expectedId = previous === undefined ? 1 : previous.id + 1;
        if (event.id !== expectedId) {
            throw new AutopilotStateStoreError('corrupt-events-jsonl', `events.jsonl line ${String(index + 1)} id ${String(event.id)} must equal ${String(expectedId)}`);
        }
        events.push(event);
    }
...
async function readJsonObject(path, label) {
    try {
        ...
        throw new AutopilotStateStoreError('missing-reference', `${label} is not readable at ${path}: ${errorMessage(error)}`);
    }
...
function validateQueueStateCoherence(state) {
    ...
    for (const [queueName, unitIds, expectedState] of queueExpectations) {
        for (const unitId of unitIds) {
            ...
            if (unit.state !== expectedState) {
                throw new AutopilotStateStoreError('queue-state-mismatch', `${queueName} contains ${unitId} but units.${unitId}.state is ${unit.state}, expected ${expectedState}`);
            }
        }
    }
}
