...
export async function readAutopilotPurposeSnapshot(input) {
    ...
    if (!Number.isInteger(decisionTailLimit) || decisionTailLimit < 0 || decisionTailLimit > 10_000) {
        throw new AutopilotPurposeStoreError('invalid-decision-tail-limit', `decisionTailLimit must be an integer in [0, 10000], got ${String(decisionTailLimit)}`);
    }
    ...
    if (input.requirePurpose === true) {
        if (mission === null)
            throw new AutopilotPurposeStoreError('missing-mission', `missing mission.md at ${missionPath}`);
        if (masterPlan === null) {
    ...
    if (masterPlan !== null) {
        ...
        if (masterPlan.last_decision_id !== newestDecisionId) {
            throw new AutopilotPurposeStoreError('master-plan-decision-id-mismatch', `master-plan last_decision_id ${String(masterPlan.last_decision_id)} does not match decision log tail ${String(newestDecisionId)}`);
        }
    }
...
export async function readAutopilotMissionIfPresent(path) {
    if (!existsSync(path))
        return null;
...
export async function readAutopilotMasterPlanIfPresent(path) {
    if (!existsSync(path))
        return null;
    const stats = await stat(path);
    if (!stats.isFile()) {
...
export async function appendAutopilotDecisionRow(input) {
    ...
    if (decision.id !== expectedId) {
        throw new AutopilotPurposeStoreError('decision-id-not-monotonic', `decision id ${String(decision.id)} must equal next monotonic id ${String(expectedId)}`);
    }
    await mkdir(dirname(input.decisionLogPath), { recursive: true });
    await appendFile(input.decisionLogPath, `${JSON.stringify(decision)}\n`, { encoding: 'utf8' });
}
export async function readAutopilotDecisionsIfPresent(decisionLogPath) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        try {
            ...
            throw new AutopilotPurposeStoreError('corrupt-decision-jsonl', `decision-log.jsonl line ${String(index + 1)} is not valid JSON: ${errorMessage(error)}`);
        }
        const decision = parseAutopilotDecisionRow(parsed);
        const previous = decisions.length === 0 ? undefined : decisions[decisions.length - 1];
        const expectedId = previous === undefined ? 1 : previous.id + 1;
        if (decision.id !== expectedId) {
            throw new AutopilotPurposeStoreError('corrupt-decision-jsonl', `decision-log.jsonl line ${String(index + 1)} id ${String(decision.id)} must equal ${String(expectedId)}`);
        }
        decisions.push(decision);
    }
    return Object.freeze(decisions);
}
function extractMissionSections(text) {
    const sections = [];
...
async function readJsonObject(path, label) {
    ...
    try {
        ...
        throw new AutopilotPurposeStoreError('corrupt-json-reference', `${label} is not valid JSON at ${path}: ${errorMessage(error)}`);
    }
