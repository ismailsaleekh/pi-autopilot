...
import { parseCoordinationAuthoritativeArtifact, parseCoordinationReconciliationEvidence, parseCoordinationRun, parseCoordinationRunTerminalIntent, parseCoordinationSessionLease, parseCoordinationWorktree } from "./coordination/contracts.js";
import { assertD65OrdinaryBoundaryFromEnvironment, assertD65RecoveryBoundaryFromEnvironment, ensureD65ProgramHeartbeatForGraphFromEnvironment } from "./coordination/d65-runtime-dispatch.js";
import { publishD65CoordinatorOnlySuccessorFromEnvironment } from "./coordination/d65-graph-successor-runtime.js";
...
import { publishS2ColdTerminalProof, verifyS2HotTerminalProofSummary } from "./coordination/s2-retention-archive.js";
import { defineS2RetentionPolicy } from "./coordination/s2-retention-policy.js";
...
async function isD65CoordinatorRun(active, env) {
    ...
    if (!Array.isArray(artifacts))
        throw new AutopilotCloseError('coordination-authority-invalid', 'D65 classification lacks authoritative_artifacts.');
...
async function assertD65TerminalCleanupBoundary(active, env) {
    if (await isD65CoordinatorRun(active, env))
        await assertD65RecoveryBoundaryFromEnvironment('terminal-tail', TERMINAL_AFTER_COMMIT_BINDINGS, env);
}
async function publishAndAuthenticateD65PreparedTerminalSuccessor(env, createdAt) {
    const published = await publishD65CoordinatorOnlySuccessorFromEnvironment({ env, createdAt });
    if (published.semanticEventType !== null && published.semanticEventType !== 'run-terminal-prepared')
        throw new AutopilotCloseError('terminal-successor-event-mismatch', 'D65 terminal successor publication covered a semantic event other than terminal preparation.', [published.semanticEventType]);
            ...
            try {
                ...
                if (d65 && existsSync(existingManifestPath)) {
                    ...
                    if (manifest.outcome !== 'closed')
                        fail('terminal-intent-outcome-mismatch', 'existing D65 terminal cleanup manifest is not a close intent.', [existingManifestPath]);
                    const workstreamAfter = gitHead(active.main_worktree_path);
                    if (manifest.terminal_sha !== workstreamAfter || manifest.result.target_after !== workstreamAfter || manifest.result.workstream_after !== workstreamAfter)
                        fail('terminal-successor-context-mismatch', 'prepared D65 close manifest no longer equals the immutable workstream terminal authority.', [manifest.terminal_sha, workstreamAfter]);
                    ...
                    if (terminalIntent.outcome !== 'closed')
                        fail('terminal-intent-outcome-mismatch', 'prepared coordinator terminal intent is for abort, not close.', [terminalIntent.terminal_intent_id]);
                    ...
                    if (targetAfter !== manifest.terminal_sha) {
                        if (targetAfter !== manifest.result.target_before)
                            fail('terminal-successor-context-mismatch', 'D65 close resume target is neither the immutable pre-effect authority nor the terminal authority.', [targetAfter, manifest.result.target_before ?? '<null>', manifest.terminal_sha]);
                    ...
                    if (targetAfter !== manifest.terminal_sha)
                        fail('terminal-successor-context-mismatch', 'D65 close terminal Git effect did not land the immutable prepared authority.', [targetAfter, manifest.terminal_sha]);
                    targetLanded = true;
                ...
                if (coordinatorAuthority && !d65 && terminalIntent === null)
                    terminalIntent = await (await ReservationCoordinationClient.fromEnvironment(env)).prepareRunTerminal('closed');
                ...
                if (d65 && terminalIntent === null)
                    await assertD65OrdinaryBoundaryFromEnvironment('unit-merge', env);
                ...
                await assertPrivateTerminalArchiveAncestry(context, true);
                const proposedClosedActive = { ...active, status: 'closed', active_epoch_started_at: now.toISOString() };
                const proposedClosedContext = { ...context, active: proposedClosedActive };
                ...
                if (d65)
                    await fastForwardD65TerminalTarget({ active, sourceRepo: context.repo.repoRoot, targetBefore });
                else
                ...
                if (targetAfter !== preparedTargetAfter)
                    fail('terminal-successor-context-mismatch', 'close terminal Git effect differs from its immutable prepared workstream authority.', [targetAfter, preparedTargetAfter]);
                const mergeEvent = {
                ...
                if (d65 && terminalIntent !== null)
                    fail('terminal-successor-required', `D65 close is durably prepared${targetLanded ? ' after final product mutation' : ''}; publish/accept its prepared-terminal successor graph and governing heartbeat, then resume the same intent: ${errorMessage(error)}`, [terminalIntent.terminal_intent_id]);
                ...
                if (terminalIntent !== null && !targetLanded) {
                    try {
                        ...
                        recoveryFailures.push(`terminal-intent cancellation/cadence failed: ${errorMessage(recoveryError)}`);
                    }
                }
                try {
    ...
    try {
        return await withAutopilotFileLock(closeMergeLockPath(prepared, env), `autopilot-abort:${prepared.active.autopilot_id}:${prepared.active.workstream_run}`, async () => {
            ...
            try {
                ...
                if (d65 && existsSync(existingManifestPath)) {
                    const manifest = await readTerminalCleanupManifest(active);
                    if (manifest.outcome !== 'aborted')
                        fail('terminal-intent-outcome-mismatch', 'existing D65 terminal cleanup manifest is not an abort intent.', [existingManifestPath]);
                    const terminalSha = existsSync(active.main_worktree_path) ? gitHead(active.main_worktree_path) : manifest.terminal_sha;
                    if (manifest.terminal_sha !== terminalSha || manifest.result.workstream_after !== terminalSha)
                        fail('terminal-successor-context-mismatch', 'prepared D65 abort manifest no longer equals the preserved workstream authority.', [manifest.terminal_sha, terminalSha]);
                    if (terminalIntent === null)
                        terminalIntent = await (await ReservationCoordinationClient.fromEnvironment(env)).prepareRunTerminal('aborted');
                    if (terminalIntent.outcome !== 'aborted')
                        fail('terminal-intent-outcome-mismatch', 'prepared coordinator terminal intent is for close, not abort.', [terminalIntent.terminal_intent_id]);
                    await publishAndAuthenticateD65PreparedTerminalSuccessor(env, manifest.prepared_at);
                    await assertD65RecoveryBoundaryFromEnvironment('terminal-tail', TERMINAL_RECOVERY_BINDINGS, env);
                ...
                if (d65 && terminalIntent !== null)
                    fail('terminal-successor-required', `D65 abort is durably prepared; publish/accept its prepared-terminal successor graph and governing heartbeat, then resume the same intent: ${errorMessage(error)}`, [terminalIntent.terminal_intent_id]);
                const recoveryFailures = [];
                if (terminalIntent !== null) {
                    try {
...
export async function recoverAutopilotTerminalCleanup(options) {
    ...
    const terminalRowsByCoordinator = [];
    for (const row of candidateRows) {
        ...
        if (candidateRuns.length === 1 && candidateRun !== undefined && (candidateRun.status === 'closed' || candidateRun.status === 'aborted'))
            terminalRowsByCoordinator.push(row);
    }
...
async function completeTerminalCleanup(recovery, observer) {
    ...
    if (context.active.coordination_authority === 'coordinator-edit-leases-v1')
        await verifyS2TerminalRetentionBinding(context.active, manifest);
...
async function fastForwardD65TerminalTarget(input) {
    const desired = gitHead(input.active.main_worktree_path);
    const current = gitHead(input.sourceRepo);
    if (current === desired)
        return;
    if (current !== input.targetBefore)
        fail('terminal-successor-context-mismatch', 'D65 terminal target moved away from its immutable pre-effect authority.', [current, input.targetBefore, desired]);
    if (!isAncestor(input.sourceRepo, current, desired))
        fail('terminal-successor-context-mismatch', 'D65 terminal authority is not a fast-forward descendant of the immutable target.', [current, desired]);
    ...
    if (mutation.kind !== 'reported' || mutation.exitCode !== 0 || gitHead(input.sourceRepo) !== desired)
        throw new CoordinationRuntimeError('git-partial-effect', 'D65 terminal Git effect did not report and verify one exact fast-forward', [mutation.kind === 'reported' ? mutation.diagnostic : mutation.reason, current, desired]);
}
...
async function writeCloseAttempt(context, now, outsideRunAuthority = false) {
...
async function writeAndRecordRunTerminalEvidence(active, outcome, terminalSha, env, now) {
    ...
    if (accepted !== null)
        await publishAndVerifyS2TerminalRetention({ active, outcome, evidencePath, accepted, now });
}
function s2RetentionRootForActive(active) {
    return join(active.worktree_root, '_retention');
}
function s2TerminalRetentionBindingPath(active) {
...
async function publishAndVerifyS2TerminalRetention(input) {
    ...
    const evidenceDocument = requireRecord(JSON.parse(new TextDecoder().decode(evidenceBytes)), 'S2 terminal proof evidence');
    const evidence = input.accepted.evidence.release_condition.evidence;
    if (evidence === null)
        fail('s2-retention-evidence-missing', 'S2 terminal retention requires accepted terminal evidence identity.', [input.active.workstream_run]);
    const published = await publishS2ColdTerminalProof({
        repoId: input.active.repo_key,
        workstreamRun: input.active.workstream_run,
        terminalEventSeq: input.accepted.evidence.accepted_event_seq,
        terminalKind: input.outcome,
        terminalProof: {
            ...evidenceDocument,
            reconciliation_evidence_id: input.accepted.evidence.reconciliation_evidence_id,
            accepted_event_seq: input.accepted.evidence.accepted_event_seq,
    ...
    const summary = await verifyS2HotTerminalProofSummary({
        ...
        archiveRoot: join(retentionRoot, 'cold'),
        policy,
        expected: { repoId: input.active.repo_key, workstreamRun: input.active.workstream_run, terminalEventSeq: input.accepted.evidence.accepted_event_seq, terminalKind: input.outcome },
    });
    if (!published.hotEligible || summary.cold_archive_sha256 !== published.coldArchiveSha256)
        fail('s2-retention-hot-ineligible', 'S2 hot terminal summary is not backed by the verified cold archive.', [input.active.workstream_run]);
    const binding = {
        schema_version: 'autopilot.s2_retention.terminal_binding.v1',
        repo_id: input.active.repo_key,
        workstream_run: input.active.workstream_run,
        terminal_kind: input.outcome,
        terminal_event_seq: input.accepted.evidence.accepted_event_seq,
        evidence_ref: evidence.ref,
        evidence_sha256: evidence.sha256,
        reconciliation_evidence_id: input.accepted.evidence.reconciliation_evidence_id,
...
async function verifyS2TerminalRetentionBinding(active, manifest) {
    ...
    const path = existsSync(activePath) ? activePath : archivePath;
    if (!existsSync(path))
        fail('s2-retention-binding-missing', 'Coordinator terminal cleanup requires S2 cold retention binding before cleanup proceeds.', [active.workstream_run]);
    const row = requireRecord(JSON.parse(await readFile(path, 'utf8')), 'S2 terminal retention binding');
    ...
    const unknown = Object.keys(row).filter((field) => !fields.includes(field));
    if (unknown.length > 0 || fields.some((field) => !(field in row)))
        fail('s2-retention-binding-invalid', 'S2 terminal retention binding has an incompatible closed shape.', unknown);
    const terminalKind = manifest.outcome;
    if (row['schema_version'] !== 'autopilot.s2_retention.terminal_binding.v1' || row['repo_id'] !== active.repo_key || row['workstream_run'] !== active.workstream_run || row['terminal_kind'] !== terminalKind || row['hot_eligible'] !== true)
        fail('s2-retention-binding-mismatch', 'S2 terminal retention binding does not match exact run terminal identity.', [active.workstream_run]);
    const eventSeq = row['terminal_event_seq'];
    const hotSummaryRef = row['hot_summary_ref'];
    if (typeof eventSeq !== 'number' || !Number.isSafeInteger(eventSeq) || eventSeq < 0 || typeof hotSummaryRef !== 'string')
        fail('s2-retention-binding-invalid', 'S2 terminal retention binding lost event or summary identity.', [active.workstream_run]);
