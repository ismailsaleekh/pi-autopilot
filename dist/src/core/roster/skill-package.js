...
function fail(reason, message, diagnostics = []) {
...
function parseJsonObject(text, label) {
    ...
    if (!isRecord(parsed))
        fail('payload-contract', `${label} must be a JSON object`);
    return parsed;
}
function sortedCopy(values) {
    return [...values].sort((left, right) => left.localeCompare(right));
}
function expectKeys(record, expectedKeys, label) {
...
function expectRecord(value, label) {
    if (!isRecord(value))
        fail('payload-contract', `${label} must be an object`);
    return value;
}
function expectString(value, expected, label) {
    if (value !== expected)
        fail('payload-contract', `${label} changed`, [`expected=${expected}`, `actual=${String(value)}`]);
}
function expectNumber(value, expected, label) {
    if (value !== expected)
        fail('payload-contract', `${label} changed`, [`expected=${expected}`, `actual=${String(value)}`]);
}
function expectBoolean(value, expected, label) {
    if (value !== expected)
        fail('payload-contract', `${label} changed`, [`expected=${String(expected)}`, `actual=${String(value)}`]);
}
function expectStringArray(value, expected, label) {
    if (!Array.isArray(value))
        fail('payload-contract', `${label} must be an array`);
    const actual = [];
    for (const entry of value) {
        if (typeof entry !== 'string')
            fail('payload-contract', `${label} must contain only strings`);
        actual.push(entry);
    }
    if (actual.length !== expected.length) {
...
function packageRelativeSegments(relativePath) {
...
function readPhysicalPackageFile(packageRoot, relativePath, label) {
    ...
    for (const segment of segments) {
        ...
        if (info.isSymbolicLink())
            fail('symlink', `${label} contains a symbolic link`, [`resource=${cursor}`]);
    }
    const info = lstatSync(target);
    if (!info.isFile())
        fail('not-file', `${label} is not a regular file`, [`resource=${target}`]);
...
function assertPhysicalPackageRoot(packageRoot) {
    const normalizedRoot = resolve(packageRoot);
    let info;
    try {
    ...
    if (info.isSymbolicLink())
        fail('symlink', 'roster setup package root is a symbolic link', [`package_root=${normalizedRoot}`]);
    if (!info.isDirectory())
        fail('package-layout', 'roster setup package root is not a directory', [`package_root=${normalizedRoot}`]);
    return normalizedRoot;
}
function verifyPackageManifest(packageRoot) {
...
function assertHash(label, bytes, expectedHash, expectedByteCount) {
...
function assertSkillContentContract(skillText) {
...
function expectAuthorities(value) {
    if (!Array.isArray(value))
...
function packageRootForModulePath(modulePath) {
    ...
    if (modulePath === join(sourceRoot, SOURCE_MODULE_RELATIVE_PATH))
        return sourceRoot;
    const distRoot = resolve(dirname(modulePath), '..', '..', '..', '..');
    if (modulePath === join(distRoot, DIST_MODULE_RELATIVE_PATH))
        return distRoot;
...
export function verifyAutopilotRosterSetupSkillPackageRoot(packageRoot) {
...
export function resolveAutopilotRosterSetupSkillPackage(moduleUrl = import.meta.url) {
    let modulePath;
    try {
