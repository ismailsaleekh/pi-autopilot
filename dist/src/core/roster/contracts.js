import { AUTOPILOT_ROSTER_CHILD_ROLE_ORDER, AUTOPILOT_ROSTER_PROFILE_VALUES, AUTOPILOT_ROSTER_ROLE_ORDER, AUTOPILOT_ROSTER_SCHEMA_VERSION_VALUES, } from "../contracts/types.js";
import { canonicalRosterJson, parseRosterJsonWithDuplicateKeyRejection, rosterCanonicalSha256, rosterCanonicalSha256Hex, rosterCanonicalSha256OmittingOwnField, } from "./canonical.js";
function deepFreezeRosterAuthority(value, seen = new WeakSet()) {
    ...
    for (const key of Reflect.ownKeys(objectValue)) {
        deepFreezeRosterAuthority(objectValue[key], seen);
    }
...
export { AUTOPILOT_ROSTER_CHILD_ROLE_ORDER, AUTOPILOT_ROSTER_FREEZE_ID, AUTOPILOT_ROSTER_PROFILE_VALUES, AUTOPILOT_ROSTER_ROLE_ORDER, AUTOPILOT_ROSTER_SCHEMA_VERSION_VALUES, } from "../contracts/types.js";
export const AUTOPILOT_ROSTER_SCHEMA_ID_BASE = 'urn:pi-autopilot:schemas:roster';
export const AUTOPILOT_ROSTER_PACKAGE_VERSION_TARGET = '1.3.0';
export const AUTOPILOT_ROSTER_PI_CONTRACT_BASELINE = '0.80.6';
export const AUTOPILOT_ROSTER_DEFAULT_USER_STATE_ROOT = '~/.pi/agent/autopilot/';
export class AutopilotRosterContractValidationError extends Error {
    schemaVersion;
    issues;
    constructor(schemaVersion, issues) {
...
const AUTOPILOT_ROSTER_CONTRACT_SCHEMA_CATALOG = AUTOPILOT_ROSTER_CONTRACT_SCHEMA_DEFINITIONS;
const ROSTER_SCHEMA_VERSION_SET = new Set(AUTOPILOT_ROSTER_SCHEMA_VERSION_VALUES);
...
const ZERO_WRITE_RESULT_SCHEMAS = new Set([
    'autopilot.existing_run_resolution_result.v1',
    'autopilot.historical_fixed_roster_adapter_result.v1',
    'autopilot.receipt_validation_result.v1',
]);
export const AUTOPILOT_ROSTER_CONTRACT_JSON_SCHEMAS = buildRosterJsonSchemas();
export function isAutopilotRosterContractSchemaVersion(value) {
    return ROSTER_SCHEMA_VERSION_SET.has(value);
}
export function getAutopilotRosterContractSchemaDefinition(schemaVersion) {
    return AUTOPILOT_ROSTER_CONTRACT_SCHEMA_CATALOG[schemaVersion];
}
export function getAutopilotRosterJsonSchema(schemaVersion) {
    return AUTOPILOT_ROSTER_CONTRACT_JSON_SCHEMAS[schemaVersion];
}
export function autopilotRosterContractJsonSchemaSha256(schemaVersion) {
    return rosterCanonicalSha256(AUTOPILOT_ROSTER_CONTRACT_JSON_SCHEMAS[schemaVersion]);
}
export function autopilotRosterContractCanonicalJson(value) {
    return canonicalRosterJson(value);
}
export function autopilotRosterContractSha256(value) {
    return rosterCanonicalSha256(value);
}
export function autopilotRosterContractSha256OmittingOwnField(value, omittedField) {
    return rosterCanonicalSha256OmittingOwnField(value, omittedField);
}
export function autopilotRosterContractHashField(schemaVersion) {
    return AUTOPILOT_ROSTER_CONTRACT_SCHEMA_CATALOG[schemaVersion].hash_field ?? null;
}
export function autopilotRosterContractIssues(schemaVersion, value) {
    const issues = [];
    validateSchemaValue(schemaVersion, value, schemaVersion, issues);
    if (issues.length === 0)
        issues.push(...semanticIssues(schemaVersion, requireRecord(value, schemaVersion, issues)));
    return Object.freeze(issues);
}
export function assertAutopilotRosterContract(schemaVersion, value) {
    const issues = autopilotRosterContractIssues(schemaVersion, value);
    if (issues.length > 0)
        throw new AutopilotRosterContractValidationError(schemaVersion, issues);
}
export function parseAutopilotRosterContract(schemaVersion, value) {
    assertAutopilotRosterContract(schemaVersion, value);
    return value;
}
export function parseAutopilotRosterContractJson(schemaVersion, text) {
    return parseAutopilotRosterContract(schemaVersion, parseRosterJsonWithDuplicateKeyRejection(text));
}
export function parseAutopilotRoster(value) {
    return parseAutopilotRosterContract('autopilot.roster.v1', value);
}
export function parseAutopilotRosterCandidateSet(value) {
    return parseAutopilotRosterContract('autopilot.roster_candidate_set.v1', value);
}
export function parseAutopilotRosterConfig(value) {
    return parseAutopilotRosterContract('autopilot.roster_config.v1', value);
}
export function parseAutopilotPreRunSelection(value) {
    return parseAutopilotRosterContract('autopilot.pre_run_selection.v1', value);
}
export function parseAutopilotUnitSpecV2(value) {
    return parseAutopilotRosterContract('autopilot.unit_spec.v2', value);
}
export function parseAutopilotReceiptV2(value) {
    return parseAutopilotRosterContract('autopilot.receipt.v2', value);
}
export function parseAutopilotHistoricalFixedRosterAdapterRequest(value) {
    return parseAutopilotRosterContract('autopilot.historical_fixed_roster_adapter_request.v1', value);
}
export function parseAutopilotHistoricalFixedRosterAdapterAdmission(value) {
    return parseAutopilotRosterContract('autopilot.historical_fixed_roster_adapter_admission.v1', value);
}
export function parseAutopilotHistoricalFixedRosterAdapterResult(value) {
    return parseAutopilotRosterContract('autopilot.historical_fixed_roster_adapter_result.v1', value);
}
export function computeAutopilotRosterCandidateSetId(candidateSet) {
    const hex = rosterCanonicalSha256Hex(omitCandidateSetIdentity(candidateSet));
    return `candidate-set-${hex.slice(0, 16)}`;
}
export function computeAutopilotAssignmentSetSha256(assignments) {
...
export function computeAutopilotRosterContractObjectHash(schemaVersion, value) {
...
function buildRosterJsonSchemas() {
    const output = {};
    ...
    return deepFreezeRosterAuthority(output);
}
function fieldToJsonSchema(field) {
    const schema = fieldToNonNullJsonSchema(field);
    if (!field.nullable || (field.type === 'enum' && field.values?.includes(null) === true))
        return schema;
    return Object.freeze({ anyOf: [schema, { type: 'null' }] });
}
function fieldToNonNullJsonSchema(field) {
    if (field.type === 'enum')
        return Object.freeze({ enum: [...(field.values ?? [])] });
...
function validateSchemaValue(schemaVersion, value, label, issues) {
    const definition = AUTOPILOT_ROSTER_CONTRACT_SCHEMA_CATALOG[schemaVersion];
    ...
    for (const fieldName of definition.field_order) {
        ...
        const field = definition.fields[fieldName];
        if (field !== undefined)
            validateField(field, record[fieldName], `${label}/${fieldName}`, issues);
    }
}
function validateField(field, value, label, issues) {
    if (value === null) {
        if (!field.nullable)
            issues.push(`${label} must not be null`);
        return;
    }
    if (field.type === 'enum') {
...
function validateArray(field, value, label, issues) {
    ...
    for (let index = 0; index < value.length; index += 1) {
        if (!hasOwn(value, String(index)))
            issues.push(`${label}/${String(index)} is a sparse array hole`);
        if (field.items !== undefined)
            validateField(field.items, value[index], `${label}/${String(index)}`, issues);
    }
    if (field.uniqueItems === true)
        pushUniqueItemIssues(value, label, issues);
    if (field.uniqueBy !== undefined)
        pushUniqueByIssues(value, field.uniqueBy, label, issues);
    if (field.orderedBy !== undefined)
        pushOrderIssues(value, field.orderedBy, label, issues);
}
function semanticIssues(schemaVersion, record) {
    if (record === undefined)
        return [];
    const issues = [];
    const hashField = AUTOPILOT_ROSTER_CONTRACT_SCHEMA_CATALOG[schemaVersion].hash_field;
    if (hashField !== undefined)
        pushHashFieldIssues(schemaVersion, record, hashField, issues);
    ...
    if (schemaVersion === 'autopilot.roster.v1')
        pushRosterIssues(record, issues);
    if (schemaVersion === 'autopilot.roster_candidate_set.v1')
        pushCandidateSetIssues(record, issues);
    if (schemaVersion === 'autopilot.roster_config.v1')
        pushRosterConfigIssues(record, issues);
    if (schemaVersion === 'autopilot.roster_setup_receipt.v1')
        pushRosterSetupReceiptIssues(record, issues);
    if (schemaVersion === 'autopilot.unit_spec.v2')
        pushUnitSpecV2Issues(record, issues);
    if (schemaVersion === 'autopilot.receipt.v2')
        pushReceiptV2Issues(record, issues);
    ...
    if (schemaVersion === 'autopilot.historical_fixed_roster_adapter_admission.v1')
        pushHistoricalAdmissionIssues(record, issues);
    if (schemaVersion === 'autopilot.historical_fixed_roster_adapter_result.v1')
        pushHistoricalResultIssues(record, issues);
    if (ZERO_WRITE_RESULT_SCHEMAS.has(schemaVersion))
        pushZeroWriteResultIssues(record, schemaVersion, issues);
    if (schemaVersion === 'autopilot.roster_tool_result.v1')
        pushRosterToolResultIssues(record, issues);
    if (schemaVersion === 'autopilot.receipt_validation_result.v1')
        pushReceiptValidationResultIssues(record, issues);
    return issues;
}
function pushHashFieldIssues(schemaVersion, record, hashField, issues) {
    const actual = stringField(record, hashField);
    if (actual === undefined)
        return;
    const expected = rosterCanonicalSha256OmittingOwnField(record, hashField);
    if (actual !== expected)
        issues.push(`${schemaVersion}/${hashField} hash mismatch: expected ${expected}, got ${actual}`);
}
function pushModelRelationIssues(record, label, issues) {
...
function pushRosterIssues(record, issues) {
    ...
    if (assignments !== undefined && assignmentSet !== undefined) {
        ...
        for (const assignment of assignments) {
            ...
            if (assignmentHash !== undefined)
                assignmentHashes.push(assignmentHash);
        }
        if (assignmentHashes.length === assignments.length) {
            ...
            if (assignmentSet !== expected)
                issues.push(`roster assignment_set_sha256 mismatch: expected ${expected}, got ${assignmentSet}`);
        }
    }
    if (record['scope'] !== record['selected_scope'])
        issues.push('roster scope and selected_scope must match');
    if (record['generation_source'] === 'w0-non-certifying-seed') {
        if (record['certification_manifest_id'] !== null)
            issues.push('W0 seed roster certification_manifest_id must be null');
        if (record['certification_manifest_sha256'] !== null)
            issues.push('W0 seed roster certification_manifest_sha256 must be null');
    }
}
function pushCandidateSetIssues(record, issues) {
    ...
    if (candidateSetId !== undefined) {
        ...
        if (candidateSetId !== expected)
            issues.push(`candidate_set_id mismatch: expected ${expected}, got ${candidateSetId}`);
    }
        ...
        for (const candidate of candidates) {
            ...
            if (sortKey !== undefined) {
                if (sortKeys.has(sortKey))
                    issues.push(`candidates duplicate candidate_sort_key ${JSON.stringify(sortKey)}`);
                sortKeys.add(sortKey);
            }
        }
    }
}
function pushRosterConfigIssues(record, issues) {
    ...
    if (defaultId === undefined || defaultRevision === undefined || defaultHash === undefined || rosters === undefined)
        return;
    let matches = 0;
    for (const roster of rosters) {
        ...
        if (rosterRecord === undefined)
            continue;
        if (stringField(rosterRecord, 'roster_id') === defaultId &&
    ...
    if (matches !== 1)
        issues.push('roster_config default roster tuple must match exactly one saved roster ref');
}
function pushRosterSetupReceiptIssues(record, issues) {
    if (record['fresh_session_required'] !== true)
        issues.push('roster_setup_receipt fresh_session_required must be true');
    if (record['zero_secrets'] !== true)
        issues.push('roster_setup_receipt zero_secrets must be true');
    ...
    if (savedRosters !== undefined && approved !== undefined) {
        ...
        if (savedHashes.every((entry) => entry !== undefined)) {
...
function pushUnitSpecV2Issues(record, issues) {
    if (record['template'] !== record['role'])
        issues.push('unit_spec.v2 template must equal role');
    ...
    if (verificationPlan !== null)
        pushVerificationPlanV1Issues(verificationPlan, 'unit_spec.v2/verification_plan', issues);
    const upstreamRefs = recordArray(record, 'upstream_refs');
    if (upstreamRefs !== undefined) {
        for (const [index, ref] of upstreamRefs.entries())
            pushUpstreamRefV1Issues(ref, `unit_spec.v2/upstream_refs/${String(index)}`, issues);
    }
}
function pushReceiptV2Issues(record, issues) {
    ...
    if (requestProfile === undefined || observedProfile === undefined)
        return;
    ...
    if (requestModelId !== undefined) {
        if (stringField(observedProfile, 'requested_model_id') !== requestModelId)
            issues.push('receipt.v2 observed requested_model_id must equal request_profile.model_id');
        if (stringField(observedProfile, 'executed_model_id') !== requestModelId)
            issues.push('receipt.v2 observed executed_model_id must equal request_profile.model_id');
    }
    if (providerIdentity !== undefined) {
        if (stringField(providerIdentity, 'provider_id') !== stringField(requestProfile, 'provider_id'))
            issues.push('receipt.v2 provider_identity provider_id must equal request_profile.provider_id');
        if (stringField(providerIdentity, 'requested_model_id') !== requestModelId)
            issues.push('receipt.v2 provider_identity requested_model_id must equal request_profile.model_id');
        if (stringField(providerIdentity, 'executed_model_id') !== requestModelId)
            issues.push('receipt.v2 provider_identity executed_model_id must equal request_profile.model_id');
        if (stringField(providerIdentity, 'api') !== stringField(requestProfile, 'api'))
            issues.push('receipt.v2 provider_identity api must equal request_profile.api');
        if (stringField(providerIdentity, 'thinking_level') !== stringField(requestProfile, 'thinking'))
            issues.push('receipt.v2 provider_identity thinking_level must equal request_profile.thinking');
    }
}
function pushHistoricalAdmissionIssues(record, issues) {
    const admitted = record['admitted'];
    const reason = record['reason'];
    if (admitted === true && reason !== 'admitted')
        issues.push('historical admission admitted=true requires reason admitted');
    if (admitted === false && reason === 'admitted')
        issues.push('historical admission admitted=false requires a fail-closed reason');
    if (record['historical_bytes_mutated'] !== false)
        issues.push('historical admission must preserve historical bytes');
    if (admitted === true) {
        if (record['pre_run_selection_absent'] !== true)
            issues.push('historical admission requires absent pre-run selection');
        if (record['no_conflicting_evidence'] !== true)
            issues.push('historical admission requires no conflicting evidence');
    }
}
function pushHistoricalResultIssues(record, issues) {
    if (record['historical_bytes_mutated'] !== false)
        issues.push('historical adapter result must preserve historical bytes');
    const ok = record['ok'];
    ...
    if (ok === true) {
        for (const field of selectedFields)
            if (record[field] === null)
                issues.push(`historical adapter ok result requires ${field}`);
    }
    if (ok === false) {
        for (const field of selectedFields)
            if (record[field] !== null)
                issues.push(`historical adapter blocked result requires null ${field}`);
    }
}
function pushZeroWriteResultIssues(record, schemaVersion, issues) {
    if (record['write_count'] !== 0)
        issues.push(`${schemaVersion} must be zero-write`);
    if (record['lock_count'] !== 0)
        issues.push(`${schemaVersion} must be zero-lock`);
    const filesTouched = recordArray(record, 'files_touched');
    if (filesTouched !== undefined && filesTouched.length !== 0)
        issues.push(`${schemaVersion} must not touch files`);
}
function pushRosterToolResultIssues(record, issues) {
...
function pushReceiptValidationResultIssues(record, issues) {
    if (record['action'] !== 'validate-receipt')
        issues.push('receipt_validation_result action must be validate-receipt');
}
function pushProviderIdentityV1Issues(value, label, issues) {
    const record = requireRecord(value, label, issues);
    if (record === undefined)
        return;
    ...
    for (const key of V1_PROVIDER_IDENTITY_KEYS) {
        if (!hasOwn(record, key))
            ...
            expectString(record[key], `${label}/${key}`, issues, { type: 'string', required: true, nullable: false, minLength: 1, maxLength: 200 });
    }
}
function pushVerificationPlanV1Issues(value, label, issues) {
    const record = requireRecord(value, label, issues);
    if (record === undefined)
        return;
    ...
    for (const key of V1_VERIFICATION_PLAN_KEYS) {
        ...
        if (entries === undefined)
            continue;
        for (const [index, witness] of entries.entries())
            pushWitnessV1Issues(witness, `${label}/${key}/${String(index)}`, issues);
    }
}
function pushWitnessV1Issues(value, label, issues) {
    const record = requireRecord(value, label, issues);
    if (record === undefined)
        return;
    checkKnownKeys(record, V1_WITNESS_KEYS, label, issues);
    for (const key of ['id', 'expected_signal']) {
        if (!hasOwn(record, key))
            issues.push(`${label} missing required property ${JSON.stringify(key)}`);
        else
            expectString(record[key], `${label}/${key}`, issues, { type: 'string', required: true, nullable: false, minLength: 1, maxLength: 1000 });
    }
    if (!hasOwn(record, 'required'))
        issues.push(`${label} missing required property "required"`);
    else if (typeof record['required'] !== 'boolean')
        issues.push(`${label}/required must be boolean`);
    for (const key of ['command', 'inspection_target', 'blocker_reason']) {
        if (hasOwn(record, key))
            expectString(record[key], `${label}/${key}`, issues, { type: 'string', required: true, nullable: false, minLength: 1, maxLength: 1000 });
    }
}
function pushUpstreamRefV1Issues(value, label, issues) {
    const record = requireRecord(value, label, issues);
    if (record === undefined)
        return;
    checkKnownKeys(record, V1_UPSTREAM_REF_KEYS, label, issues);
    for (const key of ['unit_id', 'purpose']) {
        if (!hasOwn(record, key))
            issues.push(`${label} missing required property ${JSON.stringify(key)}`);
        else
            expectString(record[key], `${label}/${key}`, issues, { type: 'string', required: true, nullable: false, minLength: 1, maxLength: 1000 });
    }
    for (const key of ['status_ref', 'audit_ref']) {
        if (hasOwn(record, key))
            expectString(record[key], `${label}/${key}`, issues, { type: 'string', required: true, nullable: false, minLength: 1, maxLength: 4096 });
    }
}
function compareRecordFields(left, right, fields, label, issues) {
    for (const field of fields) {
        if (left[field] !== right[field])
            issues.push(`${label} ${field} mismatch`);
    }
}
function omitCandidateSetIdentity(value) {
    const record = requireRecord(value, 'candidate_set_id preimage', []);
    if (record === undefined)
        return {};
    const output = {};
    for (const key of Object.keys(record)) {
        if (key !== 'candidate_set_id' && key !== 'candidate_set_sha256')
...
function expectString(value, label, issues, field) {
    ...
    if (field.format === 'utc-ms-z' && !UTC_MS_Z_PATTERN.test(value))
        issues.push(`${label} must be UTC millisecond timestamp ending in Z`);
}
function expectInteger(value, label, issues, minimum, maximum) {
    if (typeof value !== 'number' || !Number.isInteger(value)) {
        issues.push(`${label} must be integer`);
        return;
    }
    if (minimum !== undefined && value < minimum)
        issues.push(`${label} must be >= ${String(minimum)}`);
    if (maximum !== undefined && value > maximum)
        issues.push(`${label} must be <= ${String(maximum)}`);
}
function pushUniqueItemIssues(values, label, issues) {
    const seen = new Set();
...
function pushUniqueByIssues(values, uniqueBy, label, issues) {
    const seen = new Set();
    for (const value of values) {
        ...
        if (seen.has(key))
            issues.push(`${label} duplicates ${uniqueBy} ${JSON.stringify(key)}`);
        seen.add(key);
    }
}
function uniqueByKey(record, uniqueBy) {
    if (uniqueBy === 'model_id+api')
        return stringTupleKey(record, 'model_id', 'api');
    if (uniqueBy === 'roster_id+revision')
        return tupleKey(record, 'roster_id', 'roster_revision');
    const uniqueValue = record[uniqueBy];
    return typeof uniqueValue === 'string' || typeof uniqueValue === 'number' ? String(uniqueValue) : undefined;
}
function pushOrderIssues(values, orderedBy, label, issues) {
    ...
    for (let index = 1; index < keys.length; index += 1) {
        ...
        if (previous === undefined || current === undefined)
            continue;
        if (previous > current)
            issues.push(`${label} must be ordered by ${orderedBy}`);
...
function orderKey(value, orderedBy) {
    if (orderedBy === 'lexicographic')
    ...
    if (orderedBy === 'roster_id_then_revision' || orderedBy === 'roster_id,roster_revision')
        return tupleKey(record, 'roster_id', 'roster_revision');
    if (orderedBy === 'route_policy_id_then_revision')
        return tupleKey(record, 'route_policy_id', 'revision');
    if (orderedBy === 'recipe_id_then_recipe_revision')
        return tupleKey(record, 'recipe_id', 'recipe_revision');
    if (orderedBy === 'model_id,api')
        return stringTupleKey(record, 'model_id', 'api');
...
function tupleKey(record, leftKey, rightKey) {
...
function stringTupleKey(record, leftKey, rightKey) {
    const left = record[leftKey];
    const right = record[rightKey];
...
export function makeAutopilotRosterDiagnostic(code, severity) {
