import { autopilotRosterContractSha256, parseAutopilotRosterCandidateSet, } from "./contracts.js";
import { assertRosterSha256, assertValidRosterId, assertValidRosterRevision } from "./paths.js";
import { isOriginalAutopilotCommandReceiptSafe } from "./setup-receipt.js";
const ZERO_WRITE = Object.freeze({ write_count: 0, lock_count: 0, files_touched: Object.freeze([]) });
const AUTOPILOT_COMMAND_PATTERN = /^\/autopilot(?:\s|$)/u;
export function createRosterSetupApprovalSession(options) {
    return new RosterSetupApprovalSessionImpl(options);
}
class RosterSetupApprovalSessionImpl {
    #storage;
    #trustedProject;
    #originalCommand;
    #originalCommandSha256;
    #originalCommandUsable;
    #phase = 'awaiting-presentation';
    #proof = null;
    #authorizedProofSha256 = null;
    ...
    constructor(options) {
        ...
        this.#originalCommandSha256 = autopilotRosterContractSha256({ original_command: options.originalCommand });
    ...
    authorize(input) {
        ...
        if (input.authorization_source !== undefined &&
    ...
    async save(input) {
        ...
        const staleCodes = [];
        if (freshness.scope !== proof.scope ||
        ...
        if (input.current_previous_config_sha256 !== undefined &&
...
function isUsableOriginalAutopilotCommand(command) {
    return AUTOPILOT_COMMAND_PATTERN.test(command) && isOriginalAutopilotCommandReceiptSafe(command);
}
function candidateIdentityFromPresentation(input) {
    ...
    if (scope !== 'user' && scope !== 'trusted-project')
        throw new Error('scope required');
    const candidateSetSha = requireRosterSha256(input.candidate_set_sha256 ?? parsedCandidateSet?.candidate_set_sha256, 'candidate_set_sha256');
    const inventorySha = requireRosterSha256(input.inventory_sha256 ?? parsedCandidateSet?.inventory_sha256, 'inventory_sha256');
    const recipeRegistrySha = requireRosterSha256(input.recipe_registry_sha256 ?? parsedCandidateSet?.recipe_registry_sha256, 'recipe_registry_sha256');
    if (parsedCandidateSet !== null) {
        if (parsedCandidateSet.scope !== scope)
            throw new Error('candidate set scope drift');
        if (parsedCandidateSet.candidate_set_sha256 !== candidateSetSha)
            throw new Error('candidate set sha drift');
        if (parsedCandidateSet.inventory_sha256 !== inventorySha)
            throw new Error('inventory sha drift');
        if (parsedCandidateSet.recipe_registry_sha256 !== recipeRegistrySha)
            throw new Error('recipe registry sha drift');
    }
    return Object.freeze({
        scope,
...
function candidateIdentityFromSave(input, proof) {
    if (input.current_candidate_set !== undefined) {
        const parsed = parseAutopilotRosterCandidateSet(input.current_candidate_set);
        if (input.current_candidate_set_sha256 !== undefined && input.current_candidate_set_sha256 !== parsed.candidate_set_sha256)
            throw new Error('current candidate set sha drift');
        if (input.current_inventory_sha256 !== undefined && input.current_inventory_sha256 !== parsed.inventory_sha256)
            throw new Error('current inventory sha drift');
        if (input.current_recipe_registry_sha256 !== undefined && input.current_recipe_registry_sha256 !== parsed.recipe_registry_sha256)
            throw new Error('current recipe registry sha drift');
        return Object.freeze({
            scope: parsed.scope,
            candidate_set_sha256: parsed.candidate_set_sha256,
            inventory_sha256: parsed.inventory_sha256,
            recipe_registry_sha256: parsed.recipe_registry_sha256,
            candidateSet: parsed,
        });
    }
    return Object.freeze({
...
function buildRestatementProof(input, candidateSet) {
    ...
    if (input.expected_previous_config_sha256 !== null)
        assertRosterSha256(input.expected_previous_config_sha256, 'expected_previous_config_sha256');
    ...
    const withoutHash = Object.freeze({
        schema_version: 'autopilot.roster_setup_restatement_proof.v1',
        proof_contract: 'phase37-w2-approval-restatement-v1',
        scope: input.scope,
    ...
    return Object.freeze({
        ...withoutHash,
        proof_sha256: autopilotRosterContractSha256(withoutHash),
    });
}
function validateApprovedRosterHashes(approved, defaultHash, candidateSet, defaultId, defaultRevision) {
    if (approved.length === 0 || new Set(approved).size !== approved.length)
        throw new Error('approved roster list must be unique and non-empty');
    for (const hash of approved)
        assertRosterSha256(hash, 'approved_roster_sha256s');
    if (!approved.includes(defaultHash))
        throw new Error('default roster hash must be approved');
    ...
    const defaultMatches = candidateSet.candidates.filter((candidate) => candidate.roster_id === defaultId &&
        candidate.roster_revision === defaultRevision &&
        candidate.roster_sha256 === defaultHash);
    if (defaultMatches.length !== 1)
        throw new Error('default roster tuple absent from candidate set');
}
function requireRosterSha256(value, label) {
    if (value === undefined)
