...
import { rosterCanonicalSha256OmittingOwnField } from "./canonical.js";
...
export async function publishRuntimeRosterSnapshot(input) {
    ...
    try {
        ...
        if (input.expected_selection_sha256 !== undefined && input.expected_selection_sha256 !== selection.selection_sha256) {
            return snapshotPublicationResult(false, 'blocked', mirrorPath, selection.selection_sha256, false, [runSelectionDiagnostic('ROSTER_READBACK_MISMATCH')], 0, []);
        }
        ...
        await emitSnapshotStage(input.hooks, { stage: 'after-mirror-publish', path: mirrorPath, selection_sha256: selection.selection_sha256 });
        if (publish.status === 'conflict') {
            return snapshotPublicationResult(false, 'blocked', mirrorPath, selection.selection_sha256, false, [runSelectionDiagnostic('ROSTER_CREATE_ONLY_CONFLICT')], 0, []);
        }
        const writeCount = publish.status === 'created' ? 1 : 0;
        const filesTouched = publish.status === 'created' ? [mirrorPath] : [];
        if (input.crashStage === 'after-mirror-publish-before-readback') {
            return snapshotPublicationResult(false, 'failed', mirrorPath, selection.selection_sha256, publish.status === 'idempotent', [runSelectionDiagnostic('ROSTER_READBACK_MISMATCH')], writeCount, filesTouched);
        }
        ...
        await emitSnapshotStage(input.hooks, { stage: 'after-mirror-readback', path: mirrorPath, selection_sha256: selection.selection_sha256 });
        return snapshotPublicationResult(true, publish.status === 'idempotent' ? 'inspected' : 'published', mirrorPath, selection.selection_sha256, publish.status === 'idempotent', [], writeCount, filesTouched);
    }
    catch (error) {
        return snapshotPublicationResult(false, 'failed', mirrorPath, null, false, [diagnosticFromSnapshotError(error)], 0, []);
    }
}
export async function recoverRuntimeRosterSelection(input) {
    ...
    try {
        ...
        const request = existingRunRequestFromSelection({
            ...
            runtimeMirrorSha256: mirrorSelection.selection_sha256,
        ...
        return recoveryResult(existing.ok, existing.status, existing.ok ? externalSelection : null, externalSelectionPath, runtimeMirrorPath, existing, existing.diagnostics);
    }
    catch (error) {
...
function existingRunRequestFromSelection(input) {
    ...
    return Object.freeze({
        ...withPlaceholder,
        request_sha256: rosterCanonicalSha256OmittingOwnField(withPlaceholder, 'request_sha256'),
    });
}
function authenticateSpecIdentity(spec, workstream, selection, required) {
    if (spec === null || spec === undefined)
        return required ? transitionDiagnostics() : [];
    assertValidRosterId(spec.roster_id, 'spec.roster_id');
    assertValidRosterRevision(spec.roster_revision, 'spec.roster_revision');
    if (spec.workstream !== undefined && spec.workstream !== workstream)
...
function snapshotPublicationResult(ok, status, mirrorPath, selectionSha256, idempotentReplay, diagnostics, writeCount, filesTouched) {
    return Object.freeze({
        schema_version: 'autopilot.runtime_roster_snapshot_publication_result.v1',
        ok,
        status,
...
function recoveryResult(ok, status, selection, externalSelectionPath, runtimeMirrorPath, existingResolution, diagnostics) {
    return Object.freeze({
        schema_version: 'autopilot.runtime_roster_selection_recovery_result.v1',
        ok,
        status,
        selection,
...
function readbackTransitionDiagnostics() {
    return [runSelectionDiagnostic('ROSTER_READBACK_MISMATCH'), runSelectionDiagnostic('ROSTER_TRANSITION_REQUIRED')];
}
function diagnosticFromSnapshotError(error) {
    if (error instanceof RosterStorageError)
        return runSelectionDiagnostic(error.code);
    if (error instanceof SyntaxError)
        return runSelectionDiagnostic('ROSTER_READBACK_MISMATCH');
    return runSelectionDiagnostic('ROSTER_READBACK_MISMATCH');
}
function normalizeAbsoluteWorktreeRoot(value, options) {
    if (value.length === 0 || value.includes('\0'))
        throw new RosterStorageError('ROSTER_STORAGE_PATH_INVALID', 'mainWorktreeRoot must be non-empty and contain no NUL');
    if (!isAbsolute(value))
        throw new RosterStorageError('ROSTER_STORAGE_PATH_INVALID', `mainWorktreeRoot must be absolute: ${value}`);
