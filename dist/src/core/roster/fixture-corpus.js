...
const AUTHORITY_BOUNDARY_KEYS = ['binds_to', 'forbidden_bindings', 'synthetic_ready_rule'];
...
const CANONICALIZATION_KEYS = ['algorithm_id', 'definition_freeze_id', 'terminal_lf', 'digest_encoding'];
...
const CANONICAL_VECTOR_KEYS = ['vector_id', 'hash_omission_field', 'canonical_json_utf8', 'sha256'];
...
const HASH_FIELD_TO_SCHEMA_VECTOR_PREFIX = {
...
const EXPECTED_SAVE_ACCOUNTING = {
...
function assertUtcMs(value, path) {
...
function readImmutableBoundedBytes(path, maximumBytes, source) {
    let descriptor = null;
    try {
        ...
        if (error instanceof RosterFixtureCorpusError)
            throw error;
...
function isJsonValue(value) {
    ...
    if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1) {
            if (!Object.hasOwn(value, index) || !isJsonValue(value[index]))
                return false;
        }
        return true;
    }
...
function lastFrame(stack) {
    return stack[stack.length - 1];
}
function assertCanStartValue(stack, rootSeen, source, index) {
    const frame = lastFrame(stack);
    if (frame === undefined) {
        if (rootSeen)
            fail('invalid-json', `${source} has trailing root value`, [`index=${String(index)}`]);
        return;
    }
...
function markValueComplete(stack, rootSeen, source, index) {
    const frame = lastFrame(stack);
    if (frame === undefined) {
        if (rootSeen)
            fail('invalid-json', `${source} has multiple root values`, [`index=${String(index)}`]);
        return true;
    }
    if (frame.kind === 'object') {
        if (frame.expect !== 'value')
            fail('invalid-json', `${source} has object value in ${frame.expect} state`, [`index=${String(index)}`]);
        frame.expect = 'commaOrEnd';
        return rootSeen;
    }
    if (frame.expect !== 'valueOrEnd')
        fail('invalid-json', `${source} has array value in ${frame.expect} state`, [`index=${String(index)}`]);
    frame.expect = 'commaOrEnd';
    return rootSeen;
}
function parseStringToken(text, index, source) {
    let cursor = index + 1;
    while (cursor < text.length) {
        ...
        if (char === '"') {
            ...
            if (typeof parsed !== 'string')
                fail('invalid-json', `${source} string token did not parse as a string`);
...
function skipNumberToken(text, index, source) {
    const match = /^-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?/u.exec(text.slice(index));
    if (match === null)
        fail('invalid-json', `${source} has invalid number token`, [`index=${String(index)}`]);
...
function assertNoDuplicateObjectKeys(text, source) {
    ...
    while (index < text.length) {
        ...
        if (char === undefined)
            fail('invalid-json', `${source} scanner reached an impossible byte`);
        const frame = lastFrame(stack);
        if (char === '{') {
        ...
        if (char === '}') {
            if (frame === undefined || frame.kind !== 'object')
                fail('invalid-json', `${source} has unmatched }`, [`index=${String(index)}`]);
            if (frame.expect !== 'keyOrEnd' && frame.expect !== 'commaOrEnd')
                fail('invalid-json', `${source} closes object while expecting ${frame.expect}`, [`index=${String(index)}`]);
            stack.pop();
        ...
        if (char === ']') {
            ...
            if (frame.expect !== 'valueOrEnd' && frame.expect !== 'commaOrEnd')
                fail('invalid-json', `${source} closes array while expecting ${frame.expect}`, [`index=${String(index)}`]);
            stack.pop();
            rootSeen = markValueComplete(stack, rootSeen, source, index);
            index += 1;
            continue;
        ...
        if (char === ':') {
            if (frame === undefined || frame.kind !== 'object' || frame.expect !== 'colon')
                fail('invalid-json', `${source} has unexpected colon`, [`index=${String(index)}`]);
            frame.expect = 'value';
            index += 1;
            continue;
        }
        if (char === ',') {
            if (frame === undefined || frame.expect !== 'commaOrEnd')
                fail('invalid-json', `${source} has unexpected comma`, [`index=${String(index)}`]);
        ...
        if (char === '"') {
            ...
            if (frame !== undefined && frame.kind === 'object' && frame.expect === 'keyOrEnd') {
                if (frame.keys.has(token.value))
                    fail('invalid-json', `${source} has duplicate object key`, [token.value]);
    ...
    if (stack.length !== 0)
        fail('invalid-json', `${source} has unclosed JSON containers`);
    if (!rootSeen)
        fail('invalid-json', `${source} contains no JSON root value`);
}
function parseJsonObject(text, source) {
    ...
    if (!isJsonValue(parsed))
        fail('invalid-json', `${source} is not an RFC-8259 JSON value`);
    if (!isJsonObject(parsed))
        fail('invalid-json', `${source} root must be an object`);
    return parsed;
}
function at(object, key, path) {
...
function asArray(value, path) {
    if (!Array.isArray(value))
        fail('fixture-contract-violation', `${path} must be an array`);
    return value;
}
function arrayAt(object, key, path) {
    return asArray(at(object, key, path), `${path}.${key}`);
}
function asString(value, path) {
    if (typeof value !== 'string')
        fail('fixture-contract-violation', `${path} must be a string`);
    return value;
}
...
function asBoolean(value, path) {
    if (typeof value !== 'boolean')
        fail('fixture-contract-violation', `${path} must be a boolean`);
    return value;
}
...
function asInteger(value, path) {
    if (typeof value !== 'number' || !Number.isSafeInteger(value))
        fail('fixture-contract-violation', `${path} must be a safe integer`);
    return value;
}
function integerAt(object, key, path) {
    return asInteger(at(object, key, path), `${path}.${key}`);
}
function optionalIntegerAt(object, key, path) {
    const value = object[key];
    if (value === undefined || value === null)
        return null;
    return asInteger(value, `${path}.${key}`);
}
function stringsAt(object, key, path) {
...
function exactKeys(object, expected, path) {
    ...
    if (!sameOrder)
        fail('closed-field-violation', `${path} has unknown, missing, or reordered fields`, [`actual=${actual.join(',')}`, `expected=${expected.join(',')}`]);
}
...
function assertStringSet(actual, expected, path) {
...
function canonicalEncode(value, ancestors) {
    if (value === null || typeof value === 'boolean' || typeof value === 'string')
        return JSON.stringify(value);
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            fail('canonical-drift', 'canonical JSON rejects non-finite numbers');
        return JSON.stringify(value);
    }
    if (typeof value !== 'object')
        fail('canonical-drift', 'canonical JSON accepts only JSON values');
    if (ancestors.has(value))
        fail('canonical-drift', 'canonical JSON rejects cyclic values');
    ancestors.add(value);
    try {
        if (Array.isArray(value)) {
            const entries = [];
            for (let index = 0; index < value.length; index += 1) {
                if (!Object.hasOwn(value, index))
                    fail('canonical-drift', 'canonical JSON rejects sparse arrays');
                entries.push(canonicalEncode(value[index], ancestors));
            }
            return `[${entries.join(',')}]`;
        }
        const prototype = Object.getPrototypeOf(value);
        if (prototype !== Object.prototype && prototype !== null)
            fail('canonical-drift', 'canonical JSON rejects non-plain objects');
        const object = value;
        const keys = Object.keys(object).sort(compareStrings);
        const pairs = keys.map((key) => {
            const entry = object[key];
            if (entry === undefined)
                fail('canonical-drift', 'canonical JSON rejects undefined values', [key]);
...
export function canonicalJsonForRosterFixture(value) {
    return canonicalEncode(value, new WeakSet());
}
function canonicalPreimageWithTerminalLf(object, omittedHashField) {
    const copy = {};
    for (const [key, value] of Object.entries(object)) {
        if (key === omittedHashField)
            continue;
        if (value === undefined)
            fail('canonical-drift', 'canonical JSON preimage rejects undefined values', [key]);
        copy[key] = value;
    }
    return `${canonicalJsonForRosterFixture(copy)}\n`;
}
function assertCanonicalHash(object, hashField, path, vectors, vectorId) {
...
function typedVector(object, path) {
...
function typedFixtureCase(object, path) {
...
function validateTopLevel(corpus) {
...
function validateAuthorityBoundary(corpus) {
...
function validateEvidenceNotice(corpus) {
...
function validateCanonicalizationBlock(corpus) {
...
function validateFixedValues(corpus) {
    ...
    for (const key of FIXED_VALUES_KEYS) {
        ...
        if (key.startsWith('clock'))
            assertUtcMs(value, `corpus.fixed_values.${key}`);
    }
}
function validateDiagnosticRegistry(corpus) {
...
function validateCanonicalVectors(corpus) {
    ...
    for (const [index, entry] of entries.entries())
        vectors.push(typedVector(entry, `corpus.canonical_vectors[${String(index)}]`));
...
function validateFixtureCases(corpus, vectors, diagnosticRegistry) {
    ...
    for (const tag of REQUIRED_TAGS)
        assertCondition(actualTags.includes(tag), 'fixture-contract-violation', 'fixture corpus missing required tag', [tag]);
    return new Map(cases.map((fixtureCase) => [fixtureCase.fixture_id, fixtureCase]));
}
function validateFixtureCase(fixtureCase, vectors, diagnosticRegistry, path) {
    ...
    for (const [index, postcondition] of postconditions.entries())
        validateFilesystemPostcondition(postcondition, `${path}.filesystem_postconditions[${String(index)}]`);
    ...
    for (const code of diagnostics)
        assertCondition(diagnosticRegistry.includes(code), 'fixture-contract-violation', `${path}.expected.diagnostics contains unknown code`, [code]);
...
function validateFilesystemPostcondition(postcondition, path) {
    const keys = Object.keys(postcondition);
    const allowedKeys = ['path', 'state', 'sha256', 'count'];
    for (const key of keys)
        assertCondition(allowedKeys.includes(key), 'closed-field-violation', `${path} has an unknown filesystem postcondition field`, [key]);
    ...
    if (sha !== undefined)
        assertDigest(asString(sha, `${path}.sha256`), `${path}.sha256`);
    const count = optionalIntegerAt(postcondition, 'count', path);
    if (count !== null)
        assertCondition(count >= 0, 'fixture-contract-violation', `${path}.count must be non-negative`);
}
function validateAccounting(fixtureId, action, ok, status, writeCount, lockCount, filesTouched, path) {
...
function validateExpectedNestedResult(expected, expectedDiagnosticCodes, diagnosticRegistry, path) {
    ...
    for (const [index, diagnostic] of diagnostics.entries())
        validateDiagnosticObject(diagnostic, diagnosticRegistry, `${path}.expected.result.diagnostics[${String(index)}]`);
}
function validateDiagnosticObject(diagnostic, diagnosticRegistry, path) {
...
function validateObjectRegistry(corpus, vectors, cases) {
    ...
    for (const artifact of historicalArtifacts)
        validateHistoricalArtifact(artifact, vectors);
...
function validateHistoricalArtifact(artifact, vectors) {
...
function validateHistoricalRequestBytes(request, path) {
...
function validateHistoricalFixtureCases(cases) {
    for (const fixtureCase of cases.values()) {
        if (!fixtureCase.fixture_id.startsWith('historical.'))
            continue;
        ...
        for (const postcondition of objectsAt(fixtureCase, 'filesystem_postconditions', fixtureCase.fixture_id)) {
            ...
            if (state === 'byte-equal-preserved')
                assertDigest(stringAt(postcondition, 'sha256', `fixture ${fixtureCase.fixture_id}.filesystem_postconditions`), `fixture ${fixtureCase.fixture_id}.filesystem_postconditions.sha256`);
        }
    }
}
function validateSecretFreeAndSynthetic(value, path) {
    if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1)
            validateSecretFreeAndSynthetic(value[index], `${path}[${String(index)}]`);
        return;
    }
    if (!isJsonObject(value))
        return;
    for (const [key, entry] of Object.entries(value)) {
        if (entry === undefined)
            continue;
        if (key === 'secret_free')
            assertCondition(entry === true, 'fixture-contract-violation', `${path}.${key} must remain true`);
        if (key === 'secret_fields_present')
            assertCondition(entry === false, 'fixture-contract-violation', `${path}.${key} must remain false`);
        if (key === 'zero_secrets')
            assertCondition(entry === true, 'fixture-contract-violation', `${path}.${key} must remain true`);
        if (key === 'secrets_included')
            assertCondition(entry === false, 'fixture-contract-violation', `${path}.${key} must remain false`);
        if (key === 'provider_evidence_is_certifying')
            assertCondition(entry === false, 'fixture-contract-violation', `${path}.${key} must remain false`);
        if (key === 'live_provider_calls_required')
            assertCondition(entry === false, 'fixture-contract-violation', `${path}.${key} must remain false`);
        if (key === 'non_certifying_seed')
            assertCondition(entry === true, 'fixture-contract-violation', `${path}.${key} must remain true`);
...
function buildLoadResult(source, sha256, corpus, caseMap, vectorMap, diagnosticCodes) {
    const frozenCorpus = deepFreezeJson(corpus);
    ...
    const result = {
        ...
        getCase(fixtureId) {
            ...
            if (fixtureCase === undefined)
                fail('fixture-contract-violation', 'unknown Phase 37 W0 fixture case requested', [fixtureId]);
            return fixtureCase;
        },
        getCanonicalVector(vectorId) {
            const vector = vectorMap.get(vectorId);
            if (vector === undefined)
                fail('fixture-contract-violation', 'unknown Phase 37 W0 canonical vector requested', [vectorId]);
            return vector;
        },
    };
...
function validateParsedCorpus(corpus, source) {
...
export function parsePhase37W0FixtureCorpusText(text, options) {
    ...
    if (actualSha256 !== options.expectedSha256)
        fail('digest-drift', 'Phase 37 W0 fixture corpus digest drift', [`source=${options.source}`, `actual=${actualSha256}`, `expected=${options.expectedSha256}`]);
 ...
 */
export function loadPhase37W0FixtureCorpus(options) {
    ...
    if (actualSha256 !== expectedSha256)
        fail('digest-drift', 'Phase 37 W0 fixture corpus digest drift', [`source=${source}`, `actual=${actualSha256}`, `expected=${expectedSha256}`]);
