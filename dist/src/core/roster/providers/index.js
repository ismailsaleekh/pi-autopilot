...
import { KIMI_CODING_PROVIDER_ID, KIMI_CODING_RECIPE_ID, KIMI_CODING_RECIPE_REVISION, KIMI_CODING_REQUIRED_EVIDENCE_REFS, KIMI_CODING_ROUTE_POLICY_ID, KIMI_CODING_ROUTE_POLICY_REVISION, } from "./kimi-coding.js";
import { OPENCODE_GO_PROVIDER_ID, OPENCODE_GO_RECIPE_ID, OPENCODE_GO_RECIPE_REVISION, OPENCODE_GO_REQUIRED_EVIDENCE_REFS, OPENCODE_GO_ROUTE_POLICY_ID, OPENCODE_GO_ROUTE_POLICY_REVISION, } from "./opencode-go.js";
import { ZAI_PROVIDER_ID, ZAI_RECIPE_ID, ZAI_RECIPE_REVISION, ZAI_REQUIRED_EVIDENCE_REFS, ZAI_ROUTE_POLICY_ID, ZAI_ROUTE_POLICY_REVISION, } from "./zai.js";
...
function registryEntryForCandidate(candidate) {
    return W4_PROVIDER_PACK_REGISTRY.find((entry) => entry.recipe_id === candidate.recipe_id &&
        entry.recipe_revision === candidate.recipe_revision &&
        entry.route_policy_id === candidate.route_policy_id &&
        entry.route_policy_revision === candidate.route_policy_revision) ?? null;
}
export function isCentrallyTrustedW4CertifiedRoster(roster) {
    const trustedRegistry = W4_PROVIDER_PACK_REGISTRY;
    if (roster.generation_source !== 'w4-certified-recipe')
        return false;
    if (roster.certification_manifest_id === null || roster.certification_manifest_sha256 === null)
        return false;
    if (!DIGEST_PATTERN.test(roster.certification_manifest_sha256))
        return false;
    if (!roster.assignments.every((assignment) => assignment.qualification_state === 'w4-certified-ready'))
        return false;
    const entries = trustedRegistry.filter((entry) => entry.readiness === 'strict-w3-manifest' &&
        ...
        entry.trusted_manifest_sha256s.includes(roster.certification_manifest_sha256) &&
        entry.trusted_certified_roster_sha256s.includes(roster.roster_sha256));
    if (entries.length !== 1)
        return false;
    const entry = entries[0];
    if (entry === undefined)
        return false;
    return roster.assignments.every((assignment) => assignment.provider_id === entry.provider_id &&
...
function isTrustedLiveW3EvidenceRef(ref, entry) {
...
function roleResultsCoverAllRoles(manifest, entry) {
    ...
    for (const role of ROSTER_ROLE_ORDER) {
        ...
        if (required === null || result === undefined || result.state !== 'pass')
            return false;
        if (!result.evidence_refs.some((ref) => ref.kind === 'execution-proof' && ref.evidence_id === required.evidence_id))
            return false;
        for (const ref of result.evidence_refs) {
            if (ref.evidence_id !== required.evidence_id || ref.kind !== required.kind)
                return false;
            if (!isTrustedLiveW3EvidenceRef(ref, entry))
                return false;
            if (!liveEvidenceKeys.has(evidenceKey(ref)))
                return false;
        }
    }
    return true;
}
function liveEvidenceCoversGlobalProofs(manifest, entry) {
    ...
    if (routeRequired === null || billingRequired === null)
        return false;
...
function requiredEvidenceRefForKind(entry, kind) {
    return entry.required_evidence.find((ref) => ref.kind === kind) ?? null;
}
function requiredExecutionEvidenceRefForRole(entry, role) {
...
function trustedManifestHashes(entry) {
...
function candidateWithW4Readiness(input) {
    ...
    const withoutHash = {
        ...
        candidate_state: 'w4-certified-ready',
        launch_readiness: 'w4-certified-ready',
        ...
        diagnostic_codes,
        readiness_authority: 'w4-provider-registry.v1',
...
export function applyW4ProviderRegistryReadinessToCandidateSet(input) {
    ...
    const candidates = input.candidateSet.candidates.map((candidate) => {
        ...
        if (!verification.ok || verification.entry === null || verification.manifest === null)
            return candidate;
