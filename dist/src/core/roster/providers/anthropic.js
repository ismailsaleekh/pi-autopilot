...
import { PHASE37_FIXTURE_CLOCK, PHASE37_FREEZE_ID, PHASE37_PACKAGE_VERSION, PHASE37_PI_VERSION, ROSTER_ROLE_ORDER, canonicalSha256, findRoutePolicy, } from "../route-policies.js";
function deepFreezeAnthropicAuthority(value, seen = new WeakSet()) {
    ...
    for (const key of Reflect.ownKeys(objectValue)) {
        deepFreezeAnthropicAuthority(objectValue[key], seen);
    }
    return Object.freeze(objectValue);
}
export const ANTHROPIC_PROVIDER_PACK_ID = 'anthropic-sanitized';
export const ANTHROPIC_PROVIDER_PACK_REVISION = 1;
export const ANTHROPIC_PROVIDER_ID = 'anthropic';
export const ANTHROPIC_ROUTE_POLICY_ID = 'anthropic-sanitized-v1';
export const ANTHROPIC_ROUTE_POLICY_REVISION = 1;
export const ANTHROPIC_SYSTEM_PROMPT_PROFILE = 'anthropic-autopilot-sanitized.v1';
export const ANTHROPIC_QUALIFICATION_ARTIFACT_SCHEMA_VERSION = 'autopilot.anthropic_provider_qualification.v1';
export const ANTHROPIC_QUALIFICATION_INPUT_SCHEMA_VERSION = 'autopilot.anthropic_qualification_input.v1';
export const ANTHROPIC_SANITIZER_RESULT_SCHEMA_VERSION = 'autopilot.anthropic_system_prompt_transform_result.v1';
export const ANTHROPIC_SANITIZER_PAYLOAD_SCHEMA_VERSION = 'autopilot.anthropic_system_prompt_payload.v1';
...
function anthropicDiagnostic(code) {
...
function getFrozenCentralAnthropicRoutePolicy() {
...
export const ANTHROPIC_FROZEN_ROUTE_POLICY = getFrozenCentralAnthropicRoutePolicy();
function roleSeed(preimage) {
    return {
        ...preimage,
...
export const ANTHROPIC_ROLE_SEEDS = deepFreezeAnthropicAuthority([
...
export const ANTHROPIC_ROLE_SEED_SET_SHA256 = canonicalSha256({
...
export const ANTHROPIC_SANITIZER_HEADER_SHA256 = sha256Utf8(ANTHROPIC_SANITIZER_HEADER_BYTES);
function renderSanitizerPayloadJson(rawPrompt, rawPromptByteLength, rawPromptSha256) {
    return [
        '{',
...
export function transformAnthropicAutopilotSystemPrompt(rawPrompt) {
...
export function decodedRawPromptFromAnthropicTransform(transformedPromptBytesUtf8) {
...
export function validateAnthropicSystemPromptSemanticInvariants(rawPrompt, transformResult) {
    const diagnostics = [];
...
export const ANTHROPIC_TRUSTED_LIVE_PROVENANCE_CLASS = 'w3-authenticated-receipt-execution.v1';
export const ANTHROPIC_TRUSTED_HASH_PROVENANCE = 'w3-receipt-content-sha256.v1';
...
function isForbiddenEvidenceUri(uri) {
...
function isTrustedW3IdentityRef(value, kind) {
    return value.startsWith(`w3:${kind}:`) && value.length > `w3:${kind}:`.length && !isForbiddenEvidenceUri(value);
}
function isSelfHashedEvidenceRef(ref, requirement) {
...
function validateAnthropicEvidenceRef(ref, requirement, qualificationIssuedAt) {
    const diagnostics = [];
    if (ref === null) {
    ...
    if (ref.provenance_class !== ANTHROPIC_TRUSTED_LIVE_PROVENANCE_CLASS ||
    ...
    if (ref.freeze_id !== PHASE37_FREEZE_ID ||
...
function validateAnthropicRouteEvidence(route, qualificationIssuedAt) {
    const diagnostics = [];
    if (route.route_policy_id !== ANTHROPIC_FROZEN_ROUTE_POLICY.route_policy_id ||
        ...
        !ANTHROPIC_FROZEN_ROUTE_POLICY.allowed_auth_sources.includes(route.auth_source) ||
    ...
    if (route.provider_id !== ANTHROPIC_PROVIDER_ID ||
    ...
    diagnostics.push(...validateAnthropicEvidenceRef(route.live_route_evidence, {
        kind: 'route-proof',
        subject_id: frozenRouteSubjectId(),
        subject_sha256: ANTHROPIC_FROZEN_ROUTE_POLICY.route_policy_sha256,
    }, qualificationIssuedAt), ...validateAnthropicEvidenceRef(route.billing_consent_evidence, {
        kind: 'billing-proof',
        subject_id: derivedSubjectId('billing-consent'),
        subject_sha256: ANTHROPIC_FROZEN_ROUTE_POLICY.route_policy_sha256,
    }, qualificationIssuedAt), ...validateAnthropicEvidenceRef(route.non_metered_entitlement_evidence, {
        kind: 'billing-proof',
...
function validateAnthropicPromptEvidence(prompt, qualificationIssuedAt) {
    const diagnostics = [];
    const promptDigests = [
    ...
    if (isDigest(prompt.transformed_prompt_sha256) &&
    ...
    diagnostics.push(...validateAnthropicEvidenceRef(prompt.prompt_transform_evidence, {
        kind: 'prompt-proof',
        subject_id: `${ANTHROPIC_SYSTEM_PROMPT_PROFILE}/transform`,
        subject_sha256: prompt.transformed_prompt_sha256,
    }, qualificationIssuedAt), ...validateAnthropicEvidenceRef(prompt.request_evidence, {
        kind: 'request-proof',
        subject_id: `${ANTHROPIC_SYSTEM_PROMPT_PROFILE}/request`,
        subject_sha256: prompt.request_prompt_sha256,
    }, qualificationIssuedAt), ...validateAnthropicEvidenceRef(prompt.response_evidence, {
        kind: 'response-proof',
...
function validateAnthropicCacheEvidence(cache, qualificationIssuedAt) {
    const diagnostics = [];
    if (cache.requested_cache_policy !== 'provider-default' ||
...
function validateAnthropicRoleExecutionEvidence(roleEvidence, promptEvidence, qualificationIssuedAt) {
    const diagnostics = [];
    ...
    for (const role of ROSTER_ROLE_ORDER) {
        ...
        if (record.requested_model_id !== seed.model_id ||
        ...
        else if (record.request_prompt_sha256 !== promptEvidence.transformed_prompt_sha256 ||
    ...
    for (const role of byRole.keys()) {
        if (!ROSTER_ROLE_ORDER.includes(role)) {
            diagnostics.push('ANTHROPIC_ROLE_COVERAGE_MISSING');
        }
    }
    return diagnostics;
}
function collectAnthropicEvidenceRefs(input) {
    return [
...
function validateAnthropicEvidenceDistinctness(input) {
...
function summarizeQualificationInput(input) {
    return {
...
export function buildAnthropicQualificationArtifact(input) {
    const diagnosticCodes = [
    ...
    const preimage = {
        ...
        artifact_id: 'phase37-w4-anthropic-offline',
        artifact_revision: 1,
        freeze_id: PHASE37_FREEZE_ID,
        ...
        status: 'blocked-live-certification',
        qualification_state: 'blocked-live-certification',
        ...
        sanitizer_proves_billing_or_readiness: false,
        diagnostics,
...
export function createAnthropicStrictCompatibilityInput(transformResult, issuedAt = PHASE37_FIXTURE_CLOCK) {
...
export function verifyAnthropicProviderPackAuthority() {
    const issues = [];
