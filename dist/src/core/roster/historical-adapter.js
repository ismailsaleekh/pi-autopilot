import { AutopilotRosterCanonicalizationError, parseRosterJsonWithDuplicateKeyRejection, sha256Utf8, } from "./canonical.js";
import { AUTOPILOT_ROSTER_PACKAGE_VERSION_TARGET, makeAutopilotRosterDiagnostic, parseAutopilotHistoricalFixedRosterAdapterAdmission, parseAutopilotHistoricalFixedRosterAdapterRequest, parseAutopilotHistoricalFixedRosterAdapterResult, parseAutopilotRosterContract, computeAutopilotRosterContractObjectHash, } from "./contracts.js";
export const AUTOPILOT_HISTORICAL_FIXED_ROSTER_CHAIN_ID = 'openai-codex-sol-terra-luna-v1';
...
const FIXED_HISTORICAL_ROLES = Object.freeze([
...
const HISTORICAL_ROLE_NAMES = new Set([
    'parent',
    'strategy',
    'implement',
    'validate',
...
export function adaptHistoricalFixedRosterRequest(request) {
    ...
    const diagnostics = diagnosticCodesForReason(reason).map((code) => makeAutopilotRosterDiagnostic(code, code === 'ROSTER_HISTORICAL_V1_BYTES_PRESERVED' ? 'info' : 'error'));
    const resultRecord = {
    ...
    resultRecord['result_sha256'] = requiredHash('autopilot.historical_fixed_roster_adapter_result.v1', resultRecord);
    return parseAutopilotHistoricalFixedRosterAdapterResult(resultRecord);
}
export function historicalFixedRosterRoles() {
    return FIXED_HISTORICAL_ROLES;
}
function buildAdmission(request, evidence, reason, admitted) {
    ...
    admissionRecord['admission_sha256'] = requiredHash('autopilot.historical_fixed_roster_adapter_admission.v1', admissionRecord);
    return parseAutopilotHistoricalFixedRosterAdapterAdmission(admissionRecord);
}
function parseHistoricalEvidence(request) {
    ...
    const proofOk = hasOnlyKeys(unit, UNIT_KEYS) &&
    ...
    const versionSupported = typeof unit['package_version'] === 'string' &&
    ...
    const fixedRosterMatches = unit['fixed_roster_chain_id'] === AUTOPILOT_HISTORICAL_FIXED_ROSTER_CHAIN_ID &&
...
function historicalAdmissionReason(request, evidence) {
    if (!evidence.proofOk)
        return 'proof-required';
    if (!evidence.versionSupported)
        return 'historical-version-unsupported';
    if (!preRunSelectionAbsent(request))
        return 'pre-run-selection-present';
    if (!evidence.fixedRosterMatches)
        return 'fixed-roster-mismatch';
    if (request.conflicting_evidence_sha256s.length > 0)
        return 'conflicting-evidence';
    return 'admitted';
}
function diagnosticCodesForReason(reason) {
    if (reason === 'admitted')
        return ['ROSTER_HISTORICAL_V1_BYTES_PRESERVED'];
    if (reason === 'proof-required')
        return ['ROSTER_HISTORICAL_PROOF_REQUIRED'];
    if (reason === 'historical-version-unsupported') {
        return ['ROSTER_HISTORICAL_PROOF_REQUIRED', 'ROSTER_HISTORICAL_VERSION_UNSUPPORTED'];
    }
    if (reason === 'pre-run-selection-present')
        return ['ROSTER_HISTORICAL_SELECTION_PRESENT'];
    if (reason === 'fixed-roster-mismatch')
        return ['ROSTER_HISTORICAL_FIXED_ROSTER_MISMATCH'];
    return ['ROSTER_HISTORICAL_CONFLICTING_EVIDENCE'];
}
function preRunSelectionAbsent(request) {
    return request.pre_run_selection_state === 'absent' && request.pre_run_selection_sha256 === null;
}
function parseHistoricalJsonObject(text) {
    try {
        const parsed = parseRosterJsonWithDuplicateKeyRejection(text);
        return recordValue(parsed);
    }
    catch (error) {
        if (error instanceof SyntaxError || error instanceof AutopilotRosterCanonicalizationError)
            return null;
        throw error;
    }
}
function parseHistoricalRoleArray(value) {
    if (!Array.isArray(value) || value.length !== FIXED_HISTORICAL_ROLES.length)
        return null;
    const roles = [];
    const seenRoles = new Set();
    for (const item of value) {
        const record = recordValue(item);
        if (record === null || !hasExactKeys(record, ROLE_KEYS))
            return null;
        const parsedRole = parseHistoricalRoleRecord(record);
        if (parsedRole === null || seenRoles.has(parsedRole.role))
            return null;
...
function parseHistoricalRoleRecord(record) {
...
function isHistoricalRoleName(value) {
    return typeof value === 'string' && HISTORICAL_ROLE_NAMES.has(value);
}
function isHistoricalApi(value) {
    return typeof value === 'string' && HISTORICAL_API_VALUES.has(value);
}
function isHistoricalThinking(value) {
    return typeof value === 'string' && HISTORICAL_THINKING_VALUES.has(value);
}
...
function historicalRole(role, modelId, thinking) {
    return parseAutopilotRosterContract('autopilot.historical_fixed_roster_role.v1', {
        schema_version: 'autopilot.historical_fixed_roster_role.v1',
        role,
...
function sameHistoricalRoles(left, right) {
    ...
    for (let index = 0; index < left.length; index += 1) {
        ...
        if (leftRole === undefined || rightRole === undefined)
            return false;
        if (leftRole.role !== rightRole.role ||
...
function requiredHash(schemaVersion, value) {
    ...
    if (hash === null)
        throw new Error(`${schemaVersion} does not carry a hash field`);
    return hash;
}
