...
export const ANTHROPIC_OPUS5_SONNET5_RECIPE_ID = 'anthropic-opus5-sonnet5-subscription';
export const ANTHROPIC_OPUS5_SONNET5_RECIPE_REVISION = 1;
export const ANTHROPIC_OPUS5_SONNET5_ROSTER_ID = 'anthropic-precision-opus5-sonnet5-v1';
export const ANTHROPIC_OPUS5_SONNET5_CANDIDATE_ID = ANTHROPIC_OPUS5_SONNET5_ROSTER_ID;
function anthropicOpus5Sonnet5RoleTemplate(role, model_id, thinking) {
    return {
        role,
        model_id,
...
const ANTHROPIC_OPUS5_SONNET5_RECIPE_PREIMAGE = {
    schema_version: 'autopilot.provider_recipe.v1',
    ...
    recipe_state: 'unqualified-seed',
    non_certifying_seed: true,
};
export const ANTHROPIC_OPUS5_SONNET5_RECIPE = deepFreezeRecipeAuthority({
    ...ANTHROPIC_OPUS5_SONNET5_RECIPE_PREIMAGE,
    recipe_sha256: canonicalSha256(ANTHROPIC_OPUS5_SONNET5_RECIPE_PREIMAGE),
});
const ANTHROPIC_OPUS5_SONNET5_INVENTORY_PROVIDER = deepFreezeRecipeAuthority({
...
if (anthropicOpus5Sonnet5Profile === undefined)
    throw new Error('Anthropic Opus 5 / Sonnet 5 Precision profile is missing');
...
export const ANTHROPIC_OPUS5_SONNET5_ROSTER = deepFreezeRecipeAuthority({
...
export const ANTHROPIC_OPUS5_SONNET5_CANDIDATE = deepFreezeRecipeAuthority({
    ...ANTHROPIC_OPUS5_SONNET5_CANDIDATE_PREIMAGE,
    candidate_sha256: canonicalSha256(ANTHROPIC_OPUS5_SONNET5_CANDIDATE_PREIMAGE),
});
export const CODEX_GPT55_HEAVY_RECIPE_ID = 'codex-gpt55-heavy-subscription';
export const CODEX_GPT55_HEAVY_RECIPE_REVISION = 1;
export const CODEX_GPT55_HEAVY_ROSTER_ID = 'codex-gpt55-heavy-sol-terra-v1';
...
if (CODEX_GPT55_HEAVY_ROUTE_POLICY === null)
    throw new Error('Codex subscription route policy is missing');
function codexGpt55HeavyRoleTemplate(role, model_id, thinking) {
...
export const CODEX_GPT55_HEAVY_RECIPE = deepFreezeRecipeAuthority({
    ...CODEX_GPT55_HEAVY_RECIPE_PREIMAGE,
    recipe_sha256: canonicalSha256(CODEX_GPT55_HEAVY_RECIPE_PREIMAGE),
});
const CODEX_GPT55_HEAVY_INVENTORY_PROVIDER = deepFreezeRecipeAuthority({
...
if (codexGpt55HeavyProfile === undefined)
    throw new Error('Codex GPT-5.5 Heavy profile is missing');
...
export const CODEX_GPT55_HEAVY_ROSTER = deepFreezeRecipeAuthority({
...
export const CODEX_GPT55_HEAVY_CANDIDATE = deepFreezeRecipeAuthority({
    ...CODEX_GPT55_HEAVY_CANDIDATE_PREIMAGE,
    candidate_sha256: canonicalSha256(CODEX_GPT55_HEAVY_CANDIDATE_PREIMAGE),
});
function seedRosterSortKey(roster) {
    return `${roster.profile_id}:${roster.recipe_id}:${String(roster.recipe_revision).padStart(4, '0')}:${roster.roster_id}`;
}
function sortSeedRosters(rosters) {
    return [...rosters].sort((left, right) => seedRosterSortKey(left).localeCompare(seedRosterSortKey(right)));
}
export const PROVIDER_RECIPES = deepFreezeRecipeAuthority(sortProviderRecipes([
    ...JSON.parse(PROVIDER_RECIPES_JSON),
    ANTHROPIC_OPUS5_SONNET5_RECIPE,
    CODEX_GPT55_HEAVY_RECIPE,
]));
export const SEED_ROSTERS = deepFreezeRecipeAuthority(sortSeedRosters([
    ...JSON.parse(SEED_ROSTERS_JSON),
    ANTHROPIC_OPUS5_SONNET5_ROSTER,
    CODEX_GPT55_HEAVY_ROSTER,
]));
export const SEED_CANDIDATES = deepFreezeRecipeAuthority(sortRosterCandidates([
    ...JSON.parse(SEED_CANDIDATES_JSON),
...
function preserveRegisteredRosterIdentity(recipe, profile, roster) {
    const seedCandidate = seedCandidateFor(recipe, profile);
    if (seedCandidate === null)
        return roster;
    const seedRoster = SEED_ROSTERS.find((entry) => entry.roster_id === seedCandidate.roster_id && entry.roster_revision === seedCandidate.roster_revision);
    if (seedRoster === undefined ||
