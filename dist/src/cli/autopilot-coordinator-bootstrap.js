...
function assertPhysicalPackageFile(packageRoot, target) {
    if (!isContained(packageRoot, target))
        throw new Error(`compiled coordinator entrypoint escapes package root: ${target}`);
    let cursor = packageRoot;
    const rootInfo = lstatSync(cursor);
    if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink())
        throw new Error(`coordinator package root is not physical: ${packageRoot}`);
    for (const segment of relative(packageRoot, target).split(/[\\/]/u)) {
        cursor = join(cursor, segment);
        const info = lstatSync(cursor);
        if (info.isSymbolicLink())
            throw new Error(`compiled coordinator entrypoint contains a symbolic link: ${cursor}`);
    }
    if (!lstatSync(target).isFile())
        throw new Error(`compiled coordinator entrypoint is not a regular file: ${target}`);
    const realRoot = realpathSync(packageRoot);
    const realTarget = realpathSync(target);
    if (realTarget !== join(realRoot, relative(packageRoot, target)) || !isContained(realRoot, realTarget))
        throw new Error(`compiled coordinator entrypoint real path drifted: ${target}`);
}
...
function boundedCause(error) {
    ...
    if (points.length <= MAX_ERROR_CODE_POINTS)
        return { text: sanitized, truncated: false, omitted: 0 };
...
function assertPrivateDirectory(path) {
    ...
    if (!info.isDirectory() || info.isSymbolicLink())
        throw new Error(`startup diagnostic directory is not physical: ${path}`);
}
...
function writeReport(reportPath, report) {
    ...
    if (realpathSync(coordinatorRoot) !== join(realStateRoot, 'coordinator'))
        throw new Error('startup diagnostic coordinator root real path drifted');
    assertPrivateDirectory(reportRoot);
    if (realpathSync(reportRoot) !== join(realStateRoot, 'coordinator', 'startup-reports'))
        throw new Error('startup diagnostic report root real path drifted');
    const text = `${JSON.stringify(report)}\n`;
    if (Buffer.byteLength(text, 'utf8') > MAX_REPORT_BYTES)
        throw new Error('bootstrap startup report exceeded its package bound');
    try {
        const finalInfo = lstatSync(reportPath);
        if (!finalInfo.isFile() || finalInfo.isSymbolicLink())
            throw new Error(`startup diagnostic path is not a physical file: ${reportPath}`);
    }
    catch (error) {
    ...
    try {
        ...
        if (!info.isFile())
            throw new Error('bootstrap startup report temporary is not a regular file');
    }
    finally {
        closeSync(descriptor);
    }
...
function bootstrapInputs() {
    ...
    if (attemptId === undefined || !/^startup-[a-f0-9]{24,64}$/u.test(attemptId))
        throw new Error('compiled coordinator bootstrap requires a valid startup attempt identity');
    const stateRootValue = process.env['AUTOPILOT_STATE_ROOT'];
    if (stateRootValue === undefined || !isAbsolute(stateRootValue))
        throw new Error('compiled coordinator bootstrap requires an absolute state root');
    ...
    if (selected !== coordinatorPath)
        throw new Error(`compiled coordinator selection drifted: ${String(selected)}`);
    ...
    if (manifest['name'] !== 'pi-autopilot' || manifest['version'] !== PACKAGE_VERSION)
        throw new Error('compiled coordinator bootstrap package identity drifted');
    return {
        coordinatorPath,
...
try {
    ...
    await import(__rewriteRelativeImportExtension(pathToFileURL(inputs.coordinatorPath).href));
}
catch (error) {
    if (inputs !== null) {
