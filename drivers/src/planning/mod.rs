...
pub fn boundary_runtime(id: &'static str) -> BoundaryRuntime {
    let descriptor: &'static BoundaryDescriptor = match boundary_by_id(id) {
        Some(value) => value,
        None => panic!("missing planning boundary {id}"),
    };
    match BoundaryRuntime::new(descriptor) {
        Ok(value) => value,
        Err(error) => panic!("planning boundary runtime failed for {id}: {error}"),
    }
}

#[acceptance_boundary(
    id = "planning.task-atoms.v1",
...
fn parse_cap(attrs: &str) -> Result<u8, PlanningError> {
    let value = parse_attr(attrs, "default=")?;
...
fn parse_attr(attrs: &str, name: &str) -> Result<String, PlanningError> {
    ...
    Err(PlanningError::BadDeclaration(name.to_owned()))
}
