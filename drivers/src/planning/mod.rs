...
fn validate_work_map_shape(work_map: &WorkMap, runtime: &BoundaryRuntime) -> Result<(), Rejection> {
    ...
    for unit in &work_map.units {
        ...
        if let Err(error) = crate::allocation::validate_plan_unit_package_checks(
            &unit.package_checks,
            unit.criteria.len(),
        ) {
            reject_value(
                runtime,
                "planning.work-map.v1",
                "units.package_checks",
                "unique closed Core-owned package checks with nonempty expectations",
                &error,
                "Use clean-exact-package-tip for committed-tip criteria and never encode package state as child shell.",
            )?;
        }
