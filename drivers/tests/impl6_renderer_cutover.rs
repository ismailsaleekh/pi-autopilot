...
fn planning_prompt_is_the_rendered_seven_layer_prompt() {
    ...
    let spec: serde_json::Value = serde_json::from_slice(&fs::read(&issue.binding.spec_path).unwrap()).unwrap();
    ...
    assert!(prompt.contains("Terminalize only through `autopilot_submit_atoms` as the final action."));
}
...
fn no_package_rendered_sidecar_remains() {
    ...
    assert!(!sidecar.exists(), "discarded rendered sidecar still exists at {}", sidecar.display());
    assert_eq!(spec["prompt_digest"].as_str().unwrap(), prompt_digest);
    let seam_source = fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/seam/mod.rs")).unwrap();
    assert!(!seam_source.contains("package-rendered.md"), "sidecar writer remains in seam source");
}
...
fn lens_allocation_survives_resume() {
    ...
    assert_eq!(before.get("planning-ws-task-extractor-01").map(String::as_str), Some("WORK"));
    assert_eq!(before.get("planning-ws-task-extractor-07").map(String::as_str), Some("REFERENCE"));
}
...
impl Fixture {
    fn new(label: &str) -> Self {
        ...
        let root = fs::canonicalize(std::env::temp_dir()).unwrap().join(format!("pi-autopilot-impl6-{label}-{nanos}"));
    ...
    fn install_repo_task_pack(&self) -> PathBuf {
        ...
        fs::write(repo.join("task.md"), "[authority]\nauthority_set_id: auth\n\n# Task\nDo the work.\n").unwrap();
...
fn manifest_lenses(repo: &Path) -> std::collections::BTreeMap<String, String> {
    ...
    manifest["assignments"].as_array().unwrap().iter().filter_map(|assignment| {
...
fn spec_path_from_command(command: &str) -> PathBuf {
    ...
    let index = parts.iter().position(|part| *part == "--spec").expect("runner command includes --spec");
