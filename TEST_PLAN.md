...
| Bounded semantic recovery and command authority | Planning first-rejection → one Recovery Engineer → unchanged rereview, repaired and no-defect success, authority/infrastructure/unsafe fail-closed dispositions, separate create-new work-map projections, digest-bound single-subject promotion, subject-drift rejection, delivery/validation crash replay, exact authority preservation, source recovery under original lane scope, independent validation round two, exhaustion without looping, profile-specific tools, and fresh-child startup-bucket bounds. Delivery roles have no general Bash: package-derived command ids are the only input to `autopilot_run_approved_command`; bounded receipts must prove every exact command passed on the final approved-file snapshot. Core forwards command receipts plus `clean-exact-package-tip` receipts to the independent Validator, where each criterion must cite exactly its assigned evidence. A bounded pre-effect unknown-command-id denial may receive the same single Recovery Engineer only when exact audit/Git/scope facts admit it; blocked work is never auto-packaged. |
...
- Remove required `package_checks`, permit an unknown/duplicate check, accept empty/duplicate/out-of-range `criterion_ordinals`, let a package-check closure unit omit any file in the complete work-map union, attach a receipt to an unrelated criterion, tolerate nonignored untracked residue at the declared clean package tip, put package state back into the child command set, or omit/tamper/duplicate its Validator receipt: WorkMap boundary, allocation authority, delivery assignment, Host policy, and validation-context tests should fail.
- Re-enable delivery Bash, accept shell text/cwd/env/timeout, inherit `PI_*` guidance, drift a package command id/digest, omit/fail/overflow a receipt, accept a stale final-file snapshot, or substitute a command receipt onto an unrelated criterion: Host policy, runner-child, lane-delivery, role-parity, and validation-context tests should fail.
- Drop the bounded pre-effect denial ledger, permit effected/unknown/path denials, ignore overflow, admit clean/no-change, skip exact base/scope inspection, or auto-package a blocked worktree: child-terminal and lane-delivery reconciliation counterexamples should fail.
- Stop hashing blocked file contents/mode facts or trust the recorded assessment without replay recomputation: `recovery_resume_fails_closed_when_blocked_worktree_snapshot_drifts` should fail.
- Match `hard_boundary_violations` or other model prose while deciding recovery: `recovery_assessment_uses_only_typed_audit_git_and_binding_facts` and `gate:no-inference` should fail.
...
- Reintroduce hand-maintained V3 enum values, let excessive shape rows become fatal, nest a digest summary over an earlier summary, reflect receipt-looking values into a repair prompt, invert PASS/blocker guidance, mix criterion and finding ID domains, remove post-truncation sorting, admit a fourth value attempt, make RPC bytes cumulative across repairs, silently default a malformed transport limit, permit a V3 child JSONL frame or package-state/recovery Git stream to allocate before its bound or retain a pipe after the Git leader exits, retain stderr past its hard ceiling, accept shutdown before reader EOF or on a retry after timeout, persist an accepted carrier before stderr completion, or leave a SIGTERM-ignoring descendant after leader exit, let the terminal environment override exceed 4 MiB, hash Git-object bytes while serving a filtered worktree file, follow a replaced evidence path, omit the parent's canonical-submission comparison, reject an approved deletion or explicit unchanged no-defect recovery, omit a shipped child profile, enumerate the delivery profile without its closed policy fixture, or report zero network without an active OS denial sandbox: the V3 authority, RPC, Host, lane, runner-child, task-path, parent-tamper, mutation, and packed-consumer checks must fail.
