...
- Remove required `package_checks`, permit an unknown/duplicate check, accept empty/duplicate/out-of-range `criterion_ordinals`, attach a receipt to an unrelated criterion, put package state back into the child command set, or omit/tamper/duplicate its Validator receipt: WorkMap boundary, delivery assignment, Host policy, and validation-context tests should fail.
