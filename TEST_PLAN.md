...
This plan covers the current Autopilot package capabilities: `/autopilot`, `/autopilot-onboard`, `/autopilot-handoff`, `context_budget`, contracts/templates, Quality vNext contract schemas, forced-output/status, state-store, execution-audit adjudication helpers, work-item lifecycle/closure gates, compiled `autopilot-agent-run`, runner CLI/bin behavior, fake-Pi e2e witnesses, parent/onboard/handoff prompts, offline SDK/RPC/package gates, and `pack:dry-run` payload verification. Default validation is deterministic, provider-free, network-free, and isolated from user/global Pi state.
...
Current automated coverage proves commands, `context_budget`, parent/onboard/handoff prompts, contracts/templates, Quality vNext spec/status gates, durable purpose state helpers, execution-audit generation/validation, scope/protected-path adjudication helpers, work-item lifecycle and terminal closure gates, forced-output/status, state-store helpers, runner dry-run/fake-Pi paths, offline SDK/RPC/package gates, and `pack:dry-run`. Remaining gaps are compiled scheduling UI, PTY/TUI coverage, migration tooling for older runtime folders, and default live-provider child execution.
