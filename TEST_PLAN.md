...
This plan covers the current Autopilot package capabilities: `/autopilot`, `/autopilot-inject`, `/autopilot-onboard`, `/autopilot-handoff`, `/autopilot-config`, `/autopilot-claim-gc`, `/autopilot-close`, `/autopilot-abort`, `context_budget`, scheduler config/dispatch, per-unit worktrees, runtime-owned unit mergeback, validation staleness, claim GC, contracts/templates, Quality vNext contract schemas, forced-output/status, state-store, execution-audit adjudication helpers, work-item lifecycle/closure gates, runtime close/merge/abort, compiled `autopilot-agent-run`, runner CLI/bin behavior, fake-Pi e2e witnesses, parent/onboard/handoff prompts, offline SDK/RPC/package gates, and `pack:dry-run` payload verification. Default validation is deterministic, provider-free, network-free, and isolated from user/global Pi state.
...
| Runtime-owned unit mergeback writes `autopilot.unit_merge.v1` | Unit / Close | Phase 2 unit tests prove runtime mergeback from unit branch to main, claim release, unit branch retirement metadata, and merge evidence; close runtime uses accepted unit merges as the final diff union proof. |
...
| Runtime close/merge/abort is deterministic and local-only | Unit / Package | `tests/unit/close-runtime.test.ts` proves `/autopilot-close` lands validated workstream branches, accepts evidenced in-worktree commit ranges, blocks target path intersections, releases claims, writes close/merge evidence, archives runtime artifacts, removes worktrees, and retires branches without provider/network calls; Phase 2 tests prove accepted unit merge paths and validation-staleness blockers; it also proves `/autopilot-abort` archives abandoned clean workstreams without merging. |
...
Current automated coverage proves commands, `context_budget`, parent/onboard/handoff prompts, scheduler config/dispatch, per-unit worktrees, runtime-owned unit mergeback, validation staleness, claim GC, runtime close/merge/abort, contracts/templates, Quality vNext spec/status gates, durable purpose state helpers, execution-audit generation/validation, scope/protected-path adjudication helpers, work-item lifecycle and terminal closure gates, forced-output/status, state-store helpers, runner dry-run/fake-Pi paths, offline SDK/RPC/package gates, and `pack:dry-run`. Remaining gaps are compiled scheduling UI, PTY/TUI coverage, migration tooling for older runtime folders, default live-provider child execution, network push/PR creation, and hosted PR automation.
