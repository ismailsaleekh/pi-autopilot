...
| Runtime close/merge/abort is deterministic and local-only | Unit / Package | `tests/unit/close-runtime.test.ts` proves `/autopilot-close` lands validated workstream branches, preserves execution commit SHAs, blocks target path intersections, releases claims, writes close/merge evidence, archives runtime artifacts, removes worktrees, and retires branches without provider/network calls; it also proves `/autopilot-abort` archives abandoned clean workstreams without merging. |
